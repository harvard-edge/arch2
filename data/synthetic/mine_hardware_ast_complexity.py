...
def export_provenance_csv(records: List[Dict[str, Any]], output_path: Path) -> None:
    """Writes the canonical CSV dataset with complete metadata comments and verified schema."""
    output_path.parent.mkdir(parents=True, exist_ok=True)

    header_comments = [
        "# Hardware AST Complexity & Clock-Domain Disparity Dataset",
    ...
    print(
        f"  [SUCCESS] Wrote canonical hardware AST complexity dataset to: {output_path} ({len(records)} records)"
    )
...
def main():
    ...
    out_csv = REPO_ROOT / "data" / "datasets" / "hardware_ast_complexity_gap.csv"
