...
for csv in candidate_csvs:
    ...
    if SYNTH_MARK in hdr:
        notes.append(f"ok  {csv.relative_to(ROOT)}  (marked synthetic)")
        continue

    script = scraper_for(csv.name)
    if script:
        why = fabricates(script)
        if why and script.parent != SYNTHETIC:
            problems.append(
                f"{csv.relative_to(ROOT)} is GENERATED ({why}) but sits in "
                f"{csv.parent.relative_to(ROOT)}. Move it to data/synthetic/."
            )
            continue
        if script.parent == SYNTHETIC:
            problems.append(
                f"{csv.relative_to(ROOT)} is generated by quarantined {script.name} "
                f"but does not announce itself as synthetic. Add '{SYNTH_MARK}' on line 1."
            )
            continue
