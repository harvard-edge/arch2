...
This direction is **Progressive Lowering**. The idea is that when an AI agent adds a custom compute unit, it does not just output Verilog; it outputs *Intermediate Representation (IR) dialects*: the new software dialect, the lowering rules that map high-level math into it, and the hardware dialect (using frameworks like CIRCT, Circuit IR Compilers and Tools, or MLIR) that implements it. No published system yet co-generates a custom dialect, its lowering rules, and the compiler schedule end-to-end to silicon, so the pattern below is a direction the loop framing implies, sketched to show where the rejection authority would sit, not a demonstrated result. Even in that sketch, the compiler-IR verifier is a narrower gate than it first appears. A verifier can statically reject a *malformed* lowering before any simulation runs, but it checks that the IR is well-formed, not that the lowering preserves behavior: a structurally valid lowering that computes the wrong result still passes, and must be caught by translation validation or equivalence checking downstream (see @lst-mlir-co-generation).
...
// Op names below are illustrative placeholders, not real CIRCT/MLIR operations.
