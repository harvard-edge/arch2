...
2. **The Constrained Optimization Loop:** The loop structure where the AI acts as the agent exploring a massively complex state space, heavily governed by a strict Rejection Authority. The environment itself is usually too complex to fully map, so the agent (often using Reinforcement Learning[^fn-9-reinforcement-learning]) must discover paths that clear physics or timing gates.
3. **The Generator-Critic Repair Loop:** The loop structure where a weak generator (like an LLM[^fn-9-large-language-model]) proposes an artifact, fails a formal check, and uses the exact error trace as the next prompt. The rejection authority is absolute (e.g., a compiler or Verilator), and the loop iterates until the artifact parses or synthesizes correctly.

[^fn-9-reinforcement-learning]: **Reinforcement learning**: a machine learning method in which an agent learns to choose actions that maximize a cumulative reward signal from its environment [@SuttonBarto2018ReinforcementLearning].

[^fn-9-large-language-model]: **Large Language Model (LLM)**: a neural network trained on vast amounts of text to understand and generate human-like language.
...
count. In the ML domain, workload drift is hyper-aggressive (e.g., shifting from dense Transformers to Mixture-of-Experts[^fn-9-mixture-of-experts] in months). A static benchmark quickly becomes a liability; an Architecture 2.0 loop must constantly ingest new computational graphs and trace data. A workload loop should therefore record benchmark version, workload
source, coverage claims, known gaps, leakage risks, and the conditions under
which a result should not generalize.

[^fn-9-mixture-of-experts]: **Mixture-of-experts (MoE)**: a neural network architecture where different parts of the model (experts) are activated conditionally based on the input, increasing capacity without a proportional increase in compute.
...
profilers, telemetry, or canary deployment[^fn-9-canary-deployment].

[^fn-9-canary-deployment]: **Canary deployment**: a controlled, partial rollout used to detect regressions before a system-wide release.

This is the regime where stronger automation is often plausible. Autotuning
systems and learned tensor-program optimizers[^fn-9-tensor-program-optimizers] show how search spaces, cost
...
[^fn-9-tensor-program-optimizers]: **Tensor-program optimizers**: methods that automate the search for efficient execution schedules of tensor operations on target hardware.
...
This direction is **Progressive Lowering**. The idea is that when an AI agent adds a custom compute unit, it does not just output Verilog; it outputs *Intermediate Representation (IR) dialects*: the new software dialect, the lowering rules that map high-level math into it, and the hardware dialect (using MLIR and MLIR-based hardware frameworks such as CIRCT, Circuit IR Compilers and Tools) that implements it. No published system yet co-generates a custom dialect, its lowering rules, and the compiler schedule end-to-end to silicon, so the pattern below is a direction the loop framing implies, sketched to show where the rejection authority would sit, not a demonstrated result. Even in that sketch, the compiler-IR verifier is a narrower gate than it first appears. A verifier can statically reject a *malformed* lowering before any simulation runs, but it checks that the IR is well-formed, not that the lowering preserves behavior: a structurally valid lowering that computes the wrong result still passes, and must be caught by translation validation or equivalence checking downstream (see @lst-mlir-co-generation).

```mlir {#lst-mlir-co-generation lst-cap="**Co-generated hardware and software IR (schematic):** A stylized sketch, *not* compilable CIRCT/MLIR. The invented ops (`hw.predict_area`, `map_to_custom_hw`) and simplified module ports stand in for where a loop would emit the hardware dialect and the lowering strategy that targets it."}
// SCHEMATIC (not compilable): where an Arch 2.0 loop would co-generate HW + SW IR.
// The invented ops (hw.predict_area, map_to_custom_hw) are illustrative
// placeholders; the transform.* and hw.module constructs are real but wired
// together here only schematically.

// 1. The AI defines the hardware intent using CIRCT (Hardware IR)
hw.module @CustomXR_SystolicArray(
    in %clk: i1, in %act: tensor<16x16xf16>,
    out %res: tensor<16x16xf16>) {
  // Agent-generated spatial architecture constraints
  %area_est = hw.predict_area(...) 
  // Loop rejects if %area_est > mobile_XR_budget
  // Note: area is approximated; true IR drop or routing
  // congestion requires physical synthesis.
}
...
^bb1(%arg1: !pdl.operation):
  ...
  transform.structured.map_to_custom_hw %0 {
    target = @CustomXR_SystolicArray }
}
```
...
operational state, workload drift, service-level objectives[^fn-9-service-level-objectives], resource
...
[^fn-9-service-level-objectives]: **Service-level objectives (SLOs)**: target values for system reliability and performance, commonly used to manage operational tradeoffs [@BeyerEtAl2016SRE].
...
stage would have rejected, does not speed the loop; it corrupts it. Its cost is
only deferred: paid later at higher commitment, the false pass makes the
effective $f$ a fiction. So $f$ counts
...
One disanalogy is worth stating head-on, because it is what makes the two-sided
condition the substantive claim rather than a caveat on it. Amdahl's $f$ is
exogenous: the workload fixes the parallel fraction, and the answer is held
constant by construction. The loop's $f$ is endogenous. It is a disposition rate
the loop itself sets, and a loop can inflate it for free by rejecting more, up to
the degenerate $f=1$ that clears nothing worth keeping. So the loop's $f$ behaves
like Amdahl's only on the error-bounded surface above, where both false passes
and false rejects are held in check. Outside that surface the analogy breaks, and
a rising $f$ measures how aggressively the loop discards rather than how much
credible work it is actually clearing.
...
One genuine gain the bound does not measure is reaching an acceptable design in
fewer total iterations. A sharper generator can do exactly that, and it is real
value; the per-commitment ratio is simply blind to it by construction, because it
holds the answer fixed and asks only how cheaply each commitment is cleared. The
bound is a claim about throughput per credible decision, not about how many
decisions the loop needs to reach a good design.
...
a better generator legitimately lowers the number of evaluations to get there. That
gain has two parts, and only one moves the bound. To the extent the generator
proposes candidates the cheap check can decide, it raises $f$, and the
per-commitment throughput rises with it. To the extent it simply reaches an
acceptable design in fewer total iterations, the per-commitment bound does not see
it at all, exactly the blindness conceded above. The two views are therefore
consistent: per-commitment throughput still moves only through $f$, while the
separate and real gain of fewer iterations-to-target sits outside the ratio by
construction.
...
proposed fixes, and cheap instruments (build, test, sanitizers
...
engineers who owned the code. Not all of those instruments are equally
independent, and the distinction matters here: the build-and-test pass is partly
coupled to what the repair tool optimizes for, while the sanitizers and the
production monitor are the genuinely independent witnesses. The honest $f$ leans
on that independent share, so the $0.3$ figure is a generous upper reading rather
than a clean measurement. With a $(1-f)$ floor of
human-owned commitments, that is the shape the bound predicts. Progress tracked
