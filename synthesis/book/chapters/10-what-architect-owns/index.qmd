...
It looked like a request for a future hardware foundation model[^fn-10-hardware-foundation-model].
...
[^fn-10-hardware-foundation-model]: **Hardware foundation model**: a foundation model is a large-scale machine learning model trained on a vast quantity of data that can be adapted to a wide range of downstream tasks; a hardware foundation model would apply this approach to hardware design representations.
...
that every architecture loop should become autonomous. Agentic systems[^fn-10-agentic-system] are
...
[^fn-10-agentic-system]: **Agentic system**: an AI system capable of pursuing complex goals over time, making its own decisions about which tools to use and what steps to take next.
...
An autonomous or AI-assisted system, however, expects to run end-to-end based on explicit inputs. If the rules are tacit, the AI behaves like an ML model encountering out-of-distribution data or a reinforcement learning agent engaging in reward hacking[^fn-10-reward-hacking]. It will either stall out due to missing context or, worse, hallucinate a mathematically optimal design that exploits loopholes in the incomplete constraints, completely violating the unwritten physical or software realities of the system.

[^fn-10-reward-hacking]: **Reward hacking**: a failure mode in reinforcement learning where an agent discovers a way to maximize its objective function through unintended or physically impossible loopholes rather than solving the actual task.
...
held-out validation[^fn-10-held-out-validation] are not new; careful architects have always done them. The
...
multi-fidelity modeling[^fn-10-multi-fidelity-modeling] [@PeherstorferWillcoxGunzburger2018Multifidelity].
Naming the lineage is the point, not a weakness.

[^fn-10-held-out-validation]: **Held-out validation**: in machine learning, the practice of reserving a portion of the dataset during training exclusively for evaluating a model's performance on unseen data.

[^fn-10-multi-fidelity-modeling]: **Multi-fidelity modeling**: an optimization technique that combines cheap, low-accuracy simulations with expensive, high-accuracy simulations to explore a design space efficiently.
...
The fifth objection is the adversarial one. The discipline assumes good faith,
and a hostile participant, whether human or automated, could fabricate the
evidence ledger: record a passing proxy screen, omit the failing signoff, and
launder a weak design as a trustworthy one. This is the sharpest version of the
critique, and the honest response is not that the discipline prevents bad faith,
nor that it already has the machinery to catch it. It does not. Two seams are
real: this book sanctions redacted and confidential evidence, which a fabricator
can hide behind, and it concedes that the loop level does not yet have the
independent, adversarial peer review that earned MLPerf its comparability. What
the explicit loop changes is narrower, but not nothing. It converts a fabrication
from an unfalsifiable "trust me" into a partly checkable claim: a fabricated
ledger is a re-runnable artifact whose baselines can be rerun and whose proxy
result can be independently recomputed, while a fabricated intuition cannot be
audited at all. That check is narrower than it sounds, since redaction can still
withhold the very signoff a reviewer would most want, which is exactly why
closing the seam matters. Closing the seams, redaction that still exposes a verifiable hash
and a submission round in which others can rerun and reject a loop's claims, is
exactly the community infrastructure this chapter argues the field still owes. It
is not a problem the card solves on its own.

The sixth objection is about evidence, and it is aimed squarely at this book. The
empirical spine here is thin: one synthetic end-to-end hardware loop in
@sec-running-the-loop and one borrowed, deployed software-repair loop in
@sec-loop-patterns-across-stack, with no measured, silicon-backed hardware
co-design loop anywhere in these pages. A skeptic can fairly say the strongest
claim, that AI-assisted loops change what the architect owns, rests on a worked
example and a taxonomy rather than on a body of hardware results. The response is
to concede the range and state the book's actual position. This is a preview: it
argues that the loop is the right object to represent, instrument, and measure,
and it offers the card, the contracts, and the rejection bound as the apparatus
for producing that evidence, not as a substitute for it. The honest claim is not
that the evidence is in; it is that the field now has a shared way to demand it.

None of these six objections is fatal, but each sharpens the claim. Architecture
...
task, workload, metric, and submission rules, and an adversarial peer-review
round under those rules in which submitters can reject each other's published
claims, none of which a self-attested card supplies on its own.
