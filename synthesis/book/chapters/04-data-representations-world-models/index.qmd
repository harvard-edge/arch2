...
Architecture does not need data in the same way a web language model^[**Language model**: a statistical model trained to predict the next word or token in a sequence, forming the foundation of modern AI text generation.] needs text.
...
policies, much like reinforcement learning agents use latent world models^[**Latent world model**: an internal, compressed representation of an environment used by an AI to simulate and plan actions without interacting with the real world.] to plan and simulate outcomes before acting.
