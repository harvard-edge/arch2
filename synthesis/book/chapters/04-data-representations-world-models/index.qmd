...
tempting to begin Architecture 2.0 by asking which model, agent framework[^fn-4-agent-framework], or
...
[^fn-4-agent-framework]: **Agent framework**: A software structure that defines how autonomous systems perceive their environment, reason, and take actions to achieve goals.
...
Architecture does not need data in the same way a web language model[^fn-4-language-model] needs text.
...
[^fn-4-language-model]: **Language model**: a statistical model trained to predict the next word or token in a sequence, forming the foundation of modern AI text generation.
...
Labels may require expert judgment. Negative results[^fn-4-venues-negative-results] are rarely published.
...
$$
That sounds large, but it is small by web-scale pretraining[^fn-4-pretraining] standards.
...
[^fn-4-pretraining]: **Pretraining**: The computationally intensive initial phase of training a large machine learning model on a massive, general dataset before fine-tuning it for specific tasks.
...
Domain adaptation[^fn-4-domain-adaptation] matters here only if it helps construct or retrieve loop
...
[^fn-4-domain-adaptation]: **Domain adaptation**: A machine learning technique where a model trained on one data distribution is adjusted to perform well on a different, specific target domain [@PanYang2010TransferLearning].


Moving from training text to evaluation, benchmark lineages such as SQuAD and GLUE[^fn-4-squad-and-glue] offer a
...
[^fn-4-squad-and-glue]: **SQuAD and GLUE**: Standard benchmark datasets used to evaluate the performance of natural language processing models, where SQuAD is the Stanford Question Answering Dataset [@RajpurkarEtAl2016SQuAD] and GLUE is the General Language Understanding Evaluation [@WangEtAl2018GLUE].
...
policies, much like reinforcement learning agents use latent world models[^fn-4-latent-world-model] to plan and simulate outcomes before acting.

[^fn-4-latent-world-model]: **Latent world model**: an internal, compressed representation of an environment used by an AI to simulate and plan actions without interacting with the real world.
...
uncertainty. Each can be wrong under distribution shift[^fn-4-distribution-shift], new workloads,
...
[^fn-4-distribution-shift]: **Distribution shift**: A situation where the data a model encounters during deployment differs significantly from the data it was trained on, often degrading performance [@QuinoneroCandelaEtAl2009DatasetShift].
...
As emphasized by treating sample cost as architecture data (see the discussion on "Sample Cost Is Architecture Data" in @sec-sample-cost-is-architecture-data), workload provenance is critical loop state. Yet, we lack rigorous methods to detect when an automated optimizer has overfit to a specific execution phase or synthetic input distribution. A major research direction is establishing formal generalization bounds[^fn-4-generalization-bound] for architecture agents, preventing silent hardware failures under distribution shifts toward architecture world models (see the discussion on "Toward Architecture World Models" in @sec-toward-architecture-world-models).

[^fn-4-generalization-bound]: **Generalization bound**: a theoretical limit on how well a machine learning model's performance on its training data will apply to unseen, out-of-distribution data.

4. **How do we formulate representation learning over negative architecture traces?**
As established in our discussion of provenance, coverage, and negative traces (see the discussion on "Provenance, Coverage, and Negative Traces" in @sec-provenance-coverage-and-negative-traces), rejected candidates, missed timing constraints, and invalid parameters define the true boundary of the design space (@tbl-negative-traces). However, current generative models are primarily trained on successful artifacts. A critical problem for the ML-for-systems community is designing novel loss functions[^fn-4-loss-function] or preference-optimization pipelines[^fn-4-preference-optimization-pipeline] that force models to learn from high-fidelity negative records, ensuring they actively avoid—rather than rediscover—known failure modes.

[^fn-4-loss-function]: **Loss function**: a mathematical function used to measure the difference between an AI model's predicted output and the actual target, which the training process seeks to minimize.

[^fn-4-preference-optimization-pipeline]: **Preference-optimization pipeline**: a technique such as Reinforcement Learning from Human Feedback (RLHF) or Direct Preference Optimization (DPO) used to align a model's outputs with human preferences.
