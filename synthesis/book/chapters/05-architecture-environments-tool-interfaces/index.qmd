...
workload ... under a 3\ W TDP target in a 3\ nm-class LP mobile process" is not one call
...
| Memory, coherence, and consistency model | Coherence assumptions (sharing, cacheability) and consistency assumptions (ordering, DMA, and synchronization), which are distinct: coherence governs value propagation to a single location, consistency governs ordering across locations. | Correctness tests, contention behavior, latency, bandwidth, and race or ordering failures. | A candidate looks fast because it violated the program's memory assumptions. |
...
Fundamentally, this abstraction can be described as a contract. The environment is literal: it is the toolchain exposed as an executable sandbox. The environment does not compute a scalar reinforcement-learning reward. It emits raw observations and cost telemetry—such as cycle counts, gate area, and test failures. The loop's *rejection authority* evaluates this multi-dimensional telemetry against our claims. If we let tools compute a scalar reward directly, we invite reward hacking[^fn-5-reward-hacking] where the loop optimizes for simulator bugs instead of physical reality. The contract requires that the loop make a small set of obligations explicit. @tbl-environment-contract
lists the main fields.

[^fn-5-reward-hacking]: **Reward hacking** occurs when an AI agent finds an unintended shortcut to maximize its reward signal without solving the underlying problem.
...
| Observation (and reward) | Metrics returned by the simulator or tool path. ArchGym can derive a scalar reward from these, but Architecture 2.0 reads the raw multi-dimensional observations and leaves accept-or-reject to the rejection authority rather than a single reward (see Limits). |
...
| Limits | Simplified simulators, cleaner action spaces, and missing industrial constraints can bound what the result proves. The built-in scalar reward is itself a limit: collapsing multi-dimensional telemetry into one number is what invites reward hacking, which is why this book keeps the raw observations and an explicit rejection authority. |
...
with it: Bayesian optimization, reinforcement learning, evolutionary search[^fn-5-evolutionary-search],
surrogate-guided exploration[^fn-5-surrogate-guided-exploration], random search, heuristic search, or a human
...
[^fn-5-evolutionary-search]: **Evolutionary search**: In this chapter, a way to propose candidate actions by mutation, crossover, and selection, useful only when the environment can reject invalid or weak candidates.

[^fn-5-surrogate-guided-exploration]: **Surrogate-guided exploration**: In this chapter, using a fast predictor to screen candidate actions while preserving the stronger feedback needed to reject proxy wins.
...
high-fidelity ones to keep optimization, inference, and uncertainty[^fn-5-uncertainty-quantification]
...
[^fn-5-uncertainty-quantification]: **Uncertainty quantification**: In this chapter, recording where an environment's feedback is uncertain enough to force escalation rather than commitment.
...
needs sample efficiency, priors, surrogates, active learning[^fn-5-active-learning], staged gates, or
...
[^fn-5-active-learning]: **Active learning**: In this chapter, a policy for choosing which scarce simulator, synthesis, human, or deployment feedback event to spend next, with the choice logged as part of the evidence ledger [@Settles2009ActiveLearning].
...
Environment design should therefore include red-team checks[^fn-5-red-teaming] for the feedback
...
[^fn-5-red-teaming]: **Red-teaming**: intentionally attacking a system to expose vulnerabilities, flaws, and failure modes before deployment.
...
To construct this bounded task, the recipe is straightforward (@lst-environment-recipe).

```{.default #lst-environment-recipe lst-cap="**Environment recipe:** Constructing a small, bounded, rejectable Architecture 2.0 environment step by step."}
...
Operating discipline, the architecture counterpart of MLOps[^fn-5-mlops], is the practice of maintaining those layers over time.
...
[^fn-5-mlops]: **MLOps**: In this chapter, the operating discipline for deployed learning systems that motivates similar versioning, monitoring, drift, rollback, and ownership records in Architecture 2.0 loops.
