...
a loop; it is a wish. This chapter narrows the task to one XRBench-class[^fn-8-xrbench-class]
...
[^fn-8-xrbench-class]: **XRBench-class**: XRBench is a mobile and extended-reality benchmark suite [@KwonEtAl2023XRBench]. This chapter uses "XRBench-class" to mean a workload slice modeled on that benchmark family, not measured XRBench results.
...
To document this shift in ranking, @tbl-worked-loop-ledger shows the evidence ledger left behind. The exact schema of the candidate record depends on the environment, but it must be machine-readable (e.g., JSON or YAML) to allow autonomous auditing. For example, a candidate's YAML evidence ledger entry captures why it was rejected and with what evidence (@lst-evidence-ledger):
```{.yaml #lst-evidence-ledger lst-cap="**Evidence ledger entry:** A machine-readable YAML record of a rejected candidate: the gate it failed, its proxy-versus-simulation estimates, and provenance."}
...
alternative. Furthermore, it keeps the memory subsystem continuously busy, so its conservative average-power screen crosses the 3\ W budget. That is enough to reject it at this stage and to flag it for the sustained-thermal and peak-power (TDP) signoff the loop has not yet run. The screen is trustworthy in exactly this direction: a candidate whose average active power already exceeds the budget cannot pass, because peak power can only be higher, and this accelerator earns its rejection by running the memory system at near-continuous duty. The converse does not hold. A candidate that clears the average-power screen has not cleared the budget, since a bursty, low-duty design can sit under the average and still violate a peak or thermal limit, which is why a pass earns only escalation to signoff, never a commitment. These are hard *constraints*, not just soft metrics, so no
...
::: {.callout-lighthouse title="Price the interface before believing the gain"}
