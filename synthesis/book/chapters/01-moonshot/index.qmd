...
programmed in natural language by prompting a foundation model
[@Karpathy2017Software2; @Karpathy2025Software3]. Carried across to hardware, the
...
wrong unit for architecture is what motivates this book. Software 3.0 is an engine
for making plausible artifacts cheap, and this chapter's premise is that when
plausible artifacts are cheap, the scarce work shifts from producing one to
standing behind it: comparing it, rejecting weaker alternatives, and owning the
commitment when it ships in silicon, where a wrong choice is a respin, not a
re-prompt. Software 3.0 changes how we *program* computers; Architecture 2.0
...
behind it. High-level languages reshaped instruction sets and produced optimizing
compilers; operating-system abstractions pushed protection, virtual memory, and
privilege levels into the processor; machine-learning workloads pulled specialized
accelerators into the mainstream [@HennessyPatterson2019GoldenAge]. Each of those
waves changed *what* architects build. This one is different in kind: foundation
models change *how* architects build, by acting inside the design loop itself.
That is why Architecture 2.0 is best read not as a hardware counterpart to Software
3.0 but as a broader shift one level up, in the engineering discipline rather than
the programming interface (@fig-software-shifts-architecture).

![**Software abstractions have always pulled architecture, but this shift changes the loop, not the artifact:** High-level languages, operating-system abstractions, and machine-learning workloads each pulled a new architectural response, from optimizing compilers to specialized accelerators, changing *what* architects build. Foundation models, the Software 3.0 shift, pull Architecture 2.0, which changes *how* architects build by making the design loop the object of design.](images/F1d-software-shifts-architecture.svg){#fig-software-shifts-architecture width="100%" fig-alt="Two horizontal bands. The top band shows four shifts in software abstraction: high-level languages, OS abstractions, ML workloads, and foundation models (Software 3.0). Arrows pull down to the bottom band of architecture responses: optimizing compilers and ISA design, processor features, specialized accelerators, and Architecture 2.0, the design loop itself. The first three responses are tagged as changing what architects build; the last is tagged as changing how architects build."}
...
the generic version, many kinds of inputs feed a central foundation model[^fn-1-foundation-model], and
