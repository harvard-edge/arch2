...
This shift toward designing loops is easiest to state as a rule: a prompt is not yet an architecture claim. It becomes one only when the loop explicitly defines its five-part execution state: what state it saw, what actions it allowed, what alternatives it rejected, what evidence supports the result, and who owns the commitment if the claim is wrong. When plausible artifacts are cheap, that burden of proof, not the artifact, becomes the scarce work. Without treating this loop as a rigorous system-level state machine, we risk the software equivalent of dark silicon (the share of a chip that must stay powered off to fit its thermal budget [@EsmaeilzadehEtAl2011DarkSilicon]): generative capacity we can run but cannot convert into credible, committable designs, because nothing verifies and rejects its outputs fast enough.
...
[@HennessyPatterson2017QuantitativeApproach]. Just as the end of Dennard scaling forced hardware to transition from general-purpose frequency scaling to specialized accelerators, the economic and latency limits of monolithic LLMs[^fn-1-large-language-model] force agent systems to transition from raw prompting to structured execution loops. Dark silicon, data-movement energy, warehouse scale, and
carbon accounting make architectural coupling more difficult
[@DennardEtAl1974Scaling; @Horowitz2014Energy; @BarrosoHolzleRanganathan2019DatacenterAsComputer; @GuptaEtAl2021ChasingCarbon]. Today, efficiency includes performance, energy,
...
[^fn-1-large-language-model]: **Large Language Model (LLM)**: a neural network trained on vast amounts of text to predict the next token and generate human-like language.
...
It helps to name the version of that shallow reading currently in the air. Andrej
Karpathy describes a progression from Software 1.0, code written by hand, through
Software 2.0, behavior learned as neural-network weights, to Software 3.0, systems
programmed in natural language by prompting a foundation model[^fn-1-foundation-model]
[@Karpathy2017Software2; @Karpathy2025Software3].

[^fn-1-foundation-model]: **Foundation model**: a large-scale AI model trained on a vast quantity of data that can be adapted to a wide range of downstream tasks.

Carried across to hardware, the
tempting move is a Software 3.0 for chips: state the system you want and let a
model emit it. It is worth taking that move seriously, because seeing why it is the
wrong object of design for architecture is what motivates this book. Software 3.0
is an engine for making plausible artifacts cheap, so far for prose and code and
only aspirationally for silicon, since no model yet reliably emits a plausible
floorplan or RTL. The argument does not rest on which side of that line hardware
sits: this chapter's premise is that when plausible artifacts are cheap, the
scarce work shifts from producing one to standing behind it: comparing it,
rejecting weaker alternatives, and owning the commitment when it ships in silicon,
where a wrong choice is a respin, not a re-prompt. Software 3.0 changes how we *program* computers; Architecture 2.0
changes how we *engineer* computing systems. It is the discipline for the world
Software 3.0 creates, keeping the cheap generation but wrapping it in an explicit
loop with represented state, evidence, and rejection authority, and a human who
owns the final decision.

This is not the first time a shift in software has pulled architecture along
behind it. High-level languages and their optimizing compilers drove the RISC
simplification of the instruction set; operating-system abstractions pushed
protection, virtual memory, and privilege levels into the processor; and
machine-learning workloads, the Software 2.0 wave, pulled domain-specific
accelerators into the mainstream [@HennessyPatterson2019GoldenAge]. Those waves
reshaped what architects build, and some reshaped how they work too: the move to
high-level languages and RISC is inseparable from the quantitative,
benchmark-driven method that came to define modern architecture. What is new now
is narrower and sharper. Earlier waves changed the artifacts inside a design loop
that stayed implicit and human-carried; this wave makes the loop itself the object
of design, something an AI system can act inside rather than around. That is why
Architecture 2.0 is best read not as a hardware counterpart to Software 3.0 but as
a broader shift one level up, in the engineering discipline rather than the
programming interface (@fig-software-shifts-architecture).

![**Software abstractions have always pulled architecture, but this shift changes the loop, not the artifact:** High-level languages, operating-system abstractions, and machine-learning workloads each pulled a new architectural response, from RISC instruction sets to specialized accelerators, changing *what* architects build. Foundation models, the Software 3.0 shift, pull Architecture 2.0, which changes *how* architects build by making the design loop itself the object of design.](images/F1d-software-shifts-architecture.svg){#fig-software-shifts-architecture width="100%" fig-alt="Two horizontal bands. The top band shows four shifts in software abstraction: high-level languages, OS abstractions, ML workloads, and foundation models (Software 3.0). Arrows pull down to the bottom band of architecture responses: optimizing compilers and ISA design, processor features, specialized accelerators, and Architecture 2.0, the design loop itself. The first three responses are tagged as changing what architects build; the last is tagged as changing how architects build."}
...
> metric, and an adversarial peer-review round under shared rules that can reject a claim.
...
- **2010s--2020s (Agile Hardware Generation):** Agile hardware generation turns parameterized templates (e.g., Chisel and FIRRTL, hardware construction languages) into bespoke system-on-chip (SoC) instances, enabling rapid specialization through reusable IP libraries.
...
> defensible system design. It includes the instruction set architecture (ISA) and microarchitecture, memory and
...
> **Datacenter-Scale Lighthouse Prompt:** Design a scale-out TPU supercomputer topology for training 10T-parameter mixture-of-experts[^fn-1-mixture-of-experts] models. Ensure the tensor compiler (XLA/MLIR) can discover an efficient mapping, bound tail latency across 10,000 optical links, co-design network topology with deadlock-free adaptive routing and virtual channels, guarantee recovery from synchronous faults, hold the cluster within a fixed megawatt-scale power-delivery and carbon envelope (the constraint that dominates at this scale, the counterpart of the mobile prompt's 3\ W gate), and tolerate cloud multi-tenancy overheads (cold starts, cache pollution, memory disaggregation latencies). Crucially, the loop must explicitly measure and enforce software ecosystem preservation—rejecting any topology or ISA change that breaks compatibility with the dominant TPU software stack (XLA, JAX and PyTorch/XLA, and the collective-communication libraries).

[^fn-1-mixture-of-experts]: **Mixture-of-experts (MoE)**: a model that selectively routes each input to a small, specialized subset of its neural network weights, reducing compute cost per token while maintaining a large total parameter count.
...
learning[^fn-1-reinforcement-learning] systems for physical-design exploration and learned circuit-generation
...
as loop-pattern cases.

[^fn-1-reinforcement-learning]: **Reinforcement learning**: a machine learning method in which an agent learns to choose actions that maximize a cumulative reward signal from its environment [@SuttonBarto2018ReinforcementLearning]. These examples show bounded loop pieces with evidence,
...
represent, including the world models[^fn-1-world-model] introduced by the ontology.
...
2. **Adversarial Red-Teaming[^fn-1-red-teaming] for Autonomous Hardware Generation Loops.**
...
[^fn-1-red-teaming]: **Red-teaming**: proactively probing a system to discover vulnerabilities, biases, or unexpected failure modes.
