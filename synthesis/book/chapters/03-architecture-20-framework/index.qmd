...
Asking whether a generative model^[**Generative model**: a class of AI models designed to generate new data, such as text or configurations, that resembles its training data.] can design hardware is too coarse a framing. The more
...
a paper uses an LLM^[**Large Language Model (LLM)**: a neural network trained on vast amounts of text to understand and generate human-like language.], reinforcement learning^[**Reinforcement learning**: a machine learning training method based on rewarding desired behaviors and punishing negative ones.], Bayesian optimization, a surrogate
...
The practical implication is not to build a chip-specific language model^[**Language model**: a statistical model trained to predict the next word or token in a sequence, forming the foundation of modern AI text generation.] first.
...
Current AI methods often discard the failure traces that teach human architects the true boundaries of a design space. A major open question is how to continuously extract and encode these negative paths into a rigorous representation of architecture state (see the discussion on "The Architecture 2.0 Ontology" in @sec-arch2-ontology) that actively regularizes generative models against out-of-bounds hallucinations^[**Hallucination**: an instance where an AI model generates false, fabricated, or nonsensical information confidently.] and causal errors.
