...
> **Bayesian optimization**: A strategy for optimizing expensive black-box functions, where each candidate evaluation (such as a cycle-accurate simulation) is costly [@SnoekLarochelleAdams2012BayesianOptimization].
...
a paper uses an LLM[^fn-3-large-language-model], reinforcement learning[^fn-3-reinforcement-learning], Bayesian optimization, a surrogate
...
[^fn-3-large-language-model]: **Large Language Model (LLM)**: a neural network trained on vast amounts of text to understand and generate human-like language.

[^fn-3-reinforcement-learning]: **Reinforcement learning**: a machine learning training method based on rewarding desired behaviors and punishing negative ones.

The practical implication is not to build a chip-specific language model[^fn-3-language-model] first.
...
[^fn-3-language-model]: **Language model**: a statistical model trained to predict the next word or token in a sequence, forming the foundation of modern AI text generation.
