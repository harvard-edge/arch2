...
This book formalizes that loop as a five-part execution state, introduced in @sec-moonshot: what state it saw, what actions it allowed, what alternatives it rejected, what evidence supports the result, and who owns the commitment if the claim is wrong. Without those pieces, an AI
...
Asking whether a generative model[^fn-3-generative-model] can design hardware is too coarse a framing. The more
...
[^fn-3-generative-model]: **Generative model**: a class of AI models designed to generate new data, such as text or configurations, that resembles its training data.
...
loop is a Markov decision process[^fn-3-markov-decision-process]. It is a bookkeeping discipline: if a loop
...
[^fn-3-markov-decision-process]: **Markov decision process (MDP)**: A mathematical framework for modeling decision-making where outcomes are partly random and partly under the control of a decision maker; it forms the foundation of reinforcement learning.
...
it uses an LLM[^fn-3-large-language-model], reinforcement learning[^fn-3-reinforcement-learning], Bayesian optimization[^fn-3-bayesian-optimization], or a surrogate
...
[^fn-3-bayesian-optimization]: **Bayesian optimization**: A strategy for optimizing expensive black-box functions, where each candidate evaluation (such as a cycle-accurate simulation) is costly [@SnoekLarochelleAdams2012BayesianOptimization].
...
[^fn-3-large-language-model]: **Large Language Model (LLM)**: a language model, a statistical model trained to predict the next token in a sequence, scaled up as a neural network trained on vast amounts of text to understand and generate human-like language.

[^fn-3-reinforcement-learning]: **Reinforcement learning**: a machine learning method in which an agent learns to choose actions that maximize a cumulative reward signal from its environment [@SuttonBarto2018ReinforcementLearning].
...
Current AI methods often discard the failure traces that teach human architects the true boundaries of a design space. A major open question is how to continuously extract and encode these negative paths into a rigorous representation of architecture state (see the discussion on "The Architecture 2.0 Ontology" in @sec-arch2-ontology) that actively regularizes generative models against out-of-bounds hallucinations[^fn-3-hallucination] and causal errors.

[^fn-3-hallucination]: **Hallucination**: an instance where an AI model generates false, fabricated, or nonsensical information confidently.
