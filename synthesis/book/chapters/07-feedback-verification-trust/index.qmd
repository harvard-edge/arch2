...
surrogate can rank candidates. A tool-using agent[^fn-7-tool-using-agent] can call simulators and
...
[^fn-7-tool-using-agent]: **Tool-using agent**: an AI system that can programmatically invoke external software tools or APIs to solve tasks.
...
learning is possible. A Bayesian optimizer[^fn-7-bayesian-optimizer], reinforcement-learning policy[^fn-7-policy],
surrogate model[^fn-7-surrogate-model], critic[^fn-7-critic], or tool-using agent should behave differently when a
...
[^fn-7-bayesian-optimizer]: **A Bayesian optimizer** builds a probabilistic model of an objective function to efficiently find its optimal parameters with few evaluations.

[^fn-7-surrogate-model]: **A surrogate model** is a cheaper, learned approximation of a complex, expensive simulation.

[^fn-7-policy]: **Policy**: in reinforcement learning, a function that maps an agent's state to a chosen action.

[^fn-7-critic]: **Critic**: in this book, the critique method role of @sec-methods-generation-prediction-optimization, a component that challenges assumptions, evidence, or claims inside the loop; reinforcement learning uses the same word more narrowly for a learned estimator of expected future returns.
...
**assurance case[^fn-7-goal-structuring-notation]**: a structured argument that links a top-level claim to
...
[^fn-7-goal-structuring-notation]: **Goal Structuring Notation (GSN)**: a graphical argumentation notation used in safety-critical engineering to formally document the elements of an assurance case.
...
the mechanism that pays that cost. The verification authorities in @tbl-verification-authorities are its operational catalog. A compact way to remember *why* a tool or check rejects a candidate is the **4 P's of Rejection**:
1. **Proxy Mismatches:** Rejection occurs because the system was optimizing a surrogate model instead of the true metric.
2. **Protocol Violations:** Rejection occurs because the candidate breaks the ISA, hardware/software contract, or memory consistency model, where litmus tests expose ordering violations as a first-class rejection gate and formal model checking is needed wherever absence, not just presence, of a violation must be established.
...
The 4 P's name the architecture-specific reasons a candidate is rejected, and they sit between two authorities the table also lists. Below them is basic functional correctness, whether the candidate computes the right answer at all, checked by unit tests, reference outputs, and formal equivalence; it is the precondition every one of the P's assumes. Above them is expert review, the human commitment no proxy replaces and the authority the rest of this book returns to.
...
not because it understands the architecture problem. In machine learning this active exploitation of a flawed metric is called *reward hacking* [@AmodeiEtAl2016ConcreteProblems]; economics names the same failure *Goodhart's Law* (@sec-methods-generation-prediction-optimization).
...
architecture have long needed validation against held-out data[^fn-7-held-out-data], higher-fidelity
...
[^fn-7-held-out-data]: **Held-out data**: a subset of a dataset reserved during model training, used exclusively to test the model's ability to generalize to unseen inputs.
...
about whether the model ran; it was about provenance, baselines, whether an
intermediate proxy metric stood in for final power, performance, and area, and
...
As highlighted in the discussion on proxy mismatch, metric gaming, and calibration (see the discussion on "Proxy Mismatch, Metric Gaming, and Calibration" in @sec-proxy-mismatch-metric-gaming-and-calibration), autonomous generators inevitably overfit and exploit the blind spots of cheap architectural proxies. Because identifying when a candidate drifts outside a simulator's calibrated support is largely manual, generative loops are fragile. A thesis-level challenge is to develop algorithms that dynamically map the epistemic uncertainty[^fn-7-epistemic-uncertainty] of architectural surrogates in high-dimensional design spaces, automatically triggering high-fidelity checks—such as RTL synthesis—exactly when proxy trust bounds are violated. How can we design compound AI agents[^fn-7-compound-ai-agents] that natively co-optimize performance and calibration, structurally guaranteeing they escalate before committing to an adversarial proxy win?

[^fn-7-epistemic-uncertainty]: **Epistemic uncertainty** refers to uncertainty in a model's predictions caused by a lack of training data in that region, which can be reduced by gathering more data.

[^fn-7-compound-ai-agents]: **Compound AI agents** are systems composed of multiple interacting AI models, tools, and logical components working together to solve a complex task.

3. **Adversarial Red-Teaming[^fn-7-red-teaming] and the Quantification of Trust Machinery.**
...
[^fn-7-red-teaming]: **Red-teaming**: proactively testing a system using adversarial techniques to identify vulnerabilities or unintended behaviors.
