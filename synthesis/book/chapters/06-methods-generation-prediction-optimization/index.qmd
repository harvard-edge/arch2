...
::: {.column-margin}
**VJ's Note:** Lord Kelvin, the pioneering British mathematician and physicist, emphasized that what you cannot measure, you cannot improve. For our purposes this does not mean every method reduces to maximizing one reward. It means an AI loop is only as strong as the measurements that can *reject* its candidates: evidence at a fidelity high enough to say no, not a single scalar to optimize.
:::
...
design region, but fail outside the sampled space. Bayesian optimization[^fn-6-bayesian-optimization] may
spend scarce simulator or synthesis evaluations carefully, but only if the
action space and objective are well formed. Reinforcement learning[^fn-6-reinforcement-learning] may be
...
[^fn-6-bayesian-optimization]: **Bayesian optimization**: a strategy for the global optimization of black-box functions that uses a probabilistic model to balance exploration and exploitation.

[^fn-6-reinforcement-learning]: **Reinforcement learning**: a machine learning method in which an agent learns to choose actions that maximize a cumulative reward signal from its environment [@SuttonBarto2018ReinforcementLearning].

Architecture 2.0 therefore treats methods as roles in a compound design
system. These roles echo the classic components of a closed-loop controller, and each acts on part of the five-part execution state:

1. **Generators (Actuation)**: Propose candidate implementations, hypotheses, or repairs (the *actions* the loop takes).
2. **Predictors (Outcome Estimation)**: Estimate a candidate's properties and performance and check its constraints, producing the *evidence* the loop reads.
3. **Optimizers (Search Policy)**: Navigate the design space, deciding which branches to propose, evaluate, or prune next.

These three cover what the loop proposes, what it learns about each candidate, and how it moves through the space. The remaining piece of the execution state, which *alternatives were rejected* and on what authority, is deliberately not an optimizer's call: pruning a branch to save search budget is not the same as rejecting a candidate as evidence, which belongs to the verification role below.
...
search[^fn-6-neural-architecture-search] already shows the value of putting latency, energy, memory footprint,
...
[^fn-6-neural-architecture-search]: **Neural architecture search (NAS)**: a cautionary adjacent field where search only becomes reusable when the search space, data, training budget, evaluation rule, and rejected alternatives are recorded.
...
recent foundation models[^fn-6-foundation-model], architects used statistical and machine-learning
...
[^fn-6-foundation-model]: **Foundation model**: a large-scale machine learning model trained on vast quantities of data that can be adapted to a wide range of downstream tasks.
...
appropriate. Conformal prediction[^fn-6-conformal-prediction] can wrap a surrogate, regardless of its
internals, so that it emits calibrated prediction sets: intervals that contain
the true value with a user-chosen probability under an exchangeability[^fn-6-exchangeability]
...
[^fn-6-conformal-prediction]: **Conformal prediction**: a statistical technique that provides rigorous, distribution-free uncertainty bands for model predictions.

[^fn-6-exchangeability]: **Exchangeability**: A statistical property where the joint probability distribution of a sequence of random variables is invariant to their permutation; it is a weaker assumption than independent and identically distributed (i.i.d.).
...
limited feedback. That may mean finding a Pareto region[^fn-6-pareto-region], identifying a
...
[^fn-6-pareto-region]: **Pareto region**: The set of optimal solutions in a multi-objective design space where no single objective can be improved without degrading at least one other objective.
...
The contrast with the placement dispute (@sec-feedback-verification-trust) is the
lesson, not the leaderboard. PrefixRL is defensible because a bounded action space
lets a high-fidelity synthesis instrument reject every candidate. Large-space
placement, by contrast, leans on a proxy reward during search; and, separately,
its reported wins were disputed on baselines, released code, and reproducibility.
Those are two different points, not one: the search-time proxy is a method choice,
while the dispute was about the strength of the evidence behind the published
result. Method choice is inseparable from what the environment can verify.
...
itself the problem of sample efficiency[^fn-6-sample-efficiency], which is one of the reasons architecture is a hard AI domain.
...
[^fn-6-sample-efficiency]: **Sample efficiency**: In this chapter, the amount of decision-relevant evidence a loop gains per scarce feedback event, including failed runs and expert rejections, not just successful simulations.
...
probability estimate over outcomes), and GP-UCB[^fn-6-gp-ucb] (Gaussian Process Upper
Confidence Bound) gives that choice a no-regret guarantee[^fn-6-no-regret-guarantee] under stated
assumptions, so the loop can defend why it sampled where it did rather than
appealing to intuition [@SrinivasEtAl2010GPUCB]. Multi-fidelity Bayesian[^fn-6-multi-fidelity-bayesian-optimization]
...
[^fn-6-gp-ucb]: **GP-UCB (Gaussian Process Upper Confidence Bound)**: an acquisition function that selects the next point to evaluate by weighing both the expected reward and the uncertainty of a Gaussian Process model.

[^fn-6-multi-fidelity-bayesian-optimization]: **Multi-fidelity Bayesian optimization**: an extension of Bayesian optimization that jointly selects which design to evaluate and at which level of fidelity, optimizing the tradeoff between information gain and computational cost.

[^fn-6-no-regret-guarantee]: **No-regret guarantee**: In this chapter, a formal promise about a sampling rule under stated assumptions; useful only when the loop records whether those assumptions apply.
