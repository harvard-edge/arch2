...
lesson, not the leaderboard: the same method family is defensible when a bounded
action space lets a high-fidelity instrument reject every candidate, and open to
dispute over baselines and reproducibility when the evaluation rests on a weaker
proxy. Method choice is inseparable from what the environment can verify.
