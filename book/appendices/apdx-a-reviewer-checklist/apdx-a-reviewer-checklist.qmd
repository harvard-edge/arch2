...
This appendix provides a portable, external-review instrument for evaluating AI-assisted architecture papers, artifact packages, design reports, or study records. We designed this checklist so reviewers can apply it without requiring authors to adopt the design-loop card from @sec-architecture-20-ontology or any other specific schema. Our questions trace the evidence from a bounded claim and its executed environment through comparisons, independent checks, replayability, realized costs, claim limits, and technical responsibility.
...
Reward hacking through imperfect proxies is an established safety concern across our discipline [@AmodeiEtAl2016Concrete]. In hardware architecture, this translates to the risk that a search might exploit simulator inaccuracies rather than genuinely improve the design.
...
We recognize that learning-based results, particularly in deep reinforcement learning, can be highly sensitive to seeds, hyperparameters, and implementation details [@HendersonEtAl2018DeepRL].
...
- **Distribution:** Do the authors report distribution- or category-level outcomes, per-task results, failures, and uncertainties using summaries appropriate to the observed shape, including heavy tails or bimodality?
- **Noise floor:** Did the authors measure apparatus variation for their study through fixed-setup repeats and sensitivity to place-and-route seeds or tool settings where applicable [@Altera2026QuartusFitter]? Can we reliably distinguish the claimed gain from that measured variation at a relevant practical margin?
...
Our central review question regarding verification and trust applies universally to any submission (@sec-feedback-verification-trust).
...
Replay requires the exact recorded candidate, tool path, and behavior-affecting state. We use it to test that specific retained path, which is distinct from an independent reproduction derived from the stated specification.
...
While a declared budget limits the work before execution, the realized cost must honestly report the resources the study actually consumed.
...
- **Cost asymmetry:** Do the authors explicitly account for the economic imbalance between cheap, high-volume LLM generation and the astronomically expensive EDA verification required to validate those candidates?
...
Our external review process should clearly identify who qualifies each result, who integrates the system recommendation, and who holds the authority to approve a consequential design transition (@sec-what-architect-owns).
