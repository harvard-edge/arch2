...
![**Coordination sits at the hub every other role routes through:** generation, prediction, and optimization propose candidates and estimates; critique, verification, and repair challenge them; explanation traces mechanism. The coordination hub holds the shared state that keeps the roles honest, and the architect judges what the roles produce from outside the loop.](images/ch6_ai_roles){#fig-ai-method-roles width="100%" fig-alt="Hub-and-spoke diagram of eight AI method roles. A central coordination hub that orchestrates the roles and holds shared state connects to generation, prediction, and optimization on the proposer side, critique, verification, and repair on the reviewer side, and explanation below."}
...
Because of these structural differences, the choice of generative model must match the artifact geometry. For sequential text like RTL or compiler scripts, Large Language Models (LLMs) are used. To ensure syntactic legality and schema compliance, ML-Sys practitioners employ *Constrained Decoding* (using Finite State Machines [@WillardLouf2023GuidedGeneration] or context-free grammars [@GengEtAl2023GrammarConstrained] during token generation) to restrict the model's output to valid syntax by construction, solving the syntax hallucination problem. For spatial artifacts like macro placement, *Diffusion Models* are used [@LeeEtAl2024DiffusionPlacement].[^fn-ml-diffusion-c05] However, while constrained decoding can guarantee a generated RISC-V decoder is syntactically valid, it cannot guarantee it is physically realizable. Unlike analytical solvers that guarantee routability, a diffusion model's spatial layout is merely a statistical prediction and will blindly produce overlapping macros if not subsequently validated by a physical routing tool.
