...
Because of these structural differences, the choice of generative model must match the artifact geometry. For sequential text like RTL or compiler scripts, Large Language Models (LLMs) are used. To ensure syntactic legality and schema compliance, ML-Sys practitioners employ *Constrained Decoding* (using Finite State Machines or context-free grammars during token generation) to restrict the model's output to valid syntax by construction, solving the syntax hallucination problem. For spatial artifacts like macro placement, *Diffusion Models* are used.[^fn-ml-diffusion-c05] However, while constrained decoding can guarantee a generated RISC-V decoder is syntactically valid, it cannot guarantee it is physically realizable. Unlike analytical solvers that guarantee routability, a diffusion model's spatial layout is merely a statistical prediction and will blindly produce overlapping macros if not subsequently validated by a physical routing tool.
...
When the objective is a power, performance, and area vector rather than a single scalar, the fitting methods become multi-objective. Multi-objective Bayesian optimization[^fn-ml-parego-c05] and multi-objective evolutionary search such as NSGA-II and related dominance-sorting algorithms return a Pareto set rather than one best point. The architect then prunes that set against the Lighthouse power and performance frontier, since the 3\ W thermal budget rules out configurations a single scalar objective would have hidden.
