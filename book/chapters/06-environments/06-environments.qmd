...
We often forget that a tool command is not an environment. In Architecture 1.0, we interactively launched CAD tools, manually inspected log files, and adjusted scripts ad hoc. In Architecture 2.0, autonomous agentic loops generate candidate microarchitectures, synthesize gate-level netlists, and run cycle-accurate simulations asynchronously at machine scale. When we introduce AI-assisted methods, this operational gap becomes impossible to ignore, simply because automated loops dispatch tool invocations far faster than we can manually inspect the surrounding execution state for each attempt. Without an enclosing environment that enforces an immutable, state-isolated, typed boundary, Architecture 2.0 loops collapse into non-reproducible execution noise.

Suppose we want to evaluate a heterogeneous mobile XR SoC targeting a TSMC 3nm (N3) process node, balancing RISC-V RV64GCV application core execution against a $16 \times 16$ matrix-multiply NPU tile, dual LPDDR5X memory channels, UCIe chiplet interfaces, L2 cache capacity, and physical place-and-route constraints under a $3\,\mathrm{W}$ thermal design power (TDP) envelope. To evaluate candidate configurations, we use a quick analytical or SRAM model to filter candidates before we send the survivors into tightly budgeted rounds of cycle-level simulations, formal property checks, and physical synthesis runs. For this to work, our tools must reliably return physical estimates, matched workload results, warnings, run failures, and the actual resources spent. If a run loses track of a candidate's identity or mismatches our test conditions, it might hand us perfectly plausible numbers for the completely wrong comparison. Worse, if a tool silently retries without limits, we could quickly blow past our declared study budget.
...
When we dispatch a design request to a hardware toolchain, we rarely encounter a single monolithic component capable of taking our representation all the way to a verified architectural verdict. Instead, our execution path weaves through disparate compilers, simulators, formal provers, and synthesis flows, each maintaining its own assumptions and execution quirks. If we try to bundle tool invocation, state isolation, and result interpretation into a single ad hoc script, our evaluation pipeline quickly becomes impossible to debug or reproduce. To maintain clarity across these multi-stage workflows, we separate the responsibilities along this path into five distinct roles.

- A *tool* transforms an artifact or analyzes a stated property under our declared inputs and conditions. We consider compilers (such as LLVM, an open-source compiler infrastructure framework, and TVM, an open-source machine learning compiler stack), SRAM models, domain estimators (such as SCALE-Sim, a cycle-accurate NPU systolic array simulator, and Ramulator 2, a cycle-accurate DRAM memory system simulator), simulators (such as gem5, a cycle-accurate architectural simulator, Verilator, an open-source C++ RTL simulator, and Synopsys VCS, an enterprise event-driven Verilog simulator), RTL testbenches (such as Cocotb, a coroutine-based Python hardware verification environment), formal property engines (using SystemVerilog Assertions and Bounded Model Checking, SVA/BMC), synthesis tools (such as Yosys, an open-source RTL synthesis suite, and Xilinx Vivado, an FPGA synthesis and implementation toolset), and physical design flows (such as OpenROAD, an open-source physical design toolchain, and OpenSTA, an open-source static timing analyzer) to be tools.
...
Before submitting work, we must be able to inspect the candidate, fixed state, supported tool operations, resources, and prior attempts. The environment then accepts only our declared requests, returning the execution status, raw artifacts, parsed values, unclassified text, cost, and lineage. We structure this pipeline by isolating request validation, tool execution, parsing, and harness recording into dedicated stages, as illustrated in @fig-environment-tool-interface.
...
We connect our harness to a scheduler or execution backend using a *scheduler adapter*. This adapter maps our stable harness operations, like submitting a job, inspecting its status, and canceling it, onto a specific backend (such as Slurm, an open-source cluster workload manager, IBM Spectrum LSF, an enterprise workload scheduler, or cloud container schedulers). However, it does not replace our tool wrappers; we still rely on those wrappers to own the tool-specific validation, translation, invocation, and parsing.

At a minimum, we require that a caller can inspect backend capabilities and state, submit a typed request, receive an attempt identifier, inspect or cancel that attempt, and ultimately receive its raw and parsed returns, status, warnings, cost, and lineage. We structure these runtime object relationships and independent status attributes in @fig-runtime-object-state.
...
In @fig-runtime-object-state, we trace how an authorized request instantiates a single attempt record that mediates all interaction with our execution backends. Reading across the object structure, a single attempt record can correspond to zero backend jobs if admission fails, a single job for a straightforward simulation, or multiple execution segments for multi-stage physical-design flows. The attempt record acts as an immutable anchor, pointing directly to raw returned artifacts and realized compute costs while tracking six independent status properties: scheduling state, terminal cause, artifact completeness, parser status, cancellation confirmation, and retry parent link. For example, if a request fails admission, our harness still generates an attempt record even though no scheduler job exists. When we decouple object identity from state classification, we ensure that a terminal process state never falsely implies that artifacts are complete, nor does a failed job lose its realized cost accounting.
...
After the process stops, we ensure the very same attempt record retains the terminal cause, our expected-artifact check, the parser status, the raw and structured output, any retry eligibility, resource use, cost, and full lineage. When we keep these attributes distinctly separate, our runtime easily distinguishes a disastrous tool crash from a merely missing report or an outdated parser.
...
Every tool attempt must produce an immutable execution record detailing what ran, what configuration was passed, how the environment behaved, and what compute was spent. An unrecorded execution cannot justify an architectural claim.
:::
...
In @tbl-failure-taxonomy, we organize nine distinct execution outcomes across four structured columns: the Execution outcome class, How it appears during execution, What our harness preserves in the log record, and Our runtime response for recovery. Scanning down the rows, our taxonomy distinguishes operational faults like license exhaustion or syntax errors from architectural outcomes like formal proofs or timing violations. For instance, an Infrastructure failure (such as an unreachable license server) prompts a host retry without penalizing the candidate, whereas a Reported design-limit violation (such as negative slack in OpenROAD) produces a complete, schema-valid return that records the physical violation without marking the run as a tool failure. Our essential takeaway is that matching our runtime response to the exact failure category prevents transient infrastructure glitches from corrupting our candidate evaluation while ensuring that true physical design limits remain accurately recorded.
...
We organize these evaluation options along a *multi-fidelity spectrum*, establishing a structured continuum of tool execution paths ranging from fast analytical estimators to cycle-accurate simulators, emulator platforms, and physical signoff flows. We define this evaluation hierarchy in @tbl-multi-fidelity-spectrum to map what each tool class models and what state our environment must preserve to maintain evaluation integrity.
...
In @fig-chipyard-framework, we trace how a single root configuration moves through Chisel hardware generation and FIRRTL intermediate compiler passes before fanning out into three distinct tool execution branches. Following the top branch, our generated RTL enters software RTL simulators like Verilator or Synopsys VCS for cycle-exact functional verification. The middle branch routes the netlist into physical design flows like OpenROAD for place-and-route timing signoff. The bottom branch lowers the design into FireSim for FPGA-accelerated target simulation. While all three branches share an identical configuration parent, each tool path consumes different intermediate collateral and emits non-interchangeable evaluation artifacts. Our primary architectural takeaway is that shared configuration lineage guarantees parameter traceability across tool flows, but it does not make raw returns directly comparable across different fidelity tiers without explicit condition matching.

The choice of hardware evaluation environment establishes a fundamental trade-off between simulation throughput and microarchitectural cycle accuracy, as mapped in @fig-ch06-simulation-spectrum. In this plot, the horizontal axis measures simulation throughput in Kilo-Instructions Per Second ($\mathrm{KIPS}$) on a logarithmic scale spanning seven orders of magnitude, while the vertical axis tracks microarchitectural cycle error rate (where $0\%$ represents exact cycle fidelity). The physical size of each data bubble encodes the setup and build overhead required before execution can begin. Software RTL engines like Synopsys VCS and Verilator sit at the far left ($4.5\ \mathrm{KIPS}$ at $0\%$ cycle error), delivering exact RTL accuracy but requiring days of compute for long traces. Moving rightward, detailed timing simulators like gem5 increase throughput to $280\ \mathrm{KIPS}$ at a modest $4.2\%$ cycle error, while functional ISA emulators like Spike reach $48{,}000\ \mathrm{KIPS}$ at the cost of an $18.5\%$ error rate. FireSim FPGA acceleration occupies a key Pareto sweet spot along the front: running at $24{,}000\ \mathrm{KIPS}$ with $0\%$ cycle error, it delivers over $5{,}300\times$ the speed of software RTL simulators while requiring a $4\ \mathrm{hour}$ host FPGA build overhead. Our architectural takeaway is that no single platform optimizes throughput, accuracy, and setup latency simultaneously; we must dynamically route evaluation requests to the tier that satisfies our specific accuracy requirement within our compute budget.

```{python}
#| label: fig-ch06-simulation-spectrum
#| fig-cap: |
#|   **Simulation speedup versus microarchitectural fidelity spectrum across hardware verification environments.** Evaluation platforms exhibit a wide tradeoff between throughput and cycle accuracy, from slow software RTL simulators ($4.5\text{ KIPS}$) to fast functional ISA emulators ($48,000\text{ KIPS}$). FireSim FPGA acceleration provides a key Pareto sweet spot, executing cycle-exact RTL at $24,000\text{ KIPS}$ ($0\%$ cycle error), delivering over $5,300\times$ speedup compared to software RTL simulation.
#| out-width: "100%"
...
A wrapper, translator, or environment script must preserve exact architectural semantics across tool boundaries. System-level defaults, parameter mappings, or interface shims must never silently alter the candidate design being evaluated.
:::
...
Autonomous tools and agentic scripts must execute generation, simulation, and parsing actions strictly within sandboxed, resource-bounded environments. Execution wrappers and harnesses must enforce hermetic tool boundaries, explicit time and memory budgets, and complete lineage tracking, ensuring that infrastructure failures never pollute hardware evaluation records.
:::

In this chapter, we showed that tool-connected execution environments form the backbone of modern hardware evaluation. Without robust environment abstractions, automated architecture flows risk corrupting candidate state, misinterpreting tool crashes as design rejections, and losing track of evaluation lineage. When we wrap individual CAD, simulation, and synthesis tools and orchestrate them through isolated harnesses, we elevate raw tool outputs into authoritative, replayable architectural evidence.

Our environments must strictly separate prospective run specifications, which define what we authorize to execute, from immutable execution records, which record what actually ran and what compute was spent. While our infrastructure manages job scheduling, reset policies, and fault containment, it never unilaterally decides whether a return constitutes a valid measurement or a winning candidate. When we evaluate this phase of Architecture 2.0, four core takeaways govern our work:

- **Hermetic tool wrappers and contracts:** We must encapsulate each simulation or EDA tool within a typed wrapper that validates inputs, translates parameters without altering architectural intent, and parses outputs deterministically.
- **Strict runtime containment and budgets:** We must contain automated tool executions within isolated boundaries with explicit time, memory, and license limits, ensuring that infrastructure failures or preemption never masquerade as physical design defects.
- **Execution records as primary artifacts:** We must record complete, immutable lineage covering candidate state, exact tool versions, seeds, raw logs, parsed outputs, and realized compute costs for every attempt.
- **Separation of execution from evaluation:** Our harness records what ran and what returned, but it leaves measurement qualification, comparative validity, and candidate selection to downstream verification rules.

Our environment excels at establishing what we requested, executed, returned, and spent, preserving enough core identity for both replay and reproduction. However, it cannot qualify a measurement, validate a comparison, or recommend a candidate on its own. In @sec-feedback-verification-trust, we tackle this next crucial step, exploring exactly how these returned values and artifacts transform into genuinely useful feedback.
