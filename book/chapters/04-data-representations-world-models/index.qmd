...
Understanding the cost of a data sample matters because it dictates what an AI system can learn. A large asymmetry exists between the inference overhead of running an AI model, often measured in milliseconds and a few dollars per million tokens, and the cost of evaluating that AI's proposal through an electronic design automation (EDA) tool or RTL simulation, which can consume thousands of compute hours and large licensing fees. If an AI agent needs millions of trials to learn a reinforcement learning policy,[^fn-rl-policy-c04] it cannot use RTL simulation. RTL simulation captures exact cycle-by-cycle behavior, but its power figures are estimates that depend on activity annotation and a power model, and the numbers architects trust arrive later, post-synthesis with extracted parasitics. It is also slow and computationally expensive. High-fidelity simulations validate ground truth, but they are too costly to serve as the sole source of training data for data-hungry ML algorithms.
...
The representation problem extends beyond logic to physical realization. Power and timing bounds are codified in companion constraint files like UPF (Unified Power Format) and SDC (Synopsys Design Constraints), collateral that travels alongside the RTL. These constraints govern power domains and timing boundaries. In the Lighthouse SoC, for example, the need to cross clock domains between a high-frequency rendering pipeline and a low-frequency, always-on sensor hub requires strict synchronization logic governed by UPF and SDC. For AI models, they are uniquely hostile because of *topological coupling*. SDC and UPF rely on exact hierarchical paths and pin names (specific string references to internal RTL signals). Any structural mutation by the AI, such as flattening a hierarchy or renaming a buffer to improve performance, requires a synchronized co-mutation of the constraint files, otherwise the EDA tool will instantly fail with "path not found" errors. An agent must co-generate these formats to operate within a realistic physical envelope, transforming an abstract concept into a manufacturable netlist (@tbl-representation-formats).
...
Whether the predictor is a fast analytical proxy or a deep learned surrogate, what matters next is what it has actually learned. A common flaw in many early predictive surrogates is that they fall into the correlative trap. A black-box surrogate might learn a correlation (for example, that higher chip area often equals higher power) through curve-fitting on its training data, without understanding the underlying physics.
...
A later simulator execution tests the prediction and returns a high-fidelity observation. Detecting when a candidate has left the calibrated region is the job of uncertainty quantification,[^fn-uncertainty-quantification-c04] through deep ensembles [@LakshminarayananEtAl2017DeepEnsembles], Gaussian-process predictive variance, or conformal prediction [@AngelopoulosBates2021Conformal]. The same confidence signal also drives the acquisition function that decides when a ground-truth simulation is worth its cost. @sec-methods-generation-prediction-optimization develops that prediction machinery in full. The execution history must keep the AI's prediction and the simulator's observation distinct to catch these failures. Predicting one consequence reliably is already hard. Predicting a next design state faithfully enough to plan several moves ahead remains largely out of reach, so this chapter treats it as a target rather than a tool.
