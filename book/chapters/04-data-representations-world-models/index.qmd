...
: **Data generation cost rises sharply with fidelity.** Emulation and silicon evaluate a design far faster per run than RTL simulation, but bitstream compilation, board setup, and non-recurring engineering (NRE) make acquiring the first sample expensive. Note that FPGA power and thermal numbers do not transfer to the ASIC, so an emulation sample teaches functional and timing behavior but not physical dissipation. {#tbl-synthetic-data-costs tbl-colwidths="[20,20,20,20,20]"}
