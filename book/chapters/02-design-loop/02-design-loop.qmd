...
- **Carry forward:** A project stalls in one of five ways, and the remedy
  follows from that mode rather than from the layer that produced it. Match AI's
  role to the mode, keep the check it must not weaken independent of the model,
  and remember that a project's limit is how much it can evaluate well enough to
  decide, not how many candidates it can enumerate. A hardware claim is worth no
  more than the software path that realizes it and the workload snapshot it was
  measured against, so both are versioned inputs and the comparison is re-run
  when either turns over.
- **Reader test:** Can you name the measurements that would establish which of
  the five failures limits a project, say why none of them is collected today,
  name the software version and workload snapshot behind your last hardware
  comparison, and state what evidence would show that AI reduced the burden
  rather than shifted it elsewhere?
