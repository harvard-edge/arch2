...
AI can accelerate parts of that work. An automated optimizer can collect design information, propose alternatives, and predict PPA (Power, Performance, Area) costs, serving as an explorer and searcher. The first task is to identify what is actually slowing the work, because faster search helps only where search was the constraint.
...
- Refuse to read candidate count or generation speed as progress, and demand the arrival, review, and rejection measurements a bottleneck claim requires.
...
- Match a method to the failure mode it addresses, and name the check that must stay independent of the model.
...
Because the five problems in @tbl-loop-bottleneck-diagnostic differ, there is no
generic place to insert AI. A generator may broaden the available choices while
overloading evaluation. A surrogate may reduce tool cost while preserving a
blind spot in its inputs. A system that organizes evidence may save reviewer
time without deciding what action the evidence supports. Each problem instead
admits a defined job, a named piece of engineering work that job should reduce,
and a check the architect still owns (@tbl-ai-contributions-by-bound).
...
No single method supplies all of those conditions at once, which is why the
architect still holds the question and its scope, the inspectable artifacts and
tool outputs, the claim tied to its evidence, and named responsibility for the
decision.
...
Several questions from this chapter stay open. The chapter's method asks an architect to match AI to whichever bottleneck limits a project, and the instrument that would identify that bottleneck for a given project does not exist yet.

**Theme 1: Diagnosing the Binding Bottleneck.** This chapter names five ways a loop stalls, and industry-wide trends cannot tell one project which of them binds it. Nothing measured today separates a project starved of information from one that cannot afford its evaluations or cannot clear its review queue, so candidate-arrival rates, review throughput, and recorded rejection reasons are still unmeasured quantities rather than diagnostics. Two reasons keep them that way. Instrumenting the loop well enough to find the binding constraint spends the very capacity the measurement is trying to protect, so the diagnosis competes with the work. And rejection reasons are the cheapest of the three to record and the least often kept, because a rejected candidate leaves no artifact anyone is accountable for.

- What experiment would establish that human review capacity, rather than tool cost or missing information, is the limiting constraint on a project?
...
- How can a cluster-scale proposal be judged against congestion and tail latency when cycle-accurate simulation of the fleet is out of reach and analytical models miss the dynamics that decide the outcome?

**Theme 3: Comparisons That Expire.** Software turns over faster than the silicon built to run it, so a comparison can go stale between commitment and deployment. Detecting that a trace, compiler version, or model family has changed is mechanical, and deciding that the change overturns the architecture decision is the unsolved part.
