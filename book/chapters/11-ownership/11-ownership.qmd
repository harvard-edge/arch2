...
1. **Workload.** Real-time spatial tracking and neural rendering tasks from XRBench (an extended-reality benchmark suite), CPU application benchmarks from SPEC CPU2026 (an industry-standard CPU benchmark suite), and mobile inference workloads from MLPerf (an ML benchmark suite).
...
| **Missing constraints** | What has our formulation, representation, or tool path left out? | XRBench, SPEC CPU2026, and MLPerf workloads, software interface cost, 3\ W TDP subsystem power, UCIe signal integrity, physical timing closure, IP indemnification, NRE tapeout ROI, and hardware containment remain open obligations. |
...
The horizontal axis tracks advancing technology nodes, the left vertical log scale measures front-end design labor cost, and the right vertical log scale measures the fixed reticle mask set charge. Both curves are constructed to make one structural contrast visible, so read their shapes rather than their values. Front-end effort is the cost a change in methodology can act on, whether that change is a better tool, a larger team, or an automated search loop. The mask set is not. It is a fixed charge that falls due when we commit a design to fabrication, and it is indifferent to how that design was produced.

The consequence for us as architects is a gatekeeping obligation that automation cannot relieve. Suppose an assisted workflow did compress front-end effort substantially. The mask charge would still arrive unchanged at tapeout, so the saving would show up as a larger share of program capital riding on a single commitment decision rather than as a smaller bill. Whether such compression happens at all is exactly the question this book leaves open in @sec-evaluating-agentic-architect, and nothing in this figure settles it. What the figure does establish is that cheaper candidate production raises rather than lowers the evidentiary bar before a lead architect and commitment authority authorize a tapeout freeze for an RV64GCV host core and NPU tensor accelerator.
