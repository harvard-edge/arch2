...
- Classify a stalled design loop as missing information, invalid choices, unaffordable evaluation, overloaded review, or an undefined decision.
- Separate the size of a design space from the evaluation a project can afford, and refuse to read candidate count as progress.
- Demand the arrival, review, and rejection measurements a bottleneck claim requires before accepting that faster generation helped.
...
::: {.callout-failure-mode title="The silent SoC deadlock"}
**The trap.** Independently verified IP blocks can be seamlessly integrated into a working System-on-Chip.

**The mechanism.** The following is a representative composite of multi-vendor IP integration failures rather than one named incident. Two blocks with perfectly verified individual behavior are combined on the same interconnect, and a mismatched Network-on-Chip (NoC) credit-return protocol between them creates an unresolvable cycle of dependencies.

**The lesson.** An undocumented interface quirk of this kind causes a silent system hang that eludes simulation and can take months to debug in post-silicon bring-up, which is why system-level deadlocks often emerge only when the full SoC is assembled.
:::
...
**The lesson.** To stay inside power and voltage guardbands, thermal and delivery alike, early AVX-512 implementations downclocked whenever these wide units were active. Code dominated by AVX-512 still gained from the wider datapath, but the lowered frequency penalized the mixed workloads that shared the same core, which is where much of the promised gain leaked away.
:::
...
Design Rule Checks (DRC) are the wall's final course. At advanced nodes the rule decks run to thousands of geometrical rules (multi-patterning coloring, via enclosure, minimum-metal spacing), with exact counts held foundry-confidential, and a floorplanner that shifts one macro block can instantly create thousands of discrete violations. If the automated loop cannot parse these spatial markers and fix them, the output requires manual cleanup.

::: {.callout-field-note title="The DRC whack-a-mole"}
**The expectation.** A minor late-stage macro block shift to improve timing should be a quick fix.

**In practice.** This is a representative advanced-node scenario rather than one named tapeout. A physical design team resolving a single via-enclosure violation finds that the layout tool ripples the change outward, instantly spawning three new violations in adjacent polygons.

**The lesson.** A team can become stuck in a multi-week loop of fixing one error only to spawn three more, which illustrates why human-guided feedback loops break at this scale.
:::
...
- **Carry forward.** A project stalls in one of five ways, and the remedy
  ...
  decide, not how many candidates it can enumerate.
- **Reader test.** Can you say which of the five failures limits a project, name
  the measurements that would prove it is the binding one, and state what
  evidence would show that AI reduced the burden rather than shifted it
  elsewhere?
