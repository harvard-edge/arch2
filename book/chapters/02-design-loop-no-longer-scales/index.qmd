...
Learned methods have optimized real warehouse-scale mechanisms. Reconfigurable optical interconnect in TPU v4 pods [@JouppiEtAl2023TPUv4] and learned cluster scheduling [@MaoEtAl2016DeepRM] both replaced hand-built heuristics inside bounded, well-instrumented subsystems, and that bounding is the lesson. Each win sat behind an operational check, traffic telemetry or scheduler rollback, that could reject the learned policy in production. Treating the whole warehouse as one co-designed target still awaits evidence at that scale.
