...
We must also face the final physical constraints: Design Rule Checks (DRC). At advanced nodes, the rule decks run to thousands of geometrical rules (multi-patterning coloring, via enclosure, min-metal spacing), with exact counts held foundry-confidential. If a floorplanner shifts a macro block, it might instantly create thousands of discrete DRC violations. If the automated loop cannot parse these spatial markers and fix them, the output requires manual cleanup.
