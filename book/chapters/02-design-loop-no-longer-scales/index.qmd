...
governed more of the design, the mechanism by which the field has repeatedly
closed a widening gap between what can be fabricated and what a team can design,
a gap with the same scissors shape @fig-scissors-gap shows for today's loop. The shape is two diverging lines, what the process can build pulling away from what a team can design and verify, and it has a measured ancestor this chapter returns to later.
...
Those precedents describe how the field made design work explicit over time. The pressure that now makes explicitness unavoidable is better read across the stack than across the decades, so the rest of this chapter re-sorts the same material by layer, and the layers introduced here are the ones the next three sections work through in turn.
...
![**Specialization becomes a fourth axis coupled to every layer:** The older TAO stack layers optimization over architecture over technology, with technology at the bottom setting the terms for the layers above, while the newer TAOS view adds a Specialization dimension bound to optimization, architecture, and technology alike and encloses the whole stack in the physical constraints that scaling once absorbed.](images/ch2_tao_vs_taos){#fig-tao-vs-taos width="100%" fig-alt="Two side-by-side stack diagrams. On the left, the TAO stack lists Optimization, Architecture, and Technology in a vertical column with technology at the base. On the right, the TAOS view keeps the same three layers but adds a Specialization panel joined by double-headed arrows to every layer, and a red dashed boundary labeled tightly coupled physical constraints encloses the entire stack."}
...
Each lever adds its own information to the comparison. The sections that
follow do not take one lever each, because the levers do not separate that
cleanly in practice. Scaling and optimization run through all of the others
rather than standing alone, so what the chapter walks is the places where the
comparison gets harder. Microarchitecture and multicore execution add
...
the wrong candidate. The information a fair comparison must carry accumulates as
the design moves from device scaling toward specialization and system
...
AI can help screen this space by ranking microarchitectural configurations with surrogate models trained on high-fidelity data. What the ranking cannot do is certify itself. Cheap proxies can hide coherence traffic and speculative-energy costs, so a ranked shortlist earns review time only after a cycle-accurate check confirms the surrogate's ordering on the cases that matter. The open engineering problem is training these surrogates on the right corner cases so that microarchitectural risks surface early rather than in silicon.
...
diverge as two lines (@fig-scissors-gap). The upper line rises as the
...
transistors. That history sets the bar for what follows in this book. An
AI-assisted loop earns its place the way synthesis did, when its checks are as
trustworthy as the manual work it replaced.
...
For each of the five problems in @tbl-loop-bottleneck-diagnostic, AI takes on a
defined job, reduces a named piece of engineering work, and still leaves a check
the architect must own (@tbl-ai-contributions-by-bound). The second column gives AI a specific job, the
...
No single method supplies all of these conditions. The architect needs a
...
These pressures do not share a single remedy. @tbl-loop-bottleneck-diagnostic separates the five ways architecture work stalls, from missing information to an unclear decision, and @tbl-ai-contributions-by-bound pairs each one with the engineering work a method would have to reduce and the check that must stay independent of the model. The rule is to match AI to whichever bottleneck is limiting a given project.

Producing more candidates does not help if the checks remain unaffordable or the next action stays unclear. Identify the limiting pressure first, then judge any method by whether it reduced that specific work without moving the burden to a less visible part of the project.
