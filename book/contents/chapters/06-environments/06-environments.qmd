...
A single root Lighthouse intent branches into two parallel transformation paths (@fig-artifact-lineage): a software variant that compiles through an identified toolchain into a specific executable binary, and a hardware variant that elaborates into a uniquely identified RTL artifact. Our evaluation instance binds this compiled binary and elaborated RTL artifact together under a single evaluation key that incorporates tool versions, workload snapshots, and operating conditions. If we alter either child artifact or modify an operating condition, we generate a brand-new evaluation identity even if our high-level intent remains unchanged. Cryptographic hashes uniquely identify individual files, but only a complete evaluation key preserves the parent-child lineage necessary to guarantee reproducible hardware-software co-design.
