...
**The lesson.** Whirlwind's operators could not make a run succeed by hoping the storage held; they changed the component whose failure rate set the limit. Our position is the same one step removed. Mean time between failures is an expectation rather than a hard upper bound, so as uncheckpointed execution time grows relative to MTBF, the expected recovery loss from host failure or preemption grows with it. Long-running synthesis and emulation therefore need checkpoint intervals that balance recovery exposure against checkpoint overhead, plus retained attempt records that distinguish infrastructure loss from an architectural result.
:::
...
The choice of hardware evaluation environment establishes a trade-off among throughput, setup cost, and the kind of timing behavior a platform represents (@fig-ch06-simulation-spectrum). The horizontal positions and bubble sizes are constructed to make that operational trade-off visible. The vertical positions are categories, not a percentage error or a ranking of physical fidelity.
