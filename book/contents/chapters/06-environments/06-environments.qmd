...
An environment is what makes a tool run mean something. The questions below concern the parts of that job that current harnesses do not do.

**Telling a broken run from an illegal design.** Both arrive as a nonzero exit code, and they call for opposite responses.

- **How can an environment separate a tool that failed to converge from a design that was never legal?** Solvers and physical-design engines abort for numerical reasons that say nothing about the candidate, and treating those aborts as evidence removes valid regions from the search. Automatic classification of tool returns is a prerequisite for trusting any large sweep, and it currently depends on human pattern recognition over log text.

**Keeping meaning stable as intent crosses tools.** Every handoff between abstractions is a translation, and translations lose things quietly.

- **How can an environment check that a tool contract preserved what the architecture actually specified?** A parameter can survive a translation syntactically while changing what it constrains, and the resulting number is reported with full confidence. Verifying semantic preservation across a chain of tools, rather than checking that each tool ran, is open.
- **How can two results that look identical be exposed as measuring different things?** Tools at different fidelities report metrics under the same names while modeling different structures, and a study that merges them inherits a distinction it never recorded. Making that disparity visible automatically would prevent a class of comparison error that is currently caught by luck.

**Reusing a stateful tool without inheriting its last run.** Long-running sessions carry caches, working directories, and internal state across candidates.

- **What establishes that a tool session actually reset, short of tearing it down?** Full teardown is often too slow for a large sweep, and partial reset leaves contamination that surfaces as an unexplained result much later. A practical reset check would make session reuse safe rather than merely fast.
