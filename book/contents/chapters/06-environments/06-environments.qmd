...
When we enforce this narrow rule, we prevent a common failure mode. Often, an automated repair will make a tool call legal by silently altering the very design we set out to evaluate. The tool then runs cleanly and reports success, but we end up with results that belong to a different candidate. Our request validation steps should catch these mismatches before we burn any expensive cluster time. To perform this exact validation, we first need an unambiguously identifiable starting state.
