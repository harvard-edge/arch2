...
2. **On-premises and cloud NDA air-gap architecture:** Commercial process design kits (e.g., TSMC N3E, Intel 18A) operate under strict foundry non-disclosure agreements (NDAs) that strictly prohibit uploading Graphic Database System II (GDSII) streams, Library Exchange Format (LEF) / Design Exchange Format (DEF) layout views, or transistor-level SPICE (Simulation Program with Integrated Circuit Emphasis) models to third-party cloud AI endpoints. AI-native design systems bridge this boundary via an *air-gapped proxy architecture*; high-level prompts, abstract syntax tree (AST) rewrites, and anonymized feature vectors communicate with external foundation models over encrypted gRPC (gRPC Remote Procedure Calls) channels, while all PDK-dependent synthesis, physical layout, and signoff design rule check (DRC) and layout versus schematic (LVS) runs execute strictly on-premises inside isolated, air-gapped compute enclaves.
...
#|   Tool categories span six orders of magnitude in effective execution rate (from $10^2$ cycles/s for gate-level
#|   timing simulation to $10^8$ cycles/s for FPGA emulation and analytical models), while setup latency scales
#|   inversely. Physical design rates reflect toggle-driven dynamic power analysis and timing extraction throughput. Open-source flows (Icarus, Verilator, OpenROAD) provide rapid-setup evaluation environments,
#|   while commercial EDA suites and enterprise emulation platforms demand multi-day setup in exchange for high throughput
