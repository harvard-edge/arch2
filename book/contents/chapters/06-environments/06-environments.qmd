...
Eight hardware evaluation tiers span the fidelity range our environments must serve, and each tier obligates the environment to preserve different state (@tbl-multi-fidelity-spectrum). The pipeline spans fast analytical estimators (such as CACTI and SCALE-Sim), software compilers, functional ISA emulators, cycle-accurate timing simulators (gem5 and Ramulator 2), RTL verification testbenches, formal property checkers, FPGA emulators (FireSim), and physical implementation signoff flows (OpenROAD and Vivado). Higher fidelity consistently buys narrower model coverage and heavier state retention requirements. For example, while an analytical SRAM model requires preserving only technology parameters and basic circuit equations, an OpenROAD physical implementation flow requires retaining intermediate netlists, UPF power intents, Liberty library checksums, and signoff corner constraints. Our key takeaway is that fidelity is property-specific. No single tool models all microarchitectural and physical phenomena, so our environment must explicitly preserve each tool's unique execution context rather than collapsing diverse returns into a single scalar score.
