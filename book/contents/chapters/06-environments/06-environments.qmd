...
This is exactly why we build an enclosing environment to handle these messy mechanics. As the third technical building block of Architecture 2.0, tool-connected execution environments provide the containerized execution harnesses that insulate automated loops from raw CAD toolchain complexity. A robust environment proves that our requested work hit the specified tool in a known, named state, and it records what that attempt produced. However, the environment does not guarantee that the resulting metric is scientifically valid, nor does it tell us if the candidate is good. We must maintain this distinction. Executing a well-formed run is not the same as supporting an architectural claim. To interpret the data and judge the claim itself, we need targeted verification feedback in @sec-feedback-verification-trust that sits well outside the environment's responsibilities.
...
Our harness attaches every job or segment to its attempt right at submission, well before a late result could arrive. We keep our dependencies explicit. For instance, our compiler builds might wait for a source artifact, our RTL simulations in Verilator or VCS might wait for elaboration, and our physical implementations in OpenROAD or Vivado might wait for a mapped netlist from Yosys. We can run independent jobs concurrently, but we never let that concurrency relax their strict identity or resource limits. Because the scheduler might accept work before our harness records the job identifier, we explicitly tag our submitted work with the attempt identifier and reconcile our records with the scheduler before ever resubmitting. We rely on an idempotency key, a unique request identifier whose durable backend handling prevents a repeated submission from accidentally launching the same work twice. In our system, `submission_token` serves exactly this role. The backend records this key alongside the first admission result, ensuring that any later submission bearing the same key identifies the existing attempt or job. We design our harness to retain this key throughout the retry window, reconcile it cleanly after a crash, and outright reject its reuse for any different request [@Featonby2021IdempotentAPIs].
