...
- **What would show that a tool contract preserved the thing the architecture actually specified?** A parameter can survive a translation syntactically while changing what it constrains, and the resulting number is reported with full confidence. Verifying semantic preservation across a chain of tools, rather than checking that each tool ran, is open.
- **When two results look identical, what exposes them as measurements of different things?** Tools at different fidelities report metrics under the same names while modeling different structures, and a study that merges them inherits a distinction it never recorded. Making that disparity visible automatically would prevent a class of comparison error that is currently caught by luck.
