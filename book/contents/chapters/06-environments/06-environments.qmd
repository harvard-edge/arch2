...
::: {.column-margin}
**Author's Note.** John Culkin, an American media scholar explaining Marshall McLuhan's ideas, originally used this line to describe how our tools reshape our own practices. In computer architecture, a tool similarly defines what a given method can tweak and what we can actually observe. If an automated method drives that tool, we must explicitly declare those limits.
:::
...
A tool command is not an environment. An architect can launch a simulator, compiler, formal engine, or physical-design flow and still be unable to say which candidate ran, which state the tool inherited, what an unfamiliar warning meant, or whether a retry repeated the same work. Traditional workflows absorbed that gap interactively; we launched CAD tools by hand, inspected log files, and adjusted scripts as we went. AI-native loops remove that human buffer. Automated search, generation, and surrogate methods, such as those established in @sec-methods-generation-prediction-optimization, dispatch tool invocations far faster than we can inspect the surrounding execution state for each attempt, and without an enclosing environment that enforces a state-isolated, typed boundary, the loop collapses into non-reproducible execution noise.
...
- A simulator is a domain-specific category of tool that models hardware and software behavior, returning only the behavior its model exposes. It occupies the tool tier rather than serving as an enclosing environment.
- A wrapper surrounds a single tool. It validates and translates our request, invokes the tool, and houses the parser that interprets returned artifacts.
...
Building this environment is challenging because architecture tools often inherit state that a request never names and return far more material than the caller asked for.
...
A 3\ MiB cache, $16 \times 16$ NPU array, and dual LPDDR5X memory channel candidate becomes a single SRAM and NPU analytical model input under declared process, voltage, and temperature conditions. If predeclared routing conditions permit, it also becomes a gem5 and Verilator simulation configuration linked to a specific NPU model, memory traffic generators, XR workload trace, and baseline software image. While the wrapper has the freedom to adjust file syntax, paths, and command-line spelling, it is forbidden from changing the cache policy, NPU systolic topology, memory controller arbitration, compiler runtime, workload, or any other architectural property. If the translation remains exact, execution proceeds; any mismatch is refused and recorded.
:::

This narrow rule prevents a common failure mode in which an automated repair makes a tool call legal by silently altering the very design we set out to evaluate, so the tool reports success on results that belong to a different candidate. Our request validation catches these mismatches before we burn expensive cluster time. To perform this exact validation, we first need an unambiguously identifiable starting state.
...
When we introduce larger hardware and software changes, downstream evaluation artifacts must explicitly inherit their identity from a shared parent intent (@fig-artifact-lineage). A single root intent might branch out to produce an array of distinct software binaries and RTL artifacts.
...
To build a usable interface across a heterogeneous XR SoC design flow, we construct dedicated wrappers tailored to specific tool paradigms. We wrap cycle-level architectural simulators (gem5) to extract IPC and contention metrics [@BinkertEtAl2011Gem5]. Wrappers for RTL simulators (Verilator) and verification harnesses (Cocotb) compile models and parse waveform traces [@Veripool2026Verilator]. We wrap formal engines (such as SymbiYosys, an open-source formal verification front-end, and JasperGold, an enterprise formal verification platform) to serialize SystemVerilog Assertions (SVA), enforce Bounded Model Checking (BMC) cycle bounds ($k$-induction), and parse return proof certificates or counterexample Value Change Dump (VCD) waveform traces [@Clarke2018ModelChecking]. For implementation, we wrap synthesis and physical design flows (Yosys, OpenROAD, Synopsys Design Compiler, Cadence Innovus) to translate constraints into stateful executions and parse slack signoffs [@Yosys; @AjayiEtAl2019OpenROAD; @OpenROADDocs]. Finally, we wrap FPGA-accelerated simulators (FireSim, an open-source FPGA-accelerated simulation platform) to manage hardware targets and parse high-throughput traces [@KarandikarEtAl2018FireSim].

These wrappers operate under a uniform architectural interface regardless of whether the underlying engine is open-source (such as Verilator, Yosys, and OpenROAD) or proprietary commercial EDA (such as Synopsys VCS, Design Compiler, and PrimeTime, or Cadence Innovus and JasperGold). Open-source toolchains provide accessible, pre-competitive calibration environments for methodology research and public benchmark verification without commercial licensing barriers, while commercial engines slot into the exact same harness contracts and typed schemas under enterprise NDAs when physical signoff on leading-edge foundry nodes is required.
...
A familiar reinforcement-learning environment presents a synchronous `step()` interface that returns an observation, reward, termination flags, and auxiliary information [@TowersEtAl2023Gymnasium]. We generally cannot reduce architecture execution to such a simple exchange. Our runtime has to preserve the complex queueing, dependency, persistent state, and ordering behaviors we just described, along with the typed tool returns. When we actually need a reward, our methods can derive one afterward from this rich execution data. A scheduler adapter maps our stable harness operations, like submitting a job, inspecting its status, and canceling it, onto a specific backend (such as Slurm, an open-source cluster workload manager, IBM Spectrum LSF, an enterprise workload scheduler, or cloud container schedulers). However, it does not replace our tool wrappers; we still rely on those wrappers to own the tool-specific validation, translation, invocation, and parsing.
...
| **Parser failure** | A retained artifact does not match the declared schema or contains unclassified output. | Parser version, raw artifact, diagnostics, recognized elements, and residual output. | Repair the parser and reparse the retained artifact when safe to do so. |
| **Formal-tool return** | The declared method and tool (such as an SVA/BMC checker) report their exact outcome, which may include a proof, counterexample VCD trace, bounded result, `unknown`, or resource limit. | Method, property or relation, model, assumptions, bound where applicable, tool, exact outcome, artifact, and cost. | Pass the method-specific result forward without turning it into a general correctness verdict. |
| **Reported design-limit violation** | A completed run reports a timing (such as OpenROAD WNS $< 0$), power, congestion, design-rule, or functional violation. | Typed violation, units, candidate, conditions, warnings, raw artifact, and cost. | Record a complete return without rejecting or penalizing the candidate. |
| **Complete tool return** | Expected artifacts exist and parse successfully, including reports that contain violations. | Typed data, units, conditions, warnings, raw artifacts, cost, and lineage. | Retain the return for measurement qualification without deciding what it establishes. |
: **Execution status determines what ran, not what the architecture means.** Invalid requests, failed paths, partial returns, parser failures, and complete returns remain strictly separate so subsequent repairs target the precise problem. {#tbl-failure-taxonomy}
...
**The mechanism.** The timing analyzer silently loads an older library view from a cached prior process corner. The stale view lacks the setup and hold characterization for the accelerated path, so nothing fails, because the failing checks were never loaded. This example represents an illustrative composite; misapplied library data is a standing hazard class that modern signoff flows guard against with explicit view validation.
...
**The breakthrough:** Jay Forrester's MIT group described three-dimensional magnetic core storage in 1951 [@Forrester1951DigitalInformationStorage]. Whirlwind's electrostatic storage tubes were replaced with core memory in 1953, and the machine's reliability changed character rather than merely improving.
...
**The lesson.** Whirlwind's operators could not make a run succeed by hoping the storage held; they changed the component whose failure rate set the limit. Modern execution harnesses occupy a parallel position one step removed. Mean time between failures is an expectation rather than a hard upper bound, so as uncheckpointed execution time grows relative to MTBF, the expected recovery loss from host failure or preemption grows with it. Long-running synthesis and emulation therefore need checkpoint intervals that balance recovery exposure against checkpoint overhead, plus retained attempt records that distinguish infrastructure loss from an architectural result.
:::
...
The choice of hardware evaluation environment establishes an operational trade-off among throughput, setup cost, and timing abstraction (@fig-ch06-simulation-spectrum). Constructed horizontal positions and bubble sizes make relative throughput and setup regimes visible, while the vertical axis separates distinct timing categories rather than ranking physical fidelity.
...
When we construct a dependency graph, we clearly identify which branches can run concurrently and which must wait for an input. Modern workflow systems like Nextflow, a data-driven workflow management system, and Snakemake, a Python-based workflow automation tool, show us how to separate our workflow logic from the actual execution happening on local machines or computing clusters [@DiTommasoEtAl2017Nextflow; @KosterEtAl2012Snakemake]. As we construct our broader design workflow, our run specification locks in the authorized work, while our harness coordinates everything through a scheduler adapter and backend. In this setup, Nextflow- or Snakemake-style workflow logic merely expresses execution dependencies, it never takes ownership of our design ordering, methodological decisions, results interpretation, or human review processes. Our scheduler might plan the authorized work, but it cannot unilaterally add a new tool, skip a required branch, or change our chosen fidelity level without us issuing a brand new request.
...
Applying environment rules, typed contracts, and multi-fidelity pipelines to concrete engineering constraints requires a representative evaluation framework. We evaluate a mobile XR SoC co-design study targeting a TSMC N7 or 3\ nm-class process node. When evaluating an extended reality subsystem, we balance tightly coupled architectural trade-offs as we size a $16 \times 16$ NPU matrix array, configure dual LPDDR5X memory channels, integrate UCIe chiplet links, specify UPF power domains, tune L2 cache capacity, and enforce gate-level physical synthesis boundaries, all while respecting a strict 3\ W thermal design power (TDP) envelope. A multi-stage run specification manages state isolation, screening dispatch, failure containment, and lineage tracking across heterogeneous tools.
...
Before we invoke any backend tools, a prospective run specification formalizes runtime governance across state pinning, dispatch ordering, failure containment, and lineage retention (@tbl-environment-contract).

| **Run category** | **Prospective XR SoC co-design study run specification** |
| --- | --- |
| **Candidates and fixed state** | Candidate XR SoC configurations (varying `l2.capacity` between 2\ MiB and 4\ MiB, $16 \times 16$ NPU array dimensions, and memory traffic arbitration) bind to the run specification and study revision. Only declared parameters vary. Octa-core RISC-V RV64GCV CPU cores, NoC interconnect, baseline software image, and XR workload trace remain fixed. Each candidate retains its complete SRAM, NPU tile, LPDDR5X memory controller, UPF power domain, and TSMC 3\ nm technology identity. |
| **Translation and identity** | Declared parameters serialize exactly into analytical screening models (SRAM and SCALE-Sim) under declared process, voltage, and temperature (PVT) conditions, and into eligible gem5/Ramulator 2/Verilator simulator requests using declared UPF power models. Every behavior-affecting schema, wrapper, parser, tool, model, flag, host or container, seed, and reset state retains explicit identity. |
| **Order and dispatch** | Screening evaluation runs once per candidate before any dependent cycle-level work. Cycle-level gem5/Ramulator 2/Verilator simulations dispatch only when screening returns are complete, schemas are valid, combined area is at most 1.5\ mm^2^, and access latency is at most 2.5\ ns. |
| **Matched work and limits** | The shared baseline and every eligible alternative run under matched conditions. Budget permits three screening evaluations and at most four cycle-level attempts, including recovery retries. Queue-wait, run-time, and absolute deadlines are enforced, stopping when authorized work completes, no alternative qualifies, or a budget expires. |
| **Failures and recovery** | Invalid requests, timeouts, cancellations, infrastructure failures, stale state, missing outputs, and parser failures remain strictly distinct. Each recovery retry links to its parent, charges against budget, and regenerates a clean working directory from fixed state. |
| **Containment and reuse** | Access grants only to declared tools, paths, credentials, networks, and resources. Output quarantines after any unconfirmed termination. A complete return reuses only when request, candidate, conditions, translated inputs, tools, wrappers, parsers, and retained artifacts match perfectly. |
| **Returns, cost, and lineage** | Retains SRAM/NPU area, access time, energy, and leakage; matched frame records, LPDDR5X memory traffic inputs, NPU MAC utilization, subsystem power, and warnings; translated inputs, commands, raw logs, parsed outputs, residual artifacts, hashes, resource use, cost, and parent-child lineage. |
| **Decisions left outside** | The environment does not decide whether a returned value is a valid measurement, whether two returns form a valid comparison, whether a reported limit establishes rejection, or whether a candidate should advance. |
: **This prospective specification illustrates runtime responsibilities; it is not an executed study record.** The request, translation, state, expected outputs, failure behavior, cost, and limits are fixed prior to invocation. {#tbl-environment-contract}

The run specification in @tbl-environment-contract enforces these boundaries for the prospective mobile XR study. For example, under the Order and dispatch row, we set up a rule that authorizes a cycle-level attempt only after complete, schema-valid screening returns meet designated area and latency thresholds. This condition neither validates a measurement nor rejects a candidate on its own. When we codify these eight categories before calling a tool, our environment ensures that every attempt runs under strict budgets and isolated directories while leaving architectural evaluation to downstream analysis.

Our chronology starts with three screening runs. The 2\ MiB rerun baseline, alongside our 3\ MiB and 4\ MiB alternatives, would return combined subsystem areas of 1.2\ mm^2^, 1.4\ mm^2^, and 1.7\ mm^2^ and access times of 2.1\ ns, 2.4\ ns, and 2.8\ ns, respectively. While the first two candidates meet both of our declared thresholds, the 4\ MiB candidate falls short. Consequently, our run specification blocks any dependent cycle-level dispatch for it. The values alone would not establish that decision without our verified parse status and recorded dispatch outcome.

Our eligible candidates then enter matched cycle-level runs under the four-attempt budget. The baseline would return a combined subsystem dynamic and leakage power of 2.90\ W. If our first 3\ MiB attempt encounters a stale-state setup failure, the harness authorizes a controlled retry from a clean reset, followed by a separate planned repeat. These attempts exhaust our budget. Although the recovery retry and planned repeat would return frame, cache, NPU MAC utilization, and power metrics, their specific values remain unreported here. Any summary we derive from them would constitute a new artifact, rather than another tool call.
...
These are failures of the execution environment rather than of the design under study. They share the property that makes them dangerous inside an automated loop. The tool exits, a number appears, and nothing in the result records that the run was not the run anyone intended.

- **Silent drifts in gem5 architectural simulation configurations:** When executing cycle-accurate architectural studies in gem5, a full-system architectural simulator, subtle default parameter changes across minor version revisions or unverified Ruby memory hierarchy flags can corrupt execution lineage.[^fn-nix-hermeticity-c06] Automated flows that fail to lock exact gem5 build hashes and seed states produce non-reproducible cache latency and memory contention measurements.

[^fn-nix-hermeticity-c06]: **Hermetic package management**: In software engineering, pure functional package management and hermetic build graphs bind declared inputs, tool binaries, and environment dependencies to content identities [@Dolstra2006Purely]. In hardware evaluation, harness environments should likewise lock tool binaries, host OS libraries, seed states, and simulation flags. This supports replay and exposes remaining platform or tool variation; it does not guarantee identical cycle counts across compute clusters.

- **Unmanaged license preemption in VCS RTL simulation flows:** When executing Synopsys VCS, an event-driven RTL simulator, under Slurm, a high-performance computing workload manager, silent job preemptions and license check-out failures can occur.[^fn-least-privilege-c06] Harnesses that treat license denials as hard design rejections corrupt dataset provenance by masking operational cluster limits as physical hardware incompatibilities.
...
An environment is what makes a tool run mean something. The questions below concern the parts of that job current harnesses do not do.

**Telling complete signoff from silent tool dropouts.** Physical synthesis and static timing engines frequently exit with zero status even when license timeouts, missing libraries, or intermediate scripts bypass declared checks. Parsers that extract metrics without auditing report completeness risk feeding partial runs into architectural evaluation.

- **What structural checks can an execution parser perform on static timing reports to verify that all declared process corners were evaluated before recording signoff metrics?** Electronic design automation tools running multi-corner static timing analysis can silently skip individual process corners if a corner-specific Liberty library fails to load or a license checkout times out mid-run. Because the main tool process exits cleanly with partial output, naive parsers extract timing slack from surviving corners without flagging missing ones. Automatically validating reported corner lists against the input run specification prevents incomplete signoff returns from masquerading as completed evaluations.
- **How can an execution harness detect whether a physical design signoff report achieved timing closure through unauthorized timing waivers?** Physical design scripts frequently introduce timing waivers, such as false paths or multicycle constraints, to bypass routing bottlenecks during intermediate optimization iterations. If these waivers remain active during final signoff without authorization in the prospective run specification, static timing analyzers report clean timing slack for physically invalid paths. Cross-referencing active waivers against declared run specifications prevents false timing closure from corrupting candidate evaluation.

**Keeping tool inputs and residual logs honest.** Automated harnesses rely on exact parameter translations and clean log parses to produce auditable evaluation records. When tools evolve across versions or scripts quietly reuse cached library views, execution environments risk accepting corrupted observations.

- **A physical design tool will happily evaluate a stale macro netlist, so what has to be checked over the layout and timing views before it runs?** Commercial physical design tools can silently fall back to cached cell views when updated macro netlists are elaborated without explicit file-list updates. Current tool wrappers inspect process exit codes rather than cross-referencing cell characterization timestamps and checksums against candidate specifications before launching placement and routing. Enforcing cryptographic checksum invariants across all input views guarantees that every physical signoff evaluates the precise macro version declared in the candidate request.
- **Where in the execution pipeline can a parser classify residual log output to catch unmapped tool warning categories before admitting a return into a training dataset?** As electronic design automation tools and simulators evolve across version updates, they emit novel warning strings that existing regular-expression parsers do not recognize. When parsers silently drop unclassified log text, the execution harness generates clean structured records for runs that experienced downgraded timing constraints or altered clock definitions. Systematically classifying residual log output ensures that novel failure modes trigger parser exceptions rather than quiet data corruption.

**Guaranteeing clean resets across persistent execution hardware.** High-throughput hardware accelerators and shared cluster nodes carry persistent state across candidate evaluations. Failing to verify state sanitization between runs allows residual memory buffers to corrupt subsequent workload timing and cost measurements.

- **An emulator carries state across runs, so what evidence short of a full teardown shows that its buffers were actually purged?** Hardware simulation accelerators rely on persistent host memory state and FPGA bitstreams across simulation runs. When an execution harness dispatches sequential candidate evaluations without verifying register and memory resets, residual kernel buffers corrupt subsequent workload timing measurements. Performing automated differential checks across host PCIe interfaces confirms that the accelerator has returned to a pristine state before admitting the next simulation attempt.

## Summary

Tool-connected execution environments form the backbone of modern hardware evaluation. A tool command is not an environment, and the difference is structural rather than cosmetic. A tool, including a simulator, performs one transformation or analysis. A wrapper serves one tool, a harness coordinates wrappers and records what each attempt cost and returned, and the environment is everything that connects a study to those tools by fixing the state a method may observe, the requests it may issue, and the returns it can expect. Without robust environment abstractions, automated architecture flows risk corrupting candidate state, misinterpreting tool crashes as design rejections, and losing track of evaluation lineage. When we wrap individual CAD, simulation, and synthesis tools and orchestrate them through isolated harnesses, we make a tool return identifiable, repeatable, and attributable to the request that produced it. That is the necessary condition for evidence, not evidence itself. Qualifying a return against the property it actually observed belongs to @sec-feedback-verification-trust.

Our environments must strictly separate prospective run specifications, which define what we authorize to execute, from immutable execution records, which record what actually ran and what compute was spent. While our infrastructure manages job scheduling, reset policies, and fault containment, it never unilaterally decides whether a return constitutes a valid measurement or a winning candidate. When we evaluate this phase of an AI-native design system, four core takeaways govern our work:
...
- **Strict runtime containment and budgets.** Contain automated tool executions within isolated boundaries with explicit time, memory, license, and least-privilege access limits, and record infrastructure failures separately from design results.
...
Our environment establishes what we requested, executed, returned, and spent, preserving the core identities needed to attempt local replay when the declared dependencies remain available. This is narrower than the reproduction a different team with a different setup would attempt. The environment cannot qualify a measurement, validate a comparison, or recommend a candidate on its own. @sec-feedback-verification-trust examines how these returned values and artifacts become useful feedback.
