...
| **Order and dispatch** | Screening evaluation runs once per candidate before any dependent cycle-level work. Cycle-level gem5/Ramulator 2/Verilator simulations dispatch only when screening returns are complete, schemas are valid, combined area is at most 1.5 mm², and access latency is at most 2.5\ ns. |
...
Our chronology starts with three screening runs. The 2 MiB rerun baseline, alongside our 3 MiB and 4 MiB alternatives, would return combined subsystem areas of 1.2 mm², 1.4 mm², and 1.7 mm² and access times of 2.1\ ns, 2.4\ ns, and 2.8\ ns, respectively. While the first two candidates meet both of our declared thresholds, the 4 MiB candidate falls short. Consequently, our run specification blocks any dependent cycle-level dispatch for it. The values alone would not establish that decision without our verified parse status and recorded dispatch outcome.
