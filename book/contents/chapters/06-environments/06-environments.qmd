...
- **Which checksum invariant checks over layout exchange files and timing views must an execution harness enforce to prevent physical design tools from evaluating stale macro netlists?** Commercial physical design tools can silently fall back to cached cell views when updated macro netlists are elaborated without explicit file-list updates. Current tool wrappers inspect process exit codes rather than cross-referencing cell characterization timestamps and checksums against candidate specifications before launching placement and routing. Enforcing cryptographic checksum invariants across all input views guarantees that every physical signoff evaluates the precise macro version declared in the candidate request.
