...
This chapter argues that some of the work has to be done together. The questions below are the ones the community cannot answer team by team.
...
- **How can a generator and the check that judges it be kept demonstrably independent across organizations?** A model and an evaluator that share training data or heuristics can approve each other's mistakes, and no current tool architecture enforces or even exposes the separation. Making independence verifiable is a design problem for shared infrastructure rather than a policy statement.
