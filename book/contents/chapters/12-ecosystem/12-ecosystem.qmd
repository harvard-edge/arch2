...
Every chapter in this book has closed with questions rather than answers, and this one closes the device as well as the chapter. What we have laid out across these sections is not a complete research agenda, and it was never meant to be. The questions were chosen because we could see the shape of them from where the argument stood, which is a poor guarantee that they are the most important ones. Treat them as evidence that a discipline with open questions is a discipline that is still moving. The more useful exercise, once you close the book, is to write down the questions your own design problems raise that none of ours anticipated. The ones below are the subset this chapter can name, and they share the property that no single team can answer them alone.

**Building shared evaluation that is trusted enough to decide.** Open infrastructure is only useful if its results survive contact with commercial signoff.

- **Can open evaluation tools rank candidates the way commercial signoff would, without access to proprietary process data?** What matters is not absolute accuracy but decision fidelity, meaning whether the open-tool ordering of candidates holds up under a real flow, and that correlation has barely been measured. Establishing it would tell the community which claims an open stack can legitimately support.
- **What makes the independence of a generator and its judge demonstrable across organizations?** A model and an evaluator that share training data or heuristics can approve each other's mistakes, and no current tool architecture enforces or even exposes the separation. Making independence verifiable is a design problem for shared infrastructure rather than a policy statement.

**Making results comparable while the work moves.** Shared data and benchmarks age, and the systems being measured learn from them.

- **What would a public record of failed design attempts have to contain to be trustworthy and safe to contribute to?** Negative results are the community's cheapest shared asset and the hardest to publish, because a failed run carries conditions, versions, and sometimes confidential context. Working out what such a record must carry, and what it must not, is a prerequisite for the repository this chapter argues for.
- **Can a benchmark resist gaming without invalidating the comparisons already made against it?** A static suite gets absorbed into training data, while a suite that changes breaks the historical record it exists to preserve. Resolving that tension is a governance question with a real technical core, and our discipline will need an answer as soon as shared benchmarks matter.
