...
Combining machine-usable representations (@sec-data-representations-world-models), smallest sufficient methods (@sec-methods-generation-prediction-optimization), sandboxed execution environments (@sec-architecture-environments-tool-interfaces), and qualified verification feedback (@sec-feedback-verification-trust) transforms these technical foundations into an active closed loop. Putting this loop into practice tests the exact boundary between prospective planning and empirical simulation. It demonstrates what an architecture study can conclude when it halts before dispatch, and what distinct insights only a fully executed simulator study can firmly establish.
...
- **Diagnose** ranking ties, refuted directional mechanisms, and DRAM timing walls ($t_{\text{RP}} + t_{\text{RCD}} + t_{\text{CAS}}$) to formulate closed-loop multi-iteration contracts and tool escalations.
:::
...
| **Array Shape ($H \times W$)** | **Height Mapping ($K=256$)** | **Width Mapping ($N=128$)** | **Layer 1 Spatial Passes ($K\text{-folds} \times N\text{-folds}$)** | **3-Layer DRAM Writes** | **3-Layer Compute Cycles** |
| :- | :- | :- | :- | -: | -: |
...
: **Spatial tiling and fold factor decomposition across 1,024-PE systolic array geometries.** For symmetric Layer 1 $(128, 128, 256)$, all three geometries execute exactly 32 spatial passes. Across the full 3-layer workload, the wide $16 \times 64$ geometry ties baseline compute cycles (13,917) but incurs $2\times$ higher DRAM write traffic (409,726 vs 204,863 writes), while the tall $64 \times 16$ array suffers an $N$-dimension fold penalty on asymmetric layers, increasing latency to 17,757 cycles. {#tbl-systolic-folding-breakdown tbl-colwidths="[20,20,20,20,10,10]"}
...
To understand why this modeling omission is fatal, we must examine the physical reality of the memory hierarchy. SCALE-Sim reports 409,726 DRAM-word writes for the 16 by 64 array compared to 204,863 for the 32 by 32 baseline. That modeled traffic difference flags a severe physical risk. When access strides collide with DRAM bank boundaries, unmodeled row-buffer conflicts compound into multi-hundred-nanosecond latency stalls. In modern LPDDR5 and DDR5 memory systems, memory is partitioned into 16 to 32 independent banks organized across 4 to 8 bank groups. Each individual DRAM storage cell is composed of a single access transistor and a microscopic storage capacitor ($1\text{T}-1\text{C}$). Accessing a cell is inherently destructive. Asserting a wordline causes the tiny capacitor charge to share onto the high-capacitance bitline, creating a minute differential voltage ($\Delta V \approx 100\text{--}200\text{ mV}$) that sense amplifiers must detect and amplify back to the full supply rail ($V_{\mathrm{DD}}$ or ground). This amplification latches the row into the bank's active row buffer (typically 1\ KiB to 2\ KiB of sense amplifiers) and restores the cell capacitor charge.
...
3. **Column Strobe ($t_{\mathrm{CAS}}$):** Issue the column read command to multiplex the requested 64\ B data burst from the sense amplifiers onto the DQ data bus ($14\text{--}18\text{ ns}$).
...
Compounding with bank-group cycle restrictions ($t_{\mathrm{CCD\_L}}$), four-activate window ($t_{\mathrm{FAW}}$) power constraints, and write-to-read turnaround bubbles, effective read latency surges from an idealized flat 100\ ns abstraction to 440\ ns per 64\ B line read. The memory controller's command queue fills up, backpressuring the crossbar, starving the accelerator's input line buffers, and causing processing elements to stall in idle bubbles. The compute engine is paralyzed not by a lack of arithmetic units, but by memory bus command stalls.

Evaluating these access patterns in a cycle-accurate DRAM simulator reveals the severe latency penalty hidden by flat memory abstractions (@fig-dram-bank-conflict).
...
The physical mechanism behind this latency explosion is address-stride aliasing across the memory controller's physical address mapping. In modern memory controllers, physical address bits map consecutively across channel, rank, bank group, bank, row, and column. When an accelerator geometry employs wide, asymmetric tiling, the resulting memory access strides collide systematically on different rows within the same physical bank. This stride collision forces the memory controller into continuous row-buffer thrashing, serializing requests through the full $t_{\mathrm{RP}} + t_{\mathrm{RCD}} + t_{\mathrm{CAS}}$ command cycle instead of sustaining pipelined column bursts, inflating effective access latency by $+340\%$ and starving the arithmetic array.

Executing cycle-accurate SCALE-Sim v3.0 simulations paired with a JEDEC DDR4-3200 memory engine across eight representative neural network layers demonstrates the severe distortion introduced by open-loop evaluation (@fig-ch08-money-plot-aspect-ratio-pareto). Open-loop compute proxies hide the DRAM command timing wall ($t_{\mathrm{RP}} + t_{\mathrm{RCD}} + t_{\mathrm{CAS}}$). While the wide $16\times64$ candidate and balanced $32\times32$ baseline tie at 3,366 compute cycles under flat memory abstractions, cycle-accurate DRAM simulation exposes a massive DRAM stall wall (where unmodeled memory stalls account for $62.1\%\text{ to }95.8\%$ of total execution cycles, inflating execution latency by $+581\%\text{ to }+2{,}261\%$ to 47.0k to 166.9k cycles). More critically, closed-loop memory timing inverts microarchitectural candidate rankings (the *aspect ratio Pareto inversion*), proving that balanced geometries minimize page conflicts and dominate under real memory coupling. Without closed-loop memory modeling, automated optimizers systematically promote memory-choked geometries that fail physical deployment.

![**Closed-loop cycle-accurate DRAM simulation exposes the memory stall wall and inverts systolic array geometry rankings.** (a) Active compute cycles versus DRAM timing stalls ($t_{\mathrm{RP}} + t_{\mathrm{RCD}} + t_{\mathrm{CAS}}$) across five 1,024-PE array geometries, showing unmodeled memory latency inflating execution time by up to $+2{,}261\%$ [@ScaleSimProject2025V300Source; @KimEtAl2015Ramulator]. (b) DRAM page conflict distributions under asymmetric tiling ($22.7\%$ page thrashing). (c) The resulting *closed-loop Pareto inversion*, where real memory coupling resolves open-loop ties and proves the balanced $32\times32$ array is the physical winner.](images/fig-closed-loop-dram-money-plot){#fig-ch08-money-plot-aspect-ratio-pareto width="100%" fig-alt="Three-panel visualization of closed-loop DRAM simulation. (a) Stacked bar chart comparing compute cycles and DRAM timing stalls across five array geometries, showing up to a 2261% memory stall inflation. (b) 100% stacked bar chart of DRAM row buffer hits, misses, and page conflicts. (c) Scatter plot illustrating how closed-loop memory timing resolves open-loop ties and inverts candidate rankings."}

This *aspect ratio Pareto inversion* demonstrates how open-loop compute proxies deceive automated optimizers. A candidate that appears Pareto-optimal under flat memory assumptions degrades severely once coupled to a realistic memory controller. Wide, asymmetric geometries maximize spatial compute density on paper, but generate misaligned access strides that thrash DRAM row buffers. In contrast, balanced square geometries (such as the $32\times32$ array) preserve spatial locality across both activation and weight streams, distribute requests evenly across bank groups, and dominate under physical memory coupling. Open-loop optimization drives automated search straight into the DRAM timing wall.
...
Optimizing against an incomplete scalar proxy in automated exploration and iterative repair loops introduces severe architectural blind spots (@fig-proxy-mismatch). When an automated optimizer or code generator iteratively repairs an implementation to satisfy one localized constraint (such as cycle count or timing slack), it frequently causes unconstrained physical dimensions (such as memory traffic, power grid integrity, or thermal flux) to regress silently. This fundamental systems trap governs iterative optimization across both microarchitectural and physical design loops.
...
**The Hardware Trap:** When an automated search loop optimizes an accelerator datapath solely for execution latency or processing-element utilization ($S = \text{cycles}/\bar{U}$), the generator introduces wide aspect ratios that double off-chip dynamic random-access memory (DRAM) writes (as seen in the 16 by 64 systolic array candidate in @fig-proxy-mismatch). In iterative RTL and physical repair loops, an agent attempting to fix localized timing violations aggressively widens driver gates, inadvertently swelling local voltage ($IR$) drop and triggering lower-metal routing congestion.

**The Architectural Antidote:** Evaluate candidates against multi-metric Pareto radar frontiers spanning execution latency, off-chip traffic, static timing, and physical power density simultaneously across every repair iteration (see @sec-law-whac-a-mole in @sec-appendix-c-laws-paradoxes for the multi-objective Pareto trade-off and regression audit protocols).
:::
...
If an engineering team launches a second iteration or a full-space enumeration study, decision quantities and multi-metric Pareto constraints must be frozen before execution begins. At that point, the study can retain the original local score for strict comparability, introduce a separately justified memory-aware metric, or replace the scoring function entirely. However, none of these future choices can rescore the currently retained and executed study. The unresolved memory axis illustrates why neither exhaustive architectural coverage nor the current local ranking alone provides enough evidence to establish energy consumption, power draw, or overall Lighthouse suitability. The tied ranking, the failure of the directional explanation, and the omission of modeled memory traffic from the frozen score mandate a shift in how subsequent loop iterations are structured.
...
Each diagnostic return scopes a concrete next action. A ranking tie under equalized simulation budgets mandates a separately budgeted full-space reference. A refuted directional explanation requires an isolating probe with pinned dataflow mapping semantics. A provenance gap demands re-executing with full input prompts, container hashes, and toolchain locks recorded. None of those follow-ups were part of the retained initial run, and retroactively altering the original score cannot substitute for them. Five key takeaways define this phase of a design study:
...
- **Closed-loop DRAM timing and Pareto inversion.** Open-loop compute proxies hide the three-phase DRAM command timing wall ($t_{\text{RP}} + t_{\text{RCD}} + t_{\text{CAS}}$); cycle-accurate memory simulation exposes row-buffer thrashing, resolves open-loop ties, and inverts candidate rankings (the aspect ratio Pareto inversion).
- **Multi-iteration contract mutation.** Negative empirical returns and ranking ties provide diagnostic feedback to mutate study contracts (bounding DRAM traffic), expand exploration spaces, and escalate tool fidelity in subsequent loop iterations without altering historical records.
:::
