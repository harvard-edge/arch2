...
The planned evaluation path spans eight functional tool categories (@sec-architecture-environments-tool-interfaces). Full-system CPU and microarchitectural simulation relies on gem5 [@BinkertEtAl2011Gem5] and Verilator, while CACTI supplies analytical SRAM area and access-time modeling. Gate-level physical synthesis and static timing analysis use Yosys, OpenROAD, and OpenSTA, none of which a foundry accepts for production signoff at 3\ nm. The remaining categories cover power-domain validation via IEEE 1801 Unified Power Format (UPF), formal property verification via SystemVerilog Assertions (SVA) and Bounded Model Checking (BMC), thermal signoff using Ansys RedHawk-SC, DRAM timing using Ramulator, and coherent die-to-die interconnect modeling using UCIe and CXL standards. However, our repository has not yet bound these conceptual tool roles to executable scripts or retained measurement pipelines.
...
Auditing our repository state separates prospective specification identities from true execution traces (@tbl-cache-evidence-inventory). Retained artifacts divide into specification targets (such as controlled cache parameters and declared tool roles), author-constructed teaching fixtures (such as illustrative SRAM and simulator entries), and executed evidence packages. They preserve design intent and pedagogical structure yet retain zero executed cache binaries, simulator logs, or parsed measurements. Specification documents and teaching fixtures clarify methodology, but they cannot support a measured cache result.
...
Executing the systolic-array study carries a frozen plan through schema validation, interface error recovery, and cycle-accurate simulation to four independent analytical judgments (@fig-array-study-workflow). Freezing the research question, legal design space, scoring metric, and budget establishes the baseline before proposal schema validation begins. When the initial JSON schema fails validation, the workflow routes through an explicit repair and revalidation step before candidate selection and legality checks proceed. Once cleared, the pipeline dispatches twelve cycle-accurate SCALE-Sim runs, feeding an interpreted study record that branches into four separate judgments: architecture outcome, study-local AI contribution, mechanism status, and stopping point. Incorporating explicit error-recovery loops within our workflow preserves prospective experimental integrity when interface failures occur.
...
The study yields two separable empirical findings across candidate scoring and directional mechanism validation (@fig-ai-study-result):
...
Incomplete proxy metrics create significant blind spots in automated evaluation, as demonstrated when contrasting execution latency against off-chip data movement (@fig-proxy-mismatch):
