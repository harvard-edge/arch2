...
A design loop closes only when returned evidence changes the represented design state and triggers another allowed action (@sec-design-loop-no-longer-scales). Both records in this chapter are design paths under that test; each stops, deliberately, at a different operational boundary. The first record, the prospective Lighthouse cache study, distills a 3\ W system objective into a single pre-RTL decision on L2 capacity, where register-transfer level (RTL) netlists have not yet been synthesized. It formalizes candidate comparisons and prerequisites, but halts prior to tool dispatch because physical derivation rules and toolchain bindings remain incomplete.

The second record, in contrast, is a [retained systolic-array study](https://github.com/harvard-edge/arch2/tree/f5516e52e66243c2d753191e350a5cfac56c83af/labs/examples/ai_systolic_array_study) using SCALE-Sim [@RajEtAl2025ScaleSimV3]. Across twelve simulator events evaluating general matrix multiplication (GEMM) kernels, the study runs its preregistered budget to completion, encounters an exact ranking tie between candidate and baseline geometries, refutes the generative model's directional hypothesis, and executes a principled technical stop. Contrasting an unexecuted plan with an executed tool sweep demonstrates how to decouple architecture outcomes, AI contributions, physical mechanisms, and stopping rules without manufacturing artificial end-to-end claims.

Both records run the six responsibilities of @sec-architecture-20-ontology, and the sections below follow them in order rather than announcing them. Formulate appears as the architectural decision and the frozen preregistration, Explore as the legal candidate set each arm draws from, Implement as the execution prerequisites and the recorded chronology, Evaluate as the returned simulator outputs, Interpret as the mechanism probe, and Commit as the review that assigns a status and stops. The cache record halts inside Implement because its prerequisites were never bound. The array record reaches Commit and stops there, holding at a technical recommendation because no authority retired it.
...
| **Evidence not returned** | Seed, temperature, and serving state for the model call; a repository commit binding for the runner; a validation receipt; and any independent replay. |
...
| **Commitment authority** | Unassigned in the retained record, which is why the status above is a recommendation and not an outcome. |
...
The last two rows are the ones a reviewer reads first. A study whose commitment authority is unassigned cannot produce a commitment however clean its returns are, and the card makes that visible without requiring the reviewer to read the chronology. Future AI systems might systematically execute commitment authority by embedding strict policy engines. If an automated study satisfies all predefined formal property checks, physical bounds, and risk thresholds, the engine could programmatically delegate authority, advancing a technical recommendation directly into a committed design state. Without such mechanisms, the boundary between an automated simulation sweep and an authorized architectural decision remains manual.
...
Writing this contract does not mutate the retained study or close a design loop. A new execution record would begin only after authorization and dispatch; its returned evidence would have to change the represented design state and trigger another allowed action before the operational closing test is met.
...
The space is small. With fifteen legal shapes and a recorded simulator window of 19.05 seconds for twelve events, an exhaustive reference would have cost seconds of laptop time rather than a budget. It was withheld by design. The preregistration fixed a budget-matched comparison between two proposal arms, and folding a full enumeration into that record would have changed what the record tests. Without it, performance outcomes cannot be inferred for the ten unexecuted shapes, and the record cannot claim that the model tied or outperformed the complete design space; a full-space reference belongs in a separately frozen record. The frozen score also omits a returned model-local axis. The 16 by 64 shape ties the baseline on cycles and utilization while SCALE-Sim reports twice its modeled DRAM-word writes, and 64 by 16 reports fewer modeled writes and fewer total accesses than the baseline despite needing more cycles.
...
**The Hardware Trap:** In this record, the 16 by 64 candidate ties the 32 by 32 baseline on the frozen score while SCALE-Sim reports twice as many modeled DRAM-word writes for the candidate. The comparison shows that the score omits a differentiating model output. It does not show that the generator caused physical off-chip traffic or that either array experiences contention, bank conflicts, or an energy penalty.

**The Architectural Antidote:** Freeze every decision quantity that the scoped claim needs before execution, and keep modeled returns distinct from physical measurements. A future memory study must test contention, timing, and energy under its own bound configuration rather than reading those consequences out of the retained SCALE-Sim write counts (@sec-law-whac-a-mole in @sec-appendix-c-laws-paradoxes).
:::
...
Both records leave the system-level questions open. The retained SCALE-Sim configuration evaluates the isolated array geometry and nothing else. Synthetic GEMM matrices bypass compiler lowering to vector pipelines, contention-aware shared DRAM simulation, power-domain isolation, formal property verification, and chiplet interconnects, and although the broader SCALE-Sim v3 framework supports memory and energy integration [@RajEtAl2025ScaleSimV3], the retained run restricted execution to standalone PE array traffic. Neither record identifies an XR workload, validates tail deadlines under deployed conditions, or exercises the runtime, host vector cores, shared memory path, sensors, or display engine, so the array result cannot resolve the cache sizing decision for XRBench [@KwonEtAl2023XRBench], SPEC CPU [@SPEC2017CPU], or MLPerf [@ReddiEtAl2020MLPerfInference], satisfy the 3\ W envelope, or close the Lighthouse design loop. Physical design checks across timing closure, design rule checking (DRC), power delivery network integrity, and thermal signoff remain unexecuted. Naming a check in an architectural plan is never a substitute for running it.
...
- **Blurring array and cache study boundaries:** A SCALE-Sim array sweep inside a frozen accelerator container, a Layer 2 study, gets read as evidence about the subsystem that contains it, for instance the 3\ W L2 cache decision under its XR workload snapshot. The array run never modeled interconnect contention, host-side synchronization, or cache-hierarchy latency, so a claim of subsystem power signoff or memory-wall closure built on it has no measurement behind it.
...
Each diagnostic return names a possible next study rather than an executed next action. A tie under matched budgets may justify a separately frozen full-space reference, which here would cost seconds. A refuted explanation motivates an isolating probe with pinned dataflow mapping. A provenance gap calls for re-execution with the prompt, container hash, and toolchain lock recorded. None of those follow-ups ran, and rescoring the retained study cannot substitute for them.
...
Both records stop inside one workload, one simulator configuration, and one decision, and the next chapter (@sec-loop-patterns-across-stack) asks which parts of such a retained record survive when the workload, the tools, or the decision changes.
