...
We establish a fixed heuristic to serve as the budget-matched method comparator for our frozen study. Dividing execution latency by fractional layer utilization combines execution speed and hardware efficiency into a single, lower-is-better metric:
...
The study yields two separable empirical findings across candidate scoring and directional mechanism validation (@fig-ai-study-result). Candidate scoring evaluates whether model proposals outperform a fixed heuristic under equal simulator budgets (@fig-ai-study-result, Panel A), while the mechanism probe tests whether swapping matrix dimensions M and N reverses relative array performance (@fig-ai-study-result, Panel B). A local score tie between evaluation arms does not validate the model's underlying physical explanation; ranking ties and mechanistic predictions address distinct architectural questions.

```{python}
#| label: fig-ai-study-result
#| fig-cap: |
#|   **Separating candidate scoring from directional mechanism validation.** Panel A (left) compares local architectural scores (thousands; lower is better) across model-proposed (M, blue) and fixed-heuristic (H, green) arms under equal four-event simulation budgets, where the 32 by 32 baseline and 16 by 64 shape tie at 33.8k. Panel B (right) tracks the mechanism probe, showing that the 16 by 64 array remains ahead of the 64 by 16 shape across both original and transposed ($M/N$ swapped) workloads (23.5k vs 25.5k), directly refuting the predicted performance reversal. Values derive from retained SCALE-Sim simulator logs [@RajEtAl2025ScaleSimV3] rather than physical silicon measurements.
#| out-width: "100%"
...
In industrial settings, evaluating generative design proposals requires characterizing *proposal dispersion* and *incremental stability*. When models generate microarchitectural configurations, stochastic sampling produces a wide dispersion of candidate quality across runs. Furthermore, because surrounding software and memory models churn continuously, an isolated optimization against a static layer configuration can quickly degrade if memory bank mappings or tiling heuristics shift upstream. A single isolated proposal run leaves both the model dispersion profile and its resilience to incremental code churn unmeasured.
...
### Automated Structural Derivation and Macro Legality

- **Pre-simulation SRAM macro and floorplan derivation rules:** What automated derivation rules can verify SRAM macro floorplans, bank organizations, and wire-routing limits for non-power-of-two cache capacities before dispatching cycle-level simulations? Declaring an exploratory target capacity (such as 3\ MiB) leaves underlying bank counts, wordline lengths, and macro aspect ratios unspecified. In non-power-of-two configurations, irregular floorplans introduce multiplexing delays and routing congestion that violate physical timing constraints. Establishing structural derivation rules that enforce physical layout legality prior to simulation prevents wasting compute budgets on un-routable candidates.

### Budget Allocation Between Candidate Search and Mechanistic Probes

- **Optimal compute partitioning between search and diagnostic validation:** How should an automated design loop divide a fixed simulation budget between exploring novel candidate architectures and dispatching diagnostic probes to test mechanistic hypotheses? Search loops routinely exhaust compute budgets sweeping candidate parameters, only to produce ambiguous ties or unverified performance claims. Establishing formal budget-allocation policies that dynamically interleave candidate generation with hypothesis-testing diagnostic probes (such as matrix dimension transpositions or stride perturbations) ensures results yield actionable microarchitectural insight rather than raw scalar rankings.

- **Diagnostic probe design for spatial vs. temporal disentanglement:** Which diagnostic probe constructions can cleanly isolate spatial array aspect ratios from temporal loop-tiling fold counts in systolic accelerators? Transposing workload matrices to test a directional hypothesis often alters vertical and horizontal tile fold counts simultaneously when inner loop dimensions remain fixed. Developing diagnostic probe schemas that hold temporal loop iterations strictly invariant while modulating physical spatial dimensions is required to isolate true hardware mechanisms from compiler scheduling artifacts.

### Compositional Uncertainty Across Decoupled Subsystem Loops

- **Bounding composition error in multi-loop subsystem evaluation:** How can architecture frameworks mathematically bound the error introduced when combining evidence from decoupled sub-module studies into an aggregate subsystem claim? High-level SoC evaluations frequently stitch together standalone NPU accelerator simulation runs with unexecuted memory-hierarchy plans without accounting for dynamic interconnect contention, cache miss latency variability, and host synchronization stalls. Developing rigorous compositional bounds that quantify cross-loop uncertainty is an open challenge for full-system validation.
