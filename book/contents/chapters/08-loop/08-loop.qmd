...
Sizing an L2 cache for a mobile extended-reality (XR) subsystem tempts immediate simulation sweeps. However, dispatching simulation jobs without a declared baseline, fixed instruction set architecture (ISA) invariants, and pre-bound stopping criteria yields ambiguous data rather than actionable architectural decisions. To demonstrate the operational boundary where prospective planning halts before tool invocation, this first study formalizes the complete experimental intent for sizing an L2 cache under a strict 3\ W thermal envelope.
...
Expanding L2 cache capacity inside a strict 3\ W thermal design power (TDP) budget trades off memory latency against static leakage power and physical area. In the Lighthouse mobile XR subsystem (@sec-moonshot), mapped to a TSMC N7 or 3\ nm-class low-power process, this question determines whether a larger L2 cache should advance to RTL evaluation. Direct prospective comparison across the eight requirement domains (@sec-intent-vs-spec) holds the RV64GCV ISA contract, vector CPU and neural processing unit (NPU) compute topologies, XR workload snapshots, and runtime compiler passes fixed, isolating L2 capacity as the single swept variable: a 2\ MiB baseline against 3\ MiB and 4\ MiB candidates. These capacities define the distinct architectural objects within the prospective study, though they remain specification targets rather than generated RTL instances or measured artifacts in the repository.

While capacity serves as the only swept architecture parameter (limited to the 2\ MiB, 3\ MiB, and 4\ MiB choices), a larger cache capacity demands a different set count, bank organization, or static random-access memory (SRAM) macro composition. Sizing an L2 cache in an advanced process cannot be dispatched as an ungrounded capacity integer; an architectural derivation rule must compute the physical banking topology (such as asymmetric 3-bank macro compositions versus power-of-two 4-bank partitions), subarray aspect ratios, tag and data array bitcell floorplans, bit-sliced hash indexing functions (preventing bank hot-spotting under spatially correlated accesses), port arbitration, miss status holding register (MSHR) depths, hit-latency allocations, and process, voltage, and temperature (PVT) operating corners. The fixed architectural state extends far beyond the cache itself, declaring network-on-chip and memory-system configurations, the 2\ GHz host RV64GCV vector-capable CPU microarchitecture, the just-in-time (JIT) compiler and runtime lowering environment, the RISC-V software image, and an XR workload snapshot with stable frame identifiers and boundaries. Because the repository currently lacks both this structural derivation rule and explicitly bound values for controlled cache and PVT parameters, the comparison cannot yet execute.
...
Beyond spatial tiling mathematics, evaluating accelerator performance requires auditing the memory-system assumptions embedded in the simulation tool. In the retained systolic-array record, the failure of the directional compute mechanism is compounded by a fundamental limitation in simulator memory semantics. While the compute array model tracks cycle-level MAC operations, its memory interface relies on a flat abstraction that decouples compute throughput from physical memory channel realities.
...
SCALE-Sim reports 409,726 DRAM-word writes for the 16 by 64 array compared to 204,863 for the 32 by 32 baseline. That modeled traffic difference flags a severe physical risk: when access strides collide with DRAM bank boundaries, unmodeled row-buffer conflicts compound into multi-hundred-nanosecond latency stalls (@fig-dram-bank-conflict). In modern LPDDR5 and DDR5 memory systems, memory is partitioned into bank groups and individual banks containing multi-kilobyte row buffers (typically 1\ KiB to 2\ KiB page sizes). An access hitting an open row buffer incurs only the base column-access strobe latency:

$$
t_{\mathrm{hit}} = t_{\mathrm{CAS}}
$$

where $t_{\mathrm{CAS}} \approx 14\text{--}18\text{ ns}$ at typical operating frequencies. However, when streaming matrix tile strides alias with bank-addressing bits (such as stride 16 in @fig-dram-bank-conflict), consecutive requests collide on different rows within the same bank group, triggering severe *row-buffer conflicts* (page thrashing). The memory controller must first precharge the active bitlines ($t_{\mathrm{RP}}$) to close the conflicting page, issue an activate command ($t_{\mathrm{RCD}}$) to strobe the requested row into sense amplifiers, and finally issue the column read command ($t_{\mathrm{CAS}}$). The closed-page conflict access latency expands to:

$$
t_{\mathrm{conflict}} = t_{\mathrm{RP}} + t_{\mathrm{RCD}} + t_{\mathrm{CAS}}
$$

Compounding with bank-group cycle restrictions ($t_{\mathrm{CCD\_L}}$), four-activate window ($t_{\mathrm{FAW}}$) constraints, and bus turnaround stalls, effective read latency surges from an idealized flat 100\ ns abstraction to 440\ ns per 64\ B line read (+340% latency overhead). Without cycle-accurate DRAM timing models, high-level accelerator exploration loops risk selecting geometries that appear optimal in compute-only cycles while bottlenecking completely on unmodeled memory channel contention.
