...
Executing cycle-accurate SCALE-Sim v3.0 simulations paired with a JEDEC DDR4-3200 memory engine across eight representative neural network layers demonstrates the severe distortion introduced by open-loop evaluation (@fig-closed-loop-dram-money-plot). Open-loop compute proxies underestimate execution latency by $48\%\text{--}62.1\%$, completely hiding the memory stall wall. More critically, closed-loop memory timing inverts microarchitectural candidate rankings: while the wide $16\times64$ array appears superior in open-loop cycles, its high partial-sum write traffic ($2\times$ baseline) triggers persistent bank thrashing, causing its closed-loop throughput to collapse by $38.4\%$ below the balanced $32\times32$ baseline. Without closed-loop memory modeling, automated optimizers systematically promote memory-choked geometries that fail physical deployment.

![**Closed-loop cycle-accurate DRAM simulation exposes the memory stall wall and inverts systolic array geometry rankings.** Panel (a) compares open-loop execution (idealized compute-only cycles, blue) against closed-loop execution incorporating cycle-accurate JEDEC DDR4-3200 command timing ($t_{\text{RP}} + t_{\text{RCD}} + t_{\text{CAS}}$, orange) across eight neural network workloads on SCALE-Sim v3.0 [@ScaleSimProject2025V300Source; @KimEtAl2015Ramulator]. Open-loop simulation underestimates total execution latency by $48\%\text{--}62.1\%$ by ignoring row-buffer page thrashing and bank conflicts. Panel (b) reveals the resulting *geometry Pareto inversion*: while wide $16\times64$ arrays appear to outperform balanced $32\times32$ arrays in compute-only cycles, closed-loop memory contention degrades wide array throughput by $38.4\%$, making the balanced $32\times32$ geometry the true physical winner. Panel (c) provides the cycle-accurate DRAM command bus activity breakdown, illustrating that active activation ($t_{\text{RCD}}$) and precharge ($t_{\text{RP}}$) commands consume $54.2\%$ of all memory controller cycles during streaming matrix evaluation.](images/fig-closed-loop-dram-money-plot){#fig-closed-loop-dram-money-plot width="100%" fig-alt="Three-panel visualization of closed-loop DRAM simulation. Panel A shows a grouped bar chart comparing open-loop versus closed-loop execution latency across eight workloads, showing a 62.1% DRAM stall penalty. Panel B shows a Pareto scatter plot illustrating how closed-loop memory timing inverts the ranking between wide and balanced systolic arrays. Panel C shows a stacked bar chart of DRAM command bus utilization."}
