...
::: {.callout-failure-mode title="Failure Mode and Dynamic: The Whac-a-Mole Regression Effect"}
...
**The Hardware Trap:** When an automated search loop optimizes an accelerator datapath solely for execution latency or processing-element utilization ($S = \text{cycles}/\bar{U}$), the generator introduces wide aspect ratios that double off-chip dynamic random-access memory (DRAM) writes (as seen in the 16 by 64 systolic array candidate in @fig-proxy-mismatch). When optimizing physical timing slack, an agent widens driver gates, inadvertently swelling local voltage ($IR$) drop and triggering lower-metal routing congestion.
**The Architectural Antidote:** Evaluate candidates against multi-metric Pareto radar frontiers spanning execution latency, off-chip traffic, static timing, and physical power density simultaneously (see @sec-law-whac-a-mole for the multi-objective Pareto trade-off and regression audit protocols).
:::
