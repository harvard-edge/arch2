...
Expanding L2 cache capacity inside a strict 3\ W thermal design power (TDP) budget trades off memory latency against static leakage power and physical area. In our Lighthouse mobile extended-reality (XR) subsystem (@sec-moonshot), mapped to a TSMC N7 or 3\ nm-class low-power process, this question determines whether a larger L2 cache should advance to register-transfer level (RTL) evaluation. Direct prospective comparison across our eight requirement domains (@sec-intent-vs-spec) holds our RV64GCV ISA contract, vector CPU and NPU compute topologies, XR workload snapshots, and runtime compiler passes fixed, isolating L2 capacity as our single swept variable: a 2\ MiB baseline against 3\ MiB and 4\ MiB candidates. These capacities define the distinct architectural objects within our prospective study, though they remain specification targets rather than generated RTL instances or measured artifacts in our repository.
...
In contrast to our unexecuted cache study, our separate systolic-array experiment begins with concrete cycle counts and bandwidth traces from an operational simulator. This executed array study illustrates the exact operational characteristics and boundaries of Layer 2 (AI-driven subsystem optimization) (@fig-ch01-three-layers-ai-integration). Inside a fixed architectural container (the weight-stationary PE array), automated exploration searches legal geometries. However, because the container is isolated, it remains blind to DRAM bank conflicts, memory bandwidth bottlenecks, compiler lowerings, and full-subsystem power envelopes. Interpreting these results requires evaluating them against a frozen research question, ensuring that unexpected simulator returns do not prompt retrospective changes to scoring metrics or candidate filters.
...
This structured synthesis closes the loop: negative results from Iteration 1 directly shape the hypothesis, constraints, and tool fidelity of Iteration 2 without polluting baseline audit records. For students designing closed-loop search experiments, an essential open research question is how an automated loop should dynamically partition a fixed simulation budget between exploring novel candidate architectures and dispatching targeted diagnostic probes (such as workload transposition or stride perturbations) to test mechanistic hypotheses.
...
For students and researchers running end-to-end design loops, execution forces a transition from isolated component modeling to full-system empiricism. Investigating these open questions requires designing controlled experiments that isolate true hardware mechanisms from compiler scheduling artifacts and simulator quirks. The following research frontiers highlight where closed-loop systems require empirical advancement:

- **What automated derivation rules can verify SRAM macro floorplans and routing limits for non-power-of-two capacities before simulation?** Declaring an exploratory target capacity (such as 3\ MiB) leaves underlying bank counts, wordline lengths, and macro aspect ratios unspecified. In non-power-of-two configurations, irregular floorplans introduce multiplexing delays and routing congestion that violate physical timing constraints. Establishing structural derivation rules that enforce physical layout legality prior to simulation prevents wasting compute budgets on un-routable candidates.
- **How should an automated design loop divide a fixed simulation budget between exploring candidates and dispatching mechanistic diagnostic probes?** Search loops routinely exhaust compute budgets sweeping candidate parameters, only to produce ambiguous ties or unverified performance claims. Establishing formal budget-allocation policies that dynamically interleave candidate generation with hypothesis-testing diagnostic probes (such as matrix dimension transpositions or stride perturbations) ensures results yield actionable microarchitectural insight rather than raw scalar rankings.
- **Which diagnostic probe constructions can cleanly isolate spatial array aspect ratios from temporal loop-tiling fold counts in systolic accelerators?** Transposing workload matrices to test a directional hypothesis often alters vertical and horizontal tile fold counts simultaneously when inner loop dimensions remain fixed. Developing diagnostic probe schemas that hold temporal loop iterations strictly invariant while modulating physical spatial dimensions is required to isolate true hardware mechanisms from compiler scheduling artifacts.
- **How can architecture frameworks mathematically bound the composition error introduced when combining evidence from decoupled subsystem loops?** High-level SoC evaluations frequently stitch together standalone NPU accelerator simulation runs with unexecuted memory-hierarchy plans without accounting for dynamic interconnect contention, cache miss latency variability, and host synchronization stalls. Developing rigorous compositional bounds that quantify cross-loop uncertainty is an open challenge for full-system validation.
