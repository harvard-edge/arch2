...
The last two rows are the ones a reviewer reads first. A study whose commitment authority is unassigned cannot produce a commitment however clean its returns are, and the card makes that visible without requiring the reviewer to read the chronology.
...
The consequence for the probe is that a mirrored pair changes several tiling factors at once rather than one. For the first GEMM layer, with dimensions $(M,N,K) = (128, 128, 256)$, the $16 \times 64$ array requires $\lceil 256/16 \rceil = 16$ vertical $K$-folds and $\lceil 128/64 \rceil = 2$ horizontal $N$-folds ($16 \times 2 = 32$ spatial passes).
