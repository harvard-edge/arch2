...
Sizing an L2 cache for a mobile XR subsystem tempts immediate simulation sweeps. Without a declared baseline, fixed instruction set architecture (ISA) invariants, and pre-bound stopping criteria, raw simulation numbers yield ambiguous claims rather than actionable architectural decisions. Formally declaring the design space and execution prerequisites up front separates prospective intent from executed, tool-bounded evidence and later physical verification.
...
## Halting Design Studies Before Dispatch
...
We establish the comparison compactly through the empirical simulator returns from the matched evaluation arms (@tbl-array-results). Across eight total simulation runs evaluating five unique array geometries, the model-generated proposals match the deterministic aspect-ratio heuristic. The empirical returns confirm that the 32 by 32 baseline and the 16 by 64 candidate achieve identical execution latencies of 13,917 cycles and matching layer utilization of 41.196 percent, yielding an identical local score of 33,782.40. Both arms tie with this best-evaluated score, while extreme aspect ratios (such as 8 by 128 and 32 by 16) suffer performance penalties due to lower array utilization. Under equalized simulator-event budgets, generative proposals provide no local performance advantage over a simple analytical heuristic.
...
The best evaluated nonbaseline configuration, the 16 by 64 array, exactly matches the 32 by 32 baseline in total execution cycles, average layer utilization, and final local score. It fails to clear the predefined architectural change margin. Furthermore, the 8 by 128, 32 by 16, and 64 by 16 designs all rank worse under the frozen scoring metric, yielding a no-change outcome across five uniquely evaluated hardware shapes. This result alone does not prove that the 32 by 32 layout is globally optimal among all legally permitted dimensions.
...
This architectural tie demonstrates no study-local AI advantage over the deterministic heuristic under equal simulator-event budgets. Architects must not mistake this tie for evidence of equal capability, identical total design cost, or performance parity with exhaustive enumeration, an intensive expert study, specialized integer linear programming (ILP) solvers, or alternative hardware search methodologies. Executing just one proposal call supplies no statistically valid estimate of variation or broader reliability for this generative model family.
...
The generative model predicted that a wide 16 by 64 array would outperform a tall 64 by 16 layout on the baseline workload, predicting that swapping matrix dimensions M and N would reverse their relative performance. However, retained simulator records contradict this mechanistic explanation. In the [retained study results](https://github.com/harvard-edge/arch2/blob/f5516e52e66243c2d753191e350a5cfac56c83af/labs/examples/ai_systolic_array_study/recorded/reference/study_results.json), the original local scores of 33,782.40 versus 54,985.25 compare against transposed scores of 23,530.65 versus 25,458.11; the predicted architectural reversal does not occur. Observing a narrower performance gap under transposition fails to validate the proposed physical mechanism.
...
The simulator's physical hardware mapping semantics determine what the array comparison actually varied (@fig-array-mapping). Weight-stationary GEMM maps the $K$ dimension to physical array height and the $N$ dimension to array width, while matrix dimension $M$ streams temporally through the processing elements. For Layer 1 GEMM with dimensions $(M,N,K) = (128, 128, 256)$, the $16 \times 64$ array requires $\lceil 256/16 \rceil = 16$ vertical $K$-folds and $\lceil 128/64 \rceil = 2$ horizontal $N$-folds ($16 \times 2 = 32$ spatial passes). Conversely, the $64 \times 16$ array requires $\lceil 256/64 \rceil = 4$ vertical $K$-folds and $\lceil 128/16 \rceil = 8$ horizontal $N$-folds ($4 \times 8 = 32$ spatial passes). Because exchanging $M$ and $N$ leaves $K$ unchanged, evaluating mirrored array shapes alters multiple spatial tiling factors simultaneously rather than isolating a single physical dimension. Aligning diagnostic probes with simulator dataflow semantics is essential to avoid confounding hardware parameters.
...
- **How can diagnostic perturbation probes systematically decouple microarchitectural hardware mechanisms from compiler scheduling artifacts?** In specialized accelerators, candidate hardware often appears to outperform a baseline not because of superior spatial organization, but because the workload mapping happened to align favorably with compiler loop-tiling, unrolling, or buffer-allocation heuristics. Formulating principled perturbation schemas (such as controlled workload transpositions, tensor dimension swaps, and stride modulations) that isolate true hardware contributions while holding software schedules invariant is required to eliminate software-schedule confounding in hardware exploration.
- **How can architecture frameworks mathematically bound the composition error introduced when combining evidence from decoupled subsystem loops?** High-level system-on-chip (SoC) evaluations frequently stitch together standalone NPU accelerator simulation runs with unexecuted memory-hierarchy plans without accounting for dynamic interconnect contention, cache-miss latency variability, and host synchronization stalls. Developing rigorous compositional bounds that quantify cross-loop uncertainty is essential to guarantee that decoupled subsystem optimizations remain sound when integrated at the SoC level.

## Summary

Synthesizing findings across both prospective plans and executed simulator traces reveals a foundational principle of architectural evaluation. A claim can reach only as far as its retained evidence and checks.
