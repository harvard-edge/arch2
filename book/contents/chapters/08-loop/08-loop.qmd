...
| **Checks** | Require identical ISA-visible outputs and standard functional regressions for every capacity. Demand zero coherence-invariant failures, subsystem power (dynamic plus leakage) capped at 3\ W, total cache macro area $\le 1.5$\ mm^2, and modeled access time $\le 2.5$\ ns. Require uncertainty-qualified frame-time comparisons against rerun baseline. |
...
where $t_{\mathrm{CAS}} \approx 14--18\ ns$ at typical operating frequencies, multiplexing the already-latched data onto the memory bus. However, when streaming matrix tile strides alias with bank-addressing bits, consecutive read requests collide on different rows within the same bank, triggering severe *row-buffer conflicts* (page thrashing). The memory controller cannot simply strobe a column; it must execute an analog three-phase physical command sequence:

1. **Row Precharge ($t_{\mathrm{RP}}$):** Disconnect the open row, restore and equalize the active bitlines to the mid-rail reference voltage $V_{\mathrm{DD}}/2$, and close the conflicting page ($14--18\ ns$).
2. **Row Activate ($t_{\mathrm{RCD}}$):** Assert the requested wordline, allowing destructive cell charge sharing onto the bitlines and driving sense amplifiers to amplify and latch the new target row ($14--18\ ns$).
3. **Column Strobe ($t_{\mathrm{CAS}}$):** Issue the column read command to multiplex the requested 64\ B data burst from the sense amplifiers onto the DQ data bus ($14--18\ ns$).
...
On a whiteboard, the DRAM command timing wall is a devastating traffic jam caused by switching lanes. When a new request targets a closed page in the same bank, the controller must first pay $t_{\mathrm{RP}}$ to close the old page and precharge the bitlines, then $t_{\mathrm{RCD}}$ to activate the new row and latch it into the sense amplifiers, and finally $t_{\mathrm{CAS}}$ to strobe the data out. If the accelerator's aspect ratio creates access strides that constantly cross bank boundaries and thrash the row buffer, this $t_{\mathrm{RP}} + t_{\mathrm{RCD}} + t_{\mathrm{CAS}}$ sequence becomes a structural bottleneck, inflating memory latency by $340\%$ and stalling the entire arithmetic pipeline.

Executing cycle-accurate SCALE-Sim v3.0 simulations paired with a JEDEC DDR4-3200 memory engine across eight representative neural network layers demonstrates the severe distortion introduced by open-loop evaluation (@fig-ch08-money-plot-aspect-ratio-pareto). Open-loop compute proxies hide the DRAM command timing wall ($t_{\mathrm{RP}} + t_{\mathrm{RCD}} + t_{\mathrm{CAS}}$). While the wide $16\times64$ candidate and balanced $32\times32$ baseline tie at 3,366 compute cycles under flat memory abstractions, cycle-accurate DRAM simulation exposes a massive DRAM stall wall (where unmodeled memory stalls account for $62.1\%--95.8\%$ of total execution cycles, inflating execution latency by $+581\%--+2{,}261\%$ to 47.0k–166.9k cycles). More critically, closed-loop memory timing inverts microarchitectural candidate rankings (the *aspect ratio Pareto inversion*), proving that balanced geometries minimize page conflicts and dominate under real memory coupling. Without closed-loop memory modeling, automated optimizers systematically promote memory-choked geometries that fail physical deployment.
...
The aspect ratio Pareto inversion is the classic architectural plot twist: a design that looks like a Pareto-optimal winner under flat memory modeling becomes a catastrophic loser when memory is coupled. Wide, asymmetric arrays maximize theoretical compute utilization but generate misaligned memory strides that thrash the DRAM row buffer. Balanced, square arrays (like the $32\times32$) preserve spatial locality, minimize bank conflicts, and ultimately win the true physical execution. Open-loop optimization drives the search straight into the DRAM timing wall.
