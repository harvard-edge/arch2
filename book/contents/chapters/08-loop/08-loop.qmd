...
SCALE-Sim reports 409,726 DRAM-word writes for the 16 by 64 array compared to 204,863 for the 32 by 32 baseline. That modeled traffic difference flags a severe physical risk: when access strides collide with DRAM bank boundaries, unmodeled row-buffer conflicts compound into multi-hundred-nanosecond latency stalls (@fig-dram-bank-conflict). When access strides align unfavorably with bank addressing, row-buffer misses force constant precharge and activate cycles, causing the effective memory latency to spike from a flat $100\ \text{ns}$ abstraction up to $440\ \text{ns}$ per line read. Without cycle-accurate DRAM timing models, high-level accelerators risk selecting geometries that appear optimal in compute-only cycles while bottlenecking completely on unmodeled memory channel contention.
...
In automated architectural design loops, narrow proxy metrics risk masking underlying hardware trade-offs. While the local array scoring metric tracked total execution cycles and average processing-element utilization, it omitted modeled DRAM memory traffic, concealing a major discrepancy in off-chip data movement. Evaluating fifteen legal processing-element geometries against the four-event simulation budget reveals the coverage boundaries of the proposal arms (@tbl-array-shape-coverage). Across both arms, the runs evaluated only five unique shapes (8 by 128, 16 by 64, 32 by 16, 32 by 32, and 64 by 16), leaving ten legal geometries unexecuted. Repeating candidate evaluations across proposal arms consumes simulation events without expanding architectural coverage.
