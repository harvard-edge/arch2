...
The planned evaluation path spans eight functional tool categories (@sec-architecture-environments-tool-interfaces): full-system microarchitectural simulation in gem5 and Verilator, analytical SRAM area modeling in CACTI, physical synthesis and timing in Yosys and OpenROAD, power-domain validation via IEEE 1801 UPF, formal verification via SVA/BMC, thermal signoff in Ansys RedHawk-SC, DRAM timing in Ramulator, and coherent die-to-die interconnect modeling in UCIe/CXL. None of these conceptual tool roles are yet bound to executable scripts or retained measurement pipelines in our repository.
...
Evaluating access patterns in a cycle-accurate DRAM simulator reveals the severe latency penalty hidden by flat memory abstractions (@fig-dram-bank-conflict).

![**SCALE-Sim versus Ramulator DRAM bank conflict latency overhead across access strides.** While flat memory abstractions in SCALE-Sim assume constant 100\ ns access latency regardless of access pattern, cycle-accurate DRAM simulation in Ramulator exposes severe latency degradation (up to $+340\%$ overhead at stride 16) caused by bank conflicts and row-buffer thrashing.](images/fig-dram-bank-conflict){#fig-dram-bank-conflict width="100%" fig-alt="Plot comparing flat 100 ns latency in SCALE-Sim with cycle-accurate latency in Ramulator across access strides from 1 to 128, highlighting a 340 percent latency overhead penalty at stride 16 due to DRAM bank conflicts."}

SCALE-Sim reports 409,726 DRAM-word writes for the 16 by 64 array and 204,863 for the 32 by 32 baseline. That difference is a modeled traffic count, not a measured energy or latency penalty. When access strides hit unfavorable DRAM bank alignments, row-buffer miss penalties compound into multi-hundred-nanosecond stalls (@fig-dram-bank-conflict).

### Closing the Loop: Synthesizing Iteration 2

When Iteration 1 halts after observing that Candidate 1 (16 by 64 array) doubles DRAM word traffic without improving cycle time over the 32 by 32 baseline, an ad hoc workflow might blindly retry random array dimensions. In contrast, an AI-native design system synthesizes this diagnostic feedback directly into a revised scoped contract for Iteration 2:

1. **Diagnostic Extraction:** The evaluator flags a *traffic inflation defect*: the weight-stationary mapping on asymmetric $K \times N$ tiles causes frequent SRAM evictions and redundant DRAM writebacks.
2. **Contract Mutation:** Stage 1 (Formulate) mutates the scoped study plan by adding an explicit memory-traffic constraint ($\text{DRAM word writes} \le 250{,}000$) and expanding the permitted exploration space to include alternative dataflow mappings (such as output-stationary or row-stationary).
...
This structured synthesis closes the loop: negative results from Iteration 1 directly shape the hypothesis, constraints, and tool fidelity of Iteration 2 without polluting baseline audit records.
