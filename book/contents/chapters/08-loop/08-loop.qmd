...
2. **Contract mutation:** Stage 1 (Formulate, F) prospectively mutates the scoped study plan by adding an explicit memory-traffic constraint ($\text{DRAM word writes} \le 250{,}000$) and expanding the permitted exploration space to include alternative dataflow mappings (such as output-stationary or row-stationary).
3. **Targeted tool escalation:** Stage 3 (Build / Implement, B) binds Ramulator 2.0 to the execution pipeline, enabling cycle-accurate DRAM bank conflict checking for all surviving Iteration 2 proposals.
