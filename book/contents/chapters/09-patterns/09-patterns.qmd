...
3. **Physical Transistor Node Shifts (FinFET $\rightarrow$ Gate-All-Around GAAFET & BSPDN):** Floorplan heuristics and standard-cell placement patterns calibrated on FinFET nodes (7\ nm / 5\ nm) fail when applied to 2\ nm/A16 nanosheet GAAFETs with backside power delivery networks (BSPDNs). Backside power delivery shifts standard cell pin access entirely to the frontside while power grids route beneath active silicon, fundamentally altering dynamic IR-drop profiles, cell routing congestion, and thermal resistance paths.
...
| **RTL behavior** | Independently derived reference tests, linting, functional simulation (Verilator), synthesis (Yosys), formal property verification (Cadence Jasper / SystemVerilog assertions (SVA)), and target-specific fault/threat models. | No implementation-readiness, reliability, or security claims beyond checked properties. |
