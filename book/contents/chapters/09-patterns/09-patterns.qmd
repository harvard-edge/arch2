...
- **Layer 1 (AI-Assisted Engineering):** Transfer is manual and ad hoc. Engineers copy code generation prompts or testbench templates across projects. Because prompt context omits unstated application binary interface (ABI) conventions, clock domains, and physical constraints, transferred prompts silently generate invalid RTL code.
...
1. **Cross-ISA and Memory Consistency Shifts (x86 TSO $\rightarrow$ RISC-V RVWMO):** Microarchitectural synchronization patterns, lock-free queues, and speculative store buffers designed under x86 Total Store Order (TSO) assume hardware-enforced load-load, store-store, and load-store ordering. Transferring these designs to RISC-V, which defaults to the weaker RISC-V Weak Memory Ordering (RVWMO) memory consistency model [@RISCV2026UnprivilegedISA], introduces subtle multi-core data races (for instance, in the store-buffering litmus test where Core 0 executes $W(x) \to R(y)$ and Core 1 executes $W(y) \to R(x)$, x86 TSO forbids both cores from reading 0, whereas RVWMO permits it unless an explicit `fence rw, rw` is inserted).
...
3. **Physical Transistor Node Shifts (FinFET $\rightarrow$ Gate-All-Around GAAFET and BSPDN):** Floorplan heuristics and standard-cell placement patterns calibrated on FinFET nodes (7\ nm / 5\ nm) fail when applied to 2\ nm/A16 nanosheet GAAFETs with backside power delivery networks (BSPDNs). Backside power delivery shifts standard cell pin access entirely to the frontside while power grids route beneath active silicon, fundamentally altering dynamic IR-drop profiles, cell routing congestion, and thermal resistance paths.
4. **Workload Paradigm Shifts (Dense GEMMs $\rightarrow$ Mixture-of-Experts and KV-Cache):** Systolic arrays and line-buffer architectures optimized for dense matrix multiplication (GEMMs) suffer severe throughput collapse when transferred to modern mixture-of-experts (MoE) large language model (LLM) architectures. MoE workloads replace static, contiguous data movement with dynamic, data-dependent token routing across sparse expert weights and irregular, memory-bound key-value (KV) cache lookups, rendering dense systolic dataflows ineffective without dynamic crossbar routing and scatter-gather memory interfaces.
