...
Target comparison serves two primary functions: determining how broadly prior claims must be rechecked, and identifying the structural nature of the underlying shift. Determining how broadly prior claims must be rechecked depends on whether decision semantics remain fixed; selective requalification applies only while target decision and acceptance semantics remain in force, whereas material changes require reopening the specific architectural claims that depend on them.
...
1. **Cross-ISA and Memory Consistency Shifts (x86 TSO $\rightarrow$ RISC-V RVWMO):** Microarchitectural synchronization patterns, lock-free queues, and speculative store buffers designed under x86 Total Store Order (TSO) assume hardware-enforced load-load, store-store, and load-store ordering. Transferring these designs to RISC-V, which defaults to the weaker RISC-V Weak Memory Ordering (RVWMO) memory consistency model [@RISCV2026UnprivilegedISA], introduces subtle multi-core data races (e.g., in the store-buffering litmus test where Core 0 executes $W(x) \to R(y)$ and Core 1 executes $W(y) \to R(x)$, x86 TSO forbids both cores from reading 0, whereas RVWMO permits it unless an explicit `fence rw, rw` is inserted).
