...
These analytical transfer bounds translate into realized performance only if the software stack can lower high-level tensor operations onto the underlying datapath without distortion (@fig-codegen-narrow-waist). High-level domain-specific abstract syntax trees (ASTs, such as Python or Triton DSLs) descend through mid-level control-data flow graphs (CDFGs) (MLIR dialects) and LLVM IR down to where workload intent meets architectural mechanisms (CPU ISA extensions or NPU systolic dataflows) and silicon constraints. Lowering is the primary interface where hardware constraints and workload requirements must be jointly qualified for performance, correctness, portability, and maintainability.
...
Inexpensive proxy checks accelerate design-space exploration by constructing multi-tier evaluation cascades within validated coverage regions. In complex system-on-chip (SoC) synthesis, submitting every generated candidate directly to gold-standard signoff tools creates an unmanageable evaluation bottleneck. Multi-tier cascades mitigate this pressure by deploying fast analytical models, cycle-approximate simulators, and learned surrogate estimators as early screening filters. However, realizing genuine speedup requires rigorous error calibration: screening proxies must have bounded false-negative rates to prevent improperly discarding viable microarchitectures, and must possess clear escalation paths to route ambiguous candidates to high-fidelity signoff checkers.
