...
**Knowing whether a screen still ranks.** A surrogate carried to a new target keeps producing scores whether or not those scores still order candidates correctly.

- **What can be measured about a change of target that predicts whether a learned model still ranks candidates correctly?** Retraining and revalidating on every shift is what teams do when they can afford it and skip when they cannot, and the decision is currently made by intuition. A structural indicator that anticipates rank decay would tell an engineer which transfers need re-earning.
- **How can a cheap screen bound the good candidates it discards after conditions change?** Screening is only defensible if the rate at which it wrongly rejects stays bounded, and that rate is exactly what shifts when the target moves. Establishing false-rejection bounds under changed conditions would let screening survive transfer instead of quietly becoming unsafe.

**Requalifying the smallest part that actually needs it.** A change invalidates some prior results and leaves others standing, and the boundary between them is rarely obvious.

- **How can a system identify the smallest set of results a change actually invalidated?** Dependencies run through compilers and tools whose outputs are not deterministic, so tracing sensitivity from a source change to an affected conclusion is harder than tracking file dependencies. Getting this right is the difference between requalifying a subsystem and requalifying a program.
- **When does a transferred pattern stop being the same design problem?** Interconnect, host software, and workload changes can move an offload boundary far enough that the original comparison no longer applies, and no one has characterized where that happens. Answering it would tell teams when to adapt a pattern and when to formulate the question again.
