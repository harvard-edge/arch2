...
Fixed startup cost and transfer time share units with compute time, combining Amdahl's serial-fraction principle [@Amdahl1967ValiditySingleProcessor] with explicit communication costs. Excessive invocations ($N \cdot o$) or transfer overhead ($d g / B$) can erase local compute gains. For students and compiler architects exploring accelerator offloading, a pivotal open research question is how compiler fusion depth shifts the LogCA break-even offload boundary as neural network kernels grow in complexity, and how analytical cost models can guide layer fusion across heterogeneous interconnects.
...
For students exploring generalization and transfer learning in hardware design, the central challenge is identifying which design invariants survive shifts in workloads, compiler toolchains, or semiconductor process nodes. The following open questions provide research vectors for developing mathematically grounded transfer maps and screening surrogates:
