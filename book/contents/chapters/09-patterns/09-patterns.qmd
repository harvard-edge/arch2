...
- **Layer 1 (AI-Assisted Engineering):** Transfer is manual and ad hoc. Engineers copy code generation prompts or testbench templates across projects. Because prompt context omits unstated ABI conventions, clock domains, and physical constraints, transferred prompts silently generate invalid register-transfer level (RTL) code.
- **Layer 2 (AI-Driven Subsystem Optimization):** Transferred optimizers (such as a reinforcement learning (RL) policy trained on FinFET floorplans or a Bayesian design-space exploration (DSE) model fitted to a specific DRAM controller) suffer out-of-distribution failure when technology nodes or workloads shift (e.g., FinFET $\to$ GAAFET or dense general matrix multiply (GEMM) kernels $\to$ MoE models), requiring systematic requalification.
