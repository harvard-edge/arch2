...
- **What bounds the good candidates a cheap screen discards once conditions change?** Screening is only defensible if the rate at which it wrongly rejects stays bounded, and that rate is exactly what shifts when the target moves. Establishing false-rejection bounds under changed conditions would let screening survive transfer instead of quietly becoming unsafe.
...
- **Which results does a change actually invalidate, and how small can that set be made?** Dependencies run through compilers and tools whose outputs are not deterministic, so tracing sensitivity from a source change to an affected conclusion is harder than tracking file dependencies. Getting this right is the difference between requalifying a subsystem and requalifying a program.
