...
The physical reality of hardware specialization is that microarchitectural innovations remain inaccessible without software enablement. When architects introduce specialized hardware primitives (such as warp-level `wmma`, warp-group `wgmma`, tensor memory accelerators, and fifth-generation tensor cores `tcgen05`), the burden shifts to low-level software libraries. In vendor template libraries such as NVIDIA CUTLASS, architecture-specific code exploded by $29\times$ (from $13\text{K}$ to $372\text{K}$ lines of code) between 2019 and 2025, expanding from 23% to 58% of the entire library codebase (@fig-ch09-software-porting-wall). This exponential maintenance tax illustrates why manual template programming cannot scale across rapid hardware iterations, creating a *software porting wall* that isolates custom hardware from production software stacks.

![**The software porting wall in hardware specialization.** NVIDIA CUTLASS template library include line counts across releases from 2019 to 2025 [@NVIDIACUTLASS]. As basic tensor operations grow in microarchitectural complexity (from warp-level `wmma` to warp-group `wgmma` and Blackwell `tcgen05`), architecture-specific template code explodes by $29\times$ (from $13\text{K}$ to $372\text{K}$ lines of code), expanding from 23% to 58% of the entire library codebase. This exponential maintenance tax illustrates why manual template libraries fail to scale across emerging hardware and why compiler narrow waists are essential.](images/fig-ch09-software-porting-wall){#fig-ch09-software-porting-wall width="100%" fig-alt="Stacked bar chart of CUTLASS line-of-code growth from v2.0 in 2019 to v4.0 in 2025. Shared infrastructure grows from 40K to 266K lines, while architecture-specific code expands 29-fold from 13K to 372K lines, reaching 58% of total code."}
