...
@eq-logca-breakeven-g reveals a physical feasibility boundary. If per-unit data transfer overhead exceeds local compute time savings ($d/B \ge C_h(1 - 1/A)$), the denominator becomes nonpositive and $g^*$ is undefined. Under this condition, offloading cannot break even at any operation granularity, and local compute acceleration $A$ is entirely erased by interface communication. Connecting this bound to Williams' Roofline model [@Williams2009Roofline], per-work-unit transfer $d$ is the reciprocal of operational intensity ($d = 1/\mathrm{OI}$). When kernel operational intensity falls below the system balance point ($\mathrm{OI} \le \frac{1}{B \cdot C_h(1 - 1/A)}$), the offload is fundamentally link-bandwidth bound, and scaling batch size $g$ can never overcome communication latency.
...
This lowered sequence directly operationalizes the LogCA parameters. The instruction `vsetvli t0, a3, e32, m8, ta, ma` configures 32-bit floating-point elements (`e32`), sets vector register grouping ($LMUL=8$, `m8`, binding registers `v8`--`v15`, `v16`--`v23`, and `v24`--`v31` into eight-register groups), and declares tail-agnostic (`ta`) and mask-agnostic (`ma`) policies. This register grouping maximizes execution bandwidth per instruction, eliminating instruction-cache pressure and loop control branches. The instruction writes the actual number of processed elements (bounded by remaining count `a3` and hardware $\text{VLEN} \times LMUL / 32$) to `t0`, enabling single-binary portability from 128-bit mobile vector pipelines to 2,048-bit server arrays without recompilation. Keeping vector data resident in vector registers amortizes loop control overhead while avoiding redundant off-chip memory traffic $d$. End-to-end compiler, runtime, interface, and performance claims remain incomplete until workload semantics pass through lowering and execute on the target platform. For the Lighthouse architecture, a compiler-generated interface check and execution run verify correctness while measuring cycle ordering, latency, traffic, and bandwidth against a pinned configuration. Portability claims require demonstrating successful rebuilding and execution across compiler, runtime, library, interface, and hardware changes named in scope.
