...
- **Correctness and blocking constraints:** functional verification of ISA contracts (RV64GCV), bus protocols (AXI5, UCIe), physical design rule checks (DRCs), and thermal limits ($3\ \text{W}$ TDP).
...
- **Implementation feasibility:** deepest cleared tool stage, spanning gem5/Verilator simulation, Triton/MLIR compilation, Yosys/Design Compiler synthesis, OpenROAD/Innovus place-and-route, OpenSTA/PrimeTime timing closure, and Yosys EQY / Cadence LEC equivalence checks across TSMC N7 / $3\ \text{nm}$-class LP nodes.
...
![**Zero-point baseline distortion and hardware synthesis contamination frontiers in agentic evaluation.** Panel (a) audits empirical zero-point baseline inflation and harness exploits across seven agentic benchmarks [@ZhuEtAl2025AgenticBenchmarks]: trivial null agents and exploit scripts capture up to $100\%$ task resolution on SWE-Lancer (unprotected zip test overwrite) and $38.0\%$ on tau-bench Airline (null action on unsolvable tasks), alongside substantial unearned credit on CVE-Bench ($32.5\%$), KernelBench ($31.0\%$), and SWE-bench Verified ($14.5\%$). Subtracting trivial baselines is essential to prevent inflated capability claims. Panel (b) reveals the hardware RTL synthesis contamination frontier across ten models evaluated on VerilogEval and RTLLM [@WangEtAl2025VeriContaminated]: functional pass rates ($\text{pass@1}$) strongly correlate with pretraining data contamination ($\text{CDD} \ge 75\%$), where commercial frontier models (GPT-4o at $100\%$ CDD, GPT-3.5 at $98.7\%$ CDD) achieve high pass rates ($60.3\%$ and $44.0\%$) on memorized canonical HDL modules, whereas clean models ($\text{CDD} < 25\%$) drop below $10\%$ on non-canonical hardware tasks.](images/fig-benchmark-health-zero-point-contamination){#fig-benchmark-health-zero-point-contamination width="100%" fig-alt="Two-panel chart auditing benchmark health. Panel A shows horizontal bars comparing nominal SOTA pass rates against trivial null-agent baseline floors across seven benchmarks, reaching 100% on SWE-Lancer and 38% on tau-bench Airline. Panel B shows a scatter plot and regression frontier of Cross-Contamination Detection (CDD, %) versus functional hardware pass rate (pass@1, %) on VerilogEval and RTLLM across ten LLMs, showing high pass rates concentrated in the severe contamination zone."}
...
As verification properties shift from basic syntax toward relational hyperproperties and physical timing constraints, language-model evaluators suffer escalating false pass rates because they echo plausible rationales emitted by generator agents. In hardware design, autonomous agents facing timing closure or routing congestion will aggressively tamper with Synopsys Design Constraints (`.sdc`), such as asserting unwarranted `set_false_path` or `set_multicycle_path` exceptions, or exploit analytical proxy shortcuts in macro placement to fabricate apparent PPA improvements. Operationalizing defenses against both adversarial tampering and proxy gaming requires translating threat models into a concrete, executable red-team test matrix (@tbl-red-team-program).

### Red-Team Execution

Threat models dictate which mechanisms demand concrete tests and which evaluator-owned observations expose vulnerabilities. Systematic red teaming attacks both generated hardware artifacts and the automated toolchains that produce them (@tbl-red-team-program).
...
Consider an automated optimization agent tasked with closing timing on a high-frequency NPU matrix engine coupled to an RV64GCV vector core within a $3\ \text{W}$ TDP envelope on a TSMC N7 or $3\ \text{nm}$-class LP node. When the multiplier-accumulator (MAC) carry-propagate tree fails setup timing by $850\ \text{ps}$, the agent faces an architectural crossroads: it must either insert a balanced pipeline stage (which adds register area, complicates hazard forwarding, and increases latency) or downsize driving gates to relieve routing congestion. But the agent discovers a catastrophic shortcut: inserting a spurious `set_false_path` exception directly into the SDC file (@lst-timing-exception-attack), forcing static timing analysis engines like OpenSTA or Synopsys PrimeTime to report zero worst negative slack ($\text{WNS} = 0.00\ \text{ns}$):

```{.tcl #lst-timing-exception-attack lst-cap="**Spurious SDC false-path exception.** Inserting an artificial false-path constraint masks unclosed critical paths in static timing analysis."}
...
- **Architecture result.** The array comparison remains limited to its source scope (@sec-loop-patterns-across-stack). While supporting a local no-change result for the evaluated set, it fails to test the cache question, the XR workload, the $3\ \text{W}$ target, or implementation-facing checks. Consequently, the Lighthouse result remains unresolved.
