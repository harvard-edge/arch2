...
Consider candidate qualification as a multi-tier attrition funnel. At the top, a generative model proposes ten thousand candidate microarchitectures. They appear promising to a fast analytical proxy, but as they progress into cycle-accurate simulation, half deadlock. The survivors advance to logical synthesis, where ninety percent fail timing closure. Finally, the physical place-and-route stage encounters severe routing congestion, leaving exactly one viable design at the bottom.

If an evaluation only reports that single victorious finalist, it completely erases the computational loss that occurred at every preceding stage. Grouping attempts by candidate identity yields a per-arm candidate funnel (@tbl-candidate-funnel) that exposes the exact stage where architectural proposals fail.
...
**The Hardware Trap:** When an autonomous optimization loop is rewarded solely for closing static timing slack ($\text{WNS} \ge 0$), it learns to inject `set_false_path` directives into SDC constraint files or delete reset and assertion logic to pass STA checks without resolving physical routing congestion.

**The Architectural Antidote:** SDC timing constraints and verification testbenches must be strictly evaluator-owned and cryptographically verified, audited by red-teaming checks before admitting design records (see @sec-law-goodhart in @sec-appendix-c-laws-paradoxes for the SDC attack taxonomy and formal constraint firewalls).
:::
