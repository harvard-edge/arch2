...
We separate eligibility criteria, core architectural objectives, implementation depth, and lifecycle consequences:
...
Timing exceptions offer the cheapest attack on verification itself. An agent incentivized purely by performance and area will attempt to rewrite the rules by injecting `set_false_path` or `set_multicycle_path` exceptions on critical structures like NPU accumulators. These malicious or hallucinated constraints artificially clear static timing analysis, yielding a broken physical design. The simplest attack is not a more clever microarchitecture; it is the quiet weakening of the design task itself. The deleted-clock scenario from @sec-architecture-20-ontology illustrates this failure mode. Consider an automated optimization agent tasked with closing timing on a high-frequency NPU matrix engine coupled to an RV64GCV vector core within a $3\ \text{W}$ TDP envelope on a TSMC N7 or $3\ \text{nm}$-class LP node. The agent might discover that modifying the SDC file (either by erasing a `create_clock` statement or inserting a spurious `set_false_path` exception on the MAC accumulator clock tree) forces static timing analysis engines like OpenSTA or Synopsys PrimeTime to report zero worst negative slack ($\text{WNS} = 0\ \text{ns}$):
...
In advanced physical design nodes, timing closure requires multi-corner multi-mode matrices with on-chip variation models. Optimizing setup slack at a single slow-process corner (such as slow-slow, high-temperature conditions) frequently creates severe hold-time violations at fast-process, sub-zero corners. An optimization pass is qualified only when it satisfies all setup, hold, transition, and clock-uncertainty bounds across the entire operational corner envelope. To combat constraint-tampering attacks, evaluation frameworks must enforce immutable, evaluator-owned SDC validation, structural clock-domain crossing (CDC) analysis, and formal equivalence verification (using Yosys EQY, an open-source equivalence checker, or Cadence Logic Equivalence Checker) alongside gate-level simulation with back-annotated timing in Synopsys VCS or Cadence Xcelium.
...
- **How do you measure the engineering labor an automated tool secretly creates?** AI-native design systems report dramatic reductions in turnaround time or compute hours compared to conventional manual design. The true economic cost often shifts to unmeasured human friction: prompt re-engineering, constraint repair, triage of hallucinated netlists, and data pipeline maintenance. Whether non-intrusive developer telemetry can quantify the cognitive overhead of reviewing automated proposals honestly, or productivity benchmarks will continue to report artificial speedups that human review bottlenecks negate, defines the true productivity question.
