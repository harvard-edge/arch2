...
![**Zero-point baseline distortion and hardware synthesis contamination frontiers in agentic evaluation.** (a) Empirical zero-point baseline inflation and harness exploits audited across seven agentic benchmarks [@ZhuEtAl2025AgenticBenchmarks]: trivial null agents and exploit scripts capture up to $100\%$ task resolution on SWE-Lancer (unprotected zip test overwrite) and $38.0\%$ on tau-bench Airline (null action on unsolvable tasks), alongside substantial unearned credit on CVE-Bench ($32.5\%$), KernelBench ($31.0\%$), and SWE-bench Verified ($14.5\%$). Subtracting trivial baselines is essential to prevent inflated capability claims. (b) Hardware RTL synthesis contamination frontier revealed across ten models evaluated on VerilogEval and RTLLM [@WangEtAl2025VeriContaminated]: functional pass rates ($\text{pass@1}$) strongly correlate with pretraining data contamination ($\text{CDD} \ge 75\%$), where commercial frontier models (GPT-4o at $100\%$ CDD, GPT-3.5 at $98.7\%$ CDD) achieve high pass rates ($60.3\%$ and $44.0\%$) on memorized canonical HDL modules, whereas clean models ($\text{CDD} < 25\%$) drop below $10\%$ on non-canonical hardware tasks.](images/fig-benchmark-health-zero-point-contamination){#fig-benchmark-health-zero-point-contamination width="100%" fig-alt="Two-panel chart auditing benchmark health. (a) Horizontal bars comparing nominal SOTA pass rates against trivial null-agent baseline floors across seven benchmarks, reaching 100% on SWE-Lancer and 38% on tau-bench Airline. (b) Scatter plot and regression frontier of Cross-Contamination Detection (CDD, %) versus functional hardware pass rate (pass@1, %) on VerilogEval and RTLLM across ten LLMs, showing high pass rates concentrated in the severe contamination zone."}
