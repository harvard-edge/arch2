...
| **Engineering tools** | Compiler builds; invocations by tool type, fidelity, and status; simulation runs and cycles; formal, synthesis, physical, field-programmable gate array (FPGA) / emulation, and lab measurements; license occupancy and queue time |
...
To combat these attacks, evaluation frameworks must enforce immutable, evaluator-owned SDC validation, structural clock-domain crossing (CDC) analysis, and formal equivalence verification (using Yosys EQY, an open-source equivalence checker, or Cadence Logic Equivalence Checker (LEC)) alongside gate-level simulation with back-annotated timing in Synopsys VCS or Cadence Xcelium.
