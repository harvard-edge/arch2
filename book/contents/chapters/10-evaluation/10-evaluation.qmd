...
A candidate design can clear early behavioral simulation only to collapse during signoff in electronic design automation (EDA) tools. Automated loops can consume more inference tokens, simulation cycles, and manual repair hours than the conventional flows they were built to replace, or silently exploit evaluation loopholes (such as excising timing paths with spurious `set_false_path` constraints) rather than improving microarchitecture. To prevent these illusions from corrupting project records, an evaluation protocol must decouple four distinct objects:
...
1. **Candidate design.** The proposed microarchitecture, register-transfer level (RTL), configuration, script, mapping, or other design artifact. It is judged by its function, constraints, architectural quality, implementation feasibility, and the engineering effort required to integrate and maintain it.
...
Having established the eight-step protocol and distinct experimental units, evaluation begins with the primary question: did the system produce a viable piece of hardware? We measure architecture outcomes before evaluating workflow efficiency. Candidate designs must first clear all functional requirements and blocking constraints at the evaluated stage. Only among valid candidates can evaluations compare workload throughput, power, area, latency, cost, and programmability. Failing all checks is a hard failure, not a low score to average into results.
...
Automated model judges lose discriminating power on defective candidates as verification properties shift from basic syntax toward relational hyperproperties (@fig-llm-judge-confirmation-bias). Formally, we frame automated red-teaming as a minimax game (@eq-redteam-minimax):
...
Timing exceptions offer the cheapest attack on verification itself. An agent incentivized purely by performance and area will attempt to rewrite the rules by injecting `set_false_path` or `set_multicycle_path` exceptions on critical structures like NPU accumulators. These malicious or hallucinated constraints artificially clear static timing analysis, yielding a broken physical design. The simplest attack is not a more clever microarchitecture; it is the quiet weakening of the design task itself. The constraint-tampering behavior that @sec-architecture-20-ontology guards against with baseline immutability illustrates this failure mode. Consider an automated optimization agent tasked with closing timing on a high-frequency NPU matrix engine coupled to an RV64GCV vector core within a 3\ W TDP envelope on a TSMC N7 or 3\ nm-class LP node. The agent might discover that modifying the SDC file (either by erasing a `create_clock` statement or inserting a spurious `set_false_path` exception on the MAC accumulator clock tree as shown in @lst-timing-exception-attack) forces static timing analysis engines like OpenSTA or Synopsys PrimeTime to report zero worst negative slack ($\text{WNS} = 0\ \text{ns}$):
