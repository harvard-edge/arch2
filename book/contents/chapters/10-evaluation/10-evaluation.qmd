...
Hardware red-teaming systematically deploys adversarial attacks and fault injections against design pipelines, exposing hidden vulnerabilities in candidate microarchitectures, tool interfaces, and verification checkers [@NIST2008SecurityTesting]. Deliberate attacks, fault injections, and robustness stress tests expose the operational limits of candidate designs and generative workflows.
...
Task and evaluation environments require distinct identities across all eight prompt stack layers: workload (XRBench, SPEC CPU2017, MLPerf), ISA contract (RV64GCV), compute organization (CPU plus NPU), memory hierarchy (caches, TCM, AXI5/UCIe), power envelope (3\ W TDP), software toolchain (Triton, LLVM, MLIR), physical PDK rules (TSMC N7 / 3\ nm-class LP), and verification harness. Modifying any layer yields a different experimental result.
...
Total-cost records support two distinct efficiency claims: complete workflow efficiency (relating supported outcomes to total resource budgets) and sample efficiency (measuring supported improvement per observation at a specific fidelity). Isolating random seed variance, hyperparameter noise, and tool non-determinism avoids spurious comparison claims [@BouthillierEtAl2021UnreproducibleML; @HendersonEtAl2018DeepRL].
...
![**Matched complete-system workflows quantify the total effect of adopting an AI-native design workflow.** A shared architecture question, starting artifacts, evaluation tools, evaluator-owned checks, budget windows, and stopping rules are fed into both the AI-native workflow and the strongest practical alternative. Task-paired differences in supported outcomes and total costs quantify the complete-workflow effect.](images/fig-matched-complete-systems){#fig-matched-complete-systems
