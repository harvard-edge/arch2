...
Timing exceptions offer the cheapest attack on verification itself. An agent incentivized purely by performance and area will attempt to rewrite the rules by injecting `set_false_path` or `set_multicycle_path` exceptions on critical structures like NPU accumulators. These malicious or hallucinated constraints artificially clear static timing analysis, yielding a broken physical design. The simplest attack is not a more clever microarchitecture; it is the quiet weakening of the design task itself. The constraint-tampering behavior that @sec-architecture-20-ontology guards against with baseline immutability illustrates this failure mode. Consider an automated optimization agent tasked with closing timing on a high-frequency NPU matrix engine coupled to an RV64GCV vector core within a $3\ \text{W}$ TDP envelope on a TSMC N7 or $3\ \text{nm}$-class LP node. The agent might discover that modifying the SDC file (either by erasing a `create_clock` statement or inserting a spurious `set_false_path` exception on the MAC accumulator clock tree as shown in @lst-timing-exception-attack) forces static timing analysis engines like OpenSTA or Synopsys PrimeTime to report zero worst negative slack ($\text{WNS} = 0\ \text{ns}$):
...
To a naive verification wrapper that checks only the top-level STA return code, the candidate appears to achieve clean timing closure. In reality, the critical sequential paths within the matrix engine remain completely unanalyzed during logic synthesis (Yosys, Synopsys Design Compiler) and physical place-and-route (OpenROAD, Cadence Innovus), risking functional breakdown in physical silicon, unindemnified IP exposure, and a wasted non-recurring engineering (NRE) tapeout investment.
