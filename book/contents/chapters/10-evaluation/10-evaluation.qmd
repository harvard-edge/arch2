...
Recent systematic audits of agentic evaluation suites underscore how fragile reward formulations and execution environments can be when multi-step agents interact with complex tools (@fig-benchmark-health-zero-point-contamination) [@ZhuEtAl2025AgenticBenchmarks]. In systems and software benchmarks, evaluations frequently suffer from reward leaks (such as scoring empty responses or unhandled exceptions as successful resolutions) and environment isolation failures (such as the GPU memory leakage in KernelBench, where unscrubbed buffers exposed ground-truth tensors directly to candidate agents). In silicon design, these failure modes manifest with severe physical consequences: an uncalibrated testbench can score a vacuous null module as passing if it only asserts clock resets without driving bus transactions, while uncleaned temporary directories allow agents to peek at previous gate-level netlists or static timing analysis (STA) logs. Rigorous evaluation mandates evaluating a trivial "do-nothing" baseline to establish true zero-point calibration, enforcing ephemeral container isolation with complete filesystem and scratch-space memory scrubbing between episodes, and measuring stimulus sensitivity in testbenches to ensure fuzzing vectors actively stress target logic rather than trivial happy-paths [@ZhuEtAl2025AgenticBenchmarks].

![**Zero-point baseline exploitation and benchmark contamination decay in agentic hardware evaluation.** Panel (a) audits zero-point baseline vulnerability across four major agentic benchmarks [@ZhuEtAl2025AgenticBenchmarks]: trivial "do-nothing" null agents capture up to $100\%$ task resolution on SWE-Lancer and $38.2\%$ on tau-bench due to reward leakage and default status-code returns, proving that raw benchmark scores without zero-point subtraction provide an illusion of agent competence. In hardware evaluation suites, uncalibrated testbenches similarly score vacuous tie-offs as passing. Panel (b) illustrates benchmark contamination decay across model generations [@WangEtAl2025VeriContaminated; @JainEtAl2025LiveCodeBench]: when models evaluate on contaminated public repositories (such as verbatim RTL-Repo or VerilogEval problems), reported $\text{pass@1}$ is inflated by up to $1.82\times$ through training memorization. Shifting to temporal time-split holdout suites (tasks authored strictly after model training cutoff) causes performance to drop to true generalization levels, confirming that frozen public benchmarks suffer severe evaluation decay.](images/fig-benchmark-health-zero-point-contamination){#fig-benchmark-health-zero-point-contamination width="100%" fig-alt="Two-panel chart auditing benchmark health. Panel A shows a bar chart of task resolution rates by trivial null agents on four benchmarks, reaching 100% on SWE-Lancer. Panel B shows a line chart comparing model pass rates on contaminated benchmark problems versus temporal holdout problems, illustrating a 1.82x inflation on memorized test sets."}
