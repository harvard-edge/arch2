...
interquartile ranges, and granular per-task results. We must avoid relying solely on means and standard deviations, as they can mask the reality of hardware studies that exhibit bimodal outcomes, situations where one cluster of runs fails implementation while another discovers a strong design.
...
- **How can the contribution of one component be isolated without running every ablation?** Full ablation across physical design iterations is unaffordable, so most attribution claims rest on comparisons that were never run. Methods that recover component contribution from fewer experiments would make those claims testable rather than rhetorical.
- **Where does a workflow become expensive enough that its architectural advantage stops paying for it?** Human review, data upkeep, and failed runs scale with the workflow's own complexity, and the point where total cost overtakes benefit is unmeasured. Locating it is the empirical question underneath the entire book.
...
- **How can an evaluation separate tool nondeterminism from a genuine difference in a design system's behavior?** Physical design backends vary run to run by margins comparable to the effects under study, which means a reported improvement may be a seed. Distinguishing the two reliably is a prerequisite for every comparative claim in this area.

**Attacking a result on purpose.** A candidate that passes every check may have been built to pass those checks.

- **How can a review systematically surface specification gaming rather than waiting to encounter it?** Constraint exceptions, checker exploitation, and dormant failure modes survive precisely because standard evaluation does not look for them. Turning red-teaming into a repeatable practice with stated coverage, rather than an ad-hoc exercise, is open work with immediate use.
