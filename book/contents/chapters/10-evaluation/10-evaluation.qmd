...
To harden our systems, we use hardware red-teaming to systematically deploy adversarial attack strategies and fault injections against our design pipelines, exposing hidden vulnerabilities in candidate microarchitectures, tool interfaces, and verification checkers [@NIST2008SecurityTesting]. In the sections that follow, we analyze how deliberate attacks, nonmalicious fault injections, and robustness stress tests expose the limits of our candidate designs and generating workflows, recognizing that no single battery of tests provides absolute coverage.

When we evaluate a single design study, we must qualify any returned signal before our feedback loops dictate microarchitectural modifications. At scale across multiple design tasks, iterative runs, varying conditions, cost structures, engineering teams, and adversarial attacks, our complete-workflow evaluation determines whether an integrated human-plus-technical workflow improves architectural outcomes relative to traditional baselines while treating automated feedback failures as systemic errors.

A single successful synthesis run cannot pinpoint the source of an advantage. A clean layout or a passing testbench might stem from a capable underlying model, a robust conventional search heuristic, a well-engineered verification harness, intensive manual interventions by engineers, or a trivial benchmark. Conversely, a failure might originate from a flawed microarchitecture, a poor methodological choice, a misconfigured EDA environment, or an unrecognized adversarial exploit. Our evaluation methodologies must untangle these variables rather than compressing outcomes into a single scalar score.

Our role as evaluators is to deliver a supported claim, not to authorize a tapeout or accept residual project risk. As detailed in @sec-what-architect-owns, lead architects remain accountable for technical recommendations, while the named commitment authority decides whether the design advances and accepts residual project risk.
...
When we structure our analysis around distinct evaluation objects, we prevent the conflation of candidate designs, individual algorithmic methods, technical systems, and complete human-in-the-loop workflows. As we delineate these targets, we establish a taxonomy of evaluation objects and measurement protocols that guarantees interpretable, comparable assessments.
...
This protocol connects the measurements established across the evaluation framework without prescribing one
...
quantity of interest is fully observed; partial observations are retained according to the principles
established in
...
We treat a timeout as a failure when our targeted outcome is success within a fixed budget. However, we treat it as a right-censored event when our outcome metric is simply the time required to reach a supported result; in that scenario, the true completion time is mathematically known only to exceed the timeout threshold. As established in @sec-data-representations-world-models [@KleinMoeschberger2003Survival], this is the classic timeout-specific form of a censored observation. Our analysis must never discard either case, nor can we assign a fabricated completion time just to make the math look cleaner.
...
We treat hardware red-teaming as a foundational principle for architectural evaluation. We deploy structured adversarial attacks against our design methodologies, tool interfaces, and constraint handlers to expose hidden vulnerabilities long before silicon commitment. Rather than passively waiting for operational failure, we actively stress our workflows to harden AI-native systems against both malicious exploitation and systemic collapse.
...
**The trap.** A generator might rename a signal to fix a lint error, only to have the change fail equivalence checking. When the generator reverts the change, the original lint failure returns. Consequently, the proposal and checking components alternate between the exact same two failures indefinitely.

**The mechanism.** While each individual component responds correctly to its immediate input, the configured workflow lacks a cycle detector, a shared retry budget, or a definitive stopping rule for these repeated states.
...
Applying these rules to @sec-running-the-loop requires separating the unexecuted Lighthouse cache record from the executed array record. This separation allows us to state what our evaluation supports, what remains unsupported, and what we must leave unresolved.
...
**Separating tool variance from system performance.** Physical backends and stochastic environments introduce noise that can obscure whether an AI-assisted design system truly outperforms a baseline.
