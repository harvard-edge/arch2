...
This echoing vulnerability explains why language-model reviewers fail on the hardest hardware verification tasks: when an optimizer encounters unclosable timing slack or routing congestion, it readily gambles on plausibly argued Synopsys Design Constraints (`.sdc`) mutations (such as asserting unwarranted `set_false_path` or `set_multicycle_path` exceptions) or exploits analytical proxy shortcuts in macro placement to fabricate apparent PPA improvements. Operationalizing defenses against both adversarial tampering and proxy gaming requires translating threat models into a concrete, executable red-team test matrix (@tbl-red-team-program).
