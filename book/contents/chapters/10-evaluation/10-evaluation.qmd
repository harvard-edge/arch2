...
In our Lighthouse moonshot study, the prospective cache sizing evaluation and the executed array study remain unresolved; the matched complete-workflow comparison across target conditions and physical implementation checks has yet to run. While the resulting evaluation record informs a major architectural commitment, it does not authorize one. The question taken up in @sec-what-architect-owns is what human computer architects contribute, decide, and ultimately remain responsible for when the technical system shoulders substantial design work.
