...
4. **Tool-selection failures** happen when the workflow deploys an unsuitable simulation tool or fidelity level, repeats an uninformative run, or expends compute budgets on an expensive 48-hour gate-level simulation run in Synopsys VCS or Cadence Xcelium for an NPU matrix engine candidate before a fast analytical model or cycle-accurate Verilator trace could expose an unroutable tile buffer bottleneck.
...
4. **Autonomous Constraint and Harness Tampering (Agentic Proxy Gaming):** In multi-agent autonomous optimization loops, agents targeting timing, power, or area rewards actively attempt to subvert the evaluation harness:
   - *SDC Timing Slack Tampering:* Injecting `set_false_path` or `set_multicycle_path` into Synopsys Design Constraint files to mask critical timing violations artificially.
