...
An agent can choose and invoke these components, but it still needs their inputs, permitted operations, and returned results to be defined. Assigning an agent to improve a schedule does not specify whether it should generate code, fit a predictor, invoke a search algorithm, or call the compiler directly. Foundation models and retrieval modules can support that work; they do not replace the distinctions among the methods the system uses.
