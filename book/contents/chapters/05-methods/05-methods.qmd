...
The representations in @sec-data-representations-world-models make a design question evaluable; they do not determine which method should answer it. A cache study with three legal capacities presents a different computational problem from a compiler search with millions of schedules. Method selection begins by asking which engineering job limits progress, what an added method could improve, and whether that improvement justifies its total cost. The represented design and environment state, $s = \langle s_{\text{design}}, s_{\text{env}} \rangle$, supplies the conditions under which that comparison is meaningful.

For a small candidate set, the setup and qualification of a learned model may cost more than evaluating every candidate directly. An analytical roofline model[^fn-roofline-c05] may already expose the limiting resource, and a simulation sweep may resolve the remaining comparison. For a larger space, those same tools can supply reference observations to a surrogate or search procedure. The question is what the added component contributes, rather than whether the task can be described as a machine-learning problem.

Different bottlenecks demand specific methods. Direct simulation sweeps cleanly settle comparisons among small sets of microarchitectural configurations. Predictive surrogates screen candidates and search optimizers ration evaluation budgets when millions of configurations compete for scarce cycles. Generative models construct missing artifacts when synthesizable register-transfer level (RTL) code (which describes cycle-exact hardware behavior), testbenches, or compiler schedules do not exist. Direct tool execution always remains the default engineering baseline.

The comparison must include model training, surrogate calibration, and verification. An added method may earn its cost by reaching a better supported architecture result, reducing the cost of a comparable result, or both. Each choice therefore needs an explicit search space, budget, stopping rule, and checking path matched to the claimed property. These requirements carry method selection into the execution environments developed in @sec-architecture-environments-tool-interfaces.
...
- **Determine** the smallest sufficient method by comparing result quality and total cost against credible analytical, heuristic, and direct-evaluation alternatives.
- **Model** a multi-tier evaluation pipeline as a queueing network, and derive the candidate arrival rate above which the slowest stage collapses into an unbounded backlog.
- **Mitigate** the optimizer's curse, the systematic over-optimism of whichever candidate a noisy surrogate ranked first, using calibrated prediction intervals and empirical shrinkage.
- **Compose** methods with explicit handoffs, property-specific checks, and fallbacks, identifying shared assumptions that could let errors pass undetected.
:::

## Engineering Roles and Method Families

Accelerating hardware design requires decoupling functional engineering roles (what work must be done) from underlying method families (how that work is performed). In architecture studies, the goal is constructing missing artifacts, estimating physical consequences, searching decision spaces, or checking claimed properties. If the candidate set is already known and affordable to evaluate, the first comparison is whether a predictor or optimizer saves enough work to offset its setup and qualification.
...
Method families and operational layers answer different questions. An optimizer tuning macro placement inside a fixed floorplan operates within the frozen interfaces of Layer 2 (@fig-ch01-three-layers-ai-integration). Adding generation and prediction does not itself grant authority to change those interfaces, and cross-boundary exploration does not require all three families. The permitted scope of the workflow determines the layer; the work within that scope determines the methods (@fig-role-method-map).
...
- **Support, reversibility, and review capacity:** Methods must declare valid operating ranges. Reversible suggestions tolerate higher error rates than automated register-transfer level (RTL) edits or tapeout decisions.
...
#|   confidence level ($\approx 96\%$). When the workload regime shifts on the right ($x > 6.0$), the
...
A learned placement policy has a training cost before it offers reusable action choices. If every training interaction invokes a full physical implementation flow, compute and license use may dominate the budget. Cheap proxy rewards reduce that cost, while reuse across related blocks can spread it across decisions. Both advantages are conditional: proxy quality needs confirmation, and transfer must be demonstrated on the blocks that will use the policy.
...
The plan is a design path rather than a design loop, for the reason @sec-architecture-20-ontology already gives about this same study: its three candidates are fixed before the first run, so a returned measurement can only reject a candidate or end the study. What the method choice adds is why that shape is correct here. Direct evaluation of an enumerable set has nothing for a return to redirect, and a plan that promised otherwise would be claiming a loop it does not need.
...
- **Composition with independent checks.** Separate construction and checking roles, then examine their shared assumptions. A parser, formal property, or reference implementation contributes only the evidence its mechanism can establish; another agent's agreement does not ensure independence.
:::
...
[^fn-egraph-c05]: **E-graphs and Equality Saturation**: An e-graph (equivalence graph) compactly represents many equivalent expressions. Instead of applying rewrites one by one and losing the original form (which can lead to a local minimum), equality saturation applies all possible rewrite rules simultaneously, adding the new forms to the graph without deleting old ones, until no new forms can be found.
...
[^fn-ood-c05]: **Out-of-Distribution (OOD) Detection**: OOD detection methods serve as a critical complement to conformal prediction by identifying when inputs fall completely outside the model's training manifold.
