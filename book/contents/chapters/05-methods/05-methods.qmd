...
If we jumped straight from our cache comparison to a favored mechanism, we would hide the actual work required. Making our design decisions explicit means separating four levels — engineering role, method family, technique, and system composition — and evaluating them in sequence so that our engineering goal dictates the operational method family, which in turn selects a concrete conventional, learned, or hybrid technique before we compose multi-component pipelines (@tbl-role-by-stall-mode).
...
Fit alone does not make an approach usable. Before we commit engineering resources, we need an explicit checklist that compares direct tool paths against added method compositions on equal footing (@tbl-method-role-limits). Each of its nine attributes — job definition, inputs, output, search space, evaluator rigor, total cost, confidentiality, reliability, and stopping rules — pairs with a concrete pre-selection question. Requiring every proposed method to answer those questions upfront prevents comparing vague algorithm names and grounds every technical path in physical feasibility and cost.
...
Downstream tool capacity, not proposal rate, governs pipeline feasibility (@fig-candidate-check-capacity). The structural screen sits at near-zero utilization and cycle simulation scales moderately, but physical implementation screening climbs steeply because of its 24-hour execution latency. At the illustrative operating point of 32 candidates per day (@tbl-candidate-check-capacity), Stage 3 runs at 80 percent utilization ($\rho_3 = 0.80$), leaving 20 percent headroom for retries and tool licensing delays. As the arrival rate rises toward $g_{\text{max}} = 40 \text{ proposals/day}$, Stage 3 reaches the queue stability limit ($\rho_3 = 1.0$); pushing generation past 40 candidates per day drives it into the unstable backlog region, where unverified candidates accumulate faster than signoff tools can evaluate them.
...
The two paths never merge into one source of truth (@fig-architecture-cost-model). The same represented state and proposed action feed both a scoped consequence model — which incorporates a transition model when state dynamics matter — that returns a prediction, and a declared tool or simulator path that records an empirical observation, and both land in a versioned study record. The consequence model only estimates behavior within qualified bounds while the physical tools provide authoritative validation. Qualified agreement between the two lets us continue using the model within its stated support region, but we never mistake a surrogate prediction for a ground-truth observation.
...
We can only justify predictor-based screening while its calibrated coverage survives under the exact conditions where it operates. Inside the calibrated support the empirical coverage stays near nominal, and it collapses once the workload response changes while the interval remains unchanged (@fig-ch05-conformal-coverage). This synthetic construction does not estimate the true failure rate of a real architecture predictor; what it fixes is the rule that we can never carry nominal marginal coverage from one support region into a screening pass once the relevant workload or design response shifts.
...
Because different artifacts expose distinct structures, our construction methods must enforce different constraints. Each candidate generation path carries a verification bound it can honor and one it cannot (@tbl-generation-techniques): constrained decoding or spatial priors can enforce syntax and boundary geometry during synthesis, but property-specific simulation, timing analysis, and physical signoff remain mandatory whatever the artifact.
...
We must fix these conditions firmly before any execution begins (@tbl-generation-construction-pair). For one C/C++ compute kernel under HLS pragma DSE, we pin the shapes, data types, target FPGA/ASIC libraries, and synthesis engines before choosing either constructor, so that conventional autotuners and code-generation models compete under identical evaluation rules over the same tools, checks, and matched resources.
...
To guide this process, we rely on an acquisition function that balances surrogate predictions with estimated uncertainty to select our next evaluation point (@fig-bayesian-optimization).
...
Bayesian optimization trades exploitation of high-mean regions against exploration of high-uncertainty ones across a single-parameter space (@fig-bayesian-optimization). A Gaussian process surrogate mean carries three observations under a $\pm 1 \sigma$ uncertainty band that widens where we have not sampled, and the upper-confidence-bound acquisition function $\mu(x) + \kappa \cdot \sigma(x)$ combines the two: an exploitation peak sits near high predicted performance ($x \approx 3.8$), but the global acquisition maximum falls at $x \approx 7.2$, where high uncertainty meets a favorable mean, and that is where we send the next physical tool evaluation. Keeping surrogate prediction separate from acquisition selection lets us probe high-uncertainty regions to reduce ignorance without mistaking early predictions for final ground truth.
...
Reinforcement learning is plausible here because each macro placement alters the state that conditions subsequent actions (@fig-alphachip-placement). The policy emits one macro-location action at a time and receives the updated partial floorplan back as the next state; once all macros are placed, an approximate standard-cell placement produces a weighted proxy cost over cell density, wirelength, and routing congestion, and the negative of that cost is the terminal reward. Keeping action selection, partial-state return, and terminal evaluation distinct keeps us from mistaking proxy rewards for final signoff verification.

Search tactics diverge under physical design constraints because sample efficiency and asymptotic quality are different properties (@fig-ch05-search-efficiency). In the constructed scenario, zero-shot spatial prompting locates strong macro floorplans within tens of evaluations, while reinforcement learning keeps improving across thousands of environment steps, so early convergence speed trades against ultimate quality.
...
Across the four constructed trajectories, the right search choice depends on the evaluation budget rather than any universal ranking among method families (@fig-ch05-search-efficiency). LLM spatial prompting reaches roughly 0.86 normalized quality within 25 tool evaluations, the shape we would expect if learned spatial heuristics transfer to placement. Bayesian optimization holds strong early sample efficiency up to about 100 evaluations before plateauing near 0.86, the stall continuous Gaussian process surrogates can exhibit in high-dimensional placement spaces. Reinforcement learning explores extensively and then scales smoothly across policy updates, approaching the highest asymptotic score (0.95) near 1,500 evaluations, while random search grows slowly to about 0.66 at 2,000 evaluations. Sample efficiency and asymptotic quality are therefore different properties, and a method can win at one budget and lose at another. Placement-stage proxies can guide the search, but they do not simulate every downstream routing, timing, or manufacturing effect, so our later checks must bridge that gap.
...
Our Lighthouse cache study presents the exact opposite shape. The three cache-capacity settings we proposed already define the entire candidate set, and our budget safely covers a direct SRAM screen followed by matched cycle-level evaluations. If we introduced a generator, it would construct nothing new; if we added a predictor, it would demand observations we do not possess; and if we deployed an optimizer, we would just be needlessly searching through a simple three-point comparison. Set side by side across the same decision parameters, the two domains resolve oppositely (@tbl-worked-method-decisions): missing capabilities and candidate-pool size, not method reputation, govern whether a complex hybrid pipeline or a minimal direct tool path is justified.
...
Our evaluation environment must receive far more than a selected method: we keep its allowed changes, required tool roles, cost envelope, validation checks, and fallbacks tightly coupled to our approach (@tbl-lighthouse-approach-decision). For the Lighthouse cache study — an L2 SRAM cache configuration for a 3\ W Mobile XR SoC with a RISC-V RV64GCV application cluster fabricated on a TSMC N7/N3 process node with die-to-die UCIe chiplet interconnects and CXL memory expansion interfaces — bounding those allowed changes and tool contracts explicitly keeps the direct evaluation plan reproducible and verifiable.
