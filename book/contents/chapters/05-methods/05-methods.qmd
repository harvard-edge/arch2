...
Predictive surrogates require strict boundaries against physical tools. Regardless of validation accuracy, a learned model remains an inexpensive proxy rather than an authoritative oracle. If an architecture team substitutes surrogate predictions for actual simulator signoff without strict auditing, approximation errors silently compound into physical timing or area violations.
...
::: {.callout-failure-mode title="Paradox and Bound: The Optimizer's Curse in Proxy Search"}
...
**The Hardware Trap:** In DSE, an optimizer tuning an out-of-order core against an uncalibrated proxy picks a 512-entry ROB where proxy delay errors peak. The proxy ignores quadratic wire delay, yielding a microarchitecture that fails physical timing at the target 3.0\ GHz clock frequency.
**The Architectural Antidote:** Bound surrogate optimism using split-conformal prediction intervals ($\hat{C}(x)$) and mandate independent full-physics signoff on all top-ranked candidates (see @sec-law-optimizers-curse for the order-statistics proof, tail rank correlation metrics, and split-conformal coverage intervals).
:::
...
This drop-off fundamentally changes how generative methods must be evaluated. Comparing only the best generated kernel against an unoptimized baseline makes the generative method appear artificially superior. The comparison ignores discarded drafts, compilation overhead, and the possibility that a conventional autotuner given the same compute budget might discover superior kernel schedules.
...
For a one-off architectural decision, direct search, deterministic heuristics, or Bayesian optimization provide far lower total cost. When evaluation budgets are strictly limited, sample efficiency becomes the governing constraint.
