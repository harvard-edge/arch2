...
#| fig-cap: |
#|   **Physical floorplanning search efficiency and PPA quality across optimization methodologies.** Constructed illustrative trajectories. No tool sweep stands behind these curves and no ranking among method families is claimed; this chapter argues the opposite, that no procedure is universally superior. What the curves show is that sample efficiency and asymptotic quality are different properties, so a method can win at one budget and lose at another. LLM spatial prompting provides rapid early convergence, reaching roughly $0.86$ normalized quality within $25$ evaluations by exploiting learned placement heuristics. Bayesian optimization delivers strong early sample efficiency but plateaus in high-dimensional macro spaces, while reinforcement learning needs a far larger sample budget as it approaches the highest asymptotic quality ($0.95$) among the illustrated curves.
#| out-width: "100%"
#| fig-alt: "Log-scale line plot of constructed illustrative curves showing normalized floorplan quality versus tool evaluations from 1 to 2,000. LLM spatial prompting rises fastest early, Bayesian optimization plateaus, reinforcement learning approaches the highest asymptote, and random search grows slowly. The values illustrate shapes and are not measurements."
...
The semi-log plot in @fig-ch05-search-efficiency tracks normalized floorplan PPA quality scores along the vertical axis against physical design tool evaluations (from 1 to 2,000 on a logarithmic scale) on the horizontal axis. The visualization contrasts four constructed optimization trajectories. LLM spatial prompting (orange curve) achieves rapid zero-shot convergence, reaching roughly 0.86 normalized quality within 25 tool evaluations by leveraging learned spatial heuristics. Bayesian optimization (dashed blue curve) provides strong early sample efficiency up to 100 evaluations but plateaus around a 0.86 score as Gaussian process surrogates struggle with high-dimensional placement spaces. Reinforcement learning (purple curve) requires extensive initial exploration but scales smoothly across policy updates, achieving the highest asymptotic peak PPA score (0.95) at 1,500 evaluations. Random search (dotted grey baseline) grows slowly, reaching only about 0.66 quality at 2,000 evaluations. Comparing these trajectories shows that search choice depends directly on evaluation budget. Zero-shot prompting excels under tight sample limits, whereas reinforcement learning dominates when extensive tool budgets permit asymptotic policy tuning. While placement-stage proxies successfully guide search, they do not simulate every downstream routing, timing, or manufacturing effect; our later checks must explicitly bridge that gap.
