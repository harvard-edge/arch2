...
This operational limit applies strictly to uncoordinated LLM generation. When multiple prompts or agents independently propose RTL candidates or author testbenches, they act as uncoordinated stochastic arrival processes. Without centralized rate limiting, an LLM generation loop rapidly saturates the most restrictive downstream EDA queue, producing high token-generation costs but zero additional validated throughput. Agents must observe the queueing model directly, pausing generation or stiffening local rejection thresholds when downstream utilization $\rho_i$ approaches unity. The system must regulate LLM generation dynamically through a throttling loop (see the algorithm below).
