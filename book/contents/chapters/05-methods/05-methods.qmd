...
This operational limit applies strictly to multi-agent generative systems. When autonomous agents independently propose microarchitectural candidates or author testbenches, they act as uncoordinated stochastic arrival processes. Without centralized rate limiting, a multi-agent debate loop rapidly saturates the most restrictive downstream queue, producing high token-generation costs but zero additional validated throughput. Agents must observe the queueing model directly, pausing generation or stiffening local rejection thresholds when downstream utilization $\rho_i$ approaches unity. The system regulates generation dynamically through a backpressure control loop (see the backpressure control loop below).

```pseudocode
#| label: alg-dynamic-backpressure
...
\begin{algorithm}
\caption{Dynamic Backpressure for Multi-Agent Generation}
\begin{algorithmic}
...
\Loop
  ...
  \ElsIf{$\rho_{\text{max}} < \rho_{\text{limit}} - \epsilon$}
    \State $g \gets \min(g_{\text{nom}}, g \times (1 + \delta))$ \Comment{Relax backpressure}
    \State $\text{EnforceTokenRateLimit}(\mathcal{A}, g)$
