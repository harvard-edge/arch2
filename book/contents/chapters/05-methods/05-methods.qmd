...
- **What statistical tests can detect online when active search has driven surrogate predictions outside valid support?** As optimization algorithms exploit surrogate error spaces, candidate selections rapidly trigger out-of-distribution shifts that silently invalidate conformal prediction error bounds. Developing lightweight, online hypothesis tests that detect coverage loss from candidate feature dispersion alone (without consuming high-fidelity simulation budgets) is necessary to prevent optimization loops from pursuing hallucinated surrogate minima.
- **How do the reference tool runs required for surrogate calibration scale with microarchitectural parameter dimensionality?** As architectural search spaces expand from tens to hundreds of discrete and continuous variables, the calibration runs needed to guarantee ranking fidelity explode. Formulating programmatic criteria that predict the exact dimensionality threshold where surrogate calibration overhead exceeds the cost of direct tool evaluation is required to determine when high-dimensional search should abandon surrogates in favor of direct active learning.
- **To what degree does employing independent model families eliminate shared specification blind spots between generated RTL and testbenches?** When large language models generate both hardware implementation code and verification assertions, they frequently replicate specification discrepancies. Quantifying the covariance between generator and verifier error across distinct model architectures is essential to establish reliable autonomous verification.
- **What formal scheduling framework determines the optimal placement of deterministic symbolic filters within generative pipelines?** Placing strict physical and semantic filters (such as linting, CDC checks, and formal SVA properties) too early in the generative loop prematurely rejects candidates that downstream repair passes could easily fix, whereas placing them too late wastes extensive simulation compute on fatally malformed code. Establishing dynamic scheduling policies for neurosymbolic verification stages is required to maximize candidate qualification throughput under fixed compute envelopes.
- **How should generative search algorithms regulate candidate proposal rates when downstream evaluation tools exhibit extreme latency variance?** While functional linting completes in seconds, gate-level placement, routing, and physical timing closure fluctuate from minutes to tens of hours depending on design congestion. Dynamically throttling candidate proposal generation using closed-loop backpressure control to prevent memory exhaustion and cluster queue instability is essential to maintain pipeline stability across heterogeneous electronic design automation (EDA) clusters.
