...
When evaluation paths transition from one-shot prompting to these iterative structures, the physical bottleneck shifts from downstream simulation queues to the generator itself. For autonomous agent loops, intelligence alone is insufficient; generation latency strictly bounds the system. An operational sweet spot governs model selection, requiring components that balance high Software Engineering Benchmark (SWE-Bench) coding capability against high effective inference speed, measured in tokens per second (@fig-agent-latency-sweet-spot). Selecting a slow maximal-parameter model stalls the multi-agent exchange on inference latency, while a rapid but incapable model collapses under repeated syntax and verification rejections. The selected model must preserve loop throughput by maintaining both correctness and generation velocity.

![**Agent generation latency.** For autonomous agent loops, intelligence is insufficient; generation latency matters. The sweet spot contains models that balance high Software Engineering Benchmark (SWE-Bench) coding capability with high effective inference speed (tokens per second), preventing loop bottlenecks.](images/plot_ch5.png){#fig-agent-latency-sweet-spot width="100%" fig-alt="Scatter plot with Effective Inference Speed on the x-axis and SWE-bench Capability on the y-axis, highlighting a sweet spot that balances both properties."}
