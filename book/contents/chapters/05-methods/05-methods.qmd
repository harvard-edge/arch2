...
- **How can an optimizer notice that it is being fooled by its own surrogate before spending the ground-truth budget that would prove it?** Average surrogate error says little about the errors that matter, which are the ones near the decisions the search is about to make. Detecting local rank inversions cheaply, and knowing which of them deserve an expensive check, is unresolved for every method family in this chapter.
...
- **How can a generator respect structural and physical rules without collapsing the diversity that made generation worth doing?** Filtering after the fact wastes most of what was produced, while building rules into the model tends to shrink what it will propose. The trade between legality and coverage is understood informally and has not been characterized in a way a team could plan around.
