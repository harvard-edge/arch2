...
Method selection requires strict resource accounting. Model training, surrogate calibration, and manual verification costs must not exceed the expense of direct execution. The limiting engineering role must dictate the chosen method and preserve independent physical oracles to qualify every output. We can formalize this intuition by modeling the toolchain as a feed-forward queueing network. This network balances the rate at which candidates arrive against the capacity of downstream services. For any tool stage $i$, let $c_i$ be its number of parallel slots, $t_i$ its mean candidate runtime, and $p_i$ its conditional pass rate. The stage's processing capacity is then $\mu_i = c_i / t_i$.

Meanwhile, the offered load (or effective arrival rate) arriving at stage $i$ is thinned by all preceding filters, expressed as $\lambda_i = g \prod_{j=1}^{i-1} p_j$, where $g$ is the initial generation rate. Dividing load by capacity yields the stage utilization: $\rho_i = \lambda_i / \mu_i$. To maintain steady-state stability and avoid unbounded queues, utilization must remain strictly below 1 ($\rho_i < 1$) across all stages. This constraint establishes the maximum viable candidate arrival rate $g$ (@eq-queue-stability-bound) [@Kleinrock1975QueueingSystems]:
...
We must measure speedups strictly in terms of *service work*, the expected evaluation effort per comparison, explicitly excluding external queueing delays [@Amdahl1967ValiditySingleProcessor]. Consider a high-fidelity reference path, such as a cycle-accurate RTL simulation or a gem5 full-system run, with a mean cost of $c_{\mathrm{hi}}$. We can introduce a cheaper screening model (like a SCALE-Sim estimate) with cost $c_{\mathrm{cheap}} < c_{\mathrm{hi}}$.

The key metric for this screen is its *validated coverage* ($f$): the fraction of comparisons it can confidently settle without escalating to the reference path. If we submit a fraction $a$ of all candidates to the screen, the coverage $f$ can never exceed $a$. Any candidate the screen fails to confidently classify must fall back to the expensive reference path, incurring both costs. Using selective classification, we can cleanly account for this total effort [@ElYanivWiener2010SelectiveClassification]:
...
To prevent overconfident candidate filtering, surrogate point estimates must be paired with uncertainty quantification. Split-conformal prediction[^fn-ood-c05] attaches distribution-free, finite-sample prediction intervals $\hat{C}(x)$ to learned surrogates, guaranteeing marginal coverage at a declared confidence level $1-\alpha$ without parametric assumptions on error distributions [@LeiEtAl2018Conformal; @AngelopoulosBates2021Conformal]:[^fn-split-conformal-c05]
