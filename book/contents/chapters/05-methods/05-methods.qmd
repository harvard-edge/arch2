...
- **Differentiate** architecture engineering roles, method families, and concrete techniques across the three operational layers to decouple workflow requirements from automated algorithms.
...
- **Model** multi-tier candidate evaluation pipelines as feed-forward queueing networks to derive maximum arrival rates ($g < \min_i (\mu_i / \prod p_j)$) that prevent downstream queue collapse ($\rho_i \ge 1$).
...
- **Compose** generation, prediction, and search into neurosymbolic and multi-agent debate pipelines with independent verification oracles and deterministic fallbacks.
:::
...
- **Smallest sufficient approach.** Deploy the simplest method addressing the active bottleneck, defaulting to direct simulation sweeps or analytical rooflines whenever budget permits before authorizing learned surrogates.
- **Feed-forward queue stability.** Multi-stage evaluation pipelines must enforce arrival-rate bounds ($g < \min_i (\mu_i / \prod_{j=1}^{i-1} p_j)$) with active backpressure to prevent downstream physical signoff queues from collapsing ($\rho_i \ge 1$) as upstream generator yield improves.
- **Optimizer's curse and conformal bounds.** Predictive surrogates systematically suffer positive selection bias ($\mathbb{E}[\max \hat{f}] > \max f$); screening must deploy split-conformal prediction intervals ($\hat{C}(x)$) and empirical shrinkage to ensure predicted wins survive physical signoff.
- **Neurosymbolic composition and independent oracles.** Generated candidates and CUDA/RTL drafts must pass symbolic parsers, formal assertion checks, and noncorrelated verification critics rather than self-verifying testbenches.
:::
