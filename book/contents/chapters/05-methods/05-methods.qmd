...
@fig-method-selection-guide composes those criteria into a conditional flow, branching on which kind of work is limiting rather than on which method is available.
...
\begin{algorithm}
\caption{LLM Generation Throttling for Multi-Agent Generation}
\begin{algorithmic}
\Require Nominal generation rate $g_{\text{nom}}$, utilization limit $\rho_{\text{limit}} \in (0, 1)$
\Require Ordered evaluation stages $S_1 \dots S_M$ with queues $Q_1 \dots Q_M$
\Require Multi-agent proposal system $\mathcal{A}$
\State $g \gets g_{\text{nom}}$
\Loop
  \State $\rho_{\text{max}} \gets 0$
  \For{$i = 1 \dots M$}
    \State $\mu_i \gets \text{MeasureServiceCapacity}(S_i)$
    \State $\lambda_i \gets \text{MeasureArrivalRate}(Q_i)$
    \State $\rho_i \gets \lambda_i / \mu_i$
    \State $\rho_{\text{max}} \gets \max(\rho_{\text{max}}, \rho_i)$
  \EndFor
  \If{$\rho_{\text{max}} > \rho_{\text{limit}}$}
    \State $s \gets \rho_{\text{limit}} / \rho_{\text{max}}$ \Comment{Compute necessary throttling factor}
    \State $g \gets g \times s$
    \State $\text{EnforceTokenRateLimit}(\mathcal{A}, g)$
    \State $\text{RaiseAgentCriticThresholds}(\mathcal{A}, s)$ \Comment{Stiffen local rejections}
  \ElsIf{$\rho_{\text{max}} < \rho_{\text{limit}} - \epsilon$}
    \State $g \gets \min(g_{\text{nom}}, g \times (1 + \delta))$ \Comment{Relax rate-limiting}
    \State $\text{EnforceTokenRateLimit}(\mathcal{A}, g)$
    \State $\text{RestoreAgentCriticThresholds}(\mathcal{A})$
  \EndIf
  \State $\text{Wait}(\Delta t)$ \Comment{Re-evaluate across fixed control interval}
\EndLoop
\end{algorithmic}
\end{algorithm}
```
