...
Method selection begins with the physical bottleneck. Architects must answer three questions before authorizing machine learning: which engineering job acts as the active constraint, does a learned model genuinely remove that constraint, and what is the smallest method preserving a deterministic fallback? Hardware configurations and dynamic workload states first require structuring into coupled state tuples $s = \langle s_{\text{design}}, s_{\text{env}} \rangle$ (@sec-data-representations-world-models).

Teams often default to complex foundation models or reinforcement learning, assuming end-to-end automation accelerates discovery. However, this unexamined approach wastes silicon engineering budgets. An analytical roofline model,[^fn-roofline-c05] a direct simulation sweep, or a Bayesian optimizer often settles questions in minutes with verifiable uncertainty estimates. In contrast, complex autoregressive agents can consume weeks of compute for simple tasks and leave teams managing uncalibrated surrogate drift.
...
Method selection requires strict resource accounting. Model training, surrogate calibration, and manual verification costs must not exceed the expense of direct execution. The limiting engineering role must dictate the chosen method and preserve independent physical oracles to qualify every output. This choice defines search spaces, budgets, and stopping rules. It explicitly links upstream representations (@sec-data-representations-world-models) to the execution environments and tool interfaces developed in @sec-architecture-environments-tool-interfaces.
...
When an automated design workflow deploys language models inside closed repair loops, raw benchmark accuracy ceases to be the sole evaluation metric. Because agentic loops execute tens or hundreds of code-generation and test iterations per task, token generation throughput directly dictates overall iteration turnaround. The operational frontier therefore trades marginal software problem-solving capability against inference speed (@fig-agentic-sweet-spot).

![**The agentic sweet spot in multi-turn design loops.** Software problem-solving capability (measured via SWE-bench Verified pass rate [@JimenezEtAl2024SWEBench]) plotted against effective token generation throughput (measured in tokens per second via Artificial Analysis provider benchmarks [@ArtificialAnalysis2026Briefcase]). Fast-tier models ($>100\ \text{tok/s}$) provide rapid syntax turnaround but plateau in reasoning depth, while heavyweight frontier models ($>60\%$ pass rate) incur high turn latencies ($<25\ \text{tok/s}$) that choke tight interactive design loops. The operational sweet spot balances capability and throughput for automated repair cycles.](images/plot_ch5.png){#fig-agentic-sweet-spot width="95%" fig-alt="Scatter plot showing SWE-bench Verified pass rate against token generation throughput for eight large language models, categorizing sweet spot, balanced trade-off, and throughput-bound regions."}

Instead of treating capability as the sole metric, an evaluation of model utility must account for iteration latency. Delayed output severely limits multi-step, iterative repair processes in autonomous workflows, meaning that higher absolute task performance offers little practical advantage if generation latency starves downstream simulation and linting queues. As @fig-agentic-sweet-spot demonstrates, the models that most efficiently support automated loops operate within a balanced sweet spot, avoiding the throughput collapse of frontier reasoning models while providing sufficient reasoning fidelity for automated code repair.
...
This operational limit applies strictly to uncoordinated large language model (LLM) generation. When multiple prompts or agents independently propose RTL candidates or author testbenches, they act as uncoordinated stochastic arrival processes. Without centralized rate limiting, an LLM generation loop rapidly saturates the most restrictive downstream electronic design automation (EDA) queue, producing high token-generation costs but zero additional validated throughput. Agents must observe the queueing model directly, pausing generation or stiffening local rejection thresholds when downstream utilization $\rho_i$ approaches unity. The system must regulate LLM generation dynamically through a throttling loop.
...
To prevent overconfident candidate filtering, surrogate point estimates must be paired with uncertainty quantification. Split-conformal prediction[^fn-split-conformal-c05] (complemented by out-of-distribution detection to identify when inputs fall completely outside the model's training manifold) attaches distribution-free, finite-sample prediction intervals $\hat{C}(x)$ to learned surrogates, guaranteeing marginal coverage at a declared confidence level $1-\alpha$ without parametric assumptions on error distributions [@LeiEtAl2018Conformal; @AngelopoulosBates2021Conformal]:
...
#|   split-conformal prediction (complemented by out-of-distribution detection to identify when inputs fall completely outside the model's training manifold) band ($\pm q_{1-\alpha}$) around a latency-like surrogate curve. In the
...
#| out-width: "100%"
#| fig-alt: "Scatter plot of synthetic latency-like data with a fixed-width conformal prediction (complemented by out-of-distribution detection to identify when inputs fall completely outside the model's training manifold) band around a surrogate curve. In the calibrated support region on the left, nearly all points fall inside the band; in the distribution-shift region on the right, observed behavior rises away from the unchanged band and most points escape it, showing coverage collapse."
...
Code that parses is not code that works, and the drop-off between the two is measurable. KernelBench (noting that speedup metrics depend heavily on the underlying hardware like NVIDIA L40S and the specific compiler stack) evaluates generated CUDA kernels against PyTorch baselines [@OuyangEtAl2025KernelBench (noting that speedup metrics depend heavily on the underlying hardware like NVIDIA L40S and the specific compiler stack)], and candidate qualification falls at each verification boundary in turn (@fig-ch05-kernel-funnel).

```{python}
#| echo: false
#| label: fig-ch05-kernel-funnel
#| fig-cap: |
#|   **Attempted is not correct, and correct is not fast.** One-shot KernelBench (noting that speedup metrics depend heavily on the underlying hardware like NVIDIA L40S and the specific compiler stack) evaluation across three
#|   difficulty tiers (Levels 1–3) for four language models (Llama-3.1-70B, DeepSeek V3, OpenAI o1,
#|   DeepSeek R1) measured on an NVIDIA L40S GPU [@OuyangEtAl2025KernelBench (noting that speedup metrics depend heavily on the underlying hardware like NVIDIA L40S and the specific compiler stack)]. Across all panels, the dashed
...
#| out-width: "100%"
#| fig-alt: "Three-panel grouped bar chart of one-shot KernelBench (noting that speedup metrics depend heavily on the underlying hardware like NVIDIA L40S and the specific compiler stack) outcomes for Llama-3.1-70B, DeepSeek V3, OpenAI o1, and DeepSeek R1 at difficulty levels one, two, and three. In each panel a dashed line at one hundred percent marks tasks attempted; blue bars show the share passing the correctness oracle, from zero to sixty-seven percent, and green bars show the share both correct and faster than the PyTorch Eager baseline, from zero to thirty-six percent. Both bars sit far below the dashed line everywhere, and the fast share is always well below the correct share."
...
def dataset_values(metric, level):
    ...
    if set(selected) != set(model_order):
        raise ValueError(f"Incomplete KernelBench (noting that speedup metrics depend heavily on the underlying hardware like NVIDIA L40S and the specific compiler stack) dataset for {metric}, level {level}")
...
: **Pipeline components must perform necessary work that simpler alternatives cannot.** Increasing model count without distinct functional requirements adds complexity without improving architectural results. {#tbl-method-organization tbl-colwidths="[24,34,42]"}

The first row is the one most compositions should land on. Every row below it adds a component and therefore a qualification cost, a failure mode, and a boundary someone has to police, so a pipeline earns each addition by naming work the row above it cannot do. Read that way the rows are an escalation a study accepts only under pressure, which is what the two cases below make concrete.

## Method Selection in Two Contrasting Studies

The practical validity of hybrid composition depends on problem dimensionality and feedback latency. High-dimensional exploration across expensive toolchains demands composed prediction and search pipelines, whereas compact, well-bounded decisions with few discrete candidates favor direct execution. Two contrasting design scenarios illustrate this division: compiler phase-ordering optimization for tensor accelerator kernels versus L2 cache capacity sizing for the prospective mobile system-on-chip (SoC).

### Compiler Tuning versus Cache Evaluation

Compiler phase ordering and microarchitectural DSE justify multi-component compositions, whereas the illustrative cache study requires only direct tool evaluation. In compiler pass tuning (e.g., LLVM `PassBuilder` pipelines or AutoTVM schedules [@ChenEtAl2018AutoTVM]), candidate combinations far exceed direct evaluation capacity. Learned cost models predict throughput, search algorithms allocate compilation runs, and hardware execution confirms candidate performance.

Conversely, the illustrative cache study evaluates three discrete capacity settings (2\ MiB, 3\ MiB, and 4\ MiB) under a budget covering direct static random-access memory (SRAM) screening and cycle-level simulation. Adding generators, predictors, or optimizers would add overhead without accelerating discovery (@tbl-worked-method-decisions).
...
: **Missing capabilities determine composition requirements.** High-dimensional search spaces with expensive target evaluations justify learned surrogates and optimizers, whereas small discrete candidate sets favor direct evaluation. {#tbl-worked-method-decisions tbl-colwidths="[22,39,39]"}
...
**Context.** Selecting the smallest sufficient method ensures that we do not deploy complex, expensive learned surrogates when direct evaluation can satisfy the budget and fidelity requirements.
...
**Outcome.** The search remains bounded to realizable SRAM organizations, ensuring the cache evaluation is completed within budget without deploying unnecessary surrogate machinery.
:::

Grounded in the smallest sufficient method, the prospective illustrative cache study formalizes its execution strategy before invoking simulation tools. The Lighthouse cache evaluation specifies allowed mutations, tool contracts, cost bounds, and verification checks (@tbl-lighthouse-approach-decision), so that search remains bounded to realizable SRAM organizations without deploying unnecessary surrogate machinery.
...
[^fn-eda-signoff-c05]: **Signoff Tools**: OpenSTA verifies that all circuit paths meet timing constraints. UPF defines power intent, such as voltage domains and power gating. SVA and BMC use formal logic to mathematically prove that certain design properties always hold, rather than just simulating test cases.

[^fn-split-conformal-c05]: **Split-Conformal Prediction**: A statistical technique that provides guaranteed bounds on prediction errors. Holding out a separate calibration dataset, it calculates an error margin that contains the true value with a chosen probability (e.g., 90%), regardless of the underlying model's structure.
