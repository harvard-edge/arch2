...
When candidate production rapidly outpaces physical signoff, increasing our upstream candidate generation rate does not increase overall throughput; it merely creates an unverified queue at our inspection bottleneck. In automated architectural workflows, we must view the toolchain as a feed-forward queueing network where we match candidate production rate to downstream tool service capacity.
...
0.40 finalists per day. That product is a flow
...
Prediction and direct tool observation remain distinct records (@fig-architecture-cost-model). The same represented state and proposed action feed a scoped consequence model, which incorporates a transition model when state dynamics matter, and a declared tool or simulator path. Both returns land in a versioned study record. The consequence model estimates behavior within qualified bounds. The direct tool path supplies a reference observation under its own model, configuration, and fidelity. Qualified agreement lets us continue using the model within its stated support region, but it does not make either return authoritative outside its declared conditions.
...
Sequential placement differs from a static parameter search because every macro location we commit to permanently alters the state for our next choice. The graph-placement loop of @fig-alphachip-placement runs exactly this sequential decision process [@MirhoseiniEtAl2021GraphPlacement].
