...
**The claim.** Itanium's Explicitly Parallel Instruction Computing design moved instruction scheduling out of the hardware and into the compiler. Explicit parallelism, predication, and control and data speculation would let a simpler wide machine issue many operations per cycle, with the compiler finding the parallelism ahead of time [@SchlanskerRau2000EPIC].

**The gap.** The compiler had to settle at compile time what dynamic hardware settles at run time, including memory latencies and control flow that only the running program reveals. General-purpose codes did not yield the parallelism the architecture had assumed, while competing machines kept extracting it dynamically and kept the assumption out of the contract.

**The lesson.** A method family commits us to its assumptions as firmly as to its mechanism. The question is not whether a method is powerful but whether the information it depends on is available at the moment it needs it, and that question belongs to the architecture decision rather than to the method.
:::
...
: **Choose the smallest approach that performs the needed work.** Our four levels keep the role, method family, concrete technique, and system composition separate. {#tbl-role-by-stall-mode}
...
This separation also preserves our direct-tool path. When we decide on no added method, it means our declared tools already perform the needed work economically and reliably, even if a tool's internals happen to include learning. A strict no-AI path relies entirely on conventional methods and tools. We recognize that either choice can serve as the smallest sufficient approach. This separation preserves direct tool paths alongside augmented workflows, mapping functional engineering roles through selected method families and concrete techniques to verified execution paths (@fig-role-method-map).
...
Every algorithmic enhancement in computer architecture comes with an invisible invoice covering the data collection, surrogate qualification, hyperparameter tuning, and verification checking required to keep it trustworthy. Before we commit scarce computing cycles and engineering effort to an advanced prediction, generation, or optimization method, we must verify that the added machinery relieves a genuine bottleneck rather than shifting our workload into a saturated verification pipeline. Grounding our method selection in physical constraints prevents us from adopting elaborate frameworks that offer nominal speedups while inflating our total study cost.
...
Before we authorize any algorithmic optimizer or learned surrogate, we pass our prospective study through six pre-flight criteria. These criteria test whether direct tool execution or a conventional heuristic can resolve our architectural question before we introduce added methodology.
...
: **An approach decision is more than a method name.** These attributes make a direct tool path or an added method comparable before our work begins, regardless of whether its concrete technique is conventional, learned, or hybrid. {#tbl-method-role-limits}
...
- **Total cost:** We must count setup, predictor qualification, inference, tool time,
  ...
  prediction becomes inefficient if our qualification phase consumes the very evaluations it
  was meant to save.
...
- **Support, reversibility, and review capacity:** A method may work only for
  ...
  We must state those limits clearly before use. A disposable suggestion tolerates different
  ...
  the work we hand to the next step; generating more candidates can make our review-limited
  study slower.

Method fit can shift after our chosen approach changes the work. A generator
...
When candidate production outpaces physical signoff, generating more proposals merely inflates an unverified backlog at our inspection bottleneck. In automated architectural workflows, we must treat the toolchain as a feed-forward queueing network that balances candidate arrival rates against downstream tool service capacity.

We can formalize this balance with a first-order capacity model to identify the active tool bottleneck before selecting a search approach. Let $g$ be the rate at which candidates
...
: **An illustrative pre-review tool-path capacity model exposes our active tool limit.** The values are not measurements of a particular tool flow. They make our load calculation inspectable. {#tbl-candidate-check-capacity}

The structural screen receives all 32 candidates per day and operates at
$32/1{,}440$, or 2.2%, utilization.
8 candidates per day reach simulation, which
operates at 66.7%. Only
0.8 candidates per day reach the
implementation screen, which operates at
80%. Implementation is therefore our
...
For the illustrative values, @eq-finalist-throughput yields an expected finalist throughput of 0.40 finalists per day. That product is a flow
...
captures those returns, at this stage, we need to include
...
When our architectural design space expands into thousands of microarchitectural variations, subjecting every candidate to cycle-accurate simulation or physical synthesis quickly becomes impossible. In these high-dimensional regimes, surrogate cost models and predictive estimators allow us to trade absolute numerical accuracy for evaluation throughput. Predicting performance, power, congestion, or feasibility risks across candidate configurations enables us to screen vast search spaces long before we commit scarce compute hours to detailed simulation flows. Architectural predictors are not a modern invention born of deep learning; our discipline relied on regression models and learned surrogates, including early neural predictors, to navigate complex design trade-offs long before large language models emerged [@IpekEtAl2006PredictiveDSE; @LeeBrooks2006RegressionModeling]. Whether estimating branch direction miss rates across complex instruction streams or evaluating NPU memory tiling factors across systolic dimensions, fast regression models and neural surrogates allow us to filter thousands of configurations before dispatching costly cycle-accurate simulations.
...
> **Architecture world model:** In Architecture 2.0, an architecture world model is a learned predictor of how represented design state, a permitted action,
...
both connected without ever mistaking a prediction for a direct tool
observation.](images/fig-architecture-cost-model){#fig-architecture-cost-model
width="100%" fig-alt="A represented state and proposed action feed a scoped
...
Preserving prediction and direct tool observation as distinct records protects project state against uncalibrated drift. The same represented state and proposed action feed a scoped consequence model, which incorporates a transition model when state dynamics matter, and a declared tool or simulator path. Both returns land in a versioned study record. The consequence model estimates behavior within qualified bounds. The direct tool path supplies a reference observation under its own model, configuration, and fidelity. Qualified agreement lets us continue using the model within its stated support region, but it does not make either return authoritative outside its declared conditions.
...
where $q_{1-\alpha}$ represents the $(1-\alpha)(1 + 1/n)$ empirical quantile of calibration nonconformity scores $|y_i - f(x_i)|$, provided we rely on an exchangeability assumption and select a valid nonconformity score[^fn-stats-conformal-prerequisites-c05] [@LeiEtAl2018Conformal; @AngelopoulosBates2021Conformal]. When an optimizer steers exploration outside the calibrated support region into an uncharacterized design space (a distribution shift where $P_{\text{test}}(X,Y) \neq P_{\text{train}}(X,Y)$), empirical coverage $\mathbb{P}(Y \in \hat{C}(X))$ collapses silently below $1-\alpha$ because the underlying data distribution has changed [@TibshiraniEtAl2019CovariateShift]. In architectural surrogate modeling, conformal intervals provide margins of safety only within boundaries where exchangeability holds; the moment the workload or design corner shifts, those mathematical coverage guarantees vanish.
...
[^fn-stats-conformal-prerequisites-c05]: **Conformal prediction prerequisites**: The coverage guarantee rests on two conditions. Exchangeability holds when the joint distribution of a sequence of random variables is invariant to their permutation, a weaker assumption than being independent and identically distributed (i.i.d.). The nonconformity score measures how poorly our fitted model agrees with an observed calibration case, defined in split-conformal regression as the absolute residual between our prediction and the observation, whose quantile fixes the interval width [@LeiEtAl2018Conformal]. Because our architecture workloads, tools, and process corners frequently violate exchangeability, we must strictly audit, rather than blindly trust, any conformal interval we produce.
...
: **Generative techniques should match our artifacts and their available constraints.** Enforcing structure during construction helps us reduce wasted work, but it does not remove the property-specific checks we need to establish whether our generated artifact is ultimately usable. {#tbl-generation-techniques}
...
: **A generation comparison must match our construction paths, not only the final artifacts.** Our prospective comparison identifies exactly what we must pin down before we can compare a learned and conventional HLS pragma DSE construction pair. {#tbl-generation-construction-pair}
...
Balancing surrogate predictions against estimated uncertainty allows an acquisition function to allocate expensive physical evaluations where the expected information gain or performance improvement is highest (@fig-bayesian-optimization).
...
Reinforcement learning serves a different purpose. We can defend its use most easily when our problem is genuinely sequential. In these cases, an action changes the state, our later choices depend heavily on our earlier ones, and we can afford repeated interactions with the environment. Sequential macro placement illustrates this decision structure, where each placement decision permanently modifies the canvas available for subsequent blocks (@fig-alphachip-placement) [@MirhoseiniEtAl2021GraphPlacement].
...
](images/fig-alphachip-placement){#fig-alphachip-placement width="100%"
...
Evaluating search tactics across varying budgets underscores that a strategy leading under constrained evaluations may plateau early and fall behind under larger budgets. The curves do not dictate which algorithm follows which profile; that trajectory depends on the design representation, tuning effort, random variation, evaluator fidelity, and stopping rules. Placement-stage proxies can guide the search, but they do not cover every downstream routing, timing, or manufacturing effect, so later checks must address those missing properties.
...
: **Search-procedure selection matches our design-space structure and feedback cost.** Because no procedure is universally superior, our choice depends entirely on our cost of evaluation and the structure of our parameter space. {#tbl-optimization-methods}

The last column is the one to read first, because each mismatch names the moment the procedure stops being valid rather than merely inefficient. Enumeration fails once the space outgrows the budget, random search fails when it is treated as insurance against a bad objective, and a learned optimizer fails when it is asked to pursue a desirability that formulation never expressed. A familiar method name cannot repair a space, objective, or feedback path that violates the assumptions of our chosen search procedure.
...
We should not automatically treat generation, prediction, and optimization as a rigid three-stage pipeline. We combine them only when our problem involves distinct jobs. As we established when connecting intent to executable specifications (@sec-intent-vs-spec), tools like AutoTVM and Ansor illustrate successful optimization compositions in tensor-program tuning, but they do not establish a mandatory three-stage workflow across broader architecture research [@ChenEtAl2018AutoTVM; @ZhengEtAl2020Ansor]. Distinct studies demand different combinations: KernelBench needs candidate generation and a verification harness without a learned optimizer, while a small enumerated cache study needs none of the three.
...
: **We ensure each component earns its place by performing necessary work that another component cannot.** Simply adding more models or agents to our pipelines does not automatically yield a stronger method. {#tbl-method-organization}
...
| **Allowed change** | We change only `l2.capacity` among `2\ MiB`, `3\ MiB`, and `4\ MiB`; we keep our declared hardware, software, workload, cache policy, memory system, and exclusions strictly fixed. Any other architecture or software change reopens our study. |
...
Choosing a method is a bet about which part of the work is limiting. The questions below are the ones that bet leaves open once prediction, generation, and optimization are all available.

**Knowing when a predictive surrogate has lost its support.** Predictive models reduce evaluation costs only while their support assumptions hold true. When architectural search pushes surrogates into uncharacterized design regions, proxy predictions fail without explicit diagnostic signals.

* **What signal in a running search says that a predictor's uncertainty bands have gone stale and need recalibrating?** Selecting candidates based on surrogate predictions introduces systematic distribution shift that invalidates conformal coverage guarantees over time. Without an online recalibration trigger, screening passes can silently drop high-performing designs or flood signoff tools with invalid candidates. Establishing a statistical test that detects coverage loss without consuming excessive high-fidelity evaluations remains an unresolved problem.
* **How does the budget at which a surrogate finally pays for itself grow as the design space gains dimensions?** Qualifying a predictive model requires spending reference tool evaluations across the specific design regions where the surrogate will operate. As design-space dimensionality grows, this qualification overhead expands, threatening to consume the very simulation budget the surrogate was built to save. Programmatic bounds that predict when qualification costs exceed surrogate savings across high-dimensional hardware spaces are currently missing.

**Catching defects early in generative hardware flows.** Generative models can synthesize hardware specifications rapidly, but every unverified candidate shifts checking work onto downstream evaluation tools. Structuring generative pipelines requires balancing early deterministic filtering against downstream verification capacity.

* **How much does generating testbenches with an independent language model family reduce shared specification mistakes compared to using the model that generated the hardware design?** When a single language model produces both a hardware module and its testbench, it frequently repeats identical misinterpretations of the design specification across both artifacts. While using an independent model family as a verification oracle is widely suggested, the actual reduction in shared specification errors has not been quantified across standard hardware benchmarks. Determining the independence threshold needed for reliable automated checking remains open.
* **Where in a generative hardware pipeline does placing a deterministic clock-domain crossing and lint checker save the most total verification compute compared to relying on full simulation?** Early deterministic filters can prune syntactically or structurally invalid candidates before expensive simulation runs execute. However, placing checks too early can reject candidates that simple downstream repair steps could fix, while placing them too late wastes cycle-accurate evaluation budget on non-elaboratable drafts. The optimal placement of neurosymbolic checks within multi-stage generative flows has not been established empirically.

**Allocating search budgets under non-uniform tool latencies.** Hardware verification toolchains exhibit state-dependent execution latencies that range from milliseconds for structural screens to days for physical signoff. Optimization algorithms must manage downstream queue stability when evaluation times vary non-stationarily.

* **How can an automated exploration pipeline adjust early screening pass-through rates when downstream physical implementation tool execution times vary?** Queue-stability models typically assume fixed mean service times across verification filters, but real physical synthesis and timing signoff latencies fluctuate dramatically with routing congestion and clock targets. When downstream tool execution times spike, fixed screening thresholds cause candidate backlogs that exceed signoff queue capacity. Controlling upstream arrival rates based on live tool utilization remains an unmodeled control problem in automated exploration.

## Summary

Choosing a method for hardware design means finding the smallest sufficient approach. We must start with the specific bottleneck limiting our engineering progress, rather than selecting a model family based on industry trends or superficial appeal. Method selection holds together only when the engineering role, the method family, the concrete technique, and the system composition remain separate decisions rather than one choice of model. Whether we deploy prediction to estimate expensive metrics, generation to construct missing candidate artifacts, or optimization to search vast design spaces, every added component must earn its place by removing concrete limiting work without creating unmanageable verification overhead.

Direct evaluation and conventional heuristics remain our default starting line whenever they can settle an architectural decision within our declared budget. When we do justify adding learned or hybrid components, we must ensure they preserve explicit downstream verification checks, clear fallbacks, and stopping conditions. When evaluating this phase of Architecture 2.0, five core takeaways govern method selection:

::: {.callout-takeaways title="Matching the Method to the Limiting Work"}
- **Role first, then family, then technique.** Name the engineering role the work requires before choosing prediction, generation, or optimization, settle the concrete conventional, learned, or hybrid mechanism only after that, and compose components only when the problem contains distinct jobs.
