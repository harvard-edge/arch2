...
Prediction, generation, and optimization methods define search strategies, required tools, budgets, and checks; they operate on the machine-usable hardware representations established in @sec-data-representations-world-models. Connecting these methods to tool execution environments is developed in @sec-architecture-environments-tool-interfaces.
...
In sequential macro placement, partial floorplans and placement actions feed a transition predictor. Planners evaluate bounded rollouts, select the best initial action, and dispatch it to the placement toolchain [@JannerEtAl2019MBPO]. Tool observations qualify results, update represented state, and trigger replanning, while deterministic checkers enforce nonoverlap and design rules.
...
Multi-objective optimization constructs Pareto frontiers across conflicting metrics (power, performance, area) without collapsing trade-offs into artificially weighted scalars [@Deb2001MultiObjective].
...
These failures occur after a method has been chosen, when the machinery this chapter describes is running and its output looks like progress. Each one produces a confident answer whose error sits in the method's own assumptions rather than in the design.
