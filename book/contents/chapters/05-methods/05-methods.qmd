...
where $q_{1-\alpha}$ is the $\lceil (n+1)(1-\alpha) \rceil / n$ empirical quantile (approximated as $(1-\alpha)(1 + 1/n)$) of calibration residuals $|y_i - f(x_i)|$ across $n$ calibration samples, under exchangeability assumptions.[^fn-stats-conformal-prerequisites-c05] In hardware design, exchangeability is violated when a latency surrogate trained on compute-bound SPECint traces is evaluated on irregular graph analytics workloads with heavy cache miss streams; under this distribution shift ($P_{\text{test}}(X,Y) \neq P_{\text{train}}(X,Y)$), empirical coverage $\mathbb{P}(Y \in \hat{C}(X))$ collapses below $1-\alpha$ [@TibshiraniEtAl2019CovariateShift].
...
- **How much of a predicted win survives the real check?** This chapter states the inequality and works a case where a deeper reorder buffer looked best to the surrogate and collapsed under physical timing. The prescription that follows is to report shrinkage, and no number is ever attached to it. A search selects whatever the surrogate is most optimistic about, which is usually wherever the training data constrained it least, so the winner's margin is partly the model's own error and nobody can say what fraction. Selecting the best candidate over pools of increasing size and then confirming each winner would give that correction curve directly, on data that already exists.

- **When does qualifying a surrogate cost more than the runs it saves?** The rule given here is to keep a predictor only while qualification and confirmation cost less than evaluating directly, and both quantities become visible only after the budget is spent. Every efficiency claim in this book inherits that circularity. A pilot batch carries signals that might forecast the total, such as how fast residuals decorrelate or how quickly the candidate stream leaves the region the model was fitted on. Nobody has checked whether those signals predict the final split well enough to abandon a surrogate campaign in its first week, which is the only point at which abandoning it is worth anything.

- **What does forcing a generator to emit only legal designs cost in the good ones?** Constrained decoding appears here as free pruning, on the reasoning that a design which will not parse is worth nothing. Masking tokens to enforce a grammar also reshapes the distribution the generator was trained to sample from, and the modes it suppresses are not selected for being bad ones. Sampling the same budget with and without the constraint, then comparing the count of functionally distinct designs that survive repair rather than first-pass rates, would show whether the pruning is free. If it is not, the live question becomes where in the funnel the constraint belongs instead.

- **What happens to a pipeline when the search starts working?** The stability bound treats the fraction of candidates advancing at each stage as a constant (@eq-queue-stability-bound), and a working optimizer raises that fraction by design. A path provisioned near its bottleneck therefore tips into backlog at the moment the method succeeds, which is the opposite of how capacity gets planned. Per-stage advance rates as a function of campaign time are recorded by every real exploration run, and nobody has plotted them. Provisioning against the rate a campaign ends at rather than the one it opens with is the obvious repair and has never been tried.

- **Does composing methods beat the best single one at the same budget?** Every added component is required here to do work a simpler alternative cannot (@tbl-method-organization), which is this chapter's rule and the book's third tier in miniature. It is asserted rather than measured. A matched-budget comparison would charge each arm in one currency, counting qualification runs, failed invocations, and the hours somebody spent repairing output, then run a direct sweep against generation alone, against prediction with optimization, against the full composition. The prize is not which arm wins. It is which features of a problem flip the winner.
