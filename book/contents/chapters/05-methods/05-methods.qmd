...
**Searching against a model that can be wrong in a useful direction.** An optimizer ranking many candidates will find the regions where its surrogate is optimistic, because those regions look like wins.

- **What would tell an optimizer it is being fooled by its own surrogate, before it spends the ground-truth budget that would prove it?** Average surrogate error says little about the errors that matter, which are the ones near the decisions the search is about to make. Detecting local rank inversions cheaply, and knowing which of them deserve an expensive check, is unresolved for every method family in this chapter.

**Generating candidates that are legal without generating candidates that are all alike.** Hard rules can be enforced by construction or by rejection, and both choices cost something.

- **What does a generator give up when it enforces structural and physical rules by construction rather than by rejection?** Filtering after the fact wastes most of what was produced, while building rules into the model tends to shrink what it will propose. The trade between legality and coverage is understood informally and has not been characterized in a way a team could plan around.

**Knowing when a learned component earns its place, and when to drop it.** A learned method carries setup, data, and qualification costs that the comparison usually omits.

- **Can we predict, before building it, whether a learned method will reduce total effort on a given problem?** The costs that decide the answer include data preparation, retraining, and the checks that qualify the model's output, and they are paid whether or not the method helps. A prospective cost model would turn method selection from a preference into an estimate.
- **What signals should make an automated flow abandon a learned component mid-study and fall back to direct evaluation?** A surrogate stops being useful when the search leaves its support, when the bottleneck moves elsewhere, or when its errors start correlating with the decision, and none of those announce themselves. Fallback criteria that fire in time are missing from every published loop we know of.
