...
Conformal prediction attaches calibrated intervals $\hat{C}(x)$ to surrogates with finite-sample coverage guarantees under exchangeability [@LeiEtAl2018Conformal; @AngelopoulosBates2021Conformal]:[^fn-stats-conformal-prediction-c05]
...
In heteroscedastic hardware settings (where timing and power variance scales with pipeline depth or switching activity), locally normalized nonconformity scores $s_i = |y_i - f(x_i)| / (\hat{\sigma}(x_i) + \epsilon)$ yield adaptive interval bands. Asymmetric consequence structures (where optimistic timing estimation causes fatal post-silicon failure while pessimistic estimation merely wastes modest area) mandate asymmetric pinball loss quantiles ($L_\tau(u) = u(\tau - \mathbb{I}(u < 0))$) to penalize underestimation over overestimation. In hardware design, exchangeability is violated when a latency surrogate trained on compute-bound SPECint traces is evaluated on irregular graph analytics workloads with heavy cache miss streams; under this distribution shift ($P_{\text{test}}(X,Y) \neq P_{\text{train}}(X,Y)$), empirical coverage $\mathbb{P}(Y \in \hat{C}(X))$ collapses below $1-\alpha$ [@TibshiraniEtAl2019CovariateShift].
...
1. **Monte Carlo tree search (MCTS) for hardware code generation:** Single-pass autoregressive LLM generation frequently generates invalid ports, undeclared wires, or syntax errors that force the generator to discard the entire prompt context upon compilation failure. MCTS structures hardware generation as a sequential tree search over AST modifications and modular Verilog snippets. Intermediate search nodes are evaluated by fast syntax linters (Verilator lint) and unit assertion checkers (`assert property`), providing non-terminal reward estimates that guide rollout expansion (via Upper Confidence bounds applied to Trees [UCT]) toward syntactically valid and assertion-compliant hardware blocks before committing to whole-module elaboration.
2. **Multi-agent reflection and adversarial debate:** Structuring multiple specialized agent roles (an RTL generator, a lint and clock-domain crossing [CDC] critic, a testbench author, and a formal-proof specialist) creates an adversarial verification ring [@DuEtAl2024MultiagentDebate]. When the generator proposes a RISC-V Vector (RVV) unit, the critic flags unhandled reset state, the testbench author generates corner-case randomized stimulus in cocotb, and the formal specialist formulates bounded model checking (BMC) properties before any candidate consumes expensive FPGA or physical synthesis cycles. The topology is illustrative; the durable point is that checking roles must not share the generator's internal assumptions.
