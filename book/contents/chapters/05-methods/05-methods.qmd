...
Method selection begins with the architecture problem rather than the model. Evaluating three legal cache capacities requires direct tool runs to settle the comparison. When thousands of configurations compete for scarce simulation cycles, predictors screen candidates and optimizers direct evaluation budgets. If required register-transfer level (RTL), testbenches, kernels, or compiler schedules do not exist, generative techniques construct candidates. These distinct engineering tasks dictate the operational method family.
...
- **Layer 1 (AI-Assisted Engineering):** Deploys isolated point methods (such as prompt-based Verilog auto-completion or testbench templating) inside an existing manual workflow. The human engineer acts as the manual integration bus, validating code and resolving interface seams.
...
Chip-Chat demonstrates interactive RTL generation in which language models drafted Verilog for an 8-bit processor, reaching tapeout only through iterative human revision and strict tool feedback [@BlockloveEtAl2023ChipChat]. Generation cost remains inseparable from checking cost.
...
2. **Multi-Agent Reflection and Adversarial Debate:** Structuring multiple specialized agent roles (an RTL generator, a Lint/CDC Critic, a Testbench Author, and a Formal Proof Specialist) creates an adversarial verification ring. When the generator proposes a RISC-V vector (RVV) unit, the Critic flags unhandled reset state, the Testbench Author generates corner-case randomized stimulus in Cocotb, and the Formal Specialist formulates bounded model checking properties before any candidate consumes expensive FPGA or physical synthesis cycles.
...
Within a fixed space, *exploitation* samples promising regions, while *exploration* samples high-uncertainty areas. Neither strategy widens declared boundaries; evaluating novel topologies or instruction set architecture (ISA) extensions requires redefining the representation.
...
Evaluating method selection across concrete architectural challenges reveals how problem scale dictates composition:
...
Method choice is presented here as a decision an architect can make deliberately. What makes that decision affordable is not established.
