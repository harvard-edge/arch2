...
To see how search tactics can diverge under physical design constraints, we compare constructed search efficiency curves for floorplanning in @fig-ch05-search-efficiency. Optimization tactics can display distinct sample-efficiency profiles when trading off early convergence speed against ultimate asymptotic quality. In the constructed scenario the figure illustrates, zero-shot spatial prompting locates strong macro floorplans within tens of evaluations, while reinforcement learning continues to improve across thousands of environment steps.
...
The semi-log plot in @fig-ch05-search-efficiency tracks normalized floorplan PPA quality scores along the vertical axis against physical design tool evaluations (from 1 to 2,000 on a logarithmic scale) on the horizontal axis. The visualization contrasts four constructed optimization trajectories. LLM spatial prompting (orange curve) is drawn with rapid early convergence, reaching roughly 0.86 normalized quality within 25 tool evaluations, the shape we would expect if learned spatial heuristics transfer to placement. Bayesian optimization (dashed blue curve) is given strong early sample efficiency up to 100 evaluations before plateauing around a 0.86 score, the stall that continuous Gaussian process surrogates can exhibit in high-dimensional placement spaces. Reinforcement learning (purple curve) is plotted with extensive initial exploration followed by smooth scaling across policy updates, approaching the highest asymptotic score among the illustrated curves (0.95) near 1,500 evaluations. Random search (dotted grey baseline) grows slowly, reaching about 0.66 quality at 2,000 evaluations. The constructed contrast makes one durable point. Sample efficiency and asymptotic quality are different properties, so the right search choice depends on the evaluation budget rather than on any universal ranking among these method families. While placement-stage proxies can guide search, they do not simulate every downstream routing, timing, or manufacturing effect; our later checks must explicitly bridge that gap.
...
Whether we select direct evaluation or deploy complex prediction, generation, and optimization pipelines, hardware design introduces subtle methodological traps that rarely arise in pure software optimization. The pitfalls below are failure modes we anticipate from the mechanisms developed in this chapter, not findings from a measured survey of practice. They stem less from algorithmic flaws in our neural models or search heuristics than from optimization mechanics operating on uncalibrated proxy metrics, disconnected from physical ground-truth constraints and downstream verification checks. When surrogate models operate without deterministic safeguards, optimizers can exploit surrogate inaccuracies rather than discovering genuine hardware improvements.

- **Proxy gaming in surrogate-driven design search:** When we optimize architecture candidates against fast learned cost models or analytical throughput approximations, our search algorithms can quickly exploit regions where the surrogate is over-optimistic. The resulting candidates achieve strong predicted scores while failing timing closure or memory bandwidth checks during full simulation.[^fn-optimizers-curse-c05]
