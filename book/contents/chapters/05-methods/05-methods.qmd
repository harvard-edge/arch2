...
Method selection begins with the architectural bottleneck rather than the algorithm. Having structured hardware configurations and dynamic workload states into coupled state tuples $s = \langle s_{\text{design}}, s_{\text{env}} \rangle$ (@sec-data-representations-world-models), the architect must answer three governing questions before deploying machine learning: what engineering job is the active constraint, is an added model truly warranted, and what is the smallest sufficient technique that retains a verifiable check and an executable fallback?

In modern AI engineering, the default impulse is to reach for the most complex generative foundation model or reinforcement-learning loop, seduced by the prospect of automated, end-to-end design synthesis. In physical silicon engineering, that impulse is an expensive mistake. Setting up an autoregressive code generator or multi-agent debate ring to size a three-point cache hierarchy or optimize a five-parameter memory arbiter burns weeks of engineering effort, thousands of graphics processing unit (GPU) compute hours, and leaves the team managing uncalibrated surrogate errors. Meanwhile, an analytical roofline model, a simple sweep, or a Bayesian optimizer with a Matérn kernel settles the question in minutes with mathematically bounded confidence.

Different engineering bottlenecks demand fundamentally different method families. When a bounded study evaluates three candidate cache capacities, direct simulation sweeps settle the comparison cleanly. When millions of microarchitectural configurations compete for scarce compute cycles, predictive surrogates screen candidates and search optimizers ration evaluation budgets. When synthesizable register-transfer level (RTL), domain-specific testbenches, or tensor compiler schedules do not exist, generative models construct missing artifacts. While foundation models supply general prior knowledge, retrieval supplies live project context, and multi-agent harnesses coordinate tool actions, direct tool execution remains the default engineering baseline.

Selecting an AI method requires rigorous resource accounting. Saving simulator cycles is counterproductive if model training, surrogate calibration, and manual verification backlog exceed the cost of direct execution. The governing rule of Architecture 2.0 is to deploy the *smallest sufficient approach*: identify the limiting engineering role first, select a learned or hybrid method only when justified by active bottlenecks, and preserve independent physical oracles to qualify every output. Method selection defines search spaces, computational budgets, and stopping rules, linking upstream representations (@sec-data-representations-world-models) to the execution environments and tool harnesses developed in @sec-architecture-environments-tool-interfaces.
