...
The *queue stability blowup* paradox occurs when an upstream search optimizer or generative model improves: its pass rate rises, escalating the candidate load delivered to downstream simulation and physical signoff tools. Think of it like a funnel. If you widen the top filter without expanding the narrow neck at the bottom, the system inevitably backs up and overflows. A 24-hour physical signoff stage provisioned for a 5% candidate pass rate will drown in backlog if an upstream code generator increases its compilation success rate to 50%. If downstream service capacity is fixed, an improving upstream generator can inadvertently push downstream stages past their stability threshold, converting generative success into queueing collapse.
...
This quantitative lens fundamentally alters how architecture teams should perceive AI-driven candidate generation. Without queue analysis, a team might celebrate a generative model that doubles the daily proposal rate. However, if the downstream implementation screen is already running at 80% utilization, doubling the inbound volume will immediately overload the physical signoff stage, converting those "fast" generated candidates into an unmanageable backlog. The generated designs are not resolving decisions faster; they are simply shifting the wait time from the architect's desk to the simulator queue.
...
$$\mathbb{E}\left[\max_{i=1,\dots,K} \hat{f}(x_i)\right] > \max_{i=1,\dots,K} f(x_i)$$ Guarding against optimistic bias requires preserving original surrogate predictions, reserving independent confirmation runs, and reporting shrinkage.
...
Instead of just providing a single point estimate, conformal prediction draws a band around the prediction that is guaranteed to contain the true value a specified percentage of the time. This is like setting error bars based on past mistakes on similar data.

Mathematically, conformal prediction attaches calibrated intervals $\hat{C}(x)$ to surrogates with finite-sample coverage guarantees under exchangeability [@LeiEtAl2018Conformal; @AngelopoulosBates2021Conformal]:[^fn-stats-conformal-prediction-c05]
...
1. *Circuit Breaker and Escalation:* The system immediately trips an automated circuit breaker that suspends surrogate screening, routing all subsequent candidate proposals directly to unpruned reference simulator execution (such as cycle-accurate gem5, SCALE-Sim, or static timing analysis) to prevent ungrounded candidate promotion.
2. *Active-Learning Recalibration:* The out-of-distribution simulation measurements and failure counterexamples are appended to the surrogate's training buffer with boosted uncertainty weights, triggering offline surrogate retraining and conformal quantile recalibration before proxy-based screening is permitted to resume.
...
To see how these composition rules play out in practice, evaluating method selection across concrete architectural challenges reveals how problem scale and feedback latency dictate composition. High-dimensional exploration problems with expensive downstream tool paths demand composed prediction and search pipelines, whereas compact, well-bounded questions with few candidates favor direct execution. Two contrasting design scenarios illustrate this distinction in practice. Compiler phase-ordering optimization for accelerator kernels explores massive configuration spaces, while cache capacity selection for the prospective Lighthouse system-on-chip (SoC) examines only a few discrete options.
...
While the case studies demonstrate how to align methods with engineering constraints, deploying machine learning methods into architectural search loops often exposes subtle failure modes where algorithms emit confident recommendations that violate unmodeled hardware physics. Once an algorithm begins executing, errors often originate from the method's mathematical assumptions rather than the underlying circuit design.
