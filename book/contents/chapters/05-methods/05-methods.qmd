...
* **What statistical test on rolling audit-batch nonconformity scores reliably triggers online recalibration of conformal prediction bands before candidate selection efficiency degrades during design space exploration?** Selecting candidates based on surrogate predictions introduces systematic distribution shift that invalidates conformal coverage guarantees over time. Without an online recalibration trigger, screening passes can silently drop high-performing designs or flood signoff tools with invalid candidates. Establishing a statistical test that detects coverage loss without consuming excessive high-fidelity evaluations remains an unresolved problem.
* **How does the break-even evaluation budget where surrogate qualification overhead is fully offset by simulation savings scale with design-space dimensionality?** Qualifying a predictive model requires spending reference tool evaluations across the specific design regions where the surrogate will operate. As design-space dimensionality grows, this qualification overhead expands, threatening to consume the very simulation budget the surrogate was built to save. Programmatic bounds that predict when qualification costs exceed surrogate savings across high-dimensional hardware spaces are currently missing.
