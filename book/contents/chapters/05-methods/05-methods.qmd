...
![**The Agentic Sweet Spot.** For autonomous agent loops, intelligence isn't enough; generation latency matters. The Sweet Spot contains models that balance high SWE-Bench coding capability with high Effective Inference Speed, preventing loop bottlenecks. To map this tradeoff, we benchmarked the token generation latency and SWE-Bench scores of leading large language models.](images/plot_ch5.png){#fig-agentic-sweet-spot width="100%" fig-alt="Scatter plot of Token Generation Throughput vs Agentic Coding Score for various LLMs."}

Instead of treating capability as the sole metric, an evaluation of model utility must account for response time. Delayed output severely limits multi-step, iterative processes in autonomous workflows, meaning that higher absolute task performance offers little practical advantage if the time to generate proposals starves the rest of the pipeline. As @fig-agentic-sweet-spot demonstrates, the models that most efficiently support automated loops operate within a constrained envelope, deliberately trading marginal gains in capability for the throughput required to sustain continuous operation.
