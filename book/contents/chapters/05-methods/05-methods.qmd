...
**The Architectural Antidote:** Deploy split-conformal prediction intervals ($\hat{C}(x)$) as an empirical out-of-distribution circuit breaker (tripping automatic fallback to reference simulation when active optimizers sample extreme covariate-shift tails that violate exchangeability) and mandate independent full-physics signoff on all top-ranked candidates (see @sec-law-optimizers-curse in @sec-appendix-c-laws-paradoxes for the order-statistics proof, tail rank correlation metrics, and split-conformal coverage intervals).
:::
