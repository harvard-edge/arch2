...
#|   split-conformal prediction[^fn-ood-c05] band ($\pm q_{1-\alpha}$) around a latency-like surrogate curve. In the
...
#| out-width: "100%"
#| fig-alt: "Scatter plot of synthetic latency-like data with a fixed-width conformal prediction[^fn-ood-c05] band around a surrogate curve. In the calibrated support region on the left, nearly all points fall inside the band; in the distribution-shift region on the right, observed behavior rises away from the unchanged band and most points escape it, showing coverage collapse."
...
Code that parses is not code that works, and the drop-off between the two is measurable. KernelBench[^fn-kernelbench-c05] evaluates generated CUDA kernels against PyTorch baselines [@OuyangEtAl2025KernelBench[^fn-kernelbench-c05]], and candidate qualification falls at each verification boundary in turn (@fig-ch05-kernel-funnel).

```{python}
#| echo: false
#| label: fig-ch05-kernel-funnel
#| fig-cap: |
#|   **Attempted is not correct, and correct is not fast.** One-shot KernelBench[^fn-kernelbench-c05] evaluation across three
#|   difficulty tiers (Levels 1–3) for four language models (Llama-3.1-70B, DeepSeek V3, OpenAI o1,
#|   DeepSeek R1) measured on an NVIDIA L40S GPU [@OuyangEtAl2025KernelBench[^fn-kernelbench-c05]]. Across all panels, the dashed
...
#| out-width: "100%"
#| fig-alt: "Three-panel grouped bar chart of one-shot KernelBench[^fn-kernelbench-c05] outcomes for Llama-3.1-70B, DeepSeek V3, OpenAI o1, and DeepSeek R1 at difficulty levels one, two, and three. In each panel a dashed line at one hundred percent marks tasks attempted; blue bars show the share passing the correctness oracle, from zero to sixty-seven percent, and green bars show the share both correct and faster than the PyTorch Eager baseline, from zero to thirty-six percent. Both bars sit far below the dashed line everywhere, and the fast share is always well below the correct share."
...
def dataset_values(metric, level):
    ...
    if set(selected) != set(model_order):
        raise ValueError(f"Incomplete KernelBench[^fn-kernelbench-c05] dataset for {metric}, level {level}")
