...
Method selection begins with the physical bottleneck rather than the algorithm. After structuring hardware configurations and dynamic workload states into coupled state tuples $s = \langle s_{\text{design}}, s_{\text{env}} \rangle$ (@sec-data-representations-world-models), an architect must answer three governing questions before authorizing machine learning: which engineering job acts as the active constraint, does an added learned model genuinely remove that constraint, and what is the smallest sufficient technique that preserves an inspectable verification check and a deterministic fallback?

In modern AI engineering, teams often default to the most complex generative foundation model or reinforcement-learning loop, seduced by the prospect of automated, end-to-end chip synthesis. In physical silicon engineering, that impulse is an expensive mistake. Setting up an autoregressive code generator or a multi-agent debate ring to size a three-point cache hierarchy or tune a five-parameter memory arbiter burns weeks of engineering effort, thousands of graphics processing unit (GPU) hours, and leaves the team managing uncalibrated surrogate drift. Meanwhile, an analytical roofline model, a direct simulation sweep, or a Bayesian optimizer with a Matérn kernel settles the question in minutes with mathematically bounded confidence.
...
Surrogate models are reliable only within the empirical support of their training distributions. When optimizers steer search into uncharacterized design regions, unexpected workloads, or shifted process corners, surrogates fail silently. Standard statistical metrics like global root-mean-square error (RMSE) or mean absolute error are dangerously deceptive in silicon design. An architect might train a surrogate cost model on standard benchmark traces and achieve an impressive 3% global RMSE. But optimizers do not sample average designs; they aggressively hunt for extreme performance outliers. If the surrogate contains a slight uncalibrated positive defect in an uncharacterized corner of the design space, the optimizer will treat that mathematical artifact as a physical breakthrough. Screening surrogates must therefore preserve rank correlation ($r_s$) and top-$k$ recall at decision boundaries,[^fn-ml-top-k-recall-c05] while point accuracy governs hard constraint feasibility (such as power envelopes or timing slack).
...
Method choice is formalized across this chapter as a deliberate architectural decision governed by the smallest-sufficient-approach rule. However, evaluating the true economic trade-offs of complex method compositions exposes six unresolved scientific questions:
