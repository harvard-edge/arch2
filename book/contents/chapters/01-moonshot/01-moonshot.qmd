...
Every major era in computer architecture has advanced by redefining the boundary between human intent and automated tool execution. Rather than merely accelerating manual tasks, lasting design transformations introduce abstractions that expand expressible hardware complexity while establishing rigorous, repeatable verification contracts. The Mead–Conway Very Large Scale Integration (VLSI) methodology[^fn-vlsi-complexity-c01] connected structured design rules with automated layout tools and scalable fabrication access [@MeadConway1980VLSI]. When reduced-instruction-set computing arrived, it reshaped the hardware-software contract around quantitative principles that we could benchmark and test directly [@PattersonDitzel1980RISC].
...
| **Architecture 2.0**<br/>*(Autonomous Era)* | Closed-Loop Intent Execution | **2022 onward (Foundation Model Era):** Pretrained foundation models & closed-loop agentic signoff loops | Formulating intent, setting bounds, & owning commitments |
...
The upward feedback arrows in @fig-moonshot-prompt show that tool results continually force us to revisit and refine our middle-layer requirements. As structured in this stack, eight tightly coupled layers define the technical dependencies across our design space.
...
Examining these choices shows that the workload and process phrases demand far more detail than the compact prompt initially provides. The XRBench suite provides a concrete workload anchor, rather than leaving us with a vague application label. End-to-end XR research demonstrates how tightly perception and display stages are coupled, making mobile XR a full system-level architecture target rather than just a single kernel for us to optimize [@HuzaifaEtAl2021ILLIXR]. Here again, we can use learned methods to analyze dataset distributions and uncover coverage gaps, effectively complementing our traditional profiling, trace analysis, and expert workload judgment. For instance, an AI method could analyze a memory trace collected from a fixed XRBench workload and configuration, clustering access patterns to suggest specific cache hierarchies or local scratchpads for our simulations.
...
The scale of that gap is stark. Open-source software gives language models hundreds of billions of tokens to learn from; synthesizable hardware description code amounts to roughly a thousandth of that, curated architecture corpora sit in the same scarce tier, and physical layout data is scarcer still [@LozhkovEtAl2024TheStackV2; @WangEtAl2025OpenRTLSet; @PrakashEtAl2025QuArch; @ChaiEtAl2022CircuitNet]. This thousandfold data scarcity gap is why general-purpose language models hallucinate plausible but invalid hardware interfaces, and why AI-native design systems pair specialized domain curation with closed-loop physical verification. We measure the gap tier by tier in @sec-data-representations-world-models.
...
We contrast these two paradigms in @tbl-arch1-vs-arch2, detailing five architectural responsibilities in Column 1 across established Architecture 1.0 practice (Column 2) and AI-native Architecture 2.0 methodology (Column 3). Comparing the columns illustrates how our operational role as architects evolves. Under Architecture 1.0, engineers manually script parameter sweeps and tune point microarchitectures. Under Architecture 2.0, learned surrogates and generative search automate candidate exploration and tool orchestration. The final row of @tbl-arch1-vs-arch2 shows that decision authority remains unchanged. While AI components propose candidate designs and aggregate evidence, the human architect and Commitment Authority retain sole approval rights over baseline contracts and silicon signoff.
...
- **The believer's surrender (bypassing signoff checks):** Uncritically trusting generative AI outputs or surrogate recommendations without executing rigorous physical EDA timing, formal SystemVerilog Assertion checks, and workload simulations. Relinquishing commitment authority to learned models invites catastrophic silent bugs and silicon re-spins.
...
- **Non-delegable commitment authority.** Automated generators and learned surrogates propose candidate designs, but human architects retain sole non-delegable authority over silicon capital commitments and baseline specification updates.
:::
