...
When we measure our current AI capabilities against those historical transitions, we find that merely generating more code, RTL, or design proposals might be useful, but it does not by itself give us a new design method. Generative AI tools and foundation models can now produce candidate code blocks faster than human engineering teams can evaluate and verify them, a divergence we examine in @sec-design-loop-no-longer-scales. This widening "scissors gap" (the exponential divergence between rapid candidate generation velocity and slow physical signoff throughput) means that generating options faster helps only when our engineering process can determine what those options mean.
...
> **Architecture 2.0.** Architecture 2.0 is the discipline of building AI-native design systems that carry high-level system intent across the hardware-software boundary toward Pareto-optimal, physically realizable implementations, and of holding those systems to an architecture-grade evidence standard. It shifts the architect's primary role from manually crafting point microarchitectures to formulating multi-objective constraints, directing agentic exploration, and owning final design commitments.

This shift frames computer architecture across three historical epochs, a periodization we construct for this book's argument rather than an established community taxonomy (@tbl-architecture-epochs).
...
The upward feedback arrows in @fig-moonshot-prompt show that tool results continually force us to revisit and refine our middle-layer requirements. As structured in this stack, eight tightly coupled layers define the technical dependencies across our design space. The specific capacities, widths, and interface choices that follow are one illustrative decomposition of the prompt rather than resolved design commitments.
...
The scale of that gap is stark. Open-source software gives language models hundreds of billions of tokens to learn from; synthesizable hardware description code is orders of magnitude scarcer, curated architecture corpora and physical layout datasets are smaller still, down to thousands of validated samples [@LozhkovEtAl2024TheStackV2; @WangEtAl2025OpenRTLSet; @PrakashEtAl2025QuArch; @ChaiEtAl2022CircuitNet]. We infer that this thousandfold data scarcity gap is a central reason general-purpose language models hallucinate plausible but invalid hardware interfaces, and it is why AI-native design systems pair specialized domain curation with closed-loop physical verification. We measure the gap tier by tier in @sec-data-representations-world-models.
...
We highlight this asymmetric throughput gap in @fig-verification-scaling using constructed order-of-magnitude runtime ranges across tool execution stages, comparing AI candidate proposal latency against hardware verification check time across five design scales on a logarithmic time axis. In this constructed comparison, the circular data points place AI candidate proposal latency under one minute at every abstraction level. In contrast, the horizontal bars show verification check times expanding by orders of magnitude as design scope grows, scaling from milliseconds for tensor operators up to days or weeks for full SoC physical signoff and formal equivalence checking. This four-order-of-magnitude verification gap illustrates why accelerating candidate creation yields diminishing returns unless we pair generation with equally fast early screening filters. Generating hardware faster matters only if we retain the ability to identify and reject bad candidates at that same accelerated pace.
