...
Underneath this brief prompt lies a complete hardware-and-software SoC architecture. The multi-phase structure stacks the three operational layers of a complete architecture study (@fig-moonshot-prompt): the compact product prompt at the top, the eight coupled architectural requirement domains in the middle, and the execution and analysis pipeline at the bottom.
...
The complete AI-native design system organizes around a reusable knowledge substrate on one side and human authority on the other (@fig-foundation-system). The left side highlights the optional architecture foundation model, which ingests reusable domain knowledge to propose candidates or accelerate workload predictions. The center section contains the version-controlled project record, our authoritative source of truth for specifications, RTL sources, and EDA scripts. This record feeds conventional simulators, compilers, and physical signoff engines, which execute checks and return empirical evidence.
...
Each criterion pairs a metric dimension with its guiding architectural question and the factors that complicate comparative claims (@tbl-efficiency-dimensions). Five dimensions assess the proposed physical system, spanning performance, power and energy, reliability and correctness, scalability and cost, and sustainability, while the sixth tracks study cost itself, the simulation cycles, EDA licenses, and engineering hours expended to reach a verdict. An apparent efficiency gain in one dimension (such as peak compute throughput) is invalid if it silently inflates data movement energy, violates thermal envelopes, or incurs prohibitive study cost.
