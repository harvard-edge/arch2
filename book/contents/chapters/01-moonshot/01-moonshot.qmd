...
Executing the Architecture 2.0 moonshot requires resolving five open problems across representation, governance, physical validation, and engineering economics:

- **What machine-usable representations must exist before an unconstrained natural language prompt becomes a checkable architecture specification?** Translating a high-level goal into an executable search space requires committing to default parameters, interface contracts, memory models, and physical envelopes that the original request omitted. If these assumptions are inferred implicitly by learned models, the system explores unconstrained or ungrounded regions. Exploration becomes reproducible only when representations explicitly separate user-specified constraints from generator-inferred defaults.
- **Where must human commitment authority sit within an automated design loop to prevent governance from decaying into ceremonial rubber-stamping?** When an autonomous agent evaluates thousands of candidates per hour, human architects cannot review individual proposals. Architects need structured decision abstractions, Pareto-frontier projections, and residual risk summaries that make multi-thousand-candidate runs reviewable without slowing the release pipeline.
- **What diagnostic mechanisms can systematically expose when learned design heuristics have decoupled from physical silicon realities?** Foundation models and learned surrogates embed historical heuristics from past process nodes (such as planar CMOS or early FinFET rules) and legacy software runtimes. When applied to advanced packaging (e.g., 2.5D/3D chiplets, backside power delivery) or novel compiler IRs, these priors quietly generate obsolete microarchitectural trade-offs without triggering explicit tool crashes. Design environments need automated drift monitors that flag when learned priors from older planar or FinFET nodes violate gate-level timing and parasitics on 3D chiplet stacks.
- **Under what architectural complexity and team-scale conditions does an AI-native design system deliver a net reduction in total project cost?** Accelerating draft candidate creation is economical only if the downstream costs of formal verification, simulator license consumption, physical design rule check (DRC) and layout versus schematic (LVS) debugging, and expert review do not expand to consume the savings. Quantifying net savings requires auditable cost models that account for simulator license fees, DRC and LVS debugging hours, and human signoff effort alongside raw RTL drafting time.
- **How can multi-fidelity evaluation harnesses detect and penalize proxy exploitation before candidates reach expensive signoff stages?** Optimization algorithms aggressively exploit the blind spots of fast analytical models, promoting candidates with artificially high proxy scores that fail gate-level timing or thermal closure. Evaluation harnesses must quantify surrogate prediction uncertainty, routing high-uncertainty or borderline candidates to full gate-level timing and power checkers before decisions advance.
