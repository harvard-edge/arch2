...
: **Historical Epochs and the Emerging Architecture 2.0 Program.** As design complexity expands, automation moves from unassisted CAD scripts through localized learned surrogates toward AI-native design systems. The final row states an emerging engineering program rather than a completed historical transition. {#tbl-architecture-epochs tbl-colwidths="[22,25,28,25]"}
...
Analytical bounds, proxy scores, simulator results, and EDA reports provide very different kinds of evidence, at sharply different costs. Without early rejection bounds, accelerating candidate generation moves the verification burden downstream rather than removing it (@fig-verification-demand-scaling). Early rejection-bound filters, such as Amdahl caps, Roofline limits, and SRAM area bounds, prune non-viable candidates before they reach full signoff tools. The scaling model behind that figure is constructed, and its coefficients set where the crossover falls rather than whether it exists.
