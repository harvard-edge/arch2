...
**Carrying intent across the stack.** A compact request names an outcome and leaves the workload behavior, interface contracts, physical limits, and acceptance conditions unstated. Everything a design system does afterward depends on how those silences get filled.

- **What has to be represented before a compact design request becomes an architecture problem a system can work on?** Expanding intent means committing to assumptions about traffic, interfaces, and physical envelopes that the request never made, and those assumptions determine which candidates are reachable. The open part is not the expansion itself but the accounting: which commitments were made by the requester, which by the system, and which by nobody, so that a reviewer can tell an architectural result from an artifact of the setup.
- **Where should human commitment authority sit inside an automated exploration, and what does it need in order to hold?** A named authority can only accept or reject what is legible, so an exploration that produces thousands of candidates changes what accountability requires in practice. The question is what a system must expose, and at which points, for that authority to remain real rather than ceremonial.

**Reusing knowledge without inheriting its assumptions.** Prior designs, tuned heuristics, and learned models carry the technology, tooling, and workload conditions they were built under. Those conditions expire quietly.

- **What transfers across chip generations, and how would a design system know when it has stopped transferring?** A process node, a design kit, or a compiler change can invalidate a learned prior without producing a syntax error, a failed assertion, or any other visible signal. Detecting silent staleness in architectural knowledge is unsolved, and it is the difference between reuse that compounds and reuse that accumulates hidden debt.

**Pricing the whole loop, not the fast step.** Generation is the cheapest part of the work and the easiest to accelerate. Whether that acceleration reaches the program depends on everything downstream of it.

- **When does an AI-native design system leave a program better off after all of its costs are counted?** Data preparation, failed runs, verification load, and human review all draw on the same budget the speedup was supposed to relieve, and a local gain can move cost rather than remove it. Answering this requires a way to compare complete workflows at matched budgets, which the field does not yet have.
- **How can an evaluation loop keep an optimizer honest when the cheap evaluator is the thing being optimized?** Any screening proxy has blind regions, and a search with enough attempts will find them and report the result as a win. Building screens that surface their own uncertainty, and knowing which candidates that uncertainty should escalate, remains open across every method family this book covers.
