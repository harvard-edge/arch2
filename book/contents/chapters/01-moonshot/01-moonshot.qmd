...
This historical progression spans three distinct epochs of automation and architectural responsibility (@tbl-architecture-epochs). The periodization is constructed for this book's argument rather than adopted from an established community taxonomy; its columns track how the governing paradigm, automation mechanisms, and human responsibilities evolved across successive design eras.
...
For an architecture moonshot to be credible and testable, three core conditions must converge: a grand architecture challenge, a substantial shift in design practice, and an enabling advance in AI, data, or tools (@fig-moonshot-venn). Any two requirements without the third yield an incomplete engineering effort. Tackling a grand challenge without a new method leaves an unsolved problem; proposing a new design practice without an enabling advance remains mere aspiration; and demonstrating an enabling advance without a genuine architecture challenge yields an empty capability demonstration.
...
Only at the central intersection of all three domains does the moonshot become testable (@fig-moonshot-venn). This alignment ensures that learned methods address meaningful hardware challenges under real physical constraints rather than chasing ungrounded demonstrations. The discipline requires stating what capabilities the target depends on and identifying which evidence would challenge our baseline assumptions.
...
Tool feedback continuously refines those requirements, creating an explicit iteration loop across eight tightly coupled layers (@fig-moonshot-prompt). The specific capacities, widths, and interface choices that follow represent one illustrative decomposition of the prompt rather than finalized design commitments:
...
Navigating the eight layers of the prompt stack (@fig-moonshot-prompt) requires exposing the hidden dependencies, physical trade-offs, and specification choices that transform high-level intent into an executable study.

We start with the prompt itself. We must translate high-level intent into guiding questions and coupled design choices long before we ever see a generated chip artifact. The prompt asks us to compare several compute organizations for a real-time mobile XR workload, ensuring we respect a 3\ W thermal design power (TDP) sustained thermal envelope that our declared form factor and cooling assumptions must dissipate, alongside the realities of an LP process. As we break it down, individual phrases bound different parts of our study.
...
An AI-native design system couples a reusable knowledge substrate with deterministic project state and independent human authority (@fig-foundation-system). The optional architecture foundation model ingests domain knowledge to propose candidates or accelerate workload predictions. The version-controlled project record remains the authoritative source of truth for specifications, RTL sources, and EDA scripts, feeding conventional simulators, compilers, and physical signoff engines that execute checks and return empirical evidence.

The design system also decouples technical analysis from organizational authority (@fig-foundation-system). The human architect reviews tool evidence to formulate a supported technical recommendation, while a distinct Commitment Authority holds exclusive, non-delegable approval rights to advance, hold, or reject the design before committing silicon capital.
...
The Roofline model supplies a second screening bound by relating operational intensity to peak compute and memory bandwidth [@Williams2009Roofline]. Once our study establishes a candidate-specific operational intensity and the platform's machine balance, Roofline analysis identifies whether the design is memory- or compute-bound before simulation. In the method vocabulary that @sec-methods-generation-prediction-optimization develops, generation can propose legal vector configurations, prediction can estimate data-movement risks, and optimization can select which configurations merit full simulation. A candidate whose operational intensity falls short of the performance ceiling need not enter the simulator queue. However, operational intensity must reflect the candidate's mapping, declared memory hierarchy, and cache or scratchpad behavior; we recompute the bound when those properties shift. Roofline analysis rejects implausible candidates cheaply, but it cannot prove that a surviving design physically works.
...
To structure this multi-metric evaluation, we separate five dimensions of physical system quality from the meta-cost of the engineering study itself (@tbl-efficiency-dimensions). An apparent efficiency gain in one dimension (such as peak compute throughput) is invalid if it silently inflates data movement energy, violates thermal envelopes, or incurs prohibitive simulation and verification overhead.
...
The operational evolution across five core responsibilities contrasts established Architecture 1.0 practice with AI-native Architecture 2.0 methodology (@tbl-arch1-vs-arch2). Under Architecture 1.0, engineers manually script parameter sweeps and tune point microarchitectures. Under Architecture 2.0, learned surrogates and generative search automate candidate exploration and tool orchestration. Decision authority, however, remains fixed: while automated components propose candidates and aggregate evidence, the human architect and Commitment Authority retain exclusive approval rights over baseline contracts and silicon signoff.
...
Automated participation introduces explicit machine-usable representation requirements while preserving identical review obligations (@fig-design-loop-breaks). In established Architecture 1.0 work, project knowledge remains distributed across specifications, source files, and expert practice, with human architects manually driving tools and review loops. AI-native Architecture 2.0 work instead structures machine-usable project state and action spaces for automated agents. Both paradigms retain explicit review points and iterative feedback paths, keeping machine-driven exploration transparent to human architects.
...
The two workflows differ in one place: both carry explicit evidence, a review point, and a feedback path, but the AI-native workflow adds a representation of selected state and permitted actions that an automated method can read. That addition is the entire delta between the two paradigms, and making it safe is the central subject of this book.
