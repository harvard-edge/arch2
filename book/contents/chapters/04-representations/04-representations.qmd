...
Evaluating architectural hypotheses requires choosing among complementary observation paths with distinct coverage, acquisition costs, and blind spots (@fig-observation-source-choice). The decision space spans rapid analytical bounds through cycle-level simulations to fabricated silicon, demanding that each measurement retain its generating conditions and exact provenance.
...
Accumulating simulation logs and Verilog files does not create a usable dataset. Launching tool runs without a clear sampling strategy wastes compute while leaving decision boundaries unobserved. Systematic dataset construction follows an explicit eight-step sequence:
...
When an agent executes an action $a$ (such as modifying cache capacity, adding an NPU tile, or repartitioning clock domains), the world model evaluates the state transition $s' = T(s, a)$. To prevent rollouts from drifting into unconstructible regions, state representations enforce three mandatory properties:
...
The handoff contract in @tbl-represented-problem binds the cache capacity question to versioned design state, workload snapshots, physical constraint sets, and verification logs, delivering a defined problem to @sec-methods-generation-prediction-optimization without prescribing a search technique or claiming unverified results.
...
These failures occur during dataset assembly when representations silently drop critical structural relationships or conflate distinct physical domains:
