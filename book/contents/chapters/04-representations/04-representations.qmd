...
Architecture data aggregates across disparate formats, including specifications, source code, register-transfer level (RTL) netlists, execution traces, and tool measurements from simulators, formal provers, synthesis flows, physical signoff, and silicon. Review records and rejected design alternatives preserve human decisions that never appear in a tool report. These sources vary widely in access latency, freshness, acquisition cost, coverage, and ground-truth authority.
...
A transformation pipeline carries raw architectural sources into method-ready representations without surrendering source authority (@fig-architecture-data-progression). Heterogeneous sources (specifications, RTL, traces, electronic design automation (EDA) reports) are checked into a dataset, verified facts distill into durable knowledge while active parameters remain live project state, and downstream methods access linked native, structured, or learned views.
...
Large language corpora do not supply synthesizable modules, physical layouts, project histories, or checked architecture decisions. Reported hardware-generation failures, including nonexistent SystemVerilog syntax and invalid Advanced eXtensible Interface (AXI) interfaces, reinforce the need for domain curation and machine-checkable representations [@LiuEtAl2024CraftRTL].
...
## World-Model State Representations {#sec-hardware-representation-formats}
...
To prevent rollouts from drifting into unconstructible regions, state representations enforce three properties:
...
Representation pipelines must enforce boundaries between mutable parameters and non-negotiable invariants. In the prospective Lighthouse cache study, L2 capacity serves as the sole mutable parameter. Evaluating 2\ MiB, 3\ MiB, and 4\ MiB candidates for a 3\ W Mobile XR SoC (RISC-V RV64GCV core in TSMC N3 technology) requires verifying that every option resolves to a realizable SRAM macro organization while holding network-on-chip (NoC) topology, coherence policy, compiler, runtime, software image, and workload fixed under a 3\ W power envelope, $1.5\text{ mm}^2$ macro area, and 2.5\ ns access-time limit.
...
Every method later in this book runs on data and representations of the kind described here. What that material can actually support is not established.

- **What do you learn from the runs you threw away?** A model trained only on designs that worked has no idea where buildable stops, and it keeps proposing things past the edge because nothing it was shown told it there was an edge. One published physical-design corpus attempted 12,960 runs, kept 10,242 layouts, and discarded why the rest stopped. Survival analysis would be the repair, except it assumes the cutoff is independent of the subject and here the design causes its own run to stop. Most of those stops are not tool events anyway, since a router that cannot route completes and reports a violation count, and what a team calls a failure is usually a threshold in its own script. The cutoff belongs to whoever set it, so sweeping one design across utilizations locates its own limit directly instead of inferring it.

- **How similar is too similar, when every design in a corpus shares parents?** Proteins settled on a sequence-identity threshold by measuring it, molecules group by a shared core structure, and hardware has neither. The rule offered here is to split by the coarsest shared ancestor, which for a corpus built from one generator leaves no split at all and for a physical-design corpus leaves a test set of single digits. Parameterized generators put the same subgraphs on both sides of any line drawn by hand, so a model scores well by recognizing templates it has already seen. The cheap first measurement is not a split but a correlation, computable on the training data before any model is fit, and if it predicts the inflation that grouping would reveal then a corpus builder never has to run the grouped experiment at all.

- **Which steps in lowering a design actually destroy something?** This chapter names the loss once, when tensor shapes descend into gates and wires, and never measures it. Most of that descent is deterministic, so in the strict sense nothing goes missing and everything merely becomes harder to see. Some passes are genuinely not reversible, though. Technology mapping merges distinct source structures into the same cell graph and retiming erases stage boundaries, and those are the steps where a property stops being recoverable rather than just expensive to extract. Locating them costs nothing but a canonicalizer, since two designs a property separates either remain distinguishable one level down or they do not.

- **What does a workload description have to keep so a hardware comparison survives a compiler upgrade?** A basic block vector is defined over the basic blocks of one compiled binary, so changing an inliner does not shift those coordinates, it replaces them. This chapter conditions its surrogate on exactly that vector and separately forbids reusing a trace across software images, which together mean every compiler bump invalidates the input the surrogate depends on. Characterization designed to be independent of the machine has existed in this community for twenty years and was never independent of the compiler, since unrolling moves instruction-level parallelism and a different kernel library rewrites the access pattern. What matters is not that the error grew. It is whether the answer to two megabytes or four survived somebody upgrading a compiler.

- **How much of a reported improvement is larger than running the same design twice?** Placement and routing outcomes move with the seed, the thread count, and the tool version, and nobody publishes that spread. Every measurement in this chapter's territory is quoted against that floor implicitly and never against it explicitly. Until it is known, a surrogate reporting a small reduction in error has not been shown to beat rerunning the flow, and some published effect sizes in this area plausibly sit inside the noise. It is the cheapest missing number in the discipline and it is nobody's paper.
