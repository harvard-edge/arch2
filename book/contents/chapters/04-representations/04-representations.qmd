...
Representation pipelines must enforce boundaries between mutable parameters and nonnegotiable invariants. In the prospective Lighthouse cache study, L2 capacity serves as the sole mutable parameter. Evaluating 2\ MiB, 3\ MiB, and 4\ MiB candidates for a 3\ W mobile XR system-on-chip (SoC) powered by an RV64GCV RISC-V core in TSMC N3 technology requires verifying that every option resolves to a realizable SRAM macro organization while holding network-on-chip (NoC) topology, coherence policy, compiler, runtime, software image, and workload fixed under a 3\ W power envelope, $1.5\ mm^2$ macro area, and $2.5\ ns$ access-time limit.
...
| **Constraints and checks** | Subsystem power $\le 3\ W$, macro area $\le 1.5\ mm^2$, access time $\le 2.5\ ns$ under TSMC N3 ($3\ nm$) corners. Thermal and IR-drop reviews remain separate downstream checks. |
