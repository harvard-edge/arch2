...
Written out for the illustrative study, one row of that dataset is the tuple $\mathcal{O}$ with every field named. Explicitly, the design state $s_{\text{design}}$ and environmental state $s_{\text{env}}$ (encoding the workload context) must capture the exact hardware revision and dynamic phase dependencies:
