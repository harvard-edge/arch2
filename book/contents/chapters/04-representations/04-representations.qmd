...
**Learning from runs that did not finish.** Architecture data arrives from tools that abort, time out, and fail in ways that carry real information about the design space.

- **How should a representation record a run that failed rather than a result that is simply absent?** Discarding aborted synthesis and layout runs throws away the boundary they mapped, while treating them as ordinary bad scores misplaces that boundary. Handling truncated and censored observations correctly is a well-posed statistical problem the architecture community has largely left alone.
- **Where can a cheap model stand in for an expensive one without hiding a region the cheap model cannot see?** Mixing analytical estimates with a smaller number of physical runs expands a dataset quickly and can also fill it with confident coverage of regions nothing physical ever visited. Knowing which substitutions are safe requires characterizing where the fidelities disagree, not just how often.

**Encoding what the design is not allowed to violate.** Continuous representations make search efficient by making every direction traversable, including directions that produce illegal designs.

- **Can a learned representation be built so that nearby points remain legal designs?** Small moves in an embedding routinely break interface widths, structural invariants, or physical rules, so the search spends its budget proposing candidates that no tool can accept. Representations whose geometry respects hard constraints would change what search is even attempting.
- **How can a design claim be checked by someone who cannot be shown the design?** The most useful architecture data sits behind confidentiality obligations covering netlists, process kits, and calibrated tool traces, which is a principal reason the public corpus is thin. Representations that support independent verification without permitting reconstruction would unlock data that no amount of collection effort otherwise reaches.
