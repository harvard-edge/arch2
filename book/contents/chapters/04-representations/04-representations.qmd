...
Architecture results resist reduction to isolated numbers. An AI-native design system must resolve a critical tension: hardware data originates from shifting designs, expensive tool runs, and disparate workloads that frequently disagree. As the foundational entry to Part II (Technical Building Blocks), representation defines the semantic boundary between physical semiconductor realities and automated algorithms. A cycle count, timing slack report, or power estimate is never an unconditioned scalar. It remains bound to a specific register-transfer level (RTL) revision, software binary, workload trace, tool version, and operating corner. If a generated candidate drops a single clock gate or mishandles a reset state, the entire design fails.
...
![**Topological Explosion of Open-Source register-transfer level (RTL).** Plotting Rent's Exponent (Graph Entropy) against Total Gate Count across generations of open-source IP cores reveals the steep scaling of physical wiring complexity as systems grow. This analysis is based on gate counts and structural graph entropy extracted from a dataset of open-source IP cores.](images/plot_ch4.png){#fig-topological-explosion width="100%" fig-alt="Scatter plot showing Total Gate Count vs Rent's Exponent for various open-source hardware designs."}

This transition from simple microcontrollers to superscalar out-of-order machines fundamentally alters the physical layout problem (@fig-topological-explosion). As structural graph entropy spikes, placement engines can no longer rely on localized dataflows to resolve congestion. Instead, pervasive routing bottlenecks emerge, forcing tools to insert buffer trees and detour wires that degrade timing constraints and power envelopes. Consequently, architectures that appear syntactically correct will fail during physical synthesis unless they explicitly account for these escalating topological constraints.
...
[^fn-split-brain-c04]: **Split-Brain Causality**: Similar to LLM hallucinations in software, hardware hallucinations can silently pass syntax checks but manifest as catastrophic failures (e.g., timing violations or deadlocks) post-silicon, because the model optimized against a stale constraint.
