...
Architecture results cannot be reduced to isolated numbers and labels. A cycle count, timing report, or power estimate is an observation bound to a specific design revision, software image, workload, tool path, version, and set of operating conditions. Even if a candidate's text closely resembles valid designs, a single incorrect connection or flawed protocol assumption invalidates it.

Architecture data aggregates across disparate formats: specifications, source code and Register-Transfer-Level (RTL) netlists, execution traces, and tool measurements from simulators, formal provers, synthesis flows, physical signoff, and silicon. Review records and rejected design alternatives preserve human decisions that never appear in a tool report. These sources vary widely in access latency, freshness, acquisition cost, coverage, and ground-truth authority.

A representation denotes the specific form that exposes selected properties of a source or design to a given method. Representing an architecture problem using native RTL, typed graphs, measurement tables, retrieval indexes, or learned embeddings dictates which relationships remain visible, which designs are expressible, and which mistakes downstream methods fail to notice.

Architects use representation to denote a semantic design abstraction or structural level with legal objects and permitted transformations, whereas machine learning practitioners often use the term for an encoding or learned feature space optimized for a task [@BengioEtAl2013RepresentationLearning]. The unifying requirement is determining which semantics and distinctions survive to inform intended operations and verification checks. AI-native design systems require both exact design semantics to drive traditional tools and rich feature spaces to guide learned methods. Consequently, RTL, Intermediate Representations (IRs), constraint sets, graphs, and embeddings preserve distinct properties and cannot be swapped interchangeably.

Raw sources transform into useful data after identifying them, aligning versions and units, recording extraction conditions, and verifying that tool returns reflect declared properties. Verified facts distill into durable project knowledge, while active parameters remain live project state evolving alongside checked-in designs, software, workloads, constraints, and open issues. While foundation models supply broad prior knowledge about general mechanisms and design patterns, their weights cannot supply authoritative truth about an active project.

Machine-usable hardware representations preserve these explicit links within the six-stage design life cycle (@sec-architecture-20-ontology), connecting upstream candidate state to downstream search methods, tool execution, and verification feedback. These links must be inspected before choosing a method, invoking a tool, or interpreting a result.
...
Hardware architecture artifacts do not fit into uniform arrays. Architecture 1.0 workflows tracked design state through fragmented spreadsheets, unindexed simulation logs, and isolated Verilog files, treating tool returns as ephemeral checks. Architecture 2.0 unifies live project state, multi-fidelity surrogate models, and formal design provenance in machine-usable representation graphs.

Unlike image pixels or text tokens in standard machine learning benchmarks, an architectural measurement never exists in isolation. A timing report or cycle count is bound to a specific RTL revision, compiler flag set, workload trace, tool version, and operating corner. Treating a specification paragraph, a SystemVerilog module, a performance trace, and a routed timing report as interchangeable rows in a table strips away the physical and software context that gives measurements meaning.
...
Evaluating architectural correctness imposes strict boundaries. A model might generate a plausible token sequence that connects incorrect signals, violates an interconnect protocol, drops a reset condition, or references a stale module hierarchy. Showing a model more superficially similar examples will not repair these broken relationships. Usable representations must preserve the structure and contextual conditions that allow tools or reviewers to spot such mistakes.
...
Checked observations record what a source returned under documented conditions, while the responsible project source dictates which fact applies. Because unverified artifacts cannot fill either role, representation begins by establishing source identity and evidentiary scope.

## Heterogeneous Architectural Data Sources

Architecture data arrives from diverse media: specifications articulate requirements, RTL netlists define executable hardware, instruction traces capture dynamic workload behavior, and electronic design automation (EDA) signoff reports record physical realities like area, timing slack, and power integrity. Engineering review notes and rejected alternatives preserve human design rationale that never appears in a tool log. Every architectural data source pairs a structural format with the properties it can preserve and the context it cannot establish in isolation (@tbl-dataset-capability). A specification cannot prove that a local netlist satisfies a requirement, and a gate-level netlist captures pin-level technology mapping while remaining blind to high-level design intent or software behavior. Each source must be evaluated for the specific architectural question it is qualified to answer.
...
| Compiler AST (LLVM, Triton, CUDA PTX) | High-level control flow, language AST nodes, thread-block tiling, and memory swizzling constructs. | Target microarchitectural execution timing, resource contention, or lower-level physical constraints. |
...
| SystemC / TLM 2.0 virtual platform model | High-throughput transaction-level memory and interconnect modeling, sockets, and early software bring-up. | Pin-accurate signal timing, clock-cycle physical delays, or lower-level gate/layout parasitics. |
...
| Microarchitectural / memory trace (ChampSim, Ramulator) | Observed instruction execution, cache access patterns, and cycle-level DRAM bank/rank bus commands. | Behavior outside the captured trace population or system responses modified by timing feedback absent from the trace. |
...
Because each source preserves certain relationships while leaving other claims unsupported, collecting more volume will not fix structural gaps. Designing an evaluation study requires tracing the exact architectural path that a decision crosses:

- **Software and Compiler Stack:** Application binaries and execution traces capture dynamic phase behavior. Frontend compiler ASTs (Clang/LLVM, Triton, CUDA PTX) expose control structures, thread-block tiling, and software transformations. Hardware-oriented compiler representations (NPU CDFGs in Spatial IR or MLIR dialects) capture tensor dependencies, loop-nest tilings, and scratchpad SRAM allocations.
- **Microarchitecture and Simulation Stack:** TLM 2.0 virtual platforms supply transaction representations for early bring-up without pin-level clocking. Cycle-accurate simulators (gem5), accelerator simulators (SCALE-Sim), memory simulators (ChampSim, Ramulator 2), and compiled HDL engines (Verilator) expose core interface counters, bus contention, and queue dynamics.
- **Physical Synthesis and Signoff Stack:** Native RTL, synthesized netlists (Yosys, Design Compiler), SystemVerilog Assertions (SVA) paired with Bounded Model Checking (BMC), and coverage databases validate implementation. Physical synthesis and static timing analysis (OpenROAD, OpenSTA), alongside power integrity tools, inject signoff results before FPGA prototypes and post-silicon validation.

Expert judgment is conditioned data. For judgments to be reusable, records must capture the evidence presented to the expert, applied criteria, expert role, confidence level, and limiting context. Schedule-driven compromises or rough estimates must be labeled explicitly rather than hardening into accepted architectural facts (@sec-feedback-verification-trust).

## Data Licensing, Privacy, and Provenance

Authoritative sources in industrial architecture are governed by foundry Non-Disclosure Agreements (NDAs), Process Design Kits (PDKs, such as TSMC N7 or N3 kits), and proprietary Electronic Design Automation (EDA) intellectual property (IP) abstracts. A confidential 3\ nm cell library cannot be swapped for an open-source alternative without invalidating production guidance. Representations must retain original source identity, PDK revisions, NDA scopes, tool license terms, owners, access limits, and operating conditions, complete with immutable provenance links.

Artifact-level license and permitted-use metadata distinguish filesystem access from legal authorization for model training, retrieval, transformation, redistribution, or publication. Foundry NDA boundaries, PDK restrictions, commercial IP licensing (PCIe/DDR controllers, standard cell macro timing libraries, Liberty/LEF files), and tool vendor terms must travel alongside derived datasets, retrieval indexes, learned representations, and model weights [@LongpreEtAl2024DataProvenance]. A PDK or IP block licensed for internal chip implementation cannot be fed into external LLM endpoints or public pretraining corpora.

Generated-output provenance and project confidentiality raise distinct boundaries for EDA workflows. Retrieved passages, supplied design context, model endpoint identities, and similarity checks must be retained when examining generated RTL, synthesis scripts, or timing constraints to flag potential verbatim reproduction of proprietary IP or NDA collateral. Furthermore, netlist fragments, SPICE models, Liberty `.lib` files, and P&R reports may be prohibited from leaving secure enclaves, requiring explicit access, egress, and retention rules for model endpoints, retrieval indexes, logs, and caches.
...
Datasheets for Datasets standardizes documentation for dataset motivation, contents, collection, and governance [@GebruEtAl2021Datasheets]. For hardware AI datasets, dataset cards must track foundry PDK revisions, NDA boundaries, cell library licensing, tool terms, failed or censored runs, and verification claims.

Acquisition context establishes which source population, filters, tools, and measurement conditions produced the underlying records. Real-world and fleet observations require recording deployed hardware and software versions, workload population, sampling policy, measurement conditions, redaction rules, and collection cost. Observations describe only that specific version and population; they do not automatically become training labels for a subsequent design.

Public availability serves as only the first filter; multi-stage synthesizability and tool-flow attrition dictate the true yield of usable hardware data (@fig-public-code-to-rtl).
...
Initial repository counts are deceptive when only a fraction survive synthesizability screening and tool translation [@WangEtAl2025OpenRTLSet]. Collecting public source, screening legality, executing conversion tools, and accounting for failures are distinct, necessary steps in dataset construction.

Public corpus inventories use observation units that change with the task. General natural-language corpora report roughly $1.5\times10^{13}$ tokens [@MetaAI2024Llama31; @TogetherAI2023RedPajama], and open-source software corpora report roughly $9\times10^{11}$ tokens [@LozhkovEtAl2024TheStackV2]. Hardware collections count modules, layouts, or questions: OpenRTLSet contains roughly $1.3\times10^5$ Verilog modules [@WangEtAl2025OpenRTLSet], CircuitNet reports on the order of $10^4$ retained layouts [@ChaiEtAl2022CircuitNet], and QuArch contains question-answer pairs in the low thousands [@PrakashEtAl2025QuArch]. These quantities are not commensurate observations.

This data volume gap spans ten orders of magnitude across engineering domains (@fig-architecture-data-scarcity).

![**Domain corpus volume and data scarcity spectrum across engineering tiers.** Corpus scale spans ten orders of magnitude from web text and general software down to synthesizable RTL, physical layouts, and expert-validated architecture QA pairs. Hardware and system design operate under severe empirical data scarcity, requiring task-specific curation and physical verification rather than ungrounded language pretraining.](images/fig-architecture-data-scarcity){#fig-architecture-data-scarcity width="100%" fig-alt="Horizontal bar chart illustrating domain corpus volume and data scarcity across five engineering tiers, spanning 1.5e13 web tokens down to 1.5e3 architecture QA pairs on a log scale."}
...
Missing context is not poor data quality: QuArch validates architecture knowledge without describing a live project, and CircuitNet provides physical-design observations without recording software behavior. Architecture datasets must match the target property and transfer claim rather than treating sample count as sufficient evidence.

Community datasets preserve declared repositories and collection choices, but quality depends on contributor releases. Internal sources carry current designs, review histories, and vendor collateral, but privacy, intellectual property, and access limits constrain their use. Datasets must preserve access status rather than silently replacing restricted sources with public ones.

ChipNeMo illustrates task-specific domain adaptation using proprietary chip-design material for engineering assistance, EDA script generation, and bug summarization [@LiuEtAl2023ChipNeMo]. Internal data improves specialized tasks, but proprietary evaluations cannot substitute for independently testable claims.

## Workload Behavior, Trace Costs, and Execution Coverage

Static hardware netlists and microarchitectural specifications describe what a system can execute, but they reveal nothing about performance until software runs upon them. Application workloads dictate which hardware paths become active and which bottlenecks emerge. Data representations must capture dynamic workload phase encodings, execution scale, sampling policies, and observation costs in a unified execution state.

### Workload Phase Encodings and Dynamic Environment State

System performance, power dissipation, and thermal behavior are joint outcomes $y = f(x, w)$ of the hardware configuration $x \in \mathcal{X}$ and the dynamic workload feature vector $w \in \mathcal{W}$. Altering the application software or workload phase can shift a hardware candidate from compute-bound to memory-bound.

To expose workload behavior without full-trace simulation overhead, execution traces are transformed into compact feature representations. SimPoint extracts Basic Block Vectors ($w_{\text{BBV}} \in \mathbb{R}^d$) and clusters execution intervals into representative phases [@SherwoodEtAl2002SimPoint]. SMARTS computes statistical confidence bounds over sampled execution slices [@WunderlichEtAl2003SMARTS]. In emerging workloads, dynamic phase features extend to transformer Key-Value (KV) cache footprints and decoding burst lengths. Basic block vectors and generative phase tokens serve as dynamic environmental representations ($s_{\text{env}}$) conditioning microarchitectural evaluation.

Basic block vectors, memory stride histograms, and phase embeddings serve two representation roles:
...
For the prospective Lighthouse cache study (@sec-arch2-ontology), formulated as an L2 capacity evaluation under a 3\ W TDP envelope, workload state requires a versioned snapshot of an extended reality (XR) application alongside its software image and frame boundaries. Static memory traces cannot be substituted across software images because dynamic timing feedback alters instruction streams. Representations must bind the workload feature vector $w_{\text{BBV}}$ to its software image, collection policy, frame identifiers, and execution parameters.

### Observation Paths, Acquisition Costs, and Execution Limits

Evaluating architectural hypotheses requires choosing among complementary observation paths with distinct coverage, acquisition costs, and blind spots (@fig-observation-source-choice).

![**Observation source selection balances measurement fidelity against evaluation cost.** Representative analytical, trace-driven, cycle-level, RTL and gate-level simulation, FPGA or emulation, and silicon sources offer different coverage, costs, conditions, and blind spots. A study can combine multiple sources, ensuring each observation retains its specific context.](images/fig-observation-source-choice){#fig-observation-source-choice
...
| Synthesis and STA / power analysis (Yosys, Design Compiler, OpenSTA) | Logic transformation, technology mapping, and timing, area, or power estimates within the flow. | Libraries, modes, corners, constraints, tool versions, and physical approximations. |
...
Acquisition costs dictate dataset density: expensive observation paths yield fewer points, and direct comparisons require compatible designs, workloads, tools, and environmental conditions.

A simple calculation illustrates how coverage requirements multiply acquisition costs. Assuming a 1\ GHz target, a 1\ s measured interval, 0.2\ s of warmup, a simulator rate of 100,000 target cycles per wall-clock second, three repetitions, and four workloads:
...
During a single measured interval of 1\ s, a 1\ GHz target executes one billion cycles ($10^9$). At an assumed simulator rate of 100,000 target cycles per wall-clock second, simulating that interval demands roughly 2.8\ hours. Serial execution cost $T_{\text{serial}}$ combines target warmup cycles $C_{\text{warmup}}$, target measurement cycles $C_{\text{measure}}$, repetition count $R$, workload count $W$, and simulator throughput $v_{\text{sim}}$:
...
Under the assumptions in @eq-serial-execution-cost, the suite executes 10.0 billion target cycles and consumes 27,778 serial simulator hours. While target cycles and simulator rate dictate time per attempt, warmup requirements, repetitions, and workload coverage multiply the total expense. In BOOM-Explorer, for example, high-fidelity VLSI flow evaluations across 994 microarchitectures required between six and fourteen hours per candidate [@BaiEtAl2021BOOMExplorer].

Because expenses vary, detailed acquisition records must accompany every result. This record identifies tool fidelity, completion status, seeds, configurations, uncertainty intervals, execution time, queue time, compute resources, memory footprint, license consumption, and human effort. Preserving distinct resource metrics ensures downstream methods can evaluate whether a retained state repaid its cost.

An architecture sample links measured results to design history, property checks, acquisition status, and consumed resources (@tbl-architecture-observations-not-labels).
...
Accumulating simulation logs and Verilog files does not create a usable dataset. Launching tool runs without a clear sampling strategy wastes compute while leaving decision boundaries unobserved. Dataset construction follows an explicit sequence:
...
The first four steps shape acquisition; the next three protect identities, lineage, and evaluation splits; the final step records collection limits.

### Multi-Fidelity Sampling

Multi-fidelity sampling combines rapid analytical bounds with targeted high-fidelity evaluations from RTL synthesis or cycle-accurate simulation. Active learning algorithms adaptively select observations that provide the most utility [@Settles2009ActiveLearning]. Multi-fidelity methods combine sources varying in cost and accuracy when they estimate the same output or share an explicit relationship [@PeherstorferWillcoxGunzburger2018Multifidelity]. A cheap estimate and a costly tool result blend safely only when measuring the same property under aligned conditions.
...
Label errors and data cascades compound upstream data corruption into delayed downstream failures [@NorthcuttJiangChuang2021ConfidentLearning; @SambasivanEtAl2021DataCascades]. Provenance chains run from native artifacts through parsed measurements to architectural interpretations. Source corruption, parser errors, mismatched identities, or invalid derivations each introduce distinct failure modes. Computed attributes must retain raw returns and exact derivations.

Outcome categories must remain explicit: invalid requests, dependency failures, infrastructure failures, incomplete runs, limit violations, formal results, and unknown outcomes answer different questions. Naively collapsing them into binary labels prevents methods from distinguishing license drops from physical infeasibility.

Public benchmark contamination further complicates evaluation: models trained on public HDL corpora often contain benchmark solutions [@WangEtAl2025VeriContaminated; @Balloccu2024Leakage]. Held-out designs, source identities, and executable checks matter more than sample volume.

OpenRTLSet demonstrates this pipeline: public repositories were filtered for synthesizability and converted via HLS, yielding roughly 24,000 Verilog modules from an initial 103,000 C/C++ repositories (@fig-public-code-to-rtl) [@WangEtAl2025OpenRTLSet]. Released rows preserve repository URLs, licenses, and module descriptions for fine-tuning, but without repository-disjoint splits or failed conversion corpora, they do not certify functional correctness or general synthesizability. CircuitNet attempted 12,960 physical-design runs and retained 10,242 layouts, illustrating how excluding failures risks survivor-only coverage [@ChaiEtAl2022CircuitNet].

In the prospective Lighthouse cache comparison, the observation unit is one 2, 3, or 4\ MiB capacity candidate bound to declared design revisions, software images, XR workload snapshots, simulators, and process assumptions. The budget permits one SRAM evaluation per capacity and four cycle-level runs, logging distinct statuses for every attempt.

### Failed and Censored Runs

Purging failed tool runs introduces survivorship bias [@Rubin1976InferenceMissingData]. A timing violation or routing congestion error is an observation that defines physical feasibility boundaries. Datasets must record failed, timed-out, and censored runs alongside successful outcomes.[^fn-censored-c04]
...
A versioned execution history separates attempt status, candidate outcome, and project updates to prevent cleaning routines from conflating tool failures with design evidence (@fig-execution-history).

![**Execution history tracking preserves status, outcome, and cost attributes.** Attempt status records tool completion or failure, candidate outcome records what the result supports for the active comparison, and project update records whether accepted design state changed.](images/fig-execution-history){#fig-execution-history width="100%"
...
Prior results become comparable samples only when candidate, workload, software, tool, and physical conditions fully match study specifications (@tbl-represented-problem); unmatched records remain advisory context.

What is preserved from a failed run depends on its failure mechanism (@tbl-negative-traces).

| **Failed run or rejected alternative** | **What we keep** | **How later work should use it** |
| --- | --- | --- |
...
Timing or power failures constrain candidates when identical conditions recur, whereas a tool crash flags a need for environment repair rather than candidate rejection.

## Extracting Architectural Knowledge from Raw Data

Raw tool returns do not constitute project knowledge or active state. Raw data records what a tool returned under historical conditions; durable knowledge distills reusable design principles, verified specifications, and architectural rationale; and live project state defines authoritative hardware, software, workload, and constraint parameters governing current work.

These components change at different rates and originate from distinct sources (@tbl-knowledge-objects). Foundation models supply prior knowledge but cannot serve as the authoritative source of record for current project state.

| **Object** | **What we preserve** | **Responsible source and freshness rule** |
| --- | --- | --- |
...
![**Stale reference sources create phantom internal consistency in active design states.** When methods read earlier specifications or stale project state, they propose changes that violate the version under active design.](images/fig-split-brain-causality){#fig-split-brain-causality width="100%" fig-alt="A stale reference state and a current project state lead to an incompatible proposal. Version and freshness checks expose the mismatch before the proposal reaches a costly tool flow."}

Explicit regeneration and hash checks detect state drift before proposals reach expensive downstream toolchains. If represented state cannot reproduce its named design, dependent work must halt.

Maintaining current state incurs compute, storage, license, and review costs. Every source that influences decisions requires an owner and an invalidation trigger (new design revisions, compiler releases, workload captures, tool calibrations, expired waivers). When refresh costs exceed value, representations should be narrowed or frozen with explicit review conditions.

## Hardware Representation Formats and World-Model State {#sec-hardware-representation-formats}

Passing an architectural design into machine learning algorithms or optimization loops requires selecting a structural feature representation. The encodings chosen (flat tokens, ASTs, CDFGs, layout matrices, learned embeddings) dictate which design variations are expressible, which structural dependencies survive, and which candidates downstream methods consider similar.

Parameter schemas enumerate legal values; ASTs and CDFGs preserve structural and dataflow relationships obscured by token sequences. Clang/LLVM ASTs expose software control flow; MLIR dialects and Spatial IR capture tensor dependencies, loop tilings, and scratchpad allocations; Triton and CUDA PTX ASTs represent thread-block tiling and Tensor Core scheduling. Typed intermediate representations reject malformed constructions before execution.

Intermediate representation scaling directly governs High-Level Synthesis (HLS) and compiler turnaround time (@fig-ch04-ast-hls-latency). While unpartitioned, flat syntax trees suffer super-linear compilation blowup ($O(N^{2.45})$), progressive multi-level IRs in CIRCT/MLIR scale hierarchically ($O(N^{1.20})$), maintaining sub-minute compilation turnaround even across hundreds of thousands of operations.

![**HLS synthesis latency scaling across intermediate representation node count.** Flat unpartitioned syntax trees exhibit severe super-linear synthesis latency ($O(N^{2.45})$), triggering tool timeouts (> 1 hour) on complex SoC modules. Multi-level hierarchical representations in CIRCT/MLIR scale gracefully ($O(N^{1.20})$), achieving a $24.8\times$ latency advantage on large hardware blocks.](images/fig-ch04-ast-hls-latency){#fig-ch04-ast-hls-latency width="100%" fig-alt="Log-log plot comparing HLS synthesis latency across IR node count for flat Verilog/C ASTs, scalar LLVM IR, and multi-level CIRCT/MLIR graph representations, showing an order-of-magnitude scaling advantage for hierarchical IRs."}

Modern representation stacks deploy four specialized representations to bridge the hardware semantic gap:

1. **Progressive Dialect Lowering (CIRCT / MLIR):** Compilers lower high-level tensor operators (`linalg`, `affine`) through intermediate control and memory dialects (`scf`, `memref`) down to hardware-specific dialects (`hw`, `comb`, `seq`, `sv`, `firrtl`) [@CIRCT]. Progressive lowering preserves high-level loop nest structures and clock domain boundaries that flat Verilog ASTs lose.
2. **Equality Graphs (E-Graphs & `egg`):** Hardware optimization frequently encounters phase-ordering deadlocks, where applying one microarchitectural transform (e.g., pipelining before retiming) permanently closes off a superior solution. E-graphs represent entire equivalence classes of structural rewrites simultaneously, enabling saturation-based rewriting and optimal technology mapping without greedy heuristic traps.
...
Lowering across multi-level IR stacks (PyTorch/Triton $\rightarrow$ MLIR $\rightarrow$ CIRCT $\rightarrow$ SystemVerilog) introduces semantic entropy as high-level tensor shapes degrade into raw gates and wires. For multi-chiplet systems, topology representations capture Compute Express Link (CXL 3.1) and Universal Chiplet Interconnect Express (UCIe), modeling credit-based flow control, flit serialization latency, and NUMA memory hop penalties.

SystemVerilog specifies logic, hierarchy, and verification constructs [@IEEE2023SystemVerilog]. Technology-mapped gate-level netlists and And-Inverter Graphs (AIGs) represent cell primitives, pin-level interconnect, and gate delays. Microarchitectural traces (ChampSim, Ramulator 2) capture cycle-by-cycle memory streams. FIRRTL and typed CIRCT dialects expose compiler operations and transformations [@CIRCT; @Izraelevitz2017Chisel]. Companion formats like Synopsys Design Constraints (SDC) and Unified Power Format (UPF) carry timing and power intent [@SynopsysSDC; @IEEE2024UPF]. Each representation format makes specific properties explicit while leaving other constraints for downstream checks (@tbl-representation-formats).
...
When deploying machine learning for retrieval, surrogate prediction, or design space search, hardware artifacts project into continuous latent spaces across three primary classes:
...
Before automated optimizers or reinforcement learning agents execute multi-step planning rollouts, representation pipelines must define a formal state space. Architecture world models serve as learned surrogate transition predictors simulating environmental responses and structural consequences under proposed actions (@sec-methods-generation-prediction-optimization).

The world-model state vector $s$ is defined as a coupled tuple:
...
- **Environment and operating-condition state ($s_{\text{env}}$):** Dynamic operational context incorporating workload phase vectors ($w_{\text{BBV}}$ or trace embeddings $\mathbf{z}_{\text{trace}}$), process corners (TSMC N3 SS/TT/FF), UPF power domain states, SDC clock maps, and thermal-power envelopes.

When an agent executes an action $a$ (modifying cache capacity, adding an NPU tile, re-partitioning clock domains), the world model evaluates the state transition $s' = T(s, a)$.

To prevent rollouts from drifting into un-constructible regions, state representations enforce three properties:
...
| CPU / GPU code (C/C++, Triton, CUDA) | Compiler AST (LLVM, Triton GPU IR, PTX AST), CFG, or dataflow graph. | Analyze control flow, optimize thread tiling, transform layouts, guide kernel generation. | Native source files, compiler manifests, target ISA specs, functional validation. |
...
| Floorplan, physical database, tool report | Geometry, spatial graph, typed report entries, image tensor, physical representation. | Retrieve related layouts, localize physical pressure, estimate implementation properties. | Coordinate system, units, design/tool versions, mode, corner, constraints, waivers, status. |
...
Proposed changes based on derived views must write back through authoritative databases or reproducible tool commands, repeating legalization and implementation checks.

Representation structure governs compiler analysis and reasoning scope: explicit hierarchy enables modular analysis, whereas flattened views force whole-graph inspection. Compilation latency must be measured on the chosen toolchain before assigning budgets or claiming scalability advantages.
...
No single representation format bears the full weight of an architectural design loop: learned vector embeddings lose structural detail required for synthesis, while native SystemVerilog netlists preserve syntax without certifying correctness, workloads, or physical assumptions. AI-native design systems construct linked representation pipelines combining exact native artifacts with structured topological views and traceable context records.
...
Representation pipelines must enforce boundaries between mutable parameters and non-negotiable invariants. In the prospective Lighthouse cache study, L2 capacity serves as the sole mutable parameter. Evaluating 2\ MiB, 3\ MiB, and 4\ MiB candidates for a 3\ W Mobile XR SoC (RISC-V RV64GCV core in TSMC N3 technology) requires verifying that every option resolves to a realizable SRAM macro organization while holding NoC topology, coherence policy, compiler, runtime, software image, and workload fixed under a 3\ W power envelope, 1.5\ mm^2^ macro area, and 2.5\ ns access-time limit.
...
Parametric changes select new values within an existing framework (cache capacity or queue depth); structural changes alter components or relationships (adding pipeline stages or swapping interconnects), requiring distinct schemas and separate studies.

System and software contracts (ISA specifications, memory consistency models) must be codified directly as dependencies and invariants. Physical collateral demands equal coupling: timing constraints rely on hierarchical paths in SDC files [@Synopsys2023ConstraintMapping], and power intent requires UPF alignment with matching RTL hierarchies [@IEEE2024UPF].

Multi-corner, multi-mode analysis assesses physical designs across operating modes and timing views [@Cadence2021Tempus]. Design candidates must maintain consistent identities across this matrix, with every result naming its condition rather than substituting typical-corner values for hold checks.

Search space encodings impose geometric neighborhoods on optimization. Encoding cache capacity on a simple linear axis treats 512\ KiB to 1\ MiB identically to 4\ MiB to 4.5\ MiB, manufacturing invalid design points if 4.5\ MiB macros are unrealizable. Indexing over realizable macro organizations or using logarithmic scales avoids misleading distance metrics [@LeeBrooks2006RegressionModeling].
...
Reusing architectural artifacts requires making hidden context explicit (@tbl-representation-debt).
...
The handoff contract in @tbl-represented-problem binds the cache capacity question to versioned design state, workload snapshots, physical constraint sets, and verification logs.
...
The representation delivers a defined problem to @sec-methods-generation-prediction-optimization without prescribing a search technique or claiming unverified results.

## Common Pitfalls

These failures occur during dataset assembly when representations silently drop critical relationships:
...
- **Scrubbing confidential PDK views and Liberty timing models:** Omitting foundry pin capacitances, non-linear delay models, and corner specs due to NDA restrictions forces surrogate models to train on uncalibrated proxy libraries (such as Nangate45), inducing domain shift when transferring to commercial silicon nodes.[^fn-pdk-proxy-drift-c04]
...
This chapter treats representation as a commitment about what a later method can see. The questions below remain open:

**Keeping low-fidelity proxies honest during design search.** Fast simulation proxies accelerate exploration, but error distributions shift unevenly across microarchitectural boundaries.

* **What should a representation record about a proxy's past mistakes, so a search stops returning to the peaks that proxy invented?** Existing acquisition functions assume uniform proxy error across design spaces, ignoring rank inversions between transaction-level models and cycle-accurate simulations. Tracking multi-fidelity discrepancies as structured failure attributes within design histories is needed to prevent search loops from fixating on false proxy peaks.
* **Can a learned representation stay aligned with signoff when proxy error jumps rather than drifts across a design boundary?** Multi-fidelity surrogates typically assume smooth approximations with additive noise. When queues saturate or bus arbitration degrades, proxy error jumps discontinuously. Embedding structural saturation thresholds directly into latent state representations remains an open challenge.
...
* **How can a learned world model be held to legality at every step, rather than only when a candidate leaves it?** Continuous planning rollouts frequently propose intermediate states violating pin widths, clock boundaries, or macro dimensions. Integrating round-trip legalization directly into state transitions ensures surrogate planning operates strictly over constructible configurations.

**Maintaining live project state and provenance across revisions.** Design systems must track source identities and non-disclosure boundaries so that changing parameters or libraries does not invalidate project state.

* **Where should dependency invalidations attach within an architecture representation so that mutating a core generator parameter automatically purges stale simulation traces and physical reports?** Automated invalidation triggers tied to candidate content hashes are required to prevent optimization loops from consuming un-invalidated historical runs under split-brain causality.
* **Can feature representations of proprietary standard cell libraries be sanitized for cross-organization model pretraining without exposing confidential foundry process parameters?** Formulating privacy-preserving feature extraction frameworks would enable community pretraining while protecting confidential foundry collateral.
...
Learned embeddings and surrogate models must remain anchored to authoritative project records. Five takeaways govern architectural representation and state tracking:
...
With dataset and state representations structured, @sec-methods-generation-prediction-optimization weighs generation, prediction, and optimization against conventional methods and direct tool evaluation.
