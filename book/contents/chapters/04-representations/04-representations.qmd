...
- **Construct** datasets that retain failed, timed-out, and censored runs, so a surrogate learns where the feasible envelope ends rather than only what lies inside it.
...
A foundation model pretrained on open repositories therefore learns toy modules and syntax fragments rather than manufacturable physical designs.[^fn-foundation-model-c04] No volume of public text teaches the unwritten rules of physical design, such as resolving metastable clock-domain crossings,[^fn-metastability-c04] managing dynamic power drops across the power grid, or preventing combinational loops, because those rules are visible only in designs that were carried through signoff, and almost none of those are public. That data gap compounds the scissors gap between generation velocity and signoff throughput (@sec-design-loop-no-longer-scales), since what a generator produces quickly is exactly what the tools must then qualify slowly.
...
Written out for the illustrative study, one row of that dataset is the tuple $\mathcal{O}$ with every field named. Explicitly, the design state $s_{\text{design}}$ and environmental state $s_{\text{env}}$ (encoding the workload context) must capture the exact hardware revision and dynamic phase dependencies (@lst-ch04-state-tuple):

```{.json #lst-ch04-state-tuple lst-cap="Example serialized state tuple for hardware design states"}
{
  "s_design": {
...
Dataset scale is dictated by the architectural comparison rather than raw row counts. A dataset is large enough when one additional observation no longer changes the target decision. For this cache study, separating the 3\ MiB and 4\ MiB candidates from the 2\ MiB baseline requires enough matched cycle-level executions so the frame-time difference exceeds the run-to-run variance on the identical workload snapshot. Simultaneously, each candidate requires an SRAM evaluation that returns power, area, and access time against hard constraints, or a failure confirming the configuration is unrealizable. Unmatched observations - such as frame times from a different software image - add rows without adding evidence. If the budgeted executions cannot separate the candidates, the measurements cannot resolve the comparison. The fix is a different workload snapshot or wider trace window, not accumulating more rows of the same type.
...
In our cache study, this integrity pipeline governs the evaluation of the 2\ MiB, 3\ MiB, and 4\ MiB cache candidates. Each observation unit binds its candidate configuration to a declared RTL revision, software compiler image, mobile XR workload trace, and TSMC N3E PDK operating corner, logging exact exit codes and raw stdout streams to guarantee end-to-end reproducibility.
...
@fig-ch01-three-layers-ai-integration separates three layers, and each reads this state in a different form. First, point assistance consumes and emits flat tokens, which is why it accelerates typing without enforcing a checkable invariant. Second, an L2 (AI-Driven) optimizer works on a structured state vector or a local circuit graph inside its fixed container and cannot see a cross-boundary trade-off. Finally, an L3 (AI-Native) system needs typed multi-level intermediate representations (Circuit IR Compilers and Tools (CIRCT) and MLIR dialect lowering) together with the coupled state tuple $s$ defined below, because its proposals are the only ones that cross the hardware–software stack. Parameter schemas enumerate legal configurations directly, while ASTs, CDFGs, and MLIR spatial dialects preserve control flow, tensor dependencies, and memory-hierarchy staging that flat token sequences obscure. Typed intermediate representations reject malformed structural transformations and illegal operand types before tool execution.
...
**Evaluation Limitations:** The research community frequently evaluates generative models on shallow Verilog leaf modules due to low simulation overhead in Python testbenches. These benchmark modules are typically six times smaller than production RTL and lack submodule instantiation. High success rates on these benchmarks do not indicate capability in resolving the structural interface bugs that necessitate silicon respins.
...
This spatial-semantic dilation gap explains why sequence models and text-based large language models fail when scaling from isolated toy gates to complex hardware blocks. When two interconnected signals (such as an AXI `tready` backpressure line or a state machine reset net) are separated by thousands of intervening tokens, standard self-attention mechanisms dissipate focus across syntactic boilerplate rather than preserving electrical invariants. Multi-level typed intermediate representations such as CIRCT MLIR are designed to compress those structural relationships into direct SSA operand graphs, which is the motivation for moving above flat RTL. The measured Verilog side of that comparison appears in @fig-ch04-spatial-semantic-dilation b, where semantic operators occupy $16\%$ to $27\%$ of tokens across six corpora. We have not measured token composition for a CIRCT corpus, so the typed-dialect side of the comparison is stated as a design intent rather than a result.
...
Embeddings accelerate similarity queries and continuous optimization, but remain auxiliary views rather than substitutes for authoritative source code or native intermediate representations. Continuous latent spaces inherently collapse exact syntactic rules, pin connectivity, and physical constraints. Consequently, any optimization done in the continuous domain faces the *Inverse Legalization Problem*: proposed candidate vectors must round-trip back into discrete, legally constructible graphs (netlists or ASTs). To achieve this without generating syntax errors or floating pins, AI-native pipelines utilize discrete decoders paired with masked action spaces. The generator does not emit raw text; instead, it outputs probability distributions over a restricted vocabulary of valid graph mutations (e.g., adding a buffer, resizing a gate) enforced by a formal grammar or application programming interface (API), ensuring the resulting discrete netlist passes hard tool checks before entering project records.
...
::: {.callout-lighthouse title="Lighthouse Case Study and Representation Handoff"}
**Context.** A machine-usable representation must bind the mutable design space to physical invariants to ensure the automated search remains grounded.

**Application.** For the Lighthouse mobile XR case study, this means establishing a rigorous handoff contract between represented state and downstream method selection, formalizing the exact boundaries of the architectural exploration space.

**Outcome.** This contract ensures that any method applied to the XR cache capacity question operates within strict physical constraint sets and verification logs, delivering a defined problem without claiming unverified results.
:::

The handoff contract in @tbl-represented-problem binds the cache capacity question to versioned design state, workload snapshots, physical constraint sets, and verification logs, delivering a strictly defined problem to subsequent chapters without prescribing a specific search technique or claiming unverified results.
...
- **Negative and censored result preservation.** Failed runs, synthesis timeouts, and negative-slack results are the boundary points of the feasible envelope. A dataset that keeps only the survivors teaches an optimizer that the cliff edge is not there.
- **Structure over token streams.** Typed multi-level intermediate representations keep concurrency, connectivity, and clock-domain invariants visible at module scales where a flat text stream loses them.
