...
Contrasting these corpus scales reveals a data volume gap spanning ten orders of magnitude across engineering tiers (@fig-architecture-data-scarcity).

![**Domain corpus volume and data scarcity spectrum across engineering tiers.** The horizontal log-scale chart contrasts corpus volume across five engineering tiers: web text ($\sim 1.5\times 10^{13}$ tokens [@MetaAI2024Llama31; @TogetherAI2023RedPajama]), open-source software ($\sim 9\times 10^{11}$ tokens [@LozhkovEtAl2024TheStackV2]), synthesizable Verilog modules ($\sim 1.3\times 10^5$ modules [@WangEtAl2025OpenRTLSet]), physical layouts ($\sim 10^4$ layouts [@ChaiEtAl2022CircuitNet]), and expert-validated architecture QA pairs ($\sim 1.5\times 10^3$ pairs [@PrakashEtAl2025QuArch]). Hardware and system design operate under severe empirical data scarcity, requiring domain-specific curation and physical verification rather than ungrounded language pretraining.](images/fig-architecture-data-scarcity){#fig-architecture-data-scarcity width="100%" fig-alt="Horizontal bar chart illustrating domain corpus volume and data scarcity across five engineering tiers, spanning 1.5e13 web tokens down to 1.5e3 architecture QA pairs on a log scale."}
...
![**HLS synthesis latency scaling across intermediate representation node count.** The log-log plot evaluates High-Level Synthesis turnaround time against IR node count across benchmark kernels (PolyBench-HLS, MachSuite AES, Rosetta Rendering, and OpenRTLSet Conv2D) and complex SoC blocks [@CIRCT; @WangEtAl2025OpenRTLSet]. Flat, unpartitioned Verilog/C syntax trees exhibit super-linear latency scaling ($O(N^{2.45})$) that crosses the 1-hour tool timeout barrier at $\sim 150,000$ nodes. Scalar LLVM/polyhedral IR scales at $O(N^{1.75})$, while multi-level hierarchical IRs in CIRCT/MLIR scale at $O(N^{1.20})$, providing a $24.8\times$ latency advantage on large blocks ($\sim 500,000$ nodes) and preserving sub-minute turnaround.](images/fig-ch04-ast-hls-latency){#fig-ch04-ast-hls-latency width="100%" fig-alt="Log-log plot comparing HLS synthesis latency across IR node count for flat Verilog/C ASTs, scalar LLVM IR, and multi-level CIRCT/MLIR graph representations, showing an order-of-magnitude scaling advantage for hierarchical IRs with a 24.8x latency gap on large blocks."}
...
For students and researchers working on machine learning for computer architecture, hardware representations present unique challenges: unlike natural language or images, hardware representations must preserve strict physical, timing, and semantic invariants across discrete levels of abstraction. The following open questions define fertile research directions where novel representations can directly improve search efficiency and physical signoff fidelity:

- **What structured discrepancy histories should an architectural representation record to prevent search loops from returning to ungrounded proxy peaks?** Existing acquisition functions assume uniform Gaussian error distributions across design spaces, ignoring sharp rank inversions between transaction-level models and cycle-accurate simulations. Tracking multi-fidelity discrepancies as first-class structured attributes within design state graphs is needed to prevent optimizers from fixating on false proxy optima.
- **How can a learned representation maintain physical alignment when proxy error jumps discontinuously at architectural saturation boundaries?** Analytical surrogates assume smooth cost landscapes. When buffer queues saturate or memory bus arbiters switch into throttling modes, simulation error escalates abruptly. Embedding discrete structural saturation thresholds directly into latent hardware representations remains an open challenge.
- **How can a learned world model be held strictly to physical legality at every planning rollout step?** Continuous latent embeddings enable efficient gradient-guided exploration, but multi-step rollouts easily drift into physically impossible states (such as illegal pin pitches, overlapping macro keep-outs, or unroutable clock topologies). Formulating differentiable, round-trip projection operators that map unconstrained latent vectors onto constructible hardware configurations at each rollout step is an active research goal.
- **Where should invalidation triggers attach within an architectural representation to purge stale downstream traces automatically?** When an upstream microarchitectural parameter changes, automated, hash-chained dependency graphs are required to eliminate split-brain causality, ensuring optimization loops never evaluate candidates against obsolete downstream simulation or physical signoff reports.
- **How can standard-cell library embeddings be sanitized for cross-organizational pretraining without leaking confidential foundry design rules?** Standard-cell characteristics and layout design rules represent proprietary foundry IP. Formulating cryptographically audited, privacy-preserving feature abstractions would unlock collaborative open-source foundation model pretraining while strictly protecting foundry trade secrets.
