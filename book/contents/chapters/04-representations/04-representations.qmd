...
Lowering across multi-level IR stacks (PyTorch/Triton $\rightarrow$ MLIR $\rightarrow$ CIRCT $\rightarrow$ SystemVerilog) introduces *semantic entropy* as high-level tensor shapes degrade into raw gates and wires. For multi-chiplet systems, topology representations capture Compute Express Link (CXL 3.1) and Universal Chiplet Interconnect Express (UCIe), modeling credit-based flow control, flit serialization latency, and non-uniform memory access (NUMA) memory hop penalties.
