...
1. **Progressive Dialect Lowering (CIRCT / MLIR):** Compilers structure hardware compilation as a progressive pipeline across domain-specific dialects [@CIRCT]. High-level tensor operations (`linalg`, `tosa`) and polyhedral loop representations (`affine`) capture multi-dimensional iteration spaces and parallel data access patterns. Intermediate passes lower these into structured control flow (`scf`) and explicit multi-level buffer staging (`memref`). Finally, hardware-specific dialects (`hw`, `comb`, `seq`, `sv`, `firrtl`) lower logical structures into explicit clock domains, bitvector operations, and register-transfer primitives. This staged lowering prevents the semantic collapse of flat Verilog ASTs, allowing optimization passes at each tier to reason at the highest applicable level of abstraction before bit-level lowering.
