...
As we gather data, acquisition costs heavily dictate the shape of our resulting dataset. Expensive observation paths yield far fewer data points. We can only safely perform direct comparisons across observations that share compatible designs, workloads, tools, and environmental conditions. Simply dumping a massive collection of incompatible reports into a repository does not create a dense, usable dataset for our current study.
...
Complicating matters, teams also use the term to describe training records sampled directly from a learned generator. These records carry a fundamentally different status from the four objects above. Because no architecture tool actually measured a property, we cannot call it a simulator return. A Hardware Description Language (HDL) record remains strictly proposed RTL until our executable checks qualify it. Unlike an augmentation, a learned sample might have no observed parent, and unlike an imputation, it produces an entirely new record rather than simply filling a missing entry. Machine-learning experiments warn us that treating learned synthetic data as if it were real can impair downstream generalization [@VanBreugelEtAl2023SyntheticData]. In our architecture datasets, we therefore carefully retain the generator, its training scope, our sampling procedure, and any checks we ran against real or tool-derived observations.

These objects fail us in distinctly different ways. A generator might overproduce familiar structures while completely missing rare but legal designs, whereas a simulator might hand us highly precise values derived from a biased or poorly calibrated model. Perturbations preserve many of their parents' blind spots, and imputations often smooth over the very constraint boundaries that matter most. Because of these distinct failure modes, we must subject generated artifacts to rigorous legality checks, group our dependent variants by their ancestor, and calibrate any low-fidelity or predicted values against sources that offer stronger support for the targeted property.
...
We often rely on a powerful acquisition pattern. We apply cheap analytical checks to generated candidates across a broad legal region, and then we strategically spend our costly observations right near constraint boundaries or decision crossovers. This cheap preliminary stage prioritizes our measurements without artificially enlarging our supported claims. We constrain any claims to the specific design, workload, software, and physical regions where we calibrated the relationship, along with our explicit interpolation assumptions. Unobserved regions strictly remain unsupported.
...
We can actively plan around a known unknown, but a blind spot might only reveal itself when a downstream tool, an astute reviewer, or an unexpected real-world observation forces it into view. We must keep both categories highly visible in our representations. If we hide them, we risk having an automated method silently convert our missing information into conveniently assumed favorable values.
...
- **World-model state foundations.** World-model state vectors ($s = \langle s_{\text{design}}, s_{\text{env}} \rangle$) unify static design structures and dynamic operational environments, enabling multi-step agentic planning without physical state drift.
