...
This scarcity gradient is consistent with the error patterns reported for foundation models trained primarily on general web text, which can hallucinate non-existent SystemVerilog syntax and invalid AXI bus interfaces [@LiuEtAl2024CraftRTL]. Without specialized domain curation and machine-verifiable representations, raw generative capacity degrades rapidly when tasked with producing physically realizable hardware netlists.
...
The mechanism behind the divergence is what an architect should take away. A flat representation gives the compiler one global scope, so an analysis that must reason about every pair of nodes grows super-linearly with design size, and the curve therefore meets any fixed time limit at some node count. A hierarchical representation preserves module boundaries, so the compiler can work per unit and compose the results, which flattens the exponent. Neither claim depends on the numbers on the axes. If a team runs the sweep on its own designs and toolchain, the crossings will land elsewhere; the ordering of the exponents is the part we expect to transfer.
...
[^fn-pdk-proxy-drift-c04]: **Domain shift from uncalibrated proxy libraries**: Omitting foundry PDK pin capacitances and Liberty timing models due to NDA restrictions forces surrogate models to train on uncalibrated proxy cell libraries (such as Nangate45, an open-source 45nm standard cell library). This can induce severe domain shift when models transfer to commercial silicon nodes, where non-linear interconnect delays and pin loading dominate gate delays.
...
- **World-model state foundations.** World-model state vectors ($s = \langle s_{\text{design}}, s_{\text{env}} \rangle$) unify static design structures and dynamic operational environments, enabling multi-step agentic planning that stays free of physical state drift only while round-trip legalization checks hold.
