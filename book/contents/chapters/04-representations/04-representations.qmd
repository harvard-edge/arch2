...
A simple calculation illustrates how coverage requirements multiply acquisition costs. Assume a 1\ GHz target, a 1\ s measured interval, 0.2\ s of warmup, a simulator rate of 100,000 target cycles per wall-clock second, three repetitions, and four workloads.
...
When an agent executes an action $a$ (modifying cache capacity, adding an NPU tile, repartitioning clock domains), the world model evaluates the state transition $s' = T(s, a)$.
...
Representation pipelines must enforce boundaries between mutable parameters and nonnegotiable invariants. In the prospective Lighthouse cache study, L2 capacity serves as the sole mutable parameter. Evaluating 2\ MiB, 3\ MiB, and 4\ MiB candidates for a 3\ W mobile XR system-on-chip (SoC) (RISC-V RV64GCV core in TSMC N3 technology) requires verifying that every option resolves to a realizable SRAM macro organization while holding network-on-chip (NoC) topology, coherence policy, compiler, runtime, software image, and workload fixed under a 3\ W power envelope, $1.5\text{ mm}^2$ macro area, and 2.5\ ns access-time limit.
