...
These research frontiers address structural representation fidelity, physical constraint embedding, and provenance tracking:

### Correcting Discontinuous Proxy Error in Multi-Fidelity State

- **Recording structured proxy discrepancy histories:** What should a representation record about an analytical proxy's past mistakes so that autonomous search loops stop returning to ungrounded performance peaks? Existing acquisition functions assume uniform Gaussian error distributions across design spaces, ignoring sharp rank inversions between transaction-level models and cycle-accurate simulations. Tracking multi-fidelity discrepancies as first-class structured attributes within design state graphs is needed to prevent optimizers from fixating on false proxy optima.
- **Handling discontinuous error jumps at architectural boundaries:** How can a learned representation stay aligned with physical signoff when proxy error jumps discontinuously rather than drifting smoothly across architectural boundaries? Analytical surrogates assume smooth cost landscapes. When buffer queues saturate or memory bus arbiters switch into throttling modes, simulation error escalates abruptly. Embedding discrete structural saturation thresholds directly into latent hardware representations remains an open challenge.

### Enforcing Physical Legality in Continuous Planning Rollouts

- **Stepwise physical legalization in latent world models:** How can a learned world model be held strictly to physical legality at every planning step rather than only when a candidate exits the generative loop? Continuous latent embeddings enable efficient gradient-guided exploration, but multi-step rollouts easily drift into physically impossible states (such as illegal pin pitches, overlapping macro keep-outs, or unroutable clock topologies). Formulating differentiable, round-trip projection operators that map unconstrained latent vectors onto constructible hardware configurations at each rollout step is an active research goal.

### Provenance Tracking and Confidentiality in Shared Representations

- **Automated dependency-graph invalidation triggers:** Where should invalidation triggers attach within an architectural representation so that modifying an upstream microarchitectural parameter automatically purges stale simulation traces and physical signoff reports? Automated, hash-chained dependency graphs are required to eliminate split-brain causality, ensuring optimization loops never evaluate candidates against obsolete downstream metrics.
- **Privacy-preserving standard cell representations:** How can standard cell library embeddings be sanitized for cross-organizational foundation model pretraining without leaking confidential foundry process design rules? Formulating cryptographically audited, privacy-preserving feature abstractions would unlock collaborative open-source pretraining while protecting foundry intellectual property.
