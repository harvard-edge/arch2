...
The data availability gap across engineering domains spans roughly ten orders of magnitude, and the unit of data degrades along the way (@fig-architecture-data-scarcity). General natural language corpora reach training scales of roughly $1.5\times10^{13}$ tokens [@MetaAI2024Llama31; @TogetherAI2023RedPajama], and open-source software spans roughly $9\times10^{11}$ tokens [@LozhkovEtAl2024TheStackV2]. Synthesizable hardware stops being measured in tokens at all. The fully open OpenRTLSet collection yields roughly $1.3\times10^5$ Verilog modules [@WangEtAl2025OpenRTLSet], curated physical layout data provides on the order of $10^4$ samples [@ChaiEtAl2022CircuitNet], and expert-validated architecture question-answer pairs number in the low thousands [@PrakashEtAl2025QuArch].

This scarcity gradient explains why foundation models trained primarily on general web text hallucinate non-existent SystemVerilog syntax and invalid AXI bus interfaces. Without specialized domain curation and machine-verifiable representations, raw generative capacity degrades rapidly when tasked with producing physically realizable hardware netlists.

![**Training data volume spectrum across engineering domains.** Data availability collapses by roughly ten orders of magnitude from general web text ($\sim 10^{13}$ tokens) through open code ($\sim 10^{12}$ tokens) to synthesizable RTL modules ($\sim 10^5$), physical layouts ($\sim 10^4$), and expert-validated architecture QA pairs ($\sim 10^3$), with the unit of data degrading from raw tokens to curated samples along the way. Grouping training data into log-scale volume buckets highlights the data scarcity gap governing AI-native hardware design [@MetaAI2024Llama31; @TogetherAI2023RedPajama; @LozhkovEtAl2024TheStackV2; @WangEtAl2025OpenRTLSet; @ChaiEtAl2022CircuitNet; @PrakashEtAl2025QuArch].](images/fig-architecture-data-scarcity){#fig-architecture-data-scarcity width="88%" fig-alt="Log-scale horizontal bar chart of five corpus tiers: natural language web text near 10^13 tokens, general software code near 10^12 tokens, synthesizable hardware RTL near 10^5 modules, physical layout samples near 10^4 layouts, and curated architecture question-answer pairs near 10^3 pairs, showing a collapse of roughly ten orders of magnitude with the unit degrading from tokens to curated samples."}
...
We absolutely need negative records precisely because our successful designs represent mere survivor samples. What we preserve from a failed run depends on how it failed, and @tbl-negative-traces sets the preservation and consumption rules for seven failure categories. For example, when a synthesis or timing violation occurs, we preserve the candidate, constraints, process assumptions, and failing paths, ensuring we reconsider the candidate only after actively checking whether those exact conditions still apply. A robust record should always preserve exactly why a run failed, without inadvertently inflating a single localized result into a universal prohibition.
...
We can find a highly instructive representation example in the learned macro-placement work for chip floorplanning. The authors represented the chip using a graph structure for connectivity and a grid-like canvas for legal placement. When they made those choices, they successfully exposed relationships and spatial actions that flat RTL text simply could not [@MirhoseiniEtAl2021GraphPlacement; @ChengEtAl2023MacroPlacement]. Their result does not establish that any single placement method is universally superior. Instead, it demonstrates that our methods can perform meaningful searches only after we translate our architecture problems into forms that preserve the relationships relevant to the task. Their graph and canvas also remain derived views of the native placement state, rather than acting as independent sources of truth. To build truly usable architecture representations, we must keep those underlying links completely intact. We discuss the placement method and its objective further alongside optimization in @sec-methods-generation-prediction-optimization, as they belong there rather than in this chapter.
