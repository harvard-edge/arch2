...
Before formally defining an architectural observation, we must establish a whiteboard intuition for why hardware limits demand context. In software engineering, treating source code as flat text sequences is a convenient simplification because compilers parse text linearly and runtime exceptions fail gracefully in software. In hardware engineering, that simplification is fatal. Text tokens inherently lack the dimensionality to represent spatial routing congestion, physical wire delays, or clock skew across multiple timing domains. A hardware description language specifies concurrent spatial circuits, clock networks, reset trees, and physical interfaces. When an automated method consumes hardware netlists as flat token sequences, it remains blind to capacitive loading and clock-domain crossings.
...
The staggering difference in corpus volume between general web text and physical hardware layouts highlights the necessity of domain-curated, machine-checkable representations rather than ungrounded language tokens. While web text provides vast quantities of syntactic patterns, it completely lacks the explicit structural semantics required to synthesize functional digital logic or predict thermal limits. Hardware design operates under severe data scarcity, demanding physical verification over ungrounded language pretraining.
...
As detailed above, the structural unit of observation dictates the analytical boundaries of the dataset. A collection of Verilog modules provides excellent signals for syntax generation but offers zero visibility into dynamic workload performance or post-route timing closure. Conversely, a dataset of physical layouts captures congestion and IR-drop but obscures the high-level C++ algorithms that originally specified the hardware intent. This structural misalignment prevents direct substitution across design layers.

Therefore, an AI-native design methodology cannot rely on a single monolithic dataset to solve all architectural challenges. Instead, architects must carefully compose multiple specialized datasets, ensuring that the observation unit directly aligns with the target property being predicted or generated. When missing context is ignored, models confidently hallucinate designs that pass syntax checks but fail spectacularly in physical silicon.
...
Consequently, unsuccessful evaluations must be systematically preserved rather than silently discarded. The instinct to purge failed runs destroys the very boundary information required to train robust surrogate models. Maintaining an explicit record of what went wrong, downstream search algorithms can learn to navigate around physical cliffs rather than repeatedly driving over them. However, what is extracted from a failed run depends heavily on its exact failure mechanism (@tbl-negative-traces).
...
Classifying negative traces according to their root cause, data pipelines can distinguish between transient infrastructure faults and fundamental physical constraints. A timing violation imposes a permanent geometric boundary on the design space (unless the target clock frequency or operating voltage changes), whereas a simulator timeout may simply indicate an under-provisioned compute cluster. Preserving these exact failure classifications ensures that agentic loops avoid repeating known physical failures while preserving valid design directions.
...
As demonstrated, every representation format inevitably conceals critical dependencies, creating latent technical debt if those omissions are ignored. Adequate representations must therefore explicitly declare what they include, what they omit, and their precise trustworthiness boundaries. Without these declarations, automated optimizers will inevitably exploit unmodeled degrees of freedom, generating candidates that maximize a proxy metric while violating unstated physical constraints.

### Lighthouse Case Study and Representation Handoff

To ground these theoretical representation principles in a concrete, executable workflow, our prospective Lighthouse mobile XR case study establishes a rigorous handoff contract between represented state and downstream method selection. This contract formalizes the exact boundaries of the architectural exploration space. The handoff contract in @tbl-represented-problem binds the cache capacity question to versioned design state, workload snapshots, physical constraint sets, and verification logs, delivering a strictly defined problem to subsequent chapters without prescribing a specific search technique or claiming unverified results.
