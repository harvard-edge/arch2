...
::: {.column-margin}
**Author's Note.** Lisanne Bainbridge, an industrial psychologist who studied automated plant operations, observed that automation does not eliminate human operators; it leaves them with critical edge cases while decaying the skills disuse erodes. As generative agents handle register-transfer level (RTL) generation and simulation sweeps, our role as architects shifts from routine execution to cross-layer failure diagnosis, screening non-recurring engineering (NRE) capital risk, and named tapeout commitment.
:::
...
2. **Architects** integrate isolated results into a unified system design, frame cross-layer tradeoffs, and declare residual risk.
...
4. **Memory and data movement:** multi-level cache hierarchy ($32\ \text{KB}$ L1, $512\ \text{KB}$ L2, $4\ \text{MB}$ L3) and tightly coupled memory (TCM) scratchpads linked via Arm Microcontroller Bus Architecture (AMBA) AXI5 and UCIe chiplet interconnects.
...
As automated design systems shoulder substantial exploration, synthesis, and verification work, architectural ownership rests on explicit decision boundaries, post-silicon observability, and non-delegable commitment authority. What that governance can enforce remains unsettled.
...
**The principle:** While automated design systems execute exploration, optimization, and synthesis across the stack, human architects retain non-delegable accountability for problem framing, abstraction selection, residual risk declaration, and irreversible capital commitment.
