...
4. **Memory and data movement:** multi-level cache hierarchy (32\ KB L1, 512\ KB L2, 4\ MB L3) and tightly coupled memory (TCM) scratchpads linked via Arm Microcontroller Bus Architecture (AMBA) AXI5 and UCIe chiplet interconnects.
...
7. **Physical constraints:** TSMC N7 / 3\ nm-class LP mobile process node floorplanning, wire delays, design rule checks (DRCs), and routing congestion.
8. **Reliability and verification:** SystemVerilog assertions (SVA) evaluated via bounded model checking (BMC), formal equivalence checking, static timing analysis (STA) signoff, IP indemnification legal audits, and NRE capital allocation review.
...
Feasibility claims collapse when abstractions omit physical coupling. Reconciling a 120\ FPS XR rendering target with a 3\ W thermal envelope requires tracing how NPU DMA burst sizing affects LPDDR5 bus contention, host CPU interrupt latency, and package thermal gradients before committing to tapeout.
...
Abstraction choice dictates which components contribute to a design decision. Architects must determine what architectural state each predictor, generator, optimizer, or tool is allowed to modify and which observations constrain those changes. Aggregating optimized blocks does not yield a viable system if internal abstractions hide blocking physical effects such as PDN voltage droop or die-to-die thermal coupling.

AlphaChip illustrates this contrast in semiconductor macro placement [@MirhoseiniEtAl2021GraphPlacement]. An automated macro-placement tool can optimize an NPU floorplan locally, but its objective function omits power-delivery network (PDN) $IR$-drop transients that starve neighboring CPU cores or exceed local thermal dissipator limits across a 2.5D UCIe link. Architects define block boundaries, curate macro sets, establish timing and power constraints across host CPU and NPU cores, determine comparison baselines, and execute physical signoff checks.
