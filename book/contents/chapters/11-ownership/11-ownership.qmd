...
**Knowing when a design has left what the checks cover.** Verification suites encode what previous designs looked like, and automated search does not respect that history.

- **What would show that an automated loop has produced a design outside the envelope its verification suite was built for?** A testbench cannot report the cases it never anticipated, so an unusual candidate can pass everything and remain unexamined. Coverage measures that speak about novel structures rather than exercised lines would tell a review board when its evidence has stopped applying.

**Tracing a physical outcome back to the decision that caused it.** Thousands of automated transformations sit between architectural intent and a placed gate.

- **What keeps a chain of automated transformations traceable from architectural intent to physical result?** When a post-route violation appears, the responsible change may be a generated edit, a tool pass, or an engineering change order applied hours later, and reconstructing the path is currently manual. Provenance that survives iteration is what makes post-silicon diagnosis and accountability possible at all.

**Owning what cannot be opened.** Third-party IP and multi-die assemblies put critical behavior behind boundaries the integrating team cannot inspect.

- **What must a supplied component expose for an integrator to remain answerable for the system it goes into?** Vendors have sound reasons to withhold internals, and integrators still have to diagnose cross-boundary failures, so the open question is what minimum observable contract satisfies both. This is a technical question about interfaces and invariants rather than a contractual one.
- **What has to be designed into a multi-die package so that a post-silicon failure can be localized across die boundaries?** In 2.5D and 3D assemblies the failing die is often not the one showing symptoms, and the diagnostic structures needed to tell them apart must be committed to silicon long before anyone knows what will fail. Deciding what to instrument, under that uncertainty, is unresolved.
