...
Delegation requires satisfying four boundary conditions:
...
Consider the initial Lighthouse prompt: "Design a low-power, 64-bit RISC-V-based compute subsystem for a real-time mobile XR workload." This directive leaves representative workloads, software stacks, product boundaries, alternative design points, operating conditions, tradeoffs, and decision criteria unspecified. Framing this ambition across the eight prompt stack layers anchors the design space:
...
1. **Failure mode:** explicit architectural, logical, or physical mechanism that could fail.
...
5. **Containment action:** verified fallback path, throttling mechanism, or hardware disable fuse.
...
The engineering consequence is a capital governance obligation that automation cannot relieve. Even if assisted workflows compress front-end engineering effort, the fixed reticle mask charge falls due unchanged at tapeout. Cheaper candidate generation raises rather than lowers the evidentiary bar before authorizing a tapeout freeze for an RV64GCV host core and NPU accelerator.

Integrating generated RTL, Triton kernel passes, or foundation models introduces legal exposure: generative models can emit microarchitectural structures or HDL snippets that reproduce patented designs without contractual vendor warranty. Commercial IP blocks typically carry indemnification protecting against infringement litigation; generated code carries no comparable warranty absent negotiated terms.

Architects must enforce IP provenance tracking, independent structural-similarity screening against known IP, physical layout and static timing signoff, and legal indemnification review before committing generated RTL to tapeout.
...
Establishing architectural ownership across automated workflows presents open research challenges:

**Knowing when automated design leaves the verification envelope.** Automated search loops synthesize novel state transitions at scale, while testbenches evaluate only anticipated behaviors.
- **Which statistical representations over execution graphs can measure when generative search produces candidates outside testbench coverage?** Line and toggle metrics measure exercised structures but fail to capture temporal interactions across processing units and memory engines.
- **How can graph representations preserve structural lineage from high-level specifications down to physical edits across automated optimization loops?** Flattened hierarchies and renamed registers obscure design intent during late engineering change orders, complicating post-route debugging.

**Bounding cumulative waivers before exceptions become defects.** Schedule pressure encourages localized waivers across timing, layout, and power domains.
- **At what quantitative threshold should accumulating constraint waivers trigger a mandatory tapeout halt before exceptions become defects?** Formal models mapping aggregate waiver density across shared clock and power domains to chip failure probabilities remain an open requirement.

**Isolating failures across opaque component and package boundaries.** Multi-die assemblies and third-party IP obscure internal state.
- **What minimal telemetry contract must third-party IP export to enable fault isolation without exposing proprietary implementation details?** Observable telemetry must expose boundary health without revealing proprietary microarchitectural structures.
- **Where must on-chip diagnostic infrastructure be embedded across multi-die interfaces to uniquely attribute post-silicon failures to a specific die?** Dynamic cross-die coupling errors require placing diagnostic monitors under tight area and power budgets prior to silicon fabrication.

## Summary

Architectural ownership in an automated design era rests on cross-layer technical synthesis, explicit decision boundaries, and non-delegable commitment authority. While automated search loops accelerate microarchitectural exploration, layout, and verification, architects retain responsibility for defining system problems across the prompt stack, selecting abstractions, resolving conflicting specialist results, and declaring residual risk.
...
Sustaining technical ownership protects individual projects; scaling these capabilities across the discipline requires open standards and shared infrastructure. In @sec-bootstrapping-ecosystem, we examine how open schemas, accessible physical-design toolchains, neutral consortium benchmarks, and grand challenges build the auditable ecosystem needed for autonomous hardware design.
