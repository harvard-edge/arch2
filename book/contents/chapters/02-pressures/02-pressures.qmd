...
Historically, computer architecture managed rising complexity by elevating the level of abstraction used to describe designs while relying on lower-level checks to reject choices that fail during implementation. When abstractions were successfully raised (from polygon drafting to register-transfer level (RTL), or from ad hoc pipelines to formal instruction set architectures), the transition worked because the new abstraction was coupled to authoritative downstream enforcement. A generated RTL artifact is only useful because synthesis tools and static timing analysis can mathematically prove it works before it is manufactured.

However, as domain specialization tightly couples software mapping down to physical placement, manual loops strain under the volume of interacting constraints. Architectural state remains distributed across Synopsys Design Constraints (SDC) timing files, Unified Power Format (UPF) power intent, RTL configurations, and simulation benchmarks. The underlying technology dividend has changed, forcing architects to rely on specialization rather than free scaling.
...
The physical and economic severity of this transition becomes visible when tracking the underlying manufacturing fundamentals. Starting precisely at the 28nm node, the traditional engine of Moore's Law breaks. As shown in @fig-moores-law-breakdown, foundry capital expenditure (CapEx) skyrockets exponentially to fund extreme ultraviolet (EUV) lithography. During this same inflection point, the cost per billion yielded transistors simultaneously stops falling and begins rising. This inversion upends the classical scaling contract where each successive node historically delivered cheaper transistors, forcing architects to justify advanced node adoption through strict performance criteria rather than relying on automatic economic amortization.

![**The breakdown of Moore's Law scaling.** Foundry capital expenditure skyrockets exponentially to fund extreme ultraviolet lithography, while the cost per billion yielded transistors simultaneously stops falling and begins rising starting at the 28nm node.](images/plot_ch2.png){#fig-moores-law-breakdown width="100%" fig-alt="Plot showing foundry capital expenditure and cost per billion yielded transistors rising starting at the 28nm node."}
...
Missing or stale context acts as an admission control failure: invalid candidates consume the very simulator licenses and queue slots that would have evaluated valid candidates, triggering resource starvation. A local bottleneck of this kind cannot be broken by generating candidates faster; the repair is to the information and the decision boundaries that let bad data circulate.
