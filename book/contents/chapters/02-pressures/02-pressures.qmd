...
In @fig-waterbed-effect, we illustrate this proxy-mismatch failure using a stacked bar chart that compares total system energy between a balanced baseline design and an unoptimized accelerator candidate. The horizontal axis pairs the two design options, while the vertical axis measures total system energy broken into four stacked components ($E_{\mathrm{compute}}$, $E_{\mathrm{memory}}$, $E_{\mathrm{interconnect}}$, and combined control and leakage). Examining the stacked bars reveals how the candidate successfully shrinks compute energy ($E_{\mathrm{compute}}$), yet its added memory accesses and communication overhead cause memory and interconnect energy ($E_{\mathrm{memory}} + E_{\mathrm{interconnect}}$) to swell dramatically. This trade-off demonstrates the architectural takeaway that reducing compute energy in isolation can increase data movement and interconnect energy, inflating total system energy and causing partial evaluation metrics to rank an inferior design first.
...
In @fig-scissors-gap, the horizontal axis measures expanding design scope and result volume, while the vertical axis tracks relative work and capacity. Two lines diverge: work awaiting evaluation and review climbs steadily, while tool and reviewer capacity plateaus. On the left side of the plot, capacity comfortably absorbs incoming proposals. But as design scope expands, the lines cross, and beyond that intersection unresolved proposals accumulate in the widening region of unsettled work. This model articulates our core architectural takeaway that increasing candidate generation speed without scaling evaluation capacity creates phantom progress, filling queues with unverified designs rather than delivering tapeout-ready decisions.
