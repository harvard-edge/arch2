...
What matters in such a study is how the comparison is bounded. Defining workloads, run rules, and reporting conventions allows architects to compare different candidates fairly under clearly stated conditions. However, what makes an architectural comparison sound is not entirely written down; engineering teams carry much of this context implicitly, including excluded workloads, trusted tool configurations, failure post-mortems, and the specific decision under review. As design programs scale, synchronizing this operational context across larger teams and complex toolchains becomes increasingly difficult.
...
| **Power Density ($P / A$)** | $\mathbf{1}$ *(Constant)* | $\propto \kappa^2$ *(Surging)* | **Dark silicon and thermal limits** |
...
Because architects could no longer extract performance simply by scaling general-purpose clock rates or complex microarchitectures, the semiconductor industry pivoted toward massive data parallelism and domain-specific acceleration.
...
When single-thread performance scaling flattened and power constraints curtailed frequency gains, architects sought efficiency by restructuring how machines execute work across multiple physical scales. Rather than relying on a single dominant paradigm, modern systems compose a spectrum of architectural mechanisms: offloading compute to bespoke domain-specific accelerators, partitioning functions across multi-die chiplet packages, and weaving entire wafers into unified compute fabrics. Each step along this compositional spectrum alters the nature of architectural trade-offs. What begins as a local optimization rapidly escalates into package-level thermal coupling, inter-chiplet routing congestion, and system-wide verification liabilities. Navigating this landscape requires balancing these coupled design regimes, beginning with the choice of what to accelerate.

### Specialization Traps and Amdahl's Law

When those general-purpose microarchitectural levers exhaust their physical headroom, the next step is specialization. Specialization spans a broad spectrum rather than a binary leap from a general-purpose CPU to a custom accelerator. Designers can integrate vector and matrix units into general-purpose cores, assign data-parallel workloads to GPUs, construct domain-specific accelerators with custom dataflows, or eliminate software flexibility entirely with fixed-function application-specific integrated circuits (ASICs). Moving along this spectrum trades the breadth of workloads and software paths the hardware can run for raw efficiency.
...
The architectural diversity shown in @fig-ch02-accelerator-proliferation-mosaic demonstrates that there is no universal accelerator design. Instead, specialization binds the value of hardware directly to the efficiency of a targeted workload. Before carving out die area for any of these specialized blocks, architects must confront Amdahl's law (@eq-amdahl-communication) and determine what fraction $f$ of the deployed workload is actually accelerated by the new hardware.
...
4. **Silicon reliability and aging:** Soft errors, process variation [@Borkar2005ReliableSystems], and silicon aging (negative-bias temperature instability [NBTI] and hot-carrier injection [HCI]) constrain long-term operating margins in demanding domains like automotive systems governed by International Organization for Standardization (ISO) 26262 [@ISO2018Iso26262].
...
The escalating checking, simulation, and verification bottlenecks detailed above translate directly into a steep economic penalty (@fig-ch02-foundry-cost-inversion-and-rd-wall). Leading-edge design costs have surged past half a billion dollars per chip, driven by a dual economic squeeze: 300\ mm wafer manufacturing prices have surged $16.2\times$ from $\$1{,}850$ at $90\ \text{nm}$ to $\$30{,}000$ at $2\ \text{nm}$ (driven by the exponential capital cost of extreme ultraviolet lithography systems and the hundreds of masking steps required to pattern nanoscale FinFET and Gate-All-Around structures) while cost-per-transistor reductions have stalled (@fig-ch02-foundry-cost-inversion-and-rd-wall a), and corporate annual research and development (R&D) investments have climbed an exponential wall across audited SEC 10-K filings (with NVIDIA annual R&D expanding $230\times$ from $\$81.6\text{M}$ in FY2001 to $\$18.50\text{B}$ in FY2026) (@fig-ch02-foundry-cost-inversion-and-rd-wall b). However, the true crisis in computer architecture is not merely that design costs are rising, but that the economic model for amortizing those costs is fracturing.
