...
**The Hardware Trap:** When generative models drop the marginal cost of authoring RTL from weeks ($10^5\ \text{s}$) to seconds ($10^1\ \text{s}$), teams expect overall compute consumption to drop. Instead, autonomous search loops scale candidate generation by $10^4\times$, flooding simulation clusters and commercial EDA license pools. Because physical verification (timing closure, design rule check (DRC) and layout versus schematic (LVS) verification, gate simulation) still requires ground-truth physics, total verification compute explodes.
