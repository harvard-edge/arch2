...
Roughly 180 publicly announced AI accelerators from the MIT Lincoln Laboratory survey make this expansive design space concrete (@fig-accelerator-landscape) [@ReutherEtAl2025LAICS]. The horizontal logarithmic axis spans peak power across seven orders of magnitude (from sub-watt embedded chips to multi-kilowatt data-center clusters), while the vertical logarithmic axis plots peak compute performance across six orders of magnitude (from giga-operations to hundred-peta-operations per second). Color distinguishes form factors (purple chips, blue cards, and gold systems), while marker fill separates training architectures (solid) from inference engines (hollow). Dashed diagonal lines mark constant energy efficiency contours from 100\ GOps/W to 100\ TOPS/W.
