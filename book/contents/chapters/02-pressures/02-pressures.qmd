...
## The Rising Cost of Each New Design {#sec-design-cost-pressure}

Those tapeout decisions carry a price that has moved by an order of magnitude in under two decades. Public estimates put a new chip on a 65\ nm node at roughly 30 million dollars in 2006 [@SemiconductorIndustryAssociation2026ChipDesign]. At 10\ nm the figure reaches about 175 million dollars and at 7\ nm about 300 million dollars, counting design through IP qualification [@BauerEtAl2020SemiconductorDesignManufacturing]. By 5\ nm the same accounting reports over 540 million dollars, and an IBS estimate carried in a public filing puts a from-scratch 2\ nm design at about 725 million dollars [@ArmHoldings2023Prospectus].

We should hold those numbers loosely. Published estimates for a single node disagree, and the same filing that reports 725 million dollars at 2\ nm reports 249 million at 7\ nm where the design-through-qualification accounting reports 300 million. The sources declare different boundaries, some report a complex part built without reusable IP, and the widely cited series has been revised substantially between editions. The direction is not in question and the magnitude is. That is itself worth noticing in a discipline that intends to claim cost reductions, because a claim of reduced total cost needs a baseline someone can defend.

Where the money goes matters more here than the total. The same IBS breakdown attributes about 71 percent of that 2\ nm design cost to software development, verification, and IP qualification, leaving roughly 29 percent for the rest of design [@ArmHoldings2023Prospectus]. The dominant term is the checking work, not the drafting work, which is consistent with the verification pressures described above and which @sec-feedback-verification-trust takes up directly. Fixed physical capital such as mask sets and shuttle runs sits outside that share and no change of method reduces it. Which portion a better design process could compress is an open question that @sec-bootstrapping-ecosystem returns to.

What absorbs the cost is the harder half of the problem. Specialization, which earlier sections presented as the answer to a fading technology dividend, changes the denominator. A domain-specific accelerator earns its efficiency by serving a narrower workload, which means more distinct designs, each shipping into a smaller volume. Chiplet composition fragments the same way, turning one part into several with their own interfaces and qualification work. Cost per design rises while volume per design falls, and both move against us.
...
This turns a question about methods into a question about economics. A design method that produces better results at the same cost per decision leaves the constraint exactly where it was. For the parts that need specialization most, what matters is whether we can reach a supported decision at a cost the product can carry.
