...
Specialization forces concrete microarchitectural and system decisions: selecting vector lengths, sizing local scratchpads, choosing dataflow and precision formats, and managing runtime scheduling under thermal throttling. This specialization drive has produced a Cambrian explosion of commercial AI silicon across more than fourteen distinct architectural families (@fig-ch02-accelerator-proliferation-mosaic). From hyperscale graphics processing units (GPUs) and systolic tensor arrays to wafer-scale engines, reconfigurable dataflow grids, streaming language processing units (LPUs), and hardwired Transformer ASICs, each design optimizes a different trade-off between peak arithmetic density, memory bandwidth, and software flexibility. However, each integrated domain-specific architecture (DSA) introduces its own programming model, instruction set architecture (ISA) extensions, and toolchain maintenance burdens. In practice, systems offload dense compute to DSAs while retaining general-purpose cores for control flow and irregular tasks.
...
Specialization binds the value of hardware directly to the efficiency of a targeted workload. Before carving out die area for a specialized block, architects must confront Amdahl's law (@eq-amdahl-communication) and determine what fraction $f$ of the deployed workload is actually accelerated.
...
#| fig-cap: |
#|   **Reported design cost for a new chip rises roughly twenty-fourfold from 65 nm to 2 nm.**
#|   Estimated design costs scale from approximately \$30M at 65 nm (2006) to \$175M at 10 nm, \$300M at 7 nm,
#|   \$540M at 5 nm (2020), and \$725M at 2 nm (2024). The 65 nm and 5 nm anchors come from an industry association summary
#|   [@SemiconductorIndustryAssociation2026ChipDesign], the 10 nm, 7 nm, and 5 nm figures derive from McKinsey
#|   [@BauerEtAl2020SemiconductorDesignManufacturing], and the 2 nm breakdown comes from International Business Strategies
...
The escalating checking, simulation, and verification bottlenecks detailed above translate directly into a steep economic penalty (@fig-ch02-foundry-cost-inversion-and-rd-wall). Leading-edge design costs have surged past half a billion dollars per chip, driven by a dual economic squeeze: 300\ mm wafer manufacturing prices have surged $16.2\times$ from $\$1{,}850$ at 90\ nm to $\$30{,}000$ at 2\ nm (driven by the exponential capital cost of Extreme Ultraviolet (EUV) lithography systems and the hundreds of masking steps required to pattern nanoscale FinFET and Gate-All-Around structures) while cost-per-transistor reductions have stalled (@fig-ch02-foundry-cost-inversion-and-rd-wall a), and corporate annual research and development (R&D) investments have climbed an exponential wall across audited SEC 10-K filings (with NVIDIA annual R&D expanding $230\times$ from $\$81.6\text{M}$ in FY2001 to $\$18.50\text{B}$ in FY2026) (@fig-ch02-foundry-cost-inversion-and-rd-wall b). However, the true crisis in computer architecture is not merely that design costs are rising, but that the economic model for amortizing those costs is fracturing.

![**Foundry wafer fabrication cost escalation and the corporate semiconductor R&D wall.** (a) 300\ mm processed wafer prices surging $16.2\times$ from $\$1{,}850$ (90\ nm) to $\$30{,}000$ (2\ nm GAAFET), causing cost-per-transistor scaling to stall beyond 7\ nm [@SemiconductorIndustryAssociation2026ChipDesign; @Shilov2023IBSDesignCost]. (b) Annual R&D investments across audited SEC 10-K filings (2000–2026), showing aggregate semiconductor R&D escalating from $\$4.89\text{B}$ to $\$56.87\text{B}$ (led by NVIDIA's $230\times$ surge to $\$18.50\text{B}$) to support advanced-node physical closure and software stacks.](images/fig-ch02-foundry-cost-inversion-and-rd-wall){#fig-ch02-foundry-cost-inversion-and-rd-wall width="100%" fig-alt="Two-panel economic chart. (a) Wafer prices rising from $1,850 at 90nm to $30,000 at 2nm alongside logic transistor density and the cost per 100M transistors stalling at 7nm and 5nm. (b) SEC 10-K annual R&D expenditures from 2000 to 2026 across NVIDIA, Intel, Broadcom, Qualcomm, AMD, and Arm, highlighting aggregate industry R&D surging from $4.9B to over $56.8B and NVIDIA expanding from $82M to $18.5B."}
...
This dynamic updates the classic *design productivity gap* identified in the 2001 ITRS roadmap [@ITRS2001Design] and reiterated in the Semiconductor Research Corporation (SRC) and Semiconductor Industry Association (SIA) Decadal Plan [@SRCSIA2021DecadalPlan]. While earlier gaps were addressed by raising abstraction levels through RTL synthesis and IP reuse, today's bottleneck is not generating design artifacts, but verifying, simulating, and evaluating them under physical constraints.
