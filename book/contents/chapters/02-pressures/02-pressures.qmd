...
Consider data movement, for example, which can easily reverse an early ranking based purely on compute. As we move data through a memory hierarchy, we often consume far more energy than we do performing arithmetic operations. @Horowitz2014Energy provides widely used estimates that quantify this substantial difference. If we rely on a compute-only comparison, we risk ranking the wrong design first. Our results depend on where the data resides, how often it moves, who schedules it, what locality we can exploit, which precision we find acceptable, and what our software stack can express.

To quantify this physical reality, @fig-data-movement-energy-scale plots energy consumption per operation across arithmetic units and memory hierarchy levels using the 45\ nm reference values from @Horowitz2014Energy. The horizontal logarithmic axis measures energy in picojoules across four orders of magnitude ($0.1\,\text{pJ to }1\,\text{nJ}$). Rows progress top to bottom from baseline 32-bit integer addition ($0.1\,\text{pJ}$, $1\times$) through floating-point math, 8\ KB and 32\ KB SRAM cache reads ($10\text{--}20\,\text{pJ}$, $100\text{--}200\times$), 1\ MB on-chip SRAM accesses ($100\,\text{pJ}$, $1000\times$), and finally off-chip DRAM accesses ($1.3\text{--}2.6\,\text{nJ}$, $13{,}000\text{--}26{,}000\times$). Reading down the plot highlights our fundamental architectural takeaway that fetching data from off-chip DRAM consumes thousands of times more energy than performing arithmetic on that data, proving that any architectural comparison omitting memory hierarchy and locality produces invalid design rankings.
