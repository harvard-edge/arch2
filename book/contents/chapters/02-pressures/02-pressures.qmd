...
Carrying compact system intent through to physical silicon is increasingly unforgiving because architects can no longer reason about layers in isolation. When an architect resizes an on-chip cache or widens an issue queue, the decision is never an isolated microarchitectural adjustment. Doubling an L2 cache from 2\ MiB to 4\ MiB might appear to reduce main memory (DRAM) misses on paper. On physical silicon, however, that larger cache consumes more die area, adds parasitic wire delays (where resistance and capacitance dominate switching time) across lower metal layers, and significantly increases static leakage power. If the device operates in a passively cooled mobile enclosure, this unbudgeted thermal load triggers emergency clock throttling, completely erasing the expected latency gains. Because process scaling no longer delivers automatic frequency and efficiency gains, architects must extract performance through specialization, software co-adaptation, and system composition.
...
Cross-layer coupling forces architects to look beyond individual processing cores and restructure how machines execute work across physical scales. Rather than relying on a single dominant execution paradigm, modern systems compose a spectrum of architectural mechanisms, offloading compute to bespoke domain-specific accelerators, partitioning functions across multi-die chiplet packages, and scaling compute fabrics across entire wafers. Each step along this compositional spectrum alters the nature of architectural trade-offs. What begins as a local datapath optimization rapidly escalates into package-level thermal coupling, inter-chiplet routing congestion, and system-wide verification liabilities. Managing this scale requires balancing these coupled design regimes, beginning with the choice of what to accelerate.
...
::: {.callout-case-study title="Case Study: AMD Bulldozer and Speculative Throughput"}
**Design Trade-offs:** In the Bulldozer generation (Zambezi, 2011), AMD grouped two integer cores behind a shared front end and floating-point unit to push clock frequency [@BurgessEtAl2011Bulldozer]. When sharing these structures, the design traded single-thread performance for aggregate throughput.
**Workload Mismatch:** This approach created bottlenecks when threads competed for shared structures, and higher operating frequencies increased power consumption. For example, the FX-9590 required a 220\ W thermal design power [@AMDFXQuickReference].
**Resolution:** The subsequent Zen microarchitecture addressed this by returning the emphasis to per-core performance and efficiency [@AMDZen], demonstrating that candidates must be evaluated on metrics aligned with actual workload behavior.
:::
...
[^fn-eda-constraints-c02]: **EDA Constraint Formats**: SDC and UPF files are industry-standard electronic design automation (EDA) formats used to mathematically specify a design's timing requirements and power domains. They ensure that synthesized gates actually meet the clock speed and power limits intended by the architect.
