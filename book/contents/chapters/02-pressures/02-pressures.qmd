...
Product announcements reveal intervals between commercial releases, not the period during which an architecture decision remains exposed to software change. Public announcement intervals for client SoCs and data-center accelerators diverge substantially over time (@fig-silicon-cadence). In this sample, Apple client SoCs appear at 7- to 19-month intervals relative to November 2020, while NVIDIA data-center accelerators appear at 22- to 24-month intervals. These dates motivate the versioning question, but they do not reveal either company's internal design schedule or establish how long a hardware decision remained fixed.
...
Apple's announcement gaps shrink from 19 to 16 to 7 months, while NVIDIA's data-center GPU intervals remain near two years (22 and 24 months). We cannot use these announcement dates to bound a silicon program or to identify exactly when an architecture choice became too hard to change. They do not measure the pace of software change, rank release strategies, isolate any specific architecture contribution, or prove that AI tools caused a release to arrive sooner. Neither series establishes a definitive design-cycle mismatch. Together, they illustrate why our comparisons must record the actual hardware, workload, and software exposure, rather than attempting to infer these details from public release dates.
...
Energy consumption per operation varies by five orders of magnitude across arithmetic units and memory hierarchy levels (@fig-data-movement-energy-scale), based on 45\ nm reference values from @Horowitz2014Energy. A baseline 32-bit integer addition costs about 0.1\ pJ (1×); floating-point arithmetic runs 0.9\ pJ to 4\ pJ (9× to 40×); 8\ KB to 32\ KB SRAM cache reads run 10\ pJ to 20\ pJ (100× to 200×); a 1\ MB on-chip SRAM access costs 100\ pJ (1,000×); and off-chip DRAM accesses reach 1.3\ nJ to 2.6\ nJ (13,000× to 26,000×). Fetching data from off-chip DRAM thus consumes thousands of times more energy than performing arithmetic on that data, so an architectural comparison that omits memory hierarchy and locality can rank the wrong design first.
