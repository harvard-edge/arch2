...
Every row in @tbl-ai-contributions-by-bound is point assistance in the sense @sec-moonshot defined, a Layer 1 use that accelerates one task and leaves the workflow, the represented design state, and the authoritative checks where they were; a program often runs several such uses against different bottlenecks at once, and reorganizing the workflow around them, as the callout below explores, does not by itself make the result valid, since components fed flawed proxies or incomplete traces converge on an invalid optimum however the workflow is arranged.
...
[^fn-dark-silicon-c02]: **Dark Silicon**: The fraction of a chip that must be powered off or underclocked at any given time to stay within thermal design power limits.

[^fn-reticle-c02]: **Reticle Limit**: A physical maximum size limit imposed by the optics of standard lithography scanners (specifically a $26\text{ mm} \times 33\text{ mm}$ exposure field).
