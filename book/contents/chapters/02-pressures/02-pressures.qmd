...
Process technology remains fundamental, yet it no longer automatically converts density into the same broadly reusable combination of frequency, single-thread performance, and energy efficiency. For roughly fifty years, processor generations improved through a familiar progression. Transistor density continued to rise while voltage scaling enabled using more transistors without a proportional increase in power [@Moore1965Cramming; @DennardEtAl1974Scaling]. Under classical Dennard scaling, scaling lateral and vertical device dimensions by a factor $\kappa$ ($\kappa > 1$) reduced transistor gate area ($W \times L$) by $1/\kappa^2$. To maintain constant internal electric fields ($E = V/L$) and gate-oxide reliability, supply voltage $V_{dd}$ and threshold voltage $V_{th}$ were scaled down by $1/\kappa$, while substrate doping concentration $N_A$ increased by $\kappa$. Consequently, gate capacitance ($C_{gate} = \epsilon_{ox} W L / t_{ox}$) scaled down by $1/\kappa$, transistor drive current in saturation ($I_{on} \propto \frac{W}{L} C_{ox} (V_{dd} - V_{th})^2$) scaled by $1/\kappa$, and intrinsic gate delay ($\tau = C_{gate} V_{dd} / I_{on}$) dropped by $1/\kappa$, driving operating frequency $f$ up by $\kappa$. Because dynamic power per switching transistor ($P_{\text{dyn}} = C_{gate} \cdot V_{dd}^2 \cdot f$) scaled by $(1/\kappa) \cdot (1/\kappa^2) \cdot \kappa = 1/\kappa^2$, total chip power density ($P_{\text{density}} = P_{\text{dyn}} \times \text{Transistor Density} \propto (1/\kappa^2) \times \kappa^2$) remained remarkably constant across technology generations [@HennessyPatterson2017QuantitativeApproach].

However, this scaling contract fractured when supply voltage dropped near $0.7\text{--}0.9\,\text{V}$. The breakdown occurred because off-state subthreshold leakage current ($I_{\text{off}} = I_{\text{sub}}|_{V_{gs}=0}$) is governed by the Boltzmann tail of thermal carrier distribution (@eq-subthreshold-leakage):
$$
I_{\text{sub}} = \mu C_{ox} \left(\frac{W}{L}\right) V_T^2 \exp\left(\frac{V_{gs} - V_{th}}{n V_T}\right)
$${#eq-subthreshold-leakage}
where $V_T = \frac{k_B T}{q} \approx 26\,\text{mV}$ is the thermal voltage at $300\,\text{K}$, and $n = 1 + \frac{C_{\text{dep}}}{C_{ox}}$ is the subthreshold ideality factor. This physics dictates a fundamental lower bound on subthreshold swing ($S = n \ln(10) V_T \approx 60\text{--}80\,\text{mV/decade}$ at room temperature), requiring a fixed gate-to-source voltage swing to suppress drain current by one order of magnitude. As a result, reducing threshold voltage $V_{th}$ below $\sim 0.3\,\text{V}$ caused static leakage current ($I_{\text{off}} \propto 10^{-V_{th}/S}$) to rise exponentially, converting static dissipation into a thermal runaway hazard. When $V_{dd}$ scaling stagnated around 2005 to preserve $I_{\text{on}}/I_{\text{off}}$ switching ratios and noise margins, dynamic power ($P_{\text{dyn}} = C \cdot V_{dd}^2 \cdot f$) surged whenever frequency increased, forcing the industry to freeze clock rates and confront the dark silicon wall [@EsmaeilzadehEtAl2011DarkSilicon].
...
where $IC$ is instruction count, $CPI_{\mathrm{calc}}$ is cycles per instruction excluding memory stalls, $f$ is processor clock frequency, and $t_{\mathrm{DRAM}}$ is off-chip memory access latency. Historically, logic switching speed improved by $50\text{--}80\%$ annually during the RISC era, while DRAM access latency improved by only $\approx 7\%$ per year. As compute frequency $f$ scaled upward, computation time $T_{\mathrm{calc}}$ shrank asymptotically toward zero, leaving total execution time $T_{\mathrm{total}}$ completely dominated by the invariant memory stall floor $T_{\mathrm{mem}}$. In modern heterogeneous SoCs, this disparity elevates off-chip memory access into the primary latency and energy bottleneck.
