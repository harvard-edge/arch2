...
- **Diagnose** the limiting constraint in an architectural workflow, targeting point assistance (Layer 1), subsystem optimization (Layer 2), or an AI-native design system (Layer 3) without displacing authoritative checks.
:::
...
These historical shifts demonstrate that abstractions succeed only when coupled to authoritative downstream enforcement. Stable interface contracts (such as System/360 and CUDA [@NickollsEtAl2008CUDA]) decoupled software compilation from microarchitectural evolution, while explicit geometric and synthesis rules (Mead–Conway and logic synthesis) empowered automated tools to reject physically invalid candidate structures early in the design flow. Standardizing workload suites (SPEC) and accounting for compiler and VLSI costs (RISC) expanded the architectural scope under study while preserving rigorous empirical comparison.
...
To see why this breakdown forced the shift to specialization, consider the physics of the original scaling contract. Classical Dennard scaling relied on a balanced geometric contract where shrinking linear dimensions by a factor $\kappa > 1$ reduced transistor area by $1/\kappa^2$, gate capacitance by $1/\kappa$, and switching delay by $1/\kappa$. Scaling supply voltage $V_{dd}$ by $1/\kappa$ kept dynamic power per transistor scaling as $1/\kappa^2$, perfectly counteracting the $\kappa^2$ increase in transistor density and keeping power density constant ($P/A = 1$).

| Device Parameter | Classical Dennard Scaling ($\kappa > 1$) | Post-Dennard Reality ($\sim 2005\rightarrow\text{present}$) | Limiting Physical Constraint |
| :--- | :--- | :--- | :--- |
| **Transistor Area ($W \times L$)** | $1/\kappa^2$ | $1/\kappa^2$ | Gate pitch scaling continues |
| **Supply Voltage ($V_{dd}$)** | $1/\kappa$ | $\approx \text{Constant } (\ge 0.7\text{--}0.9\ \text{V})$ | Subthreshold slope $S \ge 60\ \text{mV/dec}$ |
| **Gate Capacitance ($C_{\text{gate}}$)** | $1/\kappa$ | $1/\kappa$ | Interconnect parasitic $RC$ dominates |
| **Operating Frequency ($f$)** | $\kappa$ | Stagnated ($\sim 3\text{--}5\ \text{GHz}$) | Thermal dissipation and leakage runaway |
| **Power Density ($P / A$)** | $1$ *(Constant)* | $\propto \kappa^2$ *(Surging)* | Dark silicon and thermal limits |
: **Classical Dennard scaling versus post-Dennard physical realities.** Shrinking linear dimensions by a factor $\kappa > 1$ historically kept power density constant while increasing operating frequency. When subthreshold leakage prevented further voltage scaling, power density began scaling with transistor density, forcing a transition to architectural specialization. {#tbl-dennard-scaling-comparison tbl-colwidths="[24,24,28,24]"}

This scaling contract fractured around 2005 when supply voltage hit the subthreshold leakage limit near $0.7\text{--}0.9\ \text{V}$. The breakdown occurred because off-state subthreshold leakage current ($I_{\text{off}} = I_{\text{sub}}|_{V_{gs}=0}$) is governed by the Boltzmann tail of the thermal carrier distribution (@eq-subthreshold-leakage).
$$
I_{\text{sub}} = \mu C_{ox} \left(\frac{W}{L}\right) V_T^2 \exp\left(\frac{V_{gs} - V_{th}}{n V_T}\right)
$${#eq-subthreshold-leakage}
where $V_T = \frac{k_B T}{q} \approx 26\ \text{mV}$ is the thermal voltage at $300\ \text{K}$, and $n = 1 + \frac{C_{\text{dep}}}{C_{ox}}$ is the subthreshold ideality factor. Because carrier energies follow Maxwell–Boltzmann statistics, the thermionic emission tail of electrons crossing the source-to-channel energy barrier cannot be modulated faster than the thermal voltage limit at room temperature.

This physics dictates a fundamental lower bound on subthreshold swing ($S = n \ln(10) V_T \approx 60\text{--}80\ \text{mV/decade}$ at $300\ \text{K}$), requiring at least $60\text{--}80\ \text{mV}$ of gate voltage swing to suppress drain current by a single order of magnitude. To achieve an acceptable $I_{\text{on}}/I_{\text{off}}$ switching ratio of $10^4$ to $10^5$, the threshold voltage $V_{th}$ must remain at least $300\text{--}400\ \text{mV}$. Lowering $V_{th}$ below this floor causes static leakage current ($I_{\text{off}} \propto 10^{-V_{th}/S}$) to rise exponentially, converting static dissipation into a thermal runaway hazard. When $V_{dd}$ scaling stalled to maintain adequate overdrive ($V_{dd} - V_{th}$) and noise margins, dynamic power density began scaling with transistor density ($\propto \kappa^2$), forcing the industry to freeze clock rates near $3\text{--}5\ \text{GHz}$ and confront the dark silicon wall [@EsmaeilzadehEtAl2011DarkSilicon].
...
This boundary highlights an essential distinction between data-center scaling and embedded edge systems. In distributed data-center training, architects often invoke Gustafson's law [@Gustafson1988AmdahlLaw], assuming problem size expands alongside available compute parallelism (weak scaling, where $S(N) = N - (1-f)(N-1)$). In large-batch model training, scaling parameter counts and dataset tokens amortizes host coordination and unaccelerated control overheads. However, in latency-critical and embedded real-time systems (such as the 3\ W mobile XR moonshot), Gustafson's weak-scaling assumption fails. The computation required per frame is bounded by sensory refresh rates, tracking pipelines, and rigid display deadlines ($8.33\ \text{ms}$ at 120\ frames per second [FPS]). Strong scaling under Amdahl's law strictly governs the delivered performance ceiling; every microsecond spent in unaccelerated serial dispatch, sensor interrupt handling, or host-to-device synchronization directly consumes the immutable frame budget. The residual work left behind on the general-purpose host core defines the ultimate ceiling of specialized gains. Therefore, an accelerator cannot be evaluated in isolation; architects must judge it alongside the compiler, runtime, memory traffic, and residual workload that collectively determine its true delivered benefit.

### Packaging and Chiplet Composition Risks

Specialized intellectual property (IP) blocks rarely ship in isolation. Design teams weave them into complex systems-on-chip (SoCs) and, increasingly, distribute them across multiple discrete chiplets. Manufacturing yield heavily drives this shift. Under classical random-defect models, die yield falls exponentially with die area [@Murphy1964CostSize], so fracturing a large monolithic die into smaller compute and I/O chiplets recovers yield. However, this strategy does not linearize total costs, since advanced packaging, chiplet reuse, and heterogeneous process nodes completely rewrite the underlying economic model [@FengMa2022ChipletActuary]. Beyond yield, chiplets allow teams to freely mix process nodes, third-party IP blocks, and specialized memory arrays on a single substrate. Open standards like Universal Chiplet Interconnect Express (UCIe), an open die-to-die interconnect standard [@UCIeConsortium2026Spec], stabilize the physical layer, with specification targets of subpicojoule energy efficiency ($<0.5\ \text{pJ/bit}$) and high shoreline bandwidth density ($>1.3\ \text{TB/s/mm}$) across micro-bump pitches ($25\text{--}55\,\mu\text{m}$). Yet they leave partitioning and implementation choices entirely to system architects.

Multi-chiplet packaging introduces physical and electrical sensitivities absent in single-die systems. Advanced packaging (such as 2.5D silicon interposers or 3D micro-bump arrays) alters latency, bandwidth, and energy budgets while introducing severe thermal coupling and rigid physical constraints. In industrial practice, SoC and multi-chiplet projects frequently integrate new specialized accelerators alongside legacy controllers and third-party proprietary physical-interface IP without full RTL visibility. For instance, Arm has supplied processor IP as fixed hard macros [@Arm2013CortexA15HardMacro], and Intel distributes encrypted RTL [@Intel2024EncryptedRTLIP], restricting access to internal state and forcing architects to rely on interface verification. Architects must also manage undocumented interface behaviors, conflicting reset sequences, and complex clock-domain crossing (CDC) constraints that resist simplified analytical modeling. Assembling the full system can uncover subtle coherence-protocol mismatches [@Arm2020AXI] or timing violations. Networks-on-chip (NoCs) can also experience deadlock if cyclic dependencies enter the channel-dependency graph.[^fn-soc-deadlocks-c02] These integration risks often remain undetected until late-stage verification or physical silicon bring-up.
...
Each additional IP block integrated into a design restricts the space of valid configurations. Partitioning a function across a chiplet boundary alters physical interfaces, shifts the software path, and requires different evaluation tools. Heterogeneous modules also compete for shared interconnect resources. The network-on-chip (NoC) must arbitrate between high-throughput bursty accelerator traffic and latency-sensitive real-time traffic under strict deadlines, where average bandwidth estimates can easily obscure microarchitectural contention that causes latency violations.

When automating exploration across multi-component systems, search algorithms encounter two primary failure modes. If the search remains unconstrained, algorithms waste compute evaluating invalid configurations that violate physical feasibility. Conversely, overly restrictive constraints risk excluding viable candidate architectures. Before executing a search algorithm, engineering teams must define valid transformations, fixed interfaces, legal configurations, and evaluation budgets. Evaluating a large candidate count alone does not resolve these critical questions. Across multi-vendor packages, formal specifications are often incomplete, as the Semiconductor Research Corporation's Microelectronics and Advanced Packaging Technologies (MAPT) roadmap notes that design automation for packaged multi-vendor systems is constrained by the absence of standardized data formats, syntax, and semantics across suppliers [@SemiconductorResearchCorporation2023MAPT]. Optimization algorithms cannot enforce an integration rule unless suppliers specify it in a machine-readable format.

System composition multiplies the physical and economic constraints candidate designs must satisfy. Long software bring-up sequences exceed the practical execution limits of detailed software simulators such as gem5 [@BinkertEtAl2011Gem5] or MacSIM (a heterogeneous many-core timing simulator), forcing teams to rely on hardware emulation or field-programmable gate array (FPGA) prototyping. Tools such as FireSim [@KarandikarEtAl2018FireSim] provide FPGA-accelerated full-system simulation to address this bottleneck. Yet each platform requires trade-offs between execution throughput, debug visibility, hardware cost, and environment setup complexity.

Beyond functional correctness, chiplet manufacturing, security, power integrity, and IP interfaces demand independent verification. In advanced 2.5D and 3D integration schemes, such as TSMC's CoWoS [@TSMCCoWoS] or Intel's Foveros [@IntelFoveros], discovering a defective die after packaging renders the entire assembly nonfunctional. While pre-bond screening mitigates yield losses, it cannot guarantee that every die survives the physical stresses of packaging and assembly. Consequently, architectural partitioning must incorporate adequate test access and support pre-bond screening (such as Institute of Electrical and Electronics Engineers (IEEE) 1838 test-access architecture) to establish known-good-die (KGD) status.[^fn-known-good-die-c02]
...
For a target such as Lighthouse mobile extended-reality (XR), the term *XR* remains far too broad to define a concrete workload, as it spans motion-to-photon latency ($<20\ \text{ms}$), sensor fusion pipelines (inertial measurement units and 4K cameras), neural rendering workloads, and a strict 3\ W thermal budget. Five workload archetypes make the ambiguity concrete: ResNet-50 vision, GPT-2/Llama language models, deep learning recommendation models, MobileNetV2 mobile perception, and always-on keyword spotting. Each archetype pulls a design toward fundamentally different memory, latency, and energy behavior. Even when employing a dedicated benchmark suite such as XRBench [@KwonEtAl2023XRBench], architects must still independently decide which specific traces, model versions, deadlines, input distributions, and quality targets matter for the target design.
...
2. **Multi-fidelity simulation throughput:** Cycle-accurate timing simulators, hardware emulators, and FPGA prototypes differ by orders of magnitude in execution speed and cost, strictly rationing how many cycles of realistic software execution can be simulated.
...
This decomposition in @eq-system-energy-decomposition directly exposes two persistent physical limits. First, the *memory wall* identified by @WulfMcKee1995HittingMemoryWall formalizes the structural divergence between microprocessor clock frequency and dynamic RAM (DRAM) access latency. Total execution time decomposes into computational core execution and memory stall latency (@eq-memory-wall-decomposition).
$$
T_{\mathrm{total}} = T_{\mathrm{calc}} + T_{\mathrm{mem}} = \frac{IC \cdot CPI_{\mathrm{calc}}}{f} + IC \cdot \left(\frac{\text{misses}}{\text{instruction}}\right) \cdot t_{\mathrm{DRAM}}
$${#eq-memory-wall-decomposition}
where $IC$ is instruction count, $CPI_{\mathrm{calc}}$ is cycles per instruction excluding memory stalls, $f$ is processor clock frequency, and $t_{\mathrm{DRAM}}$ is off-chip memory access latency. Historically, logic switching speed improved by 50% to 80% annually during the RISC era, while DRAM access latency improved by only about 7% per year. As compute frequency $f$ scaled upward, computation time $T_{\mathrm{calc}}$ shrank asymptotically toward zero, leaving total execution time $T_{\mathrm{total}}$ completely dominated by the invariant memory stall floor $T_{\mathrm{mem}}$. In modern heterogeneous SoCs, this disparity turns off-chip memory access into the primary latency and energy bottleneck.
...
The escalating checking, simulation, and verification bottlenecks detailed above translate directly into a steep economic penalty (@fig-ch02-foundry-cost-inversion-and-rd-wall). Leading-edge design costs have surged past half a billion dollars per chip, driven by a dual economic squeeze. First, 300\ mm processed wafer manufacturing prices have surged $16.2\times$ from $\$1{,}850$ at $90\ \text{nm}$ to $\$30{,}000$ at $2\ \text{nm}$ (driven by the exponential capital cost of extreme ultraviolet lithography systems and the hundreds of masking steps required to pattern nanoscale FinFET and Gate-All-Around structures), causing cost-per-transistor scaling to stall (@fig-ch02-foundry-cost-inversion-and-rd-wall a). Second, corporate annual research and development (R&D) investments have climbed an exponential wall across audited SEC 10-K filings, exemplified by NVIDIA's annual R&D expanding $230\times$ from \$81.6M in FY2001 to \$18.50B in FY2026 (@fig-ch02-foundry-cost-inversion-and-rd-wall b). However, the true crisis in computer architecture is not merely that design costs are rising, but that the economic model for amortizing those costs is fracturing.
...
In the general-purpose computing era, massive non-recurring engineering (NRE) investments were amortized across hundreds of millions of identical CPU dies sold over many years. Domain specialization completely inverts this economic equation. Because specialized accelerators achieve their efficiency by tailoring hardware to narrow workload niches, each design targets a significantly smaller market volume. Chiplet composition further fragments production volumes across multiple specialized dies. When cost per design rises eighteenfold while production volume per design drops by orders of magnitude, traditional design methodologies become economically unsustainable.

Embedded and edge products sit at the sharpest point of this economic dilemma. Devices such as the Lighthouse XR headset must deliver server-class neural rendering within a strict 3\ W thermal envelope, demanding extreme hardware specialization. Yet edge products cannot command the premium pricing or amortize the massive verification teams of hyperscale data-center accelerators. If an AI-native design methodology merely proposes higher-performing architectures while driving up verification and evaluation costs, it fails the economic test. An AI-native architecture workflow succeeds only if it reaches defensible, silicon-ready decisions within an engineering budget that specialized production volumes can support.
...
**The Hardware Trap:** When generative models drop the marginal cost of authoring RTL from weeks ($10^5\ \text{s}$) to seconds ($10^1\ \text{s}$), teams expect overall compute consumption to drop. In economic terms, candidate proposal authoring and downstream verification compute act as strong Edgeworth complements; reducing the marginal cost of candidate proposals drastically inflates aggregate demand for downstream verification compute and EDA license pools. Autonomous search loops scale candidate generation by $10^4\times$, flooding simulation clusters and commercial EDA license queues. Because physical verification (timing closure, design rule check (DRC) and layout versus schematic (LVS) verification, gate simulation) still requires ground-truth physics, total verification compute explodes.

**The Architectural Antidote:** Teams must implement admission backpressure and multi-tier filtering to size candidate arrival rates to match physical verification capacity (see @sec-law-jevons-paradox in @sec-appendix-c-laws-paradoxes for the mathematical candidate arrival scaling proof and EDA license exhaustion models).
:::
...
Emerging research illustrates how point assistance targets specific bottlenecks. Domain-adapted large language models (LLMs) such as ChipNeMo [@LiuEtAl2023ChipNeMo] and ChatEDA [@HeEtAl2024ChatEDA] retrieve collateral and automate EDA scripts; learned surrogate models such as PowerNet [@XieEtAl2020PowerNet] and predictive DSE [@IpekEtAl2006PredictiveDSE; @LeeBrooks2006RegressionModeling] screen candidates prior to detailed simulation; reinforcement learning guides high-dimensional architectural search (Apollo [@yazdanbakhsh2021apollo], DeepRM [@MaoEtAl2016DeepRM]); and generative models draft initial verification testbenches (AutoBench [@QiuEtAl2024AutoBench]).
...
Those pressures make AI assistance worth investigating, once a program measures which work actually delays its next decision rather than assuming a universal bottleneck. Specialization sharpens that measurement, because it raises the cost of each design while shrinking the volume that absorbs it, so what matters is whether a supported decision stays reachable at a cost the product can carry. Retrieval can assemble scattered context, predictors can ration expensive evaluation, generators can draft artifacts, optimizers can search large spaces, and tool-using systems can connect proposals to existing engineering checks. Point assistance applies one of those roles inside a workflow that otherwise stays the same (Layer 1), while subsystem optimizers conduct deep search inside a fixed container (Layer 2). An AI-native design system organizes the workflow, the represented state, and the verification checks across the complete vertical stack (Layer 3). None of these roles is universally helpful, and none makes its own output trustworthy. The compounding physical, architectural, and tool pressures establish the operational boundaries for automated design assistance.
...
Architectural evaluation is defensible only when bounded by the complete system workflow. Any hardware claim remains tied to its workload traces, software runtime, physical implementation corners, and verification coverage. Point assistance earns its place when a local intervention improves that complete comparison without shifting cost downstream or weakening a necessary check. A complete AI-native design system faces the same test across the broader workflow. @sec-architecture-20-ontology introduces the six-responsibility design life cycle that organizes AI-native design work.
