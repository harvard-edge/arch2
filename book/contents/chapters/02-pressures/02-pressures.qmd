...
A design path carries a question through candidate construction, tool execution, and comparison. For the cache example, that path might compare two capacities under one workload and inspect their latency and energy estimates. It becomes a design loop when those returns update the represented design state and trigger another permitted action: a disappointing energy estimate might eliminate the larger cache from consideration and lead to a smaller capacity increment or a closer examination of memory traffic. Repeating a fixed sweep without using its returns to choose what happens next remains a path. The distinction matters because a loop can spend additional work responding to what it learns, and each such response draws on finite tool and review capacity.
...
Over that same fourteen-year window, off-chip memory bandwidth failed to scale at the pace of compute, growing only thirty-twofold from $250\ \text{GB/s}$ in Graphics Double Data Rate 5 (GDDR5) to $8.0\ \text{TB/s}$ in High Bandwidth Memory (HBM3e) (@fig-ch02-accelerator-scaling-frontier b). To understand the consequence, define a workload's operational intensity as the operations performed per byte transferred across a specified memory boundary. At the off-chip boundary, reusing data on chip raises operational intensity because more operations share each byte fetched. The hardware's bandwidth divided by peak compute rate gives the complementary ratio in bytes per operation; its reciprocal is the intensity needed to feed peak arithmetic throughput, assuming the advertised bandwidth can be sustained. Across the displayed parts, the bandwidth-to-compute ratio falls from $0.064\ \text{Bytes/FLOP}$ down to $0.003\ \text{Bytes/FLOP}$. Modern accelerators now require operational intensities exceeding $300\text{ to }560\ \text{FLOPs/Byte}$ to keep their arithmetic units saturated, creating an *operational intensity wall* where memory-bound kernels and autoregressive token decoding stall on High Bandwidth Memory (HBM) transfers. In response, architectures equipped with massive on-chip static random-access memory (SRAM) arrays (such as Groq TSP sustaining $0.43\ \text{Bytes/FLOP}$ and Cerebras Wafer-Scale Engine (WSE) sustaining $0.17\ \text{Bytes/FLOP}$) trade total parameter capacity for ultra-high memory bandwidth [@AbtsEtAl2020GroqTSP; @ReutherEtAl2025LAICS], defining the central physical trade-off in specialized silicon.
...
The physical comparison in @fig-cerebras-wse-ch02 makes the area cost of the wafer-scale approach visible. Extending the design across a wafer creates room for distributed SRAM and compute, while moving communication, defect tolerance, and power delivery into the same architectural comparison.

::: {#fig-cerebras-wse-ch02}
![Cerebras WSE physical size comparison](images/cerebras_wse_scale.jpg)
...
The interconnect and invocation path together set $o$, $B$, and the regime. Where inside that regime a workload actually lands is decided by the compiler and runtime rather than the link, which is why the bound this section prices returns below as a software problem once the hardware is composed.
...
[^fn-eda-constraints-c02]: **EDA Constraint Formats**: SDC and UPF files are industry-standard electronic design automation (EDA) formats used to specify a design's timing requirements and power domains. Implementation and verification tools use those declarations to check whether the design meets the architect's intended constraints.
