...
Those tapeout decisions carry a price that has moved by an order of magnitude in under two decades. Public estimates put a new chip on a 65\ nm node at roughly 30 million dollars in 2006 [@SemiconductorIndustryAssociation2026ChipDesign]. At 10\ nm the figure reaches about 175 million dollars, and at 7\ nm about 300 million dollars when the accounting runs from design through IP qualification [@BauerEtAl2020SemiconductorDesignManufacturing]. A separate 7\ nm estimate of about 249 million dollars appears in a public filing, sourced to IBS [@ArmHoldings2023Prospectus]. Those two 7\ nm figures differ by roughly a fifth, and that gap is worth keeping in view. The sources do not declare the same accounting boundary, so we should read these as order-of-magnitude anchors on a trend rather than as a series we can subtract.

The total is not one line item, and the composition matters for what follows. It spans architecture and design labor, the verification and signoff effort that @sec-feedback-verification-trust examines directly, and fixed physical capital such as mask sets and shuttle runs that no change of method reduces. Which portion a better design process could compress remains an open question, and @sec-bootstrapping-ecosystem returns to it. The narrower point here is that reaching a supported architecture decision now costs more, and that cost lands before any product ships.
