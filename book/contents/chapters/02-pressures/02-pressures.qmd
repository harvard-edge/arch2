...
With a placed netlist and current profile, fast learned $IR$-drop estimation [@XieEtAl2020PowerNet] and authoritative thermal signoff can confirm or clear that risk. Thermal design power (TDP), which serves as a target for sizing the cooling solution rather than a strict measure of sustained power draw, is not the sole limit [@Intel2014QuarkThermalGuide]. For instance, rapid clock-gating transitions or 512-bit single instruction, multiple data (SIMD) vector lane wakeups induce severe step-current transients ($di/dt$), causing localized $L \cdot \frac{di}{dt}$ voltage droops on the power grid that force conservative frequency guardbands or dedicated emergency voltage mitigation (EVM) circuits. Employing early physical estimates, workload-aware power models, and closer coordination between microarchitecture and physical design exposes these subtle risks long before final signoff.
