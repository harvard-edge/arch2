...
While product announcements reveal the interval between commercial releases, they do not show the actual period during which an architecture decision remains exposed to software change. Release intervals for two contrasting product categories, placed on a timeline relative to November 2020, expose that gap (@fig-silicon-cadence). Consumer mobile SoCs (Apple's M-series) iterate rapidly, on 7- to 19-month gaps, whereas enterprise data-center GPUs (NVIDIA's accelerators) maintain longer 22- to 24-month cycles. In both cases, hardware design choices remain frozen for years while compiler passes, runtime libraries, and AI model architectures evolve continuously around them.
...
The toy count sits in the thousands, MAESTRO and AutoTVM cluster near $10^9$, and Timeloop's computed lower bound lies almost ten orders of magnitude beyond them (@fig-search-vs-eval-gap). Yet this near-fifteen-order span encodes only candidate count; it tells us nothing about the cost, fidelity, or decision strength of a single evaluation. As architects, we must decide which choices are truly worth testing and exactly how many reliable evaluations our architecture study can afford.
...
To quantify this physical reality, @fig-data-movement-energy-scale reports energy consumption per operation across arithmetic units and memory hierarchy levels using the 45\ nm reference values from @Horowitz2014Energy. A baseline 32-bit integer addition costs about $0.1\,\text{pJ}$ ($1\times$); floating-point math and 8\ KB to 32\ KB SRAM cache reads run $10\text{--}20\,\text{pJ}$ ($100\text{--}200\times$); a 1\ MB on-chip SRAM access costs $100\,\text{pJ}$ ($1000\times$); and off-chip DRAM accesses reach $1.3\text{--}2.6\,\text{nJ}$ ($13{,}000\text{--}26{,}000\times$). Fetching data from off-chip DRAM thus consumes thousands of times more energy than performing arithmetic on that data, so an architectural comparison that omits memory hierarchy and locality can rank the wrong design first.
