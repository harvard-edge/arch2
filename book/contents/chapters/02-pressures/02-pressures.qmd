...
The proxy-mismatch failure shows up directly when total system energy is compared between a balanced baseline design and an unoptimized accelerator candidate (@fig-waterbed-effect). The horizontal axis pairs the two design options, while the vertical axis measures total system energy broken into four stacked components ($E_{\mathrm{compute}}$, $E_{\mathrm{memory}}$, $E_{\mathrm{interconnect}}$, and combined control and leakage). Examining the stacked bars reveals how the candidate successfully shrinks compute energy ($E_{\mathrm{compute}}$), yet its added memory accesses and communication overhead cause memory and interconnect energy ($E_{\mathrm{memory}} + E_{\mathrm{interconnect}}$) to swell dramatically. This trade-off demonstrates the architectural takeaway that reducing compute energy in isolation can increase data movement and interconnect energy, inflating total system energy and causing partial evaluation metrics to rank an inferior design first.
