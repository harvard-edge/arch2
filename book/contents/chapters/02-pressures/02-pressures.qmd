...
These scaling pressures establish the boundaries where conventional design loops stall, motivating five specific research questions in design exploration and surrogate validation:

- **How much estimation error can an early-stage surrogate model tolerate before searching a larger candidate pool yields worse silicon than searching a smaller pool?** Fast surrogate models accelerate exploration by screening large candidate pools, but they introduce selection noise that can promote over-optimized outliers over genuinely superior microarchitectures. As the candidate pool expands, noisy screening tools become statistically more likely to promote false positives that fail under high-fidelity signoff tools. Establishing analytical bounds relating surrogate noise distributions to the maximum productive candidate pool size remains unresolved.
- **At what distance in microarchitectural feature space does a learned surrogate model's ranking correlation collapse?** Performance models often demonstrate high accuracy on standard benchmark test suites, but architectural exploration depends on correctly ranking novel, unconventional microarchitectures. When generative exploration produces microarchitectures outside the training distribution, surrogate model error grows nonlinearly. Quantifying this distance threshold is necessary to determine when an automated design loop must trigger expensive cycle-accurate simulation before candidate selection degrades.
- **How should a fixed compute budget be dynamically partitioned between compiler optimization and hardware parameter sweeps?** Tensor-program compilers search vast schedule spaces per operator, while hardware design loops explore microarchitectural configurations. Terminating compiler tuning too early misranks hardware candidates due to unoptimized software mapping, while allocating excessive compute to compiler sweeps starves hardware exploration. Formulating dynamic compute-allocation policies based on empirical ranking convergence requires cross-stack optimization models that balance schedule depth against microarchitectural breadth.
- **At what threshold of die-to-die interconnect energy does modular chiplet partitioning negate the manufacturing yield advantages of smaller dies?** Disaggregating a monolithic SoC into modular chiplets improves silicon manufacturing yield, but driving high-bandwidth inter-chiplet interfaces introduces a persistent energy and latency tax. Developing unified trade-off models that balance packaging yield gains against runtime interconnect energy across diverse traffic profiles remains an unaddressed requirement.
- **How does the functional verification effort required for coverage closure scale as speculative execution depth and prefetcher complexity increase?** Speculative mechanisms and complex prefetching inflate the reachable state space of memory coherence engines and pipeline arbiters. While architects routinely quantify the performance benefits of speculative features early in design, predictive models that project downstream formal verification and testbench engineering costs prior to implementation are currently lacking.
