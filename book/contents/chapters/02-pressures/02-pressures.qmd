...
The substantial costs of leading-edge chip development illustrate the magnitude of this constraint. Public estimates vary, but they agree on the order of magnitude. @SemiconductorIndustryAssociation2026ChipDesign reports that designing a latest-node chip increased from approximately \$30M for a 65\ nm node in 2006 to over \$540M for a 5\ nm node in 2020, an $18\times$ increase, and comparable industry estimates cover architecture, design, validation, verification, and IP qualification rather than masks and wafers alone [@BauerEtAl2020SemiconductorDesignManufacturing].
...
- **When spending a joint search budget between compiler schedule sweeps and hardware parameter sweeps, what fraction of compute should be allocated to software tuning before hardware candidate ranking stabilizes?** Tensor program autotuners search billions of schedule implementations per operator, while hardware space exploration sweeps architectural parameters. Stopping software tuning too early introduces noise into hardware rankings from unoptimized code paths, but spending excessive compute on compiler sweeps starves the hardware search. Establishing a compute-splitting policy based on empirical ranking stability remains an open challenge.
