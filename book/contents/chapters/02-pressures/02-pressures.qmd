...
Wafer-scale integration explores a different balance between local communication and physical scale. The Cerebras Wafer-Scale Engine (WSE) in @fig-cerebras-wse-ch02 extends computation and distributed SRAM across a wafer. Placing compute and memory at this scale can reduce the need to cross board-level links, but power delivery, communication distance, and defect tolerance constrain how much of that hardware can be used [@ReutherEtAl2025LAICS; @PalEtAl2019WaferscaleGPU].
...
The preceding examples suggest four mistakes that can make a study appear to improve while leaving the architecture question unresolved. Each has a concrete symptom that a team can look for in its own work.

- **Reporting throughput the study cannot use:** The team produces more candidates but completes no more useful comparisons. The symptom is an improving generation metric alongside a growing evaluation backlog or longer turnaround time. Count completed comparisons and total effort as well as proposals.

- **Quoting a local gain as a system gain:** An accelerator's arithmetic speedup is reported without the payload size, invocation cost, transfers, or residual host work. The offload example shows how those omissions can reverse the result. Evaluate the matching hardware and software over the workload and boundary that the claimed gain covers.

- **Advancing a candidate whose feasibility is still unknown:** A high-level score is used to select a design even though routing, power delivery, or interface compatibility could overturn the choice. The symptom is a recommendation resting on an effect that the evaluator does not model.[^fn-horowitz-wire-scaling-c02] Resolve that uncertainty with an appropriate check or keep the conclusion provisional.

[^fn-horowitz-wire-scaling-c02]: **Interconnect estimates**: Wire energy and delay depend on geometry, loading, and the implementation [@Horowitz2014Energy; @HoMaiHorowitz2001FutureOfWires]. An architectural estimate can include those costs approximately; placement and routing provide details needed to assess that approximation.

- **Searching beyond a surrogate's demonstrated accuracy:** A model calibrated on one set of organizations guides search into substantially different designs without further checks. The optimizer can favor candidates for which prediction error is most optimistic. Compare selected predictions with suitable reference evaluations, especially when the search changes the organization or workload.
