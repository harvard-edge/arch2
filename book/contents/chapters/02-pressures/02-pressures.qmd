...
For a target such as Lighthouse mobile extended-reality (XR), the term *XR* remains far too broad to define a concrete workload, as it spans motion-to-photon latency ($<\!20\text{ ms}$), sensor fusion pipelines (inertial measurement units and $4\text{K}$ cameras), neural rendering workloads, and a strict 3\ W thermal budget. Five workload archetypes make the ambiguity concrete: ResNet-50 vision, GPT-2/Llama language models, deep learning recommendation models, MobileNetV2 mobile perception, and always-on keyword spotting. Each archetype pulls a design toward fundamentally different memory, latency, and energy behavior. Even when employing a dedicated benchmark suite such as XRBench [@KwonEtAl2023XRBench], architects must still independently decide which specific traces, model versions, deadlines, input distributions, and quality targets matter for the target design.
...
::: {.callout-failure-mode title="Paradox and Pitfall: The Jevons Paradox of Automated Generation"}
...
**The Architectural Antidote:** Teams must implement admission backpressure and multi-tier filtering to size candidate arrival rates to match physical verification capacity (see @sec-law-jevons-paradox for the mathematical candidate arrival scaling proof and EDA license exhaustion models).
:::
