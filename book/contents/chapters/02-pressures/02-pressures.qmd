...
The pressures in this chapter compound because generating candidates got cheap while judging them did not. The questions below are the ones that gap leaves open for anyone with a design-space exploration project.

**Sizing a search to the evaluation it can afford.** A larger candidate pool improves the best design only if enough of the pool can be evaluated well enough to rank. Past some point, adding candidates adds selection error instead of quality.

- **At what pool size does generating more candidates start to make the final choice worse?** Early filtering leans on fast, imperfect models, so a bigger pool gives the screen more chances to promote something the strong tools would have rejected. Nobody has characterized where that turn happens, or how it moves with proxy accuracy and evaluation budget, which is exactly what a team needs to know before scaling a sweep.
- **How should an exploration loop spend a fixed evaluation budget across tiers of very different cost?** Analytical estimates, cycle-level simulation, synthesis, and physical implementation differ by orders of magnitude in both cost and trust, and the allocation among them decides what the study can conclude. Treating that allocation as a decision to optimize, rather than a convention to inherit, is open work with immediate practical value.

**Keeping proxies honest as designs get stranger.** Fast estimators are calibrated on designs that already exist. Generated candidates are valuable precisely when they do not resemble those designs.

- **How fast does a proxy stop ranking correctly as candidates move away from the designs it learned from?** A proxy can retain good average accuracy while its ordering of candidates decays, and ordering is what a screen actually uses. Measuring that decay as a function of structural novelty would tell a team when its screen has quietly stopped being a screen.

**Turning downstream failure into upstream information.** Physical design and verification return rich evidence about why a candidate failed, and most exploration loops discard it as a pass or fail bit.

- **What can a generator learn from a signoff failure without collapsing exploration onto what already passes?** Routability, thermal density, and timing failures carry structure worth learning from, yet a loop that steers hard on that feedback converges on conservative designs and stops finding anything surprising. Keeping the information while preserving diversity is unresolved.
