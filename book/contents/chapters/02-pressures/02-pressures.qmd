...
Each historical shift exposes a dependency that our people and tools must be able to inspect. System/360 and CUDA (NVIDIA's parallel computing platform and programming model [@NickollsEtAl2008CUDA]) allowed our implementations and software to evolve smoothly around a shared interface. When we make these dependencies visible, we empower a single engineer's decision to govern more of the design without obscuring the essential checks below it. Mead-Conway methods and logic synthesis connected our designs directly to the rules and tools that accept or reject them. While SPEC makes our results comparable across machines, RISC demonstrates why our hardware claims must carry the weight of their workload, compiler, and implementation costs. Our discipline could then close more of the widening gap between what a fabrication process could physically manufacture and what our teams could effectively design. We return to the measured version of that gap in @sec-compounding-design-loop-pressure, once today's version is in view.
...
To quantify this physical reality, @fig-data-movement-energy-scale reports energy consumption per operation across arithmetic units and memory hierarchy levels using the 45 nm reference values from @Horowitz2014Energy. A baseline 32-bit integer addition costs about 0.1 pJ (1×); floating-point arithmetic runs 0.9 to 4 pJ (9× to 40×); 8 KB to 32 KB SRAM cache reads run 10 to 20 pJ (100× to 200×); a 1 MB on-chip SRAM access costs 100 pJ (1,000×); and off-chip DRAM accesses reach 1.3 to 2.6 nJ (13,000× to 26,000×). Fetching data from off-chip DRAM thus consumes thousands of times more energy than performing arithmetic on that data, so an architectural comparison that omits memory hierarchy and locality can rank the wrong design first.

```{python}
#| label: fig-data-movement-energy-scale
#| fig-cap: |
#|   **Data movement can dominate arithmetic energy.** Rough Horowitz 45 nm energy values show why architecture comparisons must represent locality, buffering, precision, scheduling, and memory hierarchy rather than counting arithmetic alone [@Horowitz2014Energy]. The values are transcribed order-of-magnitude anchors, including the source's 1.3 to 2.6 nJ DRAM range, not current-node device estimates.
#| out-width: "100%"
...
![**Local compute energy reductions can obscure total system energy increases.** The illustrative candidate reduces compute energy but increases memory and interconnect energy enough to exceed the baseline total. A comparison that omits data movement can therefore rank the worse of the two designs first. The bar heights show the relationship and are not measured energy breakdowns.](images/fig-waterbed-effect){#fig-waterbed-effect width="100%" fig-alt="Bar chart comparing a baseline energy breakdown with an illustrative candidate whose lower compute energy is outweighed by higher memory and interconnect energy."}
...
The substantial costs of leading-edge chip development illustrate the magnitude of this constraint. Public estimates vary, but they agree on the order of magnitude. @SemiconductorIndustryAssociation2026ChipDesign reports that designing a latest-node chip increased from approximately \$30M for a 65 nm node in 2006 to over \$540M for a 5 nm node in 2020, an 18-fold increase, and comparable industry estimates cover architecture, design, validation, verification, and IP qualification rather than masks and wafers alone [@BauerEtAl2020SemiconductorDesignManufacturing].

```{python}
#| label: fig-design-cost-by-node
#| fig-cap: |
#|   **Reported design cost for a new chip rises roughly eighteenfold from the 65 nm node to the 5 nm node.** Each bar is a published estimate rather than a measured program cost, and the two sources use different accounting boundaries. The 65 nm and 5 nm anchors come from an industry association summary [@SemiconductorIndustryAssociation2026ChipDesign], while the 10 nm, 7 nm, and 5 nm figures come from a consulting analysis that counts architecture, design, validation, verification, and IP qualification [@BauerEtAl2020SemiconductorDesignManufacturing]. The two independent 5 nm estimates agree, which is what makes the trend usable as an order of magnitude rather than as a price list.
#| out-width: "100%"
...
Two things in @fig-design-cost-by-node matter more than the totals. The curve steepens as the node shrinks, so each additional node costs more to design than the one before it rather than the same amount. And the estimates count architecture, design, validation, verification, and IP qualification, not masks and wafers, which means most of what the chart shows is the price of the engineering and checking work this chapter has been describing. An expensive evaluation tier draws on that same budget, so deciding which candidates earn one is a program-level judgment rather than a local convenience.
...
Generating candidates got cheap while judging them did not, and the questions below are what that gap leaves open for anyone with a design-space exploration project.

**Sizing a search to the evaluation it can afford.** Early filtering relies on fast surrogate estimators, but generating more candidates increases selection errors when those surrogates make mistakes.

- **How much noise can an early performance estimator tolerate before searching a larger pool of designs yields worse chips than a smaller pool?** Fast surrogate models introduce selection errors that can promote over-optimized outliers over superior designs. As a candidate pool expands, noisy screening tools become more likely to promote false positives that fail when evaluated with high-fidelity simulators. Current analytical methods lack a quantitative bound relating estimator error rates to the maximum useful pool size in design-space sweeps.
- **At what distance in microarchitectural feature space does a surrogate model's ranking correlation fall below a random selector?** Learned performance surrogates often demonstrate low average error on standard benchmark test sets, but design-space exploration depends on relative candidate ordering rather than absolute accuracy. As candidate generators explore unconventional topologies outside the convex hull of training data, prediction error increases nonlinearly. Quantifying this distance threshold is necessary to determine when an automated design loop must trigger high-fidelity simulation before candidate selection degrades.

**Balancing evaluation budgets across hardware and software.** Evaluating unoptimized software on candidate hardware misranks valid designs, yet running full compiler optimization across every hardware candidate quickly exhausts simulation capacity.

- **How much of a joint search budget should go to compiler tuning before the hardware ranking stops moving?** Tensor program autotuners search billions of schedule implementations per operator, while hardware space exploration sweeps architectural parameters. Stopping software tuning too early introduces noise into hardware rankings from unoptimized code paths, but spending excessive compute on compiler sweeps starves the hardware search. Establishing a compute-splitting policy based on empirical ranking stability remains an open challenge.

**Accounting for physical packaging and verification overheads.** Downstream physical integration and formal verification introduce steep costs that early performance proxies routinely ignore.

- **At what ratio of die-to-die physical-layer energy to on-chip memory energy does chiplet partitioning negate the manufacturing yield benefits of smaller dies?** Fracturing a monolithic die into smaller chiplets improves silicon manufacturing yield, but driving inter-chiplet interfaces adds a continuous data movement penalty. Architects currently lack a unified trade-off model that balances physical packaging yield cost savings against the cumulative energy consumed by die-to-die interconnect traffic.
- **How does the functional verification effort required to achieve coverage closure scale as speculative execution depth and prefetching aggressiveness increase in a multicore protocol?** Adding speculative execution mechanisms and aggressive prefetchers inflates the state space of memory coherence controllers. While architects regularly model the performance benefits of speculative features early in design, there is no established framework to predict the downstream verification engineering time required before signoff.
