...
Process and device technology still supply essential improvements, but architects can no longer rely on them to deliver the broadly reusable gains once expected from each new process generation. As classical Dennard scaling and voltage reductions reach physical limits, performance gains increasingly require choices that tightly bind architecture, software, and physical implementation together. Understanding this shift requires examining how the collapse of automatic device scaling forced modern systems toward domain specialization and deep cross-layer co-design.

### The Collapse of Dennard Scaling and the Shift to Specialization

Process technology remains fundamental, yet it no longer automatically converts transistor density into the same broadly reusable combination of frequency, single-thread performance, and energy efficiency. For roughly thirty years, processor generations advanced under classical Dennard scaling: shrinking physical device dimensions reduced transistor area and delay while voltage scaling allowed more transistors to switch faster without increasing power density (@tbl-dennard-scaling-comparison) [@Moore1965Cramming; @DennardEtAl1974Scaling; @HennessyPatterson2017QuantitativeApproach].
...
This scaling contract fractured when supply voltage hit the subthreshold leakage limit near $0.7\text{--}0.9\ \text{V}$. The breakdown occurred because off-state subthreshold leakage current ($I_{\text{off}} = I_{\text{sub}}|_{V_{gs}=0}$) is governed by the Boltzmann tail of the thermal carrier distribution (@eq-subthreshold-leakage):
$$
I_{\text{sub}} = \mu C_{ox} \left(\frac{W}{L}\right) V_T^2 \exp\left(\frac{V_{gs} - V_{th}}{n V_T}\right)
$${#eq-subthreshold-leakage}
where $V_T = \frac{k_B T}{q} \approx 26\ \text{mV}$ is the thermal voltage at $300\ \text{K}$, and $n = 1 + \frac{C_{\text{dep}}}{C_{ox}}$ is the subthreshold ideality factor. Because carrier energies follow Maxwell–Boltzmann statistics, the thermionic emission tail of electrons jumping the source-to-channel energy barrier cannot be modulated faster than the thermal voltage limit at room temperature. This physics dictates a fundamental lower bound on subthreshold swing ($S = n \ln(10) V_T \approx 60\text{--}80\ \text{mV/decade}$ at room temperature), requiring a fixed gate-to-source voltage swing to suppress drain current by one order of magnitude. Lowering threshold voltage $V_{th}$ below about 0.3\ V caused static leakage current ($I_{\text{off}} \propto 10^{-V_{th}/S}$) to rise exponentially, converting static dissipation into a thermal runaway hazard. When $V_{dd}$ scaling stagnated around 2005 to preserve $I_{\text{on}}/I_{\text{off}}$ switching ratios and noise margins, dynamic power ($P_{\text{dyn}} = \alpha C \cdot V_{dd}^2 \cdot f$) surged whenever frequency increased, forcing the industry to freeze clock rates and confront the dark silicon wall [@EsmaeilzadehEtAl2011DarkSilicon].
...
Microarchitectural scaling hits steep physical limits. As @PalacharlaEtAl1997ComplexityEffective demonstrated, out-of-order execution logic scales poorly with window size and issue width. Wakeup tag-drive delay scales quadratically with instruction window size ($O(N^2)$) because broadcasting tags requires driving long capacitive lines across $N$ window entries against $N$ parallel comparators. Result bypass delay likewise scales quadratically with issue width ($O(W^2)$) due to the dense $W \times W$ bypass crossbar wiring and multiplexer loading between execution units. Eventually, wire delay on these critical paths dominates the clock cycle. In deep submicron nodes, wire resistance and capacitance ($RC$) parasitics in lower metal layers ($M_0\text{--}M_2$) (driven by electron surface scattering and grain-boundary scattering as wire cross-sections shrink) cap clock frequency long before gate speed runs out [@AgarwalEtAl2000ClockRateVersusIPC].

These microarchitectural costs translate into concrete engineering limits. In deep pipelining, chasing aggressive multi-gigahertz targets by stretching pipelines (such as the 31-stage Intel NetBurst Prescott) inflates branch-misprediction refill penalties to dozens of cycles, creating pipeline bubbles that burn dynamic power ($P_{\text{dyn}} = \alpha C \cdot V^2 \cdot f$) while instructions per cycle (IPC) collapses on branch-heavy code [@BoggsEtAl2004Pentium4NinetyNm]. Frequency cannot be evaluated in isolation, because the pipeline stages and wider windows required to push clock rates increase energy per instruction and branch-misprediction penalties.
...
The compounding pressures analyzed across this chapter explain why computer architecture is actively seeking AI assistance and automated methodologies. However, acknowledging that traditional design loops no longer scale does not automatically validate that proposed AI tools are targeting the true limiting constraints of silicon design. Translating AI-assisted design from academic benchmarks into production tapeouts exposes fundamental methodological dilemmas where empirical evidence remains unresolved:
