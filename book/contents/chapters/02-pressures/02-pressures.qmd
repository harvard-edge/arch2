...
Which regime a design occupies in the break-even phase diagram (@fig-ch09-logca-phase-diagram) depends directly on how the physical interconnect sets these LogCA variables. The horizontal floor of each curve is governed by the fixed invocation overhead ($o$) relative to baseline host execution ($C_h$) and local acceleration ($A$); lowering $o$ drops the minimum break-even payload $g^*$. The vertical asymptote, or operational intensity wall, is dictated by link bandwidth ($B$); restricting $B$ pushes the wall right, demanding higher arithmetic reuse per transferred byte before the offload can break even. Die-to-die links under Universal Chiplet Interconnect Express (UCIe) 2.0 across 2.5D packaging minimize overhead ($o \sim 5\text{--}10\ \text{ns}$) while providing high shore bandwidth density ($B > 1\ \text{TB/s/mm}$), driving both the floor and the wall inward so fine-grained submicrosecond offloads between heterogeneous chiplets can break even. Coherent socket links (CXL 3.1 over a PCIe physical layer) incur larger startup costs ($o \sim 80\text{--}150\ \text{ns}$), pushing the floor up to suit memory pooling and coarse-grained kernel offload. Board-level peripheral links (PCIe Gen 5/6) impose round-trip invocation latencies above $500\text{--}1{,}000\ \text{ns}$ and tighten bandwidth ($B$), shifting the curves sharply up and right; amortizing host–device driver serialization, DMA descriptor setup, and bus transport requires massive payloads of $g \gg 10^5$ operations.

![**LogCA break-even offload frontiers across physical interconnect regimes.** Break-even granularity ($g^*$, minimum FLOPs for net speedup $S(g) \ge 1.0$) plotted against operational intensity ($\text{FLOP/byte}$) across seven interconnect standards [@AltafWood2017LogCA; @Williams2009Roofline]. Die-to-die UCIe 2.0 (1,600\ GB/s, 2.5\ ns) breaks even at fine-grained payloads ($g^* \approx 40\ \text{FLOPs}$), whereas board-level PCIe Gen 5 and Ultra Ethernet enforce steep bandwidth walls ($\text{OI}^* \approx 0.04\ \text{FLOP/byte}$), requiring coarse payloads ($g^* > 10^5\ \text{FLOPs}$) to amortize driver and link overheads.](images/fig-ch09-logca-phase-diagram){#fig-ch09-logca-phase-diagram width="100%" fig-alt="Log-log plot of minimum break-even operation granularity in FLOPs versus operational intensity in FLOPs per byte across seven interconnect regimes from UCIe 2.0 to Ultra Ethernet, showing that die-to-die packaging enables fine-grained offloading while board-level PCIe enforces a steep operational intensity wall."}

Advanced packaging with low startup ($o$) and high shore bandwidth ($B$) breaks even after minimal work per offload. Board-level peripheral links require substantially larger payloads before offloading helps, nonzero transfer terms reduce the asymptotic speedup, and under high-cost parameters system speedup collapses toward baseline host execution. The local factor $A$ that a datapath advertises is therefore never the speedup a product path delivers, and the gap is easy to underestimate when the runtime that drives the block has not yet been written.
...
The interconnect fixes $o$, $B$, and the regime. Where inside that regime a workload actually lands is decided by the compiler and runtime rather than the link, which is why the bound this section prices returns below as a software problem once the hardware is composed.
...
Even a narrow comparison can yield a large number of combinations. Consider an illustrative rather than measured calculation. Suppose an exploration covers an accelerator and memory subsystem for the Lighthouse XR moonshot target introduced in @sec-moonshot, sweeping six of the eight requirement domains of @sec-moonshot while holding the workload and verification domains fixed. Considering five compute organizations (such as a vector-capable CPU, an accelerator, or an SoC block), four ISA and ABI contract options (such as RV64GCV vector extensions), six memory hierarchy and data movement topologies (such as NoC bisection bandwidth and DRAM timing choices), four power envelope targets (scaled around the 3\ W TDP), three compiler and runtime stack paths (Triton, an open-source GPU compiler language; LLVM/MLIR; and domain libraries), and three physical process libraries (3\ nm-class low-power variants differing in standard-cell track height and signoff corner set) yields the following product.
...
Diagnosing where evaluation constraints bind identifies the limiting work, but it provides no structure for executing it. Because modern architectural evaluation tightly couples software workloads, specialized microarchitectures, and physical layout limits, relieving these pressures requires organizing the workflow to preserve authoritative checks at every layer. Establishing that structure is the central objective of the architecture 2.0 ontology. @sec-architecture-20-ontology formalizes the design sequence into a six-stage life cycle, from capturing initial intent and modeling the environment through candidate generation, multi-fidelity evaluation, and final execution. This organization establishes the operational harness necessary to integrate automated assistance without compromising the empirical rigor of the architectural comparison.
