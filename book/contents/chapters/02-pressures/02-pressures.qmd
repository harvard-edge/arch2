...
As shown in @fig-search-vs-eval-gap, while our toy count sits in the thousands, MAESTRO and AutoTVM cluster near $10^9$, and Timeloop's computed lower bound lies almost ten orders of magnitude farther to the right. We use AutoTVM's hollow marker to distinguish its order-of-magnitude estimate from the explicitly stated or computed counts. While this full span covers nearly fifteen orders of magnitude, the horizontal position only encodes our candidate count. It tells us nothing about the cost, fidelity, or decision strength of a single evaluation. As architects, we must decide which choices are truly worth testing and exactly how many reliable evaluations our architecture study can afford.
...
Design Rule Checks (DRC) introduce yet another rigorous physical constraint. Our routing must satisfy restricted design rules, including stringent minimum wire and via spacing [@KahngEtAl2011VLSIPhysicalDesign]. Simply moving a single macro can spawn numerous violations, and repairing just one can alter neighboring routes and expose fresh errors. If our automated flow cannot interpret these markers intelligently, we inevitably dump a large, repetitive cleanup and rerouting burden onto our implementation team.
...
Because physical constraints and reliability concerns define a vast search space, no early model will ever eliminate our need for the robust physical tools required to validate our final claims. As architects, we must intuitively understand exactly which lower-cost estimates are safe for broad screening, and which rigorous checks retain the power to overturn our rankings.
...
Our industry has grappled publicly with this verification pressure for over a quarter of a century. As far back as 2001, the ITRS Design chapter declared verification "a bottleneck that has now reached crisis proportions" [@ITRS2001Design]. Today, the Semiconductor Research Corporation's MAPT roadmap still explicitly warns of a widening verification gap looming across our future transitions [@SemiconductorResearchCorporation2023MAPT]. While this rich history makes verification an indispensable constraint to check whenever someone proposes generating architectural candidates faster, it does not automatically imply that verification will be the absolute limiting constraint in every single design program.

Yet the math is inescapable. If we can produce new register-transfer level (RTL) candidates faster than we can create the properties, tests, coverage metrics, and human reviews required to bless them, our rapid generation speed does absolutely nothing to improve our final tapeout decision. This harsh reality powerfully grounds the abstract state-space argument directly into the daily work of a hardware team.
...
No single AI method diagnoses its own need, performs the required work, and supplies an independent check all at the same time. As architects, we still completely own the question, the scope, the comparison, and the final decision.
