...
3. **Build (Stage 3).** Translates abstract candidate parameters into testable representations (transaction-level models (TLMs), synthesizable register-transfer level (RTL), and target software binaries) within clean, sandboxed execution harnesses.
...
These stage seams construct a formal *failure-routing state machine*. Before diving into the complex mechanics of this state machine, it is crucial to understand the FABRIC life cycle as a cohesive conceptual journey. The six stages operate as a continuous progression from abstract human intent down to concrete physical implementation. Each stage systematically reduces uncertainty by answering a specific question: what are we trying to achieve (Formulate), which options are viable (Assess), how do they work in practice (Build), how well do they perform (Refine), why did they perform that way (Interpret), and should we proceed (Commit).

Conceptualizing the life cycle as a progressive disclosure of physical reality helps teams appreciate why failure domains must be isolated. Only after this conceptual foundation is established can we formalize the exact routing logic that handles inevitable defects.

These stage seams construct a formal *failure-routing state machine*.

- **Syntax and packaging faults $\rightarrow$ Stage 3 (Build):** An uninstantiated clock gating cell, an unconnected active-low reset, a missing top-level port, or a malformed Verilator build script is trapped at Stage 3. It routes directly to tool wrapper repair without altering microarchitectural parameters.
- **Physical boundary violations $\rightarrow$ Stage 2 (Assess):** A static random-access memory (SRAM) macro selection that exceeds hard layout boundaries ($1.72\text{ mm}^2 > 1.50\text{ mm}^2$), introduces severe routing congestion across upper metal layers, or forces extreme aspect ratios is trapped during Stage 4 evaluation. The failure routes back to Stage 2 to prune that illegal subspace and propose a mutated alternative.
...
To achieve this, the design system structures the architecture workflow around six discrete artifacts, ensuring that every failed candidate still contributes useful telemetry. An isolated failure at the implementation or refinement stage is transformed into a highly specific diagnostic payload, enriching the study record and informing subsequent generation.

Instead of silently discarding a failed run, the execution record captures the exact physical boundary that was hit. Explicitly feeding targeted constraint data back to the generator for the next iteration transforms ad-hoc debugging into a systematic, data-driven optimization process.
...
This recurrent topology ensures that iterative refinement operates as a controlled feedback loop rather than an unguided restart. The system explicitly returns targeted repairs to the responsible stage while preserving valid prior exploration history, which prevents global state corruption.

Because architectural exploration involves both rapid computational search and deliberate engineering judgment, these feedback loops must bridge vastly different operational timescales. Automated agents can synthesize and discard thousands of configurations in the time it takes a human architect to review a single study record.

Executing a multi-tool architecture study across this loop unfolds as an asynchronous protocol. Decoupling fast automated search iterations from slower human oversight processes coordinates high-frequency automated execution with low-frequency human governance without stalling the overall pipeline.
...
Maintaining this explicit structural hierarchy guarantees that every metric is contextualized by its generating constraints. An isolated cycle-count reduction is meaningless without the cryptographic attestation proving it adhered to the authorized power and area limits defined in the scoped plan.
