...
2. **Explore.** Identifies legal candidate configurations across coupled subsystem design spaces.
...
Instead, the AI-native life cycle operates as a dynamic feedback loop that routes targeted repairs directly back to the responsible stage while preserving unaffected exploration history (@fig-design-loop). When a candidate violates a downstream physical constraint, the loop does not trigger a global restart; it routes a structured diagnostic to the exploration stage to mutate the candidate within the existing scoped plan.
...
Deploying autonomous agents, learned optimizers, and automated toolchains across a multi-stage life cycle introduces subtle operational failure modes. When optimizers operate under strict objective functions, they aggressively exploit whatever physical properties or interface rules the early models omit, a phenomenon known as proxy gaming. Maintaining life-cycle integrity requires recognizing and preventing these recurring failure patterns:
