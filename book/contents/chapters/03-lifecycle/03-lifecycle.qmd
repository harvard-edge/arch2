...
Translating the architecture moonshot into engineering practice requires a disciplined operational workflow. While generative AI models can propose candidate configurations in seconds and launch parallel simulation sweeps across clusters, generation alone does not produce defensible architecture. The central operational challenge is organizing automated exploration so that promising proposals survive an unbroken chain of physical qualification, causal explanation, and audit-ready human review.

Without an explicit life cycle, automated tools encounter severe operational failure modes. Autonomous optimizers can drift from the original design question, submit illegal microarchitectural configurations, or mistake simulation infrastructure crashes for physical breakthroughs. They can game proxy metrics to maximize reward functions or extrapolate narrow simulator observations into invalid chip-wide claims [@AmodeiEtAl2016Concrete; @Goodhart1975MonetaryManagement].

Because each failure mode requires a fundamentally different correction, the design system must partition exploration into isolated failure domains [@NASA2017SystemsEngineeringHandbook]. The six life-cycle responsibilities form the *FABRIC* operational framework (*Formulate*, *Alternatives*, *Build*, *Run*, *Interpret*, and *Commit*). Rather than a rigid waterfall schedule, FABRIC operates as an iterative, error-isolated feedback loop. To ensure full transparency, every study records its iterations in a structured study record, indexed by a standardized Design-Loop Card (@sec-design-loop-card), binding each attempt to its target question, allowed mutations, tool budget, rejection criteria, and named commitment authority.
...
- **Apply** the six-stage FABRIC framework (Formulate, Alternatives, Build, Run, Interpret, and Commit) to structure error-isolated design loops.
...
## Six Core Responsibilities: The FABRIC Framework {#sec-arch2-ontology}
...
The life cycle organizes these responsibilities into the six sequential stages of the FABRIC framework:

1. **Formulate (F).** Translates product intent into an explicit architectural question, legal bounds, baseline comparators, and verifiable acceptance checks.
2. **Alternatives / Explore (A).** Explores coupled subsystem design spaces, generating legal candidate configurations and pruning invalid subspaces.
3. **Build / Implement (B).** Translates abstract candidate parameters into testable representations (transaction-level models, synthesizable register-transfer level (RTL), and target software binaries) within clean, sandboxed execution harnesses.
4. **Run / Evaluate (R).** Executes multi-fidelity evaluation flows (analytical models, cycle-accurate simulators, and physical electronic design automation (EDA) engines), measuring evidence against declared constraints.
5. **Interpret (I).** Decouples top-line metric changes from simulation artifacts, establishing direct physical causality for observed performance, power, and area (PPA) differences.
6. **Commit / Decide (C).** Synthesizes the complete study record, formulates the supported technical recommendation, and authorizes in-order architectural retirement under named human commitment authority.

To ground these responsibilities, the chapter carries forward our prospective mobile extended-reality (XR) system-on-chip (SoC) subsystem study introduced in @sec-moonshot, targeting real-time tracking and neural rendering under a composite $3\ \text{W}$ thermal design power (TDP) signoff envelope across declared process, voltage, and temperature (PVT) corners and eight requirement domains (@sec-intent-vs-spec). The six life-cycle responsibilities map this architectural intent to a reviewed decision while isolating failure domains (@fig-ontology-loop). Operating across these six stages distinguishes the three layers of AI capability defined in @sec-moonshot (@fig-ch01-three-layers-ai-integration):

- **Layer 1 (AI-Assisted Engineering):** Operates within an isolated stage (such as drafting an RTL module in Stage 2 or generating a simulation script in Stage 3), leaving the human engineer to manually transfer artifacts and bridge verification gaps across boundaries.
- **Layer 2 (AI-Driven Subsystem Optimization):** Automates an inner closed loop across Stages 2 through 4 (Explore/Alternatives, Implement/Build, and Evaluate/Run) inside a frozen container (such as autotuning an L2 cache bank schedule or macro floorplan), but cannot renegotiate interface contracts or repartition software tasks.
- **Layer 3 (AI-Native System Design):** Structures the complete six-stage FABRIC life cycle across the hardware–software stack, coupling automated multi-tool exploration (Stages 2–4) with physical causal interpretation (Stage 5) and in-order human commitment authority (Stage 6).

![**The six-stage FABRIC design life cycle carries system intent to a reviewed decision.** The life cycle organizes architectural exploration into six sequential stages: Formulate, Alternatives (Explore), Build (Implement), Run (Evaluate), Interpret, and Commit (Review and Decide). Direct return paths allow inadequate outputs or review feedback to cycle back to the owning stage (such as revising the scoped plan in Stage 1), preventing indiscriminate full-system restarts. Detailed stage-seam repairs and protocol interactions are developed in @fig-design-loop and @fig-lifecycle-interaction-protocol.](images/fig-ontology-loop){#fig-ontology-loop width="100%" fig-alt="Six numbered responsibilities arranged in two rows showing the FABRIC framework. A return path shows that review can revise the question, while targeted repair paths return inadequate outputs directly to the specific stage responsible for the defect."}
...
## The Six FABRIC Life-Cycle Stages
...
### Stage 1 (F). Formulating Core Architecture Questions
...
| **Hard constraints** | Modeled subsystem power must not exceed 3\ W. The SRAM macro must not exceed $1.5\text{ mm}^2$ and its access time must not exceed 2.5\ ns across declared PVT signoff corners. |
...
### Stage 2 (A). Exploring Alternatives
...
### Stage 4 (R). Running Candidate Evaluations
