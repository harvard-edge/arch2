...
Every architecture study begins with high-level intent, yet converting a vague goal into an actionable design space determines whether an engineering study succeeds. Without explicit scope, declared comparators, and hard constraints, automated search algorithms optimize misleading proxy metrics or propose illegal microarchitectural configurations. Formulation transforms raw intent into a precise architectural question bound by machine-readable component metadata contracts (such as IP-XACT [Institute of Electrical and Electronics Engineers (IEEE) 1685]) and strict hardware–software contracts. A scoped plan establishes these requirements before exploration begins, binding parameters for the mobile XR SoC study (@tbl-bounded-study-contract). In this study, the contract defines a comparison of 3\ MiB and 4\ MiB L2 cache candidates against a 2\ MiB baseline under strict hard constraints (3\ W TDP, $1.5\text{ mm}^2$ SRAM area limit, and 2.5\ ns access latency bound). The contract specifies objectives, workload conditions, planned checks, and a budget of four cycle-level gem5 execution attempts [@BinkertEtAl2011Gem5], evaluating a versioned XRBench trace with a pinned RV64GCV software image, a declared SRAM model for area and latency, and Verilator [@Veripool2026Verilator] for functional regression of advancing candidates.
...
Permitted changes and Exclusions fix what the study may touch, ensuring downstream observations trace directly to the capacity decision. The run budget bounds computational effort, preventing unconstrained iteration. Formulation culminates in an architectural claim, making the expected result falsifiable:
...
Evaluation results remain bounded by the fidelity of the configured tool. A cycle-level result in gem5 does not authorize silicon fabrication when the study contract scoped recommendations to RTL evaluation. Unlike software engineering benchmarks such as SWE-bench [@JimenezEtAl2024SWEBench] that evaluate functional state through a single test harness, architecture evaluation aggregates feedback across heterogeneous tools. Comparing studies requires explicit declarations; evaluating a candidate in gem5 versus FireSim [@BinkertEtAl2011Gem5; @KarandikarEtAl2018FireSim] yields valid comparisons only when workload phases, memory timing, and power models are explicitly matched.

### Stage 5. Interpreting Empirical Results

A passing evaluation score demonstrates that a candidate satisfied quantitative metrics, but leaves physical causality unverified. Stage 5 decouples top-line metric improvements from tool measurement anomalies, isolating the microarchitectural drivers behind observed gains. A larger cache may improve average latency while degrading tail latency due to network-on-chip (NoC) contention or thread scheduling. Interpretation connects measured outcomes to concrete physical accounts rather than speculative narratives, filtering out simulator artifacts such as simplified queueing models, unmodeled DRAM refresh delays, or idealized bus arbitration. Simulation fidelity gaps exist across all domains; an optimizer in SCALE-Sim might report higher throughput by omitting buffer synchronization barriers, or a compiler pass in TVM might eliminate vector spill code by assuming unconstrained register capacity. Empirical interpretation identifies whether gains reflect physical reality before advancing candidates to review.

### Stage 6. Reviewing and Deciding Handoffs

The final stage bridges empirical evidence to an architectural recommendation and organizational commitment. A collection of simulation logs and Pareto curves does not constitute either. The architect judges what technical recommendation the evidence supports, while a named commitment authority decides whether the organization should commit capital toward RTL synthesis or silicon fabrication. Review synthesizes the exploration trajectory, qualified evaluation evidence, physical explanations, and residual risk into a study record that supports both judgments without conflating them. Review may advance a candidate, reject the slate, reopen a specific stage for repair, or close the study cleanly when budget is exhausted.
...
This recurrent topology ensures that iterative refinement operates as a controlled feedback loop rather than an unguided restart. Executing a multi-tool architecture study unfolds as an asynchronous protocol coordinating automated execution with human governance (@fig-lifecycle-interaction-protocol).
