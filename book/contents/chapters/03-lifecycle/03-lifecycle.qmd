...
**Attributing a failure to the stage that caused it.** A downstream crash rarely originates downstream. When stages iterate automatically, the distance between cause and symptom grows.

- **What would let a design system trace a late failure back to the stage that produced it?** A simulator abort or a timing violation may come from an under-constrained memory specification, an illegal parameter combination, or a tool flag set three stages earlier, and current flows leave the architect to guess. Diagnosis that isolates the responsible seam would prevent the repeated backtracking that consumes automated loops.
- **When does a local repair silently invalidate the contract it was made under, and what would catch it?** Automated fixes that close timing or area frequently adjust something the upstream specification assumed fixed, and the study continues with a mismatch nobody recorded. What is missing is a way to detect that a repair crossed a stage boundary, not a policy that forbids repairs.

**Preserving provenance without preserving everything.** Reconstructing a study requires knowing what ran and under what conditions, and thousands of attempts make full retention impractical.

- **What is the smallest record that still lets a failed attempt be understood later?** Keeping every waveform, netlist, and log across a large sweep is unaffordable, while keeping only the winners destroys the negative evidence that makes a boundary reusable. Finding the minimum that preserves causality is an information-retention problem our discipline has not posed carefully.

**Keeping verification independent of the thing it verifies.** A loop that generates, checks, and interprets its own work can be internally consistent and wrong.

- **Which stages of a life cycle must be owned by something that did not produce the work?** When one component proposes a change, writes the test, and reads the result, agreement carries much less information than it appears to. The life-cycle question is structural, asking which handoffs need a separate owner, while @sec-feedback-verification-trust asks how to detect shared blind spots and @sec-bootstrapping-ecosystem asks how independence can be shown across organizations.
