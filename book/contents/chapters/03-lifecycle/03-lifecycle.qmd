...
2. **Explore (Stage 2).** Explores coupled subsystem design spaces, generating legal candidate configurations and pruning physically unviable subspaces early.
3. **Implement (Stage 3).** Translates abstract candidate parameters into testable representations (transaction-level models (TLMs), synthesizable register-transfer level (RTL), and target software binaries) within clean, sandboxed execution harnesses.
4. **Evaluate (Stage 4).** Executes multi-fidelity evaluation flows (analytical models, cycle-accurate simulators, and physical electronic design automation (EDA) engines), measuring evidence against declared constraints.
...
- **Syntax and packaging faults $\rightarrow$ Stage 3 (Implement):** An uninstantiated clock gating cell, an unconnected active-low reset, a missing top-level port, or a malformed Verilator build script is trapped at Stage 3. It routes directly to tool wrapper repair without altering microarchitectural parameters.
- **Physical boundary violations $\rightarrow$ Stage 2 (Explore):** A static random-access memory (SRAM) macro selection that exceeds hard layout boundaries ($1.72\text{ mm}^2 > 1.50\text{ mm}^2$), introduces severe routing congestion across upper metal layers, or forces extreme aspect ratios is trapped during Stage 4 evaluation. The failure routes back to Stage 2 to prune that illegal subspace and propose a mutated alternative.
...
### Stage 4: Running Candidate Evaluations
...
Empirical tracking of 4,564 closed engineering defect records across open-source silicon repositories (lowRISC OpenTitan, The OpenROAD Project, and UCB Chipyard) illustrates the compounding cost of late-stage defect escapes (@fig-lifecycle-defect-latency). While early front-end synthesis and tooling defects are triaged and resolved in a median of 8.7\ to 16.5\ days, defects that escape into macro floorplanning, hardware–software toolchain boundaries, and static timing signoff escalate super-linearly, requiring a median resolution latency of 69.6\ to 100.4\ days (with 90th-percentile tail latency reaching 814.7\ to 1,021.7\ days). Downstream physical failures require running massive multi-hour place-and-route suites to verify each fix. Enabling automated inner loops to detect and repair constraint violations at Stages 2 and 3 prevents defects from cascading into the high-latency physical signoff regime.

![**Defect resolution latency escalates super-linearly across the six silicon life-cycle stages.** Distributions and median latency markers trace resolution timelines across $N = 4{,}564$ closed engineering defects queried from open-source hardware repositories (lowRISC OpenTitan, The OpenROAD Project, and UCB Chipyard). Resolution latency escalates $11.5\times$ from front-end synthesis and tooling (median 8.7\ to 16.5\ days) through RTL implementation (median 33.0\ to 35.5\ days) to Stage 5/6 static timing signoff (median 100.4\ days, with 90th-percentile tail latency reaching 814.7\ days in OpenROAD and 1,021.7\ days at the Chipyard toolchain seam). Automated inner loops that isolate and repair defects at Stages 2 and 3 prevent expensive escalations into downstream physical closure flows.](images/fig-lifecycle-defect-latency){#fig-lifecycle-defect-latency width="100%" fig-alt="Distributions on a logarithmic scale showing defect resolution duration in days across six hardware life-cycle stages from Stage 1 formulation to Stage 5 and 6 signoff, illustrating an escalation from 8.7 days in early stages to over 100 days in static timing signoff."}

The severe latency penalty shown in the physical signoff regime illustrates why hardware development resists naive trial-and-error. When a defect escapes into place-and-route or static timing analysis, fixing it requires unraveling weeks of tightly coupled physical optimizations. This mandates a design loop that can catch and repair physical violations via fast proxies early in the cycle before they cascade into the backend.
...
The design system structures the architecture workflow around these six discrete artifacts, ensuring that every failed candidate still contributes useful telemetry. Instead of silently discarding a failed run, the execution record captures the exact physical boundary that was hit, feeding targeted constraint data back to the generator for the next iteration.
