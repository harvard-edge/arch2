...
We formalize these verification boundaries in @tbl-stage-seams. For each stage, the table names the input artifacts the stage receives, the deliverables and validation checks it must produce, and the precise return path taken when an output falls short.
...
The six responsibilities in the figure form one recurrent path rather than a pipeline that runs once. As work moves forward, backward return paths route repairs to the owning stage without discarding valid exploration history, ensuring every design revision preserves historical evidence while holding every change accountable to our original question.
