...
The AI-native life cycle is fractal. Rather than a single monolithic pass from concept to silicon, an architecture team executes hundreds of nested, concurrent scoped studies across the stack (subsystem sizing $\rightarrow$ microarchitecture $\rightarrow$ RTL implementation $\rightarrow$ physical closure), each organized by the six responsibilities and indexed by its own Design-Loop Card. A scoped study may follow a single design path. It becomes a loop only when returned evidence changes represented state and triggers another allowed action. If a check reveals that a baseline simulation used an obsolete memory model, for example, that return invalidates the prior comparison and can trigger repetition at the responsible stage; the study record preserves why the prior result was superseded. Formal stage boundaries define explicit verification contracts and targeted repair paths across the life cycle (@tbl-stage-seams).
...
- **Physical boundary violations $\rightarrow$ Stage 2 (Assess).** In a hypothetical return, a static random-access memory (SRAM) macro selection might exceed hard layout boundaries ($1.72\text{ mm}^2 > 1.50\text{ mm}^2$), introduce severe routing congestion across upper metal layers, or force illegal aspect ratios during Stage 4 evaluation. Such a failure would route back to Stage 2 to prune that illegal subspace and propose a mutated alternative.
...
Because automated agents can synthesize, simulate, and qualify candidate microarchitectures orders of magnitude faster than humans can review full study records, feedback-driven repair must bridge vastly different operational timescales. @fig-lifecycle-interaction-protocol constructs an asynchronous protocol for that coordination under an illustrative mutation contract broader than the prospective XR contract.

![**A constructed protocol separates feedback-driven repair from human governance.** Five lifelines coordinate the human architect, orchestrator, execution environment, verification engines, and commitment authority. The illustrative contract permits mutations to cache capacity, banking partition, and the Unified Power Format (UPF) table. It is neither an execution nor an instantiation of the prospective capacity-only XR study; every displayed run, value, status, causal account, recommendation, and promotion is constructed. Only a return that changes represented state and triggers an allowed mutation closes this protocol into a loop. Continuing the prospective study's fixed sweep alone remains a design path.](images/fig-lifecycle-interaction-protocol){#fig-lifecycle-interaction-protocol width="100%" fig-alt="Sequence diagram with five lifelines. Steps 2–7 show a constructed return changing state and triggering cache-capacity, banking-partition, and UPF-table mutations; Steps 1 and 8–10 show human formulation, review, and commitment."}

Viewing this interaction as an explicit protocol separates operating frequencies and authority boundaries across actors.

- **High-frequency automated inner loop (Steps 2–7).** In this separate constructed protocol, the illustrative contract permits changes to cache capacity, banking partition, and the UPF table. Suppose Candidate $C_1$ (a 4\ MiB cache) produced an SRAM area estimate of $1.72\text{ mm}^2$ against the $1.50\text{ mm}^2$ limit. That hypothetical return would close a loop only if it changed represented state, such as marking $C_1$ invalid, and triggered a permitted repair across those fields. This mutation does not instantiate the prospective XR study, whose fixed sweep permits capacity changes only. Running its predeclared 3\ MiB configuration next without a return-driven state change would continue the design path rather than synthesize a new candidate in response to evidence.
...
![**An illustrative study card separates prospective fields from decision authority.** Instantiated as a template for the prospective mobile XR L2 cache capacity study, the card indexes the claim under review, baseline comparator (2\ MiB), planned candidates (3\ MiB and 4\ MiB), a reserved infrastructure-invalid replacement attempt, planned rejection checks (power, area, and latency bounds), fields for empirical findings, and the designated commitment authority. The displayed four-execution entry denotes budget capacity rather than completed runs; any displayed rejection, finding, or decision is a hypothetical placeholder rather than an executed result.](images/fig-filled-design-loop-card){#fig-filled-design-loop-card width="100%" fig-alt="Illustrative Design-Loop Card template for the prospective XR capacity comparison. A claim under review appears above planned entries for the baseline, candidates, failed-run replacement, rejection checks, evidence support, and accountable commitment decision. The four-execution entry is a budget, and the entries do not report executed results."}

The card establishes an audit-ready provenance index by organizing three core concerns: the architectural claim (which specifies the target workload scenario, baseline comparator, and quantitative bounds), the execution provenance (which records evaluated candidate variations, physical rejection causes such as SRAM macro limits, and tool execution failures separately from microarchitectural defects), and the evidence authority (which isolates empirical findings from the named lead architect holding in-order commitment rights). In the prospective mobile XR study, the card binds the planned 2\ MiB baseline and 3\ MiB and 4\ MiB alternatives to the 3\ W TDP envelope, $1.50\text{ mm}^2$ area limit, 2.5\ ns access-time bound, and four-attempt budget. It can record an area-based rejection only after the declared model returns a valid estimate beyond the limit, and it can support a recommendation only after the planned checks return qualifying evidence. Until then, its rejection, finding, and decision fields remain unfilled rather than evidence of a promoted candidate.
