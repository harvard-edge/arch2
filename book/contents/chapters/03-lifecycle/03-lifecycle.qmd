...
- **What is the smallest record that still lets a failed attempt be understood later?** Keeping every waveform, netlist, and log across a large sweep is unaffordable, while keeping only the winners destroys the negative evidence that makes a boundary reusable. Finding the minimum that preserves causality is an information-retention problem the field has not posed carefully.
