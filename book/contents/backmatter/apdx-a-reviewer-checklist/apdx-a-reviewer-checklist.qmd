...
An architectural claim cannot be evaluated rigorously without clear boundaries around the task, design space, and operating conditions (@sec-architecture-20-ontology).
...
External review requires access to the claim-relevant environment and its represented state, even when certain artifacts remain protected or proprietary. Model weights alone do not preserve the data, constraints, and tool states that bound an architectural result.
...
Judging the validity of feedback requires identifying which component produced it and which role it performed (@sec-methods-generation-prediction-optimization).
...
Reward hacking through imperfect proxies is an established concern in machine learning safety [@AmodeiEtAl2016Concrete], and it transfers directly to hardware evaluation. Relying on uncalibrated proxy metrics risks allowing an automated search to exploit simulator inaccuracies rather than improve the underlying design.
...
- **Latent space grounding.** For methods relying on learned latent embeddings or surrogate manifolds (e.g., GNN floorplan embeddings, Transformer code tokens, or Bayesian optimization surrogates), do the authors define the physical decoding and grounding mechanism that projects latent points back to executable electronic design automation (EDA) and RTL signoff checks (@sec-appendix-b-ai-for-architecture-map)?
...
Learning-based results, particularly in deep reinforcement learning, can be sensitive to seeds, hyperparameters, and implementation details [@HendersonEtAl2018DeepRL].
...
Replaying an evaluation requires the exact recorded candidate, tool path, and behavior-affecting state. Replay tests that specific retained execution path, which remains distinct from an independent reproduction derived from the stated specification.
...
While a declared budget bounds exploration before execution, the realized cost must account for all resources the study actually consumed.
...
External review must identify who qualifies each domain result, who synthesizes the system recommendation, and who holds the authority to commit to a consequential design transition (@sec-what-architect-owns).
