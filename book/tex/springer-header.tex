...
\usepackage{array}
\usepackage{float}
% Keep a figure after the source passage that introduces it.
\usepackage{flafter}
\usepackage{algorithm}
\usepackage{algpseudocode}
\usepackage{placeins}
\usepackage{needspace}
% Float placement policy: spread figures evenly across pages and enforce a
% minimum text fraction (20%) so that at most one major float appears per page,
% preventing double-stacked figures with squished text slivers.
...
\makeatletter
\AtBeginDocument{%
  % Color and bold in-text footnote / sidenote reference numbers
  \RenewDocumentCommand{\@sidenotes@thesidenotemark}{ m }{%
    ...
    \ifhmode
      ...
      \nobreak
    \fi
    \hbox{\@textsuperscript{\normalfont\bfseries\color{archtwoblue}#1}}%
    \ifhmode
      \spacefactor\@x@sf
    \fi
    \relax
  }%
  \def\@makefnmark{\hbox{\@textsuperscript{\normalfont\bfseries\color{archtwoblue}\@thefnmark}}}%
  % Color and bold margin sidenote numbers matching the in-text mark
  \RenewDocumentCommand{\sidenotetext}{ o o +m }{%
    \IfNoValueOrEmptyTF{#1}{%
      \@sidenotes@placemarginal{#2}{\textsuperscript{\bfseries\color{archtwoblue}\thesidenote}{}~#3}%
      \refstepcounter{sidenote}%
    }{%
      \@sidenotes@placemarginal{#2}{\textsuperscript{\bfseries\color{archtwoblue}#1}~#3}%
    }%
  }%
  \RenewDocumentCommand{\@sidenotes@placemarginal}{+m +m}{\marginpar{\archsnframe{#2}}}%
}
\makeatother
...
% Caption typography: size captions at \small with bold labels to establish clear
% typographic hierarchy between multi-sentence technical captions and body text.
\usepackage{caption}
\captionsetup{
  font=small,
  labelfont={bf,small},
...
\newcommand{\ArchTwoCoverMotif}{%
  ...
  \begin{tikzpicture}[
    ...
    \node[anchor=north west, font=\tiny\bfseries, text=archtwomuted] at (0.42,4.22)
      {COMPLETE AI-ASSISTED DESIGN SYSTEM};
    ...
    \node[anchor=north east, font=\tiny\bfseries, text=archtwomuted] at (10.43,4.22) {ONE ARCHITECTURE STUDY};
...
\newif\if@archtwochruleprinted
\newcommand{\ArchTwoPrintChapterRule}{%
  \if@archtwochruleprinted\else
    \par\nobreak\vskip 12\p@
    {\color{archtwoblue}\rule{0.72in}{1.0pt}\par}%
    \vskip 18\p@
    \global\@archtwochruleprintedtrue
  \fi
}
...
\def\@makechapterhead#1{{\parindent\z@\raggedright\normalfont
  ...
  \processchapauthor
  \processmotto
  \global\@archtwochruleprintedfalse
}}
...
\makeatletter
% Epigraph environment: sits above the chapter blue rule as part of the header canopy
\newenvironment{epigraph}{%
  ...
  \ArchTwoPrintChapterRule
}

\newenvironment{epigraphtext}{%
...
\newenvironment{epigraphsource}{%
  \par\vspace*{4pt}%
  \noindent\small\upshape\hangindent=1.5em\hangafter=1
}{%
  \par
}
\AtBeginEnvironment{tcolorbox}{\ArchTwoPrintChapterRule}
\pretocmd{\section}{\ArchTwoPrintChapterRule}{}{}

% Center part titles instead of using the class default. SNmono ends a part
% immediately after the title, which otherwise leaves the explanatory paragraph
% alone on the following page. Keep the part open so its short description forms
% one visual unit with the title; the following chapter starts a fresh page.
\makeatletter
% A book without part pages still needs to leave Roman-numbered front matter.
% Only numbered chapters use \@chapter; Preface and other starred headings do not.
% \chapter has already started a fresh page, so do not clear another one here.
\pretocmd{\@chapter}{\if@mainmatter\else\@mainmattertrue\pagenumbering{arabic}\fi}{}{}
\pretocmd{\part}{\if@mainmatter\else\mainmatter\fi}{}{}
...
% spbasic.bst strips every period from formatted author names (to render
% dotless initials), which turns the \. dot-above accent in a surname such as
% Lukošiūtė into an undefined bare \e in the .bbl. Define \e to produce the
% accented letter so the stripped form compiles. (Any future name with a
% different dot-accented letter needs its own alias here.)
\providecommand{\e}{\.{e}}

% Springer Nature alt text. Quarto writes each figure's fig-alt as
% \includegraphics[alt={...}], and graphicx ignores that key under pdflatex.
% SNmono.cls defines \Description, which appends "N (Fig. X) - text" to
% DescriptionTexts.txt for Springer's production pipeline, using the number of
% the figure about to be captioned. Quarto emits \includegraphics before
% \caption, which is exactly where \Description must run, so route the key
% there. Only figure floats are described, and a figure with several images, or
% one typeset twice while being measured, is described once, by its first alt
% text. graphicx reads keys inside a group, so the last-described marker must be
% set \global or the guard forgets it. Pandoc escapes the alt value for LaTeX,
% and \Description writes its argument verbatim, so an apostrophe would reach
% Springer as \textquotesingle{}; undo the escapes Pandoc emits for characters
% the source check allows. Guarded by ./arch2 validate alt-text (the hook
% exists) and ./arch2 verify alt-text (the built file matches the manuscript
% and carries no LaTeX escape).
\makeatletter
\def\archtwo@figuretype{figure}
\def\archtwo@lastdescribed{}
\ExplSyntaxOn
\str_new:N \l__archtwo_description_str
\cs_new_protected:Npn \archtwo@plaindescription #1
  {
    \str_set:Nn \l__archtwo_description_str {#1}
    \str_replace_all:Nnn \l__archtwo_description_str { \textquotesingle {} } { ' }
    \str_replace_all:Nnn \l__archtwo_description_str { \textquotesingle } { ' }
    \str_replace_all:Nnn \l__archtwo_description_str { \textquotedbl {} } { " }
    \str_replace_all:Nnn \l__archtwo_description_str { \textquotedbl } { " }
    \str_replace_all:Nnn \l__archtwo_description_str { \ldots {} } { ... }
    \str_replace_all:Nnn \l__archtwo_description_str { \textbar {} } { | }
    \exp_args:NV \Description \l__archtwo_description_str
  }
\ExplSyntaxOff
\def\archtwo@describe#1{%
  \ifx\@captype\archtwo@figuretype
    \edef\archtwo@current{\number\value{chapter}.\number\value{figure}}%
    \ifx\archtwo@current\archtwo@lastdescribed\else
      \global\let\archtwo@lastdescribed\archtwo@current
      \archtwo@plaindescription{#1}%
    \fi
  \fi}
\AtBeginDocument{%
  \@ifundefined{Description}{}{%
    \define@key{Gin}{alt}{\archtwo@describe{#1}}}}
\makeatother
