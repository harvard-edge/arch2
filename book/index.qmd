...
![**The loop is the object this book engineers:** architect intent bounds the study, agentic participants propose and build candidates, system evidence tests what they produced, and the loop decision returns to the architect who has to stand behind it.](images/arch2-loop-diagram.svg){#fig-arch2-loop .front-loop-diagram fig-alt="Architecture 2.0 loop diagram connecting architect intent, one or more agentic participants, system evidence, and a loop decision that returns to the architect"}
:::

# Preface {.unnumbered}

Computer architecture is entering a period where design effort, not transistor count, is the binding constraint. The field's response to the end of easy scaling has been an explosion of specialized designs, and every one of them multiplies the analysis, verification, and judgment a human team must supply. At the same time, AI systems have begun to participate directly in design work, proposing layouts, writing hardware descriptions, and searching spaces no team can cover by hand. However, an AI participant can also hide a bad assumption, optimize the wrong abstraction, or make a fluent explanation look more certain than the physical evidence supports. The question this book answers is what engineering practice has to surround such a participant before its output deserves an architect's trust.
...
The discipline shifts from human-driven manual exploration to human-steered, agentic search over vast design spaces, where the architect's job is to define the boundaries and verify the evidence.
...
design loop records its state, actions, and evidence, and those records are
what make the study inspectable.
...
That loop recurs throughout the book, and it maps directly to the six stages (@fig-arch2-loop): the architect's intent sets the **Formulate** stage; the agentic participants **Explore** and **Implement** candidates; the system evidence facilitates the **Evaluate** and **Explain** stages; and the final loop decision empowers the architect to **Defend** the result.

::: {.content-visible when-format="pdf"}
![**The loop is the object this book engineers:** architect intent bounds the study, agentic participants propose and build candidates, system evidence tests what they produced, and the loop decision returns to the architect who has to stand behind it.](images/arch2-loop-diagram.pdf){#fig-arch2-loop width="100%" fig-alt="Architecture 2.0 loop diagram connecting architect intent, one or more agentic participants, system evidence, and a loop decision that returns to the architect"}
:::

Machine learning has long optimized isolated mechanisms like branch predictors, and more recently macro placement. What is new is the level at which it operates. Recent models, from language models generating hardware descriptions to reinforcement learning tuning physical-design flows, can participate in the higher-level control loop that proposes and evaluates a design.
...
- **Formulate:** Turn design intent into a scoped study with a stated baseline,
  design space, objectives, constraints, and non-goals.
- **Explore:** Compare meaningful alternatives, and preserve why the serious
  ones failed or were rejected.
- **Implement:** Lower the surviving alternatives into artifacts a tool can
  actually check.
- **Evaluate:** Test the claim at a justified fidelity against matched
  baselines, and go looking for counterevidence.
- **Explain:** Form and test an architectural account of why the result
  occurred.
- **Defend:** Recommend, state the evidence limits and who owns the decision,
  and name what would reverse it.

@sec-architecture-20-ontology builds each stage in full, names the artifacts it
produces, and maps it to the chapter that develops it. What matters here is only
the shape.
...
: **Architecture 2.0 borrows its methods rather than inventing them, and each one answers a specific way an AI-assisted study can fail:** the discipline's own contribution is knowing which method belongs at which point in a study. {#tbl-borrowed-disciplines tbl-colwidths="[22,40,38]"}

Electronic Design Automation is the exception in @tbl-borrowed-disciplines, because it supplies no method to borrow. It supplies the physical reality the borrowed methods have to survive. EDA tools are opaque and brittle, rely on decades-old Tcl scripting conventions, and carry long runtimes. The physical search space is discontinuous, since a small upstream architectural change can cause downstream failures like routing congestion or DRC errors. That friction is both why AI prediction is attractive and why integrating AI into the hardware loop is hard.
...
This book maps each borrowed idea to the exact point in an architecture study where it belongs, inside the intersection @fig-book-composition draws. It structures AI assistance to produce checkable evidence rather than fluent claims.
...
stack. @sec-evaluating-agentic-architect turns the measurement lens from the
chip to the agent driving the search. @sec-what-architect-owns returns to
architectural judgment and responsibility. The parts group the reading and the six stages name the work inside a
single study, and the two are deliberately different cuts.
@sec-architecture-20-ontology maps each stage to the chapter that builds
it. Three chapters sit outside that map on purpose. @sec-moonshot and
@sec-design-loop-no-longer-scales establish why the loop needs governing
before any stage can be defined, and @sec-loop-patterns-across-stack
tests the whole lifecycle across the stack instead of building one stage
of it.
...
![**One book, four entry points:** two axes, domain expertise and reader role, split the audience into four personas, and each persona needs a different subset of the chapters. Each persona follows a different reading route through the chapters.](images/audience-quadrant.svg){#fig-audience-quadrant width="100%" fig-alt="A 2x2 quadrant mapping four personas: AI Strategist, Chief Architect, ML Researcher, and Computer Architect across domain expertise and reader role."}

The four audience roles each approach the material with a different goal (@fig-audience-quadrant):

- **The AI Strategist:** Machine learning leaders who will read this to understand the fundamental physics bottlenecks, why evaluation rather than generation is the binding cost, and the structural limits of the hardware design space (@sec-moonshot, @sec-design-loop-no-longer-scales, @sec-methods-generation-prediction-optimization).
...
- **The Computer Architect:** (The primary audience) Hardware implementers who need to know how to integrate AI safely, manage agentic search, and evaluate the design space (@sec-architecture-20-ontology, @sec-feedback-verification-trust, @sec-running-the-loop, @sec-loop-patterns-across-stack).
...
With these interpretive principles in mind, choose a route. Each persona's chapter subset in @fig-audience-quadrant is a reading route of its own, and two more serve every reader. A fast orientation pass takes the Preface, @sec-moonshot, and @sec-what-architect-owns; the full argument reads the chapters in order.

For a graduate seminar, the material maps onto a semester directly. The five parts are Formulation, Exploration, Implementation, Evaluation, and Defense, and they do not carry equal weight. Formulation is the heaviest and wants about four weeks; Exploration and Evaluation about three each; Implementation two; and Defense one, since the closing chapter is short by design. The replay package behind @sec-running-the-loop is the lab spine, and the reviewer checklist in @sec-appendix-a-reviewer-checklist doubles as a grading rubric. Each student team runs one bounded architecture study on a problem of its own choosing and defends it against that checklist, which is the book's test of transfer applied in miniature.

Readers seeking hands-on practice should replay the worked study in
@sec-running-the-loop using that same replay package. Readers applying
