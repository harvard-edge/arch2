...
\begin{frame}{Feedback must update the right thing}
  \claim{A failed check can change a candidate, a method's support, the project record, or the operating policy. Those are different updates.}
  \begin{center}
  \begin{tikzpicture}[node distance=0.25cm and 0.42cm]
    ...
    \node[feedbackbox,fill=ArchBlueLight,draw=ArchBlue] (a)
      {\textbf{Observation}\\measurement, counterexample, failure, review};
    \node[feedbackbox,fill=ArchPanel,draw=ArchLine,right=of a] (b)
      {\textbf{Diagnose}\\candidate error, proxy error, drift, or process failure};
    \node[feedbackbox,fill=ArchGreenLight,draw=ArchGreen,right=of b] (c)
      {\textbf{Update}\\design state, calibration, data, model, or policy};
    \node[feedbackbox,fill=ArchGoldLight,draw=ArchGold,right=of c] (d)
      {\textbf{Recheck}\\independent observation under the changed conditions};
    \draw[-{Stealth},ArchMuted,thick] (a)--(b);
    \draw[-{Stealth},ArchMuted,thick] (b)--(c);
    \draw[-{Stealth},ArchMuted,thick] (c)--(d);
  \end{tikzpicture}
...
% 35
\section{Running the study}

\begin{frame}{One complete study must resolve four judgments}
  \claim{The Lighthouse is too broad to test at once. One scoped XR study must still separate architecture outcome, AI contribution, mechanism, and action.}
  \begin{center}
...
% 35
\begin{frame}{One XR subproblem produced a local tie}
  ...
  \begin{tikzpicture}[node distance=0.28cm]
    ...
    \node[rounded corners=4pt,fill=ArchRedLight,draw=ArchRed,minimum width=12.7cm,minimum height=1.0cm,
      ...
      {Outcome: no evaluated nonbaseline cleared the margin.\\Recommendation: retain 32$\times$32; architecture action pending.};
  \end{tikzpicture}
  \end{center}
  \vspace{0.12cm}
...
\begin{frame}{Four patterns recur across the stack}
  ...
  \begin{tikzpicture}[node distance=0.28cm and 0.35cm]
    ...
    \node[patternbox,fill=ArchBlueLight,draw=ArchBlue] (a)
      {\textbf{Expensive truth}\\Use cheap bounds and predictors to decide which candidates deserve stronger tools.};
    ...
    \node[patternbox,fill=ArchRedLight,draw=ArchRed,right=of c] (d)
      {\textbf{Asymmetric consequence}\\Generate cheaply when possible, but scale checks with the cost of a wrong commitment.};
  \end{tikzpicture}
  \end{center}
\end{frame}
  ...
  \begin{center}
    \begin{tikzpicture}
      \node[rounded corners=3pt,fill=ArchBlueLight,draw=ArchBlue,text width=12.7cm,
        align=center,minimum height=0.75cm,font=\small\bfseries,text=ArchBlue]
...
\begin{frame}{Metrics people forget to count}
  \claim{The result, the work that produced it, and its decision quality need separate measurements.}
  \begin{center}
...
\begin{frame}{The field now has a research agenda}
  ...
  \begin{tikzpicture}[node distance=0.12cm]
    ...
    \node[agenda,fill=ArchPanel,draw=ArchLine,below=of d] (e)
      {\textbf{Organize}\hspace{0.5cm} Define the architect's authority, review boundaries, and conditions for safe learning and reuse.};
  \end{tikzpicture}
  \end{center}
  \vspace{0.08cm}
...
\begin{frame}{Three separations to remember}
  ...
  \begin{center}
    ...
    Residual risk, containment, and reopening conditions remain part of the record.\par}
  \end{center}
\end{frame}
